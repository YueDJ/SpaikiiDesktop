...
or change arbitrary Sparkii configuration.
...
small host fact requested by the RFC. Settings writes use Sparkii'
...
`SPARKII_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<SPARKII_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Sparkii home on every operation.
...
The implementation is covered with real temporary-Sparkii-home tests for:
...
- [Issue #64227](https://github.com/NousResearch/sparkii-agent/issues/64227)
- [RFC PR #58542](https://github.com/NousResearch/sparkii-agent/pull/58542) by Topher Ross
