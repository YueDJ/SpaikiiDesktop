...
`sparkii config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.sparkii/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `SPARKII_HOME`, secret scope, and
