...
has to be thrown away or summarized. Sparkii has always done this in one batch:
...
Sparkii folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Sparkii's normal session flush is
...
With `micro_compact` unset or `false` Sparkii behaves exactly as it always has:
...
Defaults to `$SPARKII_HOME/logs/agent.log`. It reports passes, outcome mix, net
