...
Sparkii supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.sparkii/config.yaml`:
...
Or set the env var: `SPARKII_KANBAN_DISPATCH_IN_GATEWAY=false`
