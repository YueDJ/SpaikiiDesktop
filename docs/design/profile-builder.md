...
Supersedes: PR #31781 (prompt_toolkit `sparkii profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `sparkii profile wizard` in the terminal.
...
needs, and a profile is just a SPARKII_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.sparkii/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — SPARKII_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with SPARKII_HOME override |
...
### Seam #1 — hub-skill install cannot use the SPARKII_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = SPARKII_HOME / "skills"` at **module
import time**. The context-local `set_sparkii_home_override()` swap (which makes
...
2. write model (synchronous, SPARKII_HOME override)
3. write selected MCP servers (synchronous, SPARKII_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `sparkii -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, SPARKII_HOME override
...
page afterward. Mirror `_write_profile_model`'s SPARKII_HOME-override helper for
...
- Backend E2E with isolated SPARKII_HOME: POST a full create body
