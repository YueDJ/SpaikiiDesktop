# Sparkii Observer Hooks

Sparkii observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Sparkii' planner, model providers, memory, tool registry,
...
Sparkii also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "sparkii.observer.v1"
```

Hook callbacks are fail-open. Sparkii catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Sparkii only builds
