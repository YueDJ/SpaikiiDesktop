...
Sparkii includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Sparkii and does not require `sparkii plugins enable
observability/nemo_relay`. Sparkii remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Sparkii execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `sparkii-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Sparkii requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Sparkii installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Sparkii process so configured interceptors can
...
Collection remains off unless Sparkii policy enables it:
...
Sparkii core owns one Relay host and one isolated Relay session scope per Sparkii
session. Core lifecycle producers use
`sparkii_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Sparkii plugin registration. Shared-metrics marks must
...
```text
Sparkii turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Sparkii shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Sparkii sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Sparkii used for the logical call, such as
...
New calls use `sparkii.model_route.count`. The previous
`sparkii.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `sparkii.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Sparkii
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Sparkii processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `sparkii.task_run`, parented to
the owning Sparkii session. The start counter contains only bounded execution
...
provider attempts for the same Sparkii API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Sparkii tool-call
...
cancellations. Active task ownership follows the task ID if Sparkii rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`sparkii.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Sparkii
...
tool or plugin names. Sparkii does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `sparkii.tool_approval` marks and
...
Successful skill mutations emit `sparkii.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `sparkii.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Sparkii derives reuse and patch-generation
...
```text
$SPARKII_HOME/telemetry/shared_metrics/metrics.sqlite3
$SPARKII_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Sparkii version,
...
schema field name, its current scope is one `SPARKII_HOME`, so it is more
...
Deleting `$SPARKII_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SPARKII_HOME`. To reset it, stop Sparkii
processes and remove `$SPARKII_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Sparkii CLI turn against the deterministic local model server:
