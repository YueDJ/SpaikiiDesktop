...
When running Sparkii inside Docker, the default `network_mode: host` gives the
...
The Sparkii [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ sparkii-agent │   │ sparkii-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   SPARKII_UID=$(id -u) SPARKII_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=sparkii,sparkii-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://sparkii-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Sparkii trust model and vulnerability reporting
