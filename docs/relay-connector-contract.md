...
This document is the formal interface between the **Sparkii gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `sparkii gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`sparkii gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `sparkii` CLI.
