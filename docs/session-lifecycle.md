...
The canonical transcript store is SQLite via `SessionDB` (from `sparkii_state`). The
...
Counts consecutive restarts via a JSON file (`{SPARKII_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `sparkii update`, `sparkii gateway restart`,
or `/restart`.
