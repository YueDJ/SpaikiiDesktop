...
> **Heads up:** Native Windows runs Sparkii without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues](https://github.com/YueDJ/SparkiiAgent/issues).
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Sparkii `bin` folder (`%LOCALAPPDATA%\sparkii\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Sparkii bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\sparkii\bin\uv.exe"
...
**To whitelist Sparkii:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\sparkii\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Sparkii updates `uv` and the hash changes every version
...
```bash
sparkii              # Interactive CLI — start a conversation
...
Sparkii works with whatever provider you want — that's not changing. But if you'd rather not collect five separate API keys for the model, web search, image generation, TTS, and a cloud browser, **[Nous Portal](https://portal.nousresearch.com)** covers all of them under one subscription:
...
If you're coming from OpenClaw, Sparkii can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`sparkii setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
sparkii claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.sparkii/skills/openclaw-imports/`
...
See `sparkii claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
...
`~/.sparkii/sparkii-agent`). This matches the layout used by `sparkii update`, the
...
source ~/.sparkii/venvs/sparkii-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
