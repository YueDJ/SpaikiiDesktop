...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Sparkii profile."""
...
# comparison sites (four here, plus sparkii_cli/tools_config.py and
# sparkii_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Sparkii-managed-Node-only setups
...
# run exactly once, before the cleanup loop started, which meant a sparkii
...
# Hard ceiling for a daemon whose owning sparkii process is still alive but
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    sparkii restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live sparkii process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Sparkii-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Sparkii-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_sparkii_node_executable`` uses to self-heal a managed Node tree)
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``sparkii update`` (or ``sparkii doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Sparkii holds, while running registry-fetched
    npm code on every ``sparkii update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `sparkii update`/`doctor
