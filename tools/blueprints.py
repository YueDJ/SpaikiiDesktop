...
`sparkii skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.sparkii.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.sparkii.blueprint`` is a mapping containing
    ...
    sparkii = meta.get("sparkii") if isinstance(meta, dict) else None
    blueprint = sparkii.get("blueprint") if isinstance(sparkii, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.sparkii.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.sparkii.blueprint`` block) they can hand
    to ``sparkii skills publish`` to share. ``body`` is the plain-language
