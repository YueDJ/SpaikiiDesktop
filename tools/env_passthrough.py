...
def _is_sparkii_provider_credential(name: str) -> bool:
    """True if ``name`` is a Sparkii-managed provider credential (API key,
    token, or similar) per ``_SPARKII_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Sparkii API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Sparkii credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Sparkii-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Sparkii-managed provider credentials (from
    ``_SPARKII_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Sparkii-managed
    ...
    Non-Sparkii third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_sparkii_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Sparkii provider "
                "credential %r (blocked by _SPARKII_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Sparkii-managed provider credentials must not be passed
                ...
                if _is_sparkii_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Sparkii "
                        "provider credential %r from config.yaml (blocked "
                        "by _SPARKII_PROVIDER_ENV_BLOCKLIST). Operator "
