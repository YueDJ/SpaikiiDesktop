...
CHECKPOINT_PATH = get_sparkii_home() / "processes.json"
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Sparkii gateway runtime.

    Both supervisor markers and ``_SPARKII_GATEWAY`` are inherited by every
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from sparkii_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``sparkii_cli.goals``) to support
