...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Sparkii's live source checkout "
        ...
        "checkout, stop Sparkii, run the command externally, then restart "
        "Sparkii."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(sparkii_home).expanduser() if sparkii_home else Path.home() / ".sparkii"
