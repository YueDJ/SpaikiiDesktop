...
``ui_meta['sparkii-bots']`` block in its profile.yaml), a bot's canonical
...
def _sparkii_root(home: Path) -> Path:
    """Root ~/.sparkii for both the default profile and named profiles."""
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['sparkii-bots'] block.
    ...
    try:
        ...
        if "sparkii-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("sparkii-bots"), dict)
    except Exception:
        return False
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @sparkii.
    return "sparkii" if name == "default" else name


def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``sparkii peer``), for the protocol text.
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "```\n"
        "sparkii peer dm <peer>/<agent-name> < /tmp/dm.txt\n"
        "```\n"
        "Use `<peer>` alone for the peer's main agent. Run `sparkii peer list` "
        "for the live peer list."
    )


def _build_section(home: Path) -> str:
    root = _sparkii_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Sparkii profile is an agent teammate with "
        ...
        "```\n"
        f'sparkii -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file /tmp/dm.txt\n'
        "```\n"
        ...
        "which agent it came from. Run `sparkii profile list` for the LIVE teammate "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient SPARKII_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("SPARKII_HOME") or os.path.expanduser("~/.sparkii"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("SPARKII_HOME") or os.path.expanduser("~/.sparkii")))
    surface: dict = {}
    try:
    ...
    try:
        ...
        surface["peers"] = _peers(_sparkii_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
