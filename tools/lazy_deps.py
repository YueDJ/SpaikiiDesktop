"""
Lazy dependency installer for opt-in Sparkii Agent backends.

Many Sparkii features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``sparkii-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``sparkii tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``SPARKII_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/sparkii`` read-only), setting
  ``SPARKII_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Sparkii, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (SPARKII_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (sparkii trace upload / /upload-trace).
    #
    ...
    # presence, the `sparkii update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "SPARKII_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``SPARKII_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("SPARKII_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``sparkii update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="sparkii-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``sparkii_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        # which() misses the uv Sparkii installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Sparkii
    ...
    # deployment sets SPARKII_MANAGED=true *and* SPARKII_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Sparkii cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Sparkii instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``SPARKII_DISABLE_LAZY_INSTALLS=1``) and sets
      ``SPARKII_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``sparkii update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``sparkii update``. Never raises; lazy-install failures
