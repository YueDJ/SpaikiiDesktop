...
_PRIVATE_BROWSER_SENTINEL = "_SPARKII_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# sparkii: pin this named session to its own tab (once per daemon process)
def _sparkii_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "sparkii-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_sparkii_ensure_own_tab()
del _sparkii_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Sparkii's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Sparkii's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_sparkii_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `sparkii tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Sparkii' own bin dir ($SPARKII_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from sparkii_constants import get_sparkii_home

        return str(Path(get_sparkii_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Sparkii' own ``$SPARKII_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Sparkii-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Sparkii' managed uv (bootstrapped on demand via
    ``sparkii_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SPARKII_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Sparkii-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through sparkii tools).
