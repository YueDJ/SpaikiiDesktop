...
<url>``) is spawned as a direct child of the Sparkii process. Sparkii's own
...
Sparkii process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Sparkii startup's opt-in
