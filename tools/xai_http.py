...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``sparkii tools`` repaint, tool-registration scans,
    ...
    2. ``~/.sparkii/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``sparkii auth add xai-oauth``
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Sparkii home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    honors ``SPARKII_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
