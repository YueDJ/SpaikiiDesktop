...
Backend is selected during ``sparkii tools`` setup (web.backend in config.yaml).
When available, Sparkii can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Sparkii config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Sparkii' config/.env layer (``sparkii config set``, ``sparkii tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.sparkii/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``sparkii tools``). A
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `sparkii tools` repaint.
        try:
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `sparkii plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `sparkii tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `sparkii plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`sparkii plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from sparkii_cli.config import get_env_value as _gev
