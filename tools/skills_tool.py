...
# Cache validation (mirrors sparkii_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.sparkii/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set SPARKII_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped SPARKII_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from SPARKII_HOME/.env."""
    env_path = get_sparkii_home() / ".env"
    ...
    # the canonical readers in sparkii_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set SPARKII_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (SPARKII_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("SPARKII_INTERACTIVE"):
        return {
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_sparkii_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.sparkii/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``SPARKII_PLATFORM`` environment variable
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.sparkii/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``sparkii skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile SPARKII_HOME; the module-level
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: sparkii plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `sparkii skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.sparkii/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `sparkii skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `sparkii sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
