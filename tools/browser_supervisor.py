...
One ``CDPSupervisor`` runs per Sparkii ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Sparkii logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "sparkii-dialog-bridge.invalid"
...
(() => {
  if (window.__sparkiiDialogBridgeInstalled) return;
  window.__sparkiiDialogBridgeInstalled = true;
  const ENDPOINT = "http://sparkii-dialog-bridge.invalid/";
