...
_FORCE_SYNC_ENV = "SPARKII_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.sparkii") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.sparkii to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SPARKII_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.sparkii/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="sparkii-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.sparkii/skills/a.md`` →
        ``~/.sparkii/skills/a.md``, a new remote file at
        ``/root/.sparkii/skills/b.md`` maps to ``~/.sparkii/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
