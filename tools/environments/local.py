...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Sparkii was already launched from (for example ``sparkii-agent``
    while the process cwd is ``~/.sparkii/sparkii-agent``), passing it through
    unchanged makes the wrapper run ``cd sparkii-agent`` *inside* the project
    ...
    # Common recovery for config values like ``sparkii-agent`` when Sparkii was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./sparkii-agent``; use the current directory instead.
...
# Sparkii-internal env vars that should NOT leak into terminal subprocesses.
_SPARKII_PROVIDER_ENV_FORCE_PREFIX = "_SPARKII_FORCE_"

# Sparkii-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Sparkii inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    blocked.update({
        ...
        "SPARKII_DASHBOARD_SESSION_TOKEN",
    ...
    # Sparkii-managed inference credential — Claude subscription auth is not a
    # working Sparkii provider path.  Stripping it broke agent-spawned
...
_SPARKII_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Sparkii venv path instead of the project's own
# ``.venv`` — silently clobbering the Sparkii environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Sparkii venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Sparkii venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Sparkii itself treats PYTHONHOME as contamination in its own
...
# _strip_sparkii_owned_pythonpath() which removes only Sparkii-owned entries,
...
def _inject_context_sparkii_home(env: dict) -> None:
    """Bridge the context-local Sparkii home override into subprocess env."""
    try:
        from sparkii_constants import get_sparkii_home_override

        value = get_sparkii_home_override()
        if value:
            env["SPARKII_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (SPARKII_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Sparkii-managed secrets from a subprocess environment."""
    try:
    ...
    _inject_context_sparkii_home(sanitized)

    from sparkii_constants import apply_subprocess_home_env
    ...
    # the separate Sparkii runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_sparkii_owned_pythonpath_and_runtime_markers(sanitized)
...
# legitimate child Sparkii spawns needs them, and they are the highest-value
...
def sparkii_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_SPARKII_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Sparkii spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_SPARKII_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Sparkii-internal dynamic secrets
    ...
    if not inherit_credentials:
        ...
        for key in _SPARKII_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_sparkii_home(env)
    from sparkii_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_sparkii_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose SPARKII_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's SPARKII_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Sparkii home override into ``SPARKII_HOME`` and apply the
      subprocess HOME contract (``sparkii_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``SPARKII_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs SPARKII_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SPARKII_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale SPARKII_GIT_BASH_PATH pointing at one) must not
    ...
    # SPARKII_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\sparkii\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "SPARKII_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Sparkii Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set SPARKII_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Sparkii. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``sparkii`` console-script.
...
def _resolve_sparkii_bin_dir() -> str | None:
    ...
    sparkii install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``sparkii``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``sparkii`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``sparkii`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("sparkii")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``sparkii`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _SPARKII_BIN_DIR
    if _SPARKII_BIN_DIR is not _SENTINEL:
        return _SPARKII_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("sparkii")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "sparkii" or base.startswith("sparkii."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "sparkii.exe" if _IS_WINDOWS else "sparkii"
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Sparkii-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Sparkii installs for
    itself, so on a machine where Sparkii provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$SPARKII_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$SPARKII_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Sparkii-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Sparkii
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SPARKII_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if k in _SPARKII_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    _strip_sparkii_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_sparkii_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by Sparkii launchers.

    ``gateway_windows._preserve_sparkii_home_path`` maps a physical path under
    the resolved SPARKII_HOME back onto the configured SPARKII_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of SPARKII_HOME as Sparkii-owned.
    ...
    # lexically the same way get_default_sparkii_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of SPARKII_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\sparkii\sparkii-agent
    # -> C:\...\sparkii-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Sparkii-owned.  Fail-closed: a
...
# --- Sparkii venv / repo-root detection (module-level, computed once) ---

#: The Sparkii repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import sparkii_cli``,
#: etc.  Subprocesses that are NOT the Sparkii backend don't need it and it
#: can shadow local packages.
_sparkii_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Sparkii launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Sparkii-owned paths under
#: the configured SPARKII_HOME spelling (which may be a junction to another
#: drive — see ``sparkii_cli/gateway_windows.py::_preserve_sparkii_home_path``).
...
_sparkii_repo_root_aliases: tuple[Path, ...] = _build_sparkii_repo_root_aliases(
    _sparkii_repo_root,
...
#: subprocess spawn.
_sparkii_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Sparkii repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _sparkii_repo_root_aliases):
        return None
...
def _get_sparkii_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Sparkii runtime.
    ...
    """
    global _sparkii_site_packages
    if _sparkii_site_packages is not None:
        result = list(_sparkii_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _sparkii_site_packages = list(result)
...
def _strip_sparkii_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Sparkii-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_sparkii_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_sparkii_owned_pythonpath(env: dict) -> None:
    """Remove Sparkii-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Sparkii repo root and the Sparkii venv's
    ...
    user entries, so only entries proven Sparkii-owned are removed:
    ...
    sparkii_site_packages = _get_sparkii_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Sparkii-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Sparkii venv site-packages ---
        ...
        for sp in sparkii_site_packages:
        ...
        # resolved and unresolved (SPARKII_HOME/junction) spellings count as
        # Sparkii-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _sparkii_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped Sparkii-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Sparkii trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SPARKII_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SPARKII_HOME.  Using
