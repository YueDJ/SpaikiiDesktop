...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id (X-Sparkii-Session-Id) of the ORIGINATING
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ...
            ("user_id", "SPARKII_SESSION_USER_ID"),
            ("user_name", "SPARKII_SESSION_USER_NAME"),
        ):
...
def recover_abandoned_delegations() -> int:
    ...
    try:
        from core.process_utils import _pid_exists, get_process_start_time
    except Exception:
        return 0
    now = time.time()
    recovered = 0
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``SPARKII_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``SPARKII_SESSION_CHAT_ID`` binding survives child
    construction: ``_bind_api_server_session`` binds ``chat_id`` to the raw
    ``X-Sparkii-Session-Id``, and its only writer is ``set_session_vars`` —
    ...
    try:
        ...
        if get_session_env("SPARKII_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("SPARKII_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
