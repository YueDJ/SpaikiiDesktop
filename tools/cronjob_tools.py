"""
Cron job management tools for Sparkii Agent.
...
# uses 30s) — comfortably below the 1800s default SPARKII_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (SPARKII_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with SPARKII_CRON_TIMEOUT=0
...
#      saying "cat ~/.sparkii/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `sparkii-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.sparkii/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("SPARKII_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("SPARKII_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes SPARKII_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``SPARKII_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``SPARKII_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    def _creator_target() -> str:
        platform = get_session_env("SPARKII_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("SPARKII_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("SPARKII_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within SPARKII_HOME/scripts/.
    ...
    # Only relative paths within ~/.sparkii/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.sparkii/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.sparkii/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_sparkii_home() / "scripts"
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.sparkii/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``sparkii -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `sparkii
    ...
    if not session_key:
        # Direct Python callers (`sparkii cron run`, tests) have no agent
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("SPARKII_INTERACTIVE")
        or env_var_enabled("SPARKII_GATEWAY_SESSION")
        or env_var_enabled("SPARKII_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `sparkii cron create/edit --model`, or hand-edited jobs). The agent
