...
Lets the LLM write a Python script that calls Sparkii tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `sparkii_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `sparkii_tools.py` with file-based RPC stubs
...
# available on every platform Sparkii itself runs on.
...
# match a safe prefix, the operational SPARKII_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Sparkii
# code can lose the DB-layer Kanban mutation guard while still inheriting
# SPARKII_HOME.
#
# NB: the broad "SPARKII_" prefix was deliberately removed (#27303) — it leaked
# SPARKII_*-named config that lacks a secret substring (e.g. SPARKII_BASE_URL,
# SPARKII_KANBAN_DB, SPARKII_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SPARKII_CHILD_ALLOWED below; SPARKII_RPC_SOCKET /
# SPARKII_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SPARKII_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set sparkii_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_SPARKII_CHILD_ALLOWED = frozenset({
    "SPARKII_HOME",
    "SPARKII_PROFILE",
    "SPARKII_CONFIG",
    "SPARKII_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational SPARKII_* vars (_SPARKII_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret SPARKII_* vars dropped by the tightened allowlist (#27303). The
    # broad "SPARKII_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SPARKII_KANBAN_DB / SPARKII_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _SPARKII_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
...
# ---------------------------------------------------------------------------
# sparkii_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Sparkii-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Sparkii-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    sparkii_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'sparkii_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_sparkii_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the sparkii_tools.py stub module.
...
"""Auto-generated Sparkii tools RPC stubs."""
...
def _connect():
    ...
    SPARKII_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SPARKII_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SPARKII_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
def _execute_remote(
    ...
    The script and the generated sparkii_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/sparkii_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_sparkii_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/sparkii_tools.py", tools_src)
        ...
        env_prefix = (
            f"SPARKII_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SPARKII_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SPARKII_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from SPARKII_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        # Write the auto-generated sparkii_tools module.
        ...
        tools_src = generate_sparkii_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "sparkii_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SPARKII_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SPARKII_RPC_SOCKET"] = rpc_endpoint
        child_env["SPARKII_RPC_TOKEN"] = rpc_token
        ...
        # SPARKII_TIMEZONE is an internal Sparkii setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SPARKII_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SPARKII_TIMEZONE", None)
        ...
        # ``sparkii_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Sparkii's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Sparkii's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Sparkii-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Sparkii-
        ...
        _strip_sparkii_owned_pythonpath(child_env)
        _sparkii_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_sparkii_python_environment(_child_python):
            _pp_parts.append(_sparkii_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import sparkii_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Sparkii "
                "environment; sparkii root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.sparkii/.env')).
...
def _load_config() -> dict:
    ...
    ``sparkii --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (sparkii-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Sparkii environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the sparkii root from every
...
def _uses_sparkii_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Sparkii's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``sparkii tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and sparkii-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.sparkii/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Sparkii tools programmatically. "
        ...
        f"Available via `from sparkii_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from sparkii_tools import {import_str}` "
