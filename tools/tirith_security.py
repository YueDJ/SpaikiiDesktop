...
it is automatically downloaded from GitHub releases to $SPARKII_HOME/bin/tirith.
...
def _get_sparkii_home() -> str:
    """Return the Sparkii home directory, respecting SPARKII_HOME env var."""
    return str(get_sparkii_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sparkii_home(), ".tirith-install-failed")
...
def _sparkii_bin_dir() -> str:
    """Return $SPARKII_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sparkii_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SPARKII_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sparkii_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SPARKII_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SPARKII_HOME/bin/tirith
    ...
    sparkii_bin = os.path.join(_sparkii_bin_dir(), "tirith")
    if os.path.isfile(sparkii_bin) and os.access(sparkii_bin, os.X_OK):
        _resolved_path = sparkii_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sparkii_bin
