...
def auth_json_path():
    """Return the Sparkii auth store path, respecting SPARKII_HOME overrides."""
    return get_sparkii_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`sparkii tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Sparkii release, and
