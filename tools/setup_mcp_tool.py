...
``sparkii mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Sparkii desktop app. Use the "
            "terminal instead: `sparkii mcp install <name>` for catalog entries, "
            "`sparkii mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Sparkii desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `sparkii mcp catalog` "
