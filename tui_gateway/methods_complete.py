...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'sparkii' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "sparkii".startswith(prefix.lower()) and "sparkii" not in seen:
                out.append(
                    {
                        "text": "@sparkii",
                        "display": "@sparkii",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sparkii_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from sparkii_cli.inventory import build_model_options_payload
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `sparkii model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.sparkii/.env via the unified credential lifecycle
