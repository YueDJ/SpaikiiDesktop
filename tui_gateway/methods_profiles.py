...
handlers delegate to the same `sparkii_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Sparkii profiles (name, path, model, description, skill count).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's sparkii-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``sparkii setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (sparkii_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local SPARKII_HOME override — the same
        ...
        try:
            ...
            token = set_sparkii_home_override(str(path))
            try:
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the SPARKII_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `sparkii tools`
            ...
            # composites (sparkii-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from sparkii_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `sparkii tools` / Settings is where
            ...
            description = ""
            try:
                from sparkii_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from sparkii_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from sparkii_cli.skills_config import save_disabled_skills
