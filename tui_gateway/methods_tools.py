...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m sparkii_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m sparkii_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SPARKII_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_sparkii_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SPARKII_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SPARKII_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_sparkii_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SPARKII_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from sparkii_constants import set_sparkii_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from sparkii_constants import reset_sparkii_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``sparkii journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from sparkii_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from sparkii_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `sparkii mcp` offers, so
...
# (sparkii_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SPARKII_HOME via set_sparkii_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses sparkii_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sparkii_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sparkii_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.sparkiiDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``sparkii mcp login`` uses
    ...
    try:
        ...
        sparkii_home = str(get_sparkii_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(sparkii_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``sparkii plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.sparkii/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SPARKII_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Sparkii plugin.
