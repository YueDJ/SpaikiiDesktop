...
| List/filter by metadata (age, source, cost, tokens, workspace) | `sparkii sessions list` / `stats` via terminal |
| Rename | `sparkii sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `sparkii sessions archive <filters>` |
| Delete (destructive) | `sparkii sessions delete` / `sparkii sessions prune <filters>` |
| Export before deleting anything valuable | `sparkii sessions export --session-id <id> --format md` |
...
`sparkii sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `sparkii sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `sparkii sessions list`
