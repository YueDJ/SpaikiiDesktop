...
Everything operator-facing is a `sparkii teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${SPARKII_HOME:-~/.sparkii}/.env`:
...
```bash
sparkii teams-pipeline validate              # config snapshot — run first after any change
sparkii teams-pipeline token-health          # Graph token status
sparkii teams-pipeline token-health --force-refresh   # force a fresh token acquisition
sparkii teams-pipeline list                  # recent meeting jobs
sparkii teams-pipeline list --status failed  # only failed jobs
sparkii teams-pipeline show <job-id>         # full detail of one job
sparkii teams-pipeline subscriptions         # current Graph webhook subscriptions
```
...
```bash
sparkii teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

sparkii teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
sparkii teams-pipeline delete-subscription <sub-id>
sparkii teams-pipeline maintain-subscriptions            # renew near-expiry ones
sparkii teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `sparkii teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `sparkii cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
