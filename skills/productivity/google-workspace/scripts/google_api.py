#!/usr/bin/env python3
"""Google Workspace API CLI for Sparkii Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Sparkii-facing JSON contract and falls back to the Python client
...
from _sparkii_home import get_sparkii_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SPARKII_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Sparkii Agent")
