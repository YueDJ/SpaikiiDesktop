...
Sparkii runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`sparkii config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Sparkii's env scrubber dropping
...
Sparkii-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Sparkii tool and most Windows APIs. Prefer forward slashes in code
