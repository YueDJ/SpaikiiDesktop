...
Edit with `sparkii config edit` or `sparkii config set section.key value`.
Full reference: https://sparkii-agent.nousresearch.com/docs/user-guide/configuration
...
`sparkii config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `sparkii tools` (interactive) or `sparkii tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SPARKII_CORE_TOOLS` is the default bundle most platforms inherit).
