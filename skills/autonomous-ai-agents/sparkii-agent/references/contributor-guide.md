...
For occasional contributors and PR authors. Full developer docs: https://sparkii-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.sparkii/config.yaml` (settings), `~/.sparkii/.env` (API keys) — both under `$SPARKII_HOME` when it is set.
...
`_SPARKII_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_sparkii_home()` for paths,
never hardcode `~/.sparkii`. For custom/local-only tools, write a plugin in
`~/.sparkii/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `sparkii_cli/commands.py`
...
- Tests auto-redirect `SPARKII_HOME` to temp dirs — never touch real `~/.sparkii/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/sparkii_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_sparkii_home()` from `sparkii_constants` for all paths (profile-safe)
