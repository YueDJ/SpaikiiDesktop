# Themes / Skins — Author a Sparkii Color Theme

Author a Sparkii **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`sparkii_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.sparkii/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Sparkii itself.
...
- Write access to the Sparkii home dir — `~/.sparkii` by default, or `$SPARKII_HOME`
  / the active profile's dir. Skins live in `<sparkii-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `sparkii config set`).
...
3. `write_file` it to `<sparkii-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `sparkii config set display.skin default` (or they can `/skin default`).
...
```
sparkii skin set <key> <hex>      # e.g. sparkii skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `sparkii skin set background "#08201f"` changes
only the background; `sparkii skin use <name>` / `sparkii skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.sparkii`** when a profile is active — resolve the real home
  from `$SPARKII_HOME` first, falling back to `~/.sparkii`.
...
- **Never hand-edit `config.yaml` to activate.** Use `sparkii config set
...
- **You apply it, not the user.** `sparkii config set display.skin <name>` is
...
- `read_file` the written `<sparkii-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `sparkii config get display.skin` and confirm it reports `<name>`.
