...
Author widget apps for the Sparkii TUI (`sparkii --tui`): glanceable ambient
...
- The TUI must be in use (`sparkii --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.sparkii/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.sparkii/logs/tui_gateway_crash.log` if a widget never appears.
