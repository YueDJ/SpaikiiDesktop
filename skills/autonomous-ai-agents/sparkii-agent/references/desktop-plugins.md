...
Write plugins for the Sparkii desktop app: statusbar items, layout panes,
...
system (`~/.sparkii/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SPARKII_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SPARKII_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Sparkii desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SPARKII_HOME/desktop-plugins/` (usually
  `~/.sparkii/desktop-plugins/`).

## How to Run

1. Create `$SPARKII_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.sparkii/...` by default, or `~/.sparkii/profiles/<profile>/...` under a
...
The ONLY import surface is `@sparkii/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Sparkii
  ...
  (`sparkii://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.sparkii/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@sparkii/plugin-sdk`, `react`, and
