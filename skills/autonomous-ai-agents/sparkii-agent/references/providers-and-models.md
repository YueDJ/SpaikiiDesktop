...
Set via `sparkii model` (picker) or `sparkii setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://sparkii-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `sparkii auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `sparkii auth add openai-codex` |
| qwen-oauth | OAuth | `sparkii auth add qwen-oauth` |
| minimax-oauth | OAuth | `sparkii auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`sparkii auth`).
Fallback chain when the primary fails: `sparkii fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`sparkii_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   sparkii config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
