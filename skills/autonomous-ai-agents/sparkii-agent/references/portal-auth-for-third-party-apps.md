...
## Layer 1 — Is this thing a Sparkii plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Sparkii process**. Its LLM calls go through Sparkii's already-configured provider. | Already uses Portal if user's Sparkii is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Sparkii. That LLM call is already authenticated
via Sparkii's provider config. The plugin's own server auth (e.g.
...
What Sparkii Agent has that *feels* like OAuth — `sparkii login --provider nous`
opening a browser, user signs in, token lands in `~/.sparkii/auth.json` — is a
**Sparkii-specific browser flow**. Under the hood it produces a credential
Sparkii uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Sparkii's existing Portal credential out of `~/.sparkii/auth.json`.
...
rides on the credential Sparkii already holds.

Where this could live in Sparkii:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Sparkii itself.
...
1. First decide: Sparkii plugin (runs inside Sparkii) or separate app? If plugin,
   it already uses Portal via Sparkii's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Sparkii agent itself") is fully supported. Sparkii treats agent
...
pitch in the Tool Gateway announcement is specifically about Sparkii Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Sparkii is configured to use Portal as its provider. It is
