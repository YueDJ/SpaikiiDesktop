...
for the platform plumbing. The Sparkii-side `computer_use` tool exposed
in this skill is a higher-level Sparkii vocabulary; the raw cua-driver
...
Normal Sparkii sessions use `standard`, which requires a certified protected
host and fails closed when Sparkii has none. Explicit Sparkii YOLO (`--yolo`,
...
When `cua-driver skills install` autodetects Sparkii (planned follow-up
