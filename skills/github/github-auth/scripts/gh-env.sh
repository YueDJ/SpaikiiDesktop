#!/usr/bin/env bash
# GitHub environment detection helper for Sparkii Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _sparkii_env="${SPARKII_HOME:-$HOME/.sparkii}/.env"; [ -f "$_sparkii_env" ] && grep -q "^GITHUB_TOKEN=" "$_sparkii_env" 2>/dev/null; then
