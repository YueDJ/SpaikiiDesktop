...
SPARKII_HOME_DIR="${SPARKII_HOME:-$HOME/.sparkii}"
SPARKII_CFG="${SPARKII_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Sparkii config has twozero_td MCP entry ──
if [[ ! -f "$SPARKII_CFG" ]]; then
    echo -e " ${FAIL} Sparkii config not found at ${SPARKII_CFG}"
    manual_steps+=("Create ${SPARKII_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SPARKII_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Sparkii config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Sparkii config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SPARKII_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SPARKII_CFG} manually"); }
    manual_steps+=("Restart Sparkii session to pick up config change")
fi
