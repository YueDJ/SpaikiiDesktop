...
## 5. Sparkii Configuration

### Config location

`$SPARKII_HOME/config.yaml` (defaults to `~/.sparkii/config.yaml` when `SPARKII_HOME` is unset)
...
Restart the Sparkii session for changes to take effect. The MCP connection is
