import type { RunExternalProcess } from '@sparkii/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`sparkii ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching sparkii: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`sparkii ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
