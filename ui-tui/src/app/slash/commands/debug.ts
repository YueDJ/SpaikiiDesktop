...
import { terminalBackgroundHex } from '@sparkii/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SPARKII_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SPARKII_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SPARKII_TUI_BACKGROUND', process.env.SPARKII_TUI_BACKGROUND ?? '(unset)'],
            ['SPARKII_TUI_THEME', process.env.SPARKII_TUI_THEME ?? '(unset)'],
