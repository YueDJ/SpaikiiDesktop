...
import { Box, Text } from '@sparkii/ink'
...
/**
 * User widget apps — Sparkii authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SPARKII_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.sparkii/plugins/`: files under SPARKII_HOME execute
...
const widgetsDir = () => join(process.env.SPARKII_HOME?.trim() || join(homedir(), '.sparkii'), 'tui-widgets')
 ...
 *  every change, so a widget Sparkii writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Sparkii) ever writes must hot-load too — a 10s poll here read
