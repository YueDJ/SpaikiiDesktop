...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SPARKII_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Sparkii on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SPARKII_TUI_TRUECOLOR=1.
