...
const logDir = join(process.env.SPARKII_HOME?.trim() || join(homedir(), '.sparkii'), 'logs')
...
// a real ~/.sparkii (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('sparkii-tui: parent lifecycle log unavailable\n')
    }
  }
}
