 ...
 * `sparkii-boot-background` / `sparkii-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SPARKII_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.sparkii matches get_sparkii_home()'s default.
const bootFilePath = () => join(process.env.SPARKII_HOME ?? join(homedir(), '.sparkii'), 'tui-theme-boot.json')

// Never touch the user's real ~/.sparkii from test runs (the TS suite has no
// SPARKII_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SPARKII_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SPARKII_TUI_THEME || env.SPARKII_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SPARKII_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SPARKII_TUI_BACKGROUND
  ) {
    env.SPARKII_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SPARKII_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SPARKII_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SPARKII_TUI_THEME.
