import type { InputEvent, Key } from '@sparkii/ink'
import * as Ink from '@sparkii/ink'
 ...
 * but that accepts `key.meta` on macOS — and sparkii-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SPARKII_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which sparkii-ink decodes as meta+'d'; without this branch it
