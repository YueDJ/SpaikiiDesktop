...
React + Ink terminal UI for Sparkii. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SPARKII_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `sparkii setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.sparkii/.sparkii_history` or under `SPARKII_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `sparkii setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/sparkii-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external sparkii setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.sparkii/tui-parent.log
    ...
    types/
      sparkii-ink.d.ts            type declarations for @sparkii/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SparkiiCLI subprocess for slash commands
```
