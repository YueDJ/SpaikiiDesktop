"""ACP agent server — exposes Sparkii Agent via the Agent Client Protocol."""
...
try:
    from sparkii_cli import __version__ as SPARKII_VERSION
except Exception:
    SPARKII_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Sparkii running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Sparkii cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Sparkii/OpenAI-compatible user content payload."""
...
class SparkiiACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Sparkii AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Sparkii version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Sparkii maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Sparkii inventory is also used by ``sparkii model``, the TUI,
        ...
        try:
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from sparkii_cli.models import _configured_custom_provider_ids
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Sparkii estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        # sparkii_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["sparkii-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="sparkii-agent", version=SPARKII_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Sparkii' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Sparkii setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.sparkii`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"sparkii": {"compactionSummary": True}}
        if kind == "merged":
            return {"sparkii": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Sparkii
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Sparkii on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Sparkii' local kawaii waiting/status
        ...
        # callback shape — not the gateway-queue SPARKII_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind SPARKII_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Sparkii workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.sparkii/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global SPARKII_CRON_SESSION (#37968).
            ...
            previous_session_id = os.environ.get("SPARKII_SESSION_ID")
            ...
            try:
                ...
                if previous_session_id is None:
                    ...
                    os.environ["SPARKII_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Sparkii DB session id before the turn so we
        ...
        # Sparkii' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Sparkii install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Sparkii Agent v{SPARKII_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Sparkii has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
