"""ACP auth helpers — detect and advertise Sparkii authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "sparkii-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Sparkii runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Sparkii can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Sparkii.
    ...
    not have Sparkii provider credentials configured yet, so Sparkii always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Sparkii using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Sparkii provider",
            description=(
                "Open Sparkii' interactive model/provider setup in a terminal. "
                "Use this when Sparkii has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
