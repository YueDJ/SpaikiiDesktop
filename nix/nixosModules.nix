# nix/nixosModules.nix — NixOS module for sparkii-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.sparkii-agent;
    effectivePackage =
    ...
    sparkii-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;
    ...
    generatedConfigFile = pkgs.writeText "sparkii-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # SPARKII_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "sparkii-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SPARKII_DOC_EOF'\n${value}\nSPARKII_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    containerEntrypoint = pkgs.writeShellScript "sparkii-container-entrypoint" ''
      ...
      SPARKII_UID="''${SPARKII_UID:?SPARKII_UID must be set}"
      SPARKII_GID="''${SPARKII_GID:?SPARKII_GID must be set}"

      # ── Group: ensure a group with GID=$SPARKII_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SPARKII_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        ...
        if command -v groupadd >/dev/null 2>&1; then
          ...
          addgroup -g "$SPARKII_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SPARKII_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SPARKII_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          ...
          adduser -u "$SPARKII_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SPARKII_UID:$SPARKII_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SPARKII_HOME is owned by the target user.
      ...
      if [ -n "''${SPARKII_HOME:-}" ] && [ -d "$SPARKII_HOME" ]; then
        find "$SPARKII_HOME" \! -user "$SPARKII_UID" -exec chown "$SPARKII_UID:$SPARKII_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/sparkii-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SPARKII_UID" --regid="$SPARKII_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SPARKII_HOME/.env.
    ...
    # Default: /var/lib/sparkii/workspace → /data/workspace.
  ...
  in {
    options.services.sparkii-agent = with lib; {
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SPARKII_HOME/.env
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.sparkii-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.sparkii-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.sparkii/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sparkii/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."sparkii-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.sparkii/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.sparkii/$_subdir"
            chmod 2770 "${cfg.stateDir}/.sparkii/$_subdir"
            find "${cfg.stateDir}/.sparkii/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # sparkii-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.sparkii/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.sparkii/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sparkii/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.sparkii/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.sparkii/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sparkii/.managed
          chmod 0644 ${cfg.stateDir}/.sparkii/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.sparkii/.container-mode <<'SPARKII_CONTAINER_MODE_EOF'
    ...
    SPARKII_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.sparkii/.container-mode
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "sparkii-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sparkii/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.sparkii/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sparkii/auth.json
              fi
            ''}
          ''}
          ...
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ...
            cat > "$ENV_FILE" <<'SPARKII_NIX_ENV_EOF'
    ${envFileContent}
    SPARKII_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.sparkii/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.sparkii/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sparkii/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
        ...
        systemd.services.sparkii-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SPARKII_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SPARKII_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
