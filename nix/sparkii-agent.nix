...
let
  ...
  # lib.nix) — the wrapper exposes them via SPARKII_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from SPARKII_BUNDLED_PLUGINS.
  ...
  # at by SPARKII_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via SPARKII_OPTIONAL_MCPS.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sparkii-agent $out/bin
    ln -s ${bundledSkills} $out/share/sparkii-agent/skills
    ln -s ${bundledOptionalSkills} $out/share/sparkii-agent/optional-skills
