...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Sparkii derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            sparkii-agent = prev.sparkii-agent.overrideAttrs (_old: {
              SPARKII_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $SPARKII_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SPARKII_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        sparkii-agent = prev.sparkii-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "sparkii-agent-env" {
    sparkii-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "sparkii-agent-editable-env" {
    sparkii-agent = dependency-groups;
  };
}
