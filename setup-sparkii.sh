...
# 5. Symlinks the 'sparkii' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Sparkii Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sparkii-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink sparkii into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sparkii command..."

SPARKII_BIN="$SCRIPT_DIR/venv/bin/sparkii"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SPARKII_BIN" "$COMMAND_LINK_DIR/sparkii"
echo -e "${GREEN}✓${NC} Symlinked sparkii → $COMMAND_LINK_DISPLAY_DIR/sparkii"
...
# ============================================================================
# Seed bundled skills into ~/.sparkii/skills/
# ============================================================================

SPARKII_SKILLS_DIR="${SPARKII_HOME:-$HOME/.sparkii}/skills"
mkdir -p "$SPARKII_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sparkii/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SPARKII_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sparkii status        # Check configuration"
if is_termux; then
    echo "  sparkii gateway       # Run gateway in foreground"
else
    echo "  sparkii gateway install # Install gateway service (messaging + cron)"
fi
echo "  sparkii cron list     # View scheduled jobs"
echo "  sparkii doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sparkii_cli.main setup
fi
