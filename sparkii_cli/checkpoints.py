"""`sparkii checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sparkii/checkpoints/``.  Actions:

    sparkii checkpoints               # same as `status`
    sparkii checkpoints status        # total size, project count, breakdown
    sparkii checkpoints list          # per-project checkpoint counts + workdir
    sparkii checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sparkii checkpoints clear [-f]    # nuke the entire base (asks first)
    sparkii checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sparkii checkpoints
    sparkii checkpoints prune --retention-days 3 --max-size-mb 200
    sparkii checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sparkii checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sparkii checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sparkii checkpoints` → status
