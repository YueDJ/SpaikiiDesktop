"""
Unified self-relaunch for Sparkii CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``sparkii sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``sparkii`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from sparkii_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched sparkii."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_sparkii_bin() -> Optional[str]:
    """Find the sparkii entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("sparkii")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m sparkii_cli.main``.
    ...
    ``sparkii.exe`` on PATH or the ``sys.executable -m sparkii_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with sparkii.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "sparkii_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh sparkii invocation.
    ...
    relaunch contract wants: "run sparkii again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``sparkii.exe`` (a Python console-script shim that wraps
    ``python -m sparkii_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "sparkii exited, then
    new sparkii started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``sparkii`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nSparkii relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run sparkii.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
