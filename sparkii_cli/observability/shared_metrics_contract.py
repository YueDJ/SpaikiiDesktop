"""Bounded product contract for the first Sparkii shared-metrics slice."""
...
SCHEMA_KEY = "sparkii.metrics.schema_version"
SCHEMA_VERSION = "sparkii.metrics.event.v2"
MODEL_CALL_SCOPE = "sparkii.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "sparkii.task_run"
TOOL_CALL_SCOPE = "sparkii.tool_call"
CLIENT_ACTIVE_MARK = "sparkii.client.active"
TOOL_APPROVAL_MARK = "sparkii.tool_approval"
SKILL_LIFECYCLE_MARK = "sparkii.skill.lifecycle"
SKILL_LOAD_MARK = "sparkii.skill.load"
SUBSCRIBER_NAME = "sparkii.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "sparkii.client.active"
LEGACY_MODEL_CALL_METRIC = "sparkii.model_call.count"
MODEL_ROUTE_METRIC = "sparkii.model_route.count"
TASK_STARTED_METRIC = "sparkii.task_run.started"
TASK_FINISHED_METRIC = "sparkii.task_run.finished"
TOOL_CALL_METRIC = "sparkii.tool_call.count"
TOOL_APPROVAL_METRIC = "sparkii.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "sparkii.skill.lifecycle.count"
SKILL_LOAD_METRIC = "sparkii.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Sparkii installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(sparkii_version or "").strip()
    ...
    return {
        ...
        "sparkii_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("sparkii_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Sparkii logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("sparkii.call_role")
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Sparkii terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Sparkii registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Sparkii tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Sparkii."""
