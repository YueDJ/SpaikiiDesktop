"""sparkii hooks — inspect and manage shell-script hooks.

Usage::

    sparkii hooks list
    sparkii hooks test <event> [--for-tool X] [--payload-file F]
    sparkii hooks revoke <command>
    sparkii hooks doctor

Consent records live under ``~/.sparkii/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.sparkii/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``sparkii hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: sparkii hooks {list|test|revoke|doctor}")
        print("Run 'sparkii hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.sparkii/config.yaml.")
        print("See `sparkii hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.sparkii/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `sparkii hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `sparkii hooks test` and `sparkii hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `sparkii hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `sparkii hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `sparkii hooks doctor`.")
