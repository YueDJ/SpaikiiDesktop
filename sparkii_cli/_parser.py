"""
Top-level argparse construction for the sparkii CLI.
...
# argparse runs (it sets ``SPARKII_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``sparkii_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    sparkii                        Start interactive chat
    sparkii chat -q "Hello"        Single query mode
    sparkii --tui                  Launch the modern TUI (or set display.interface: tui)
    sparkii --cli                  Force the classic REPL (overrides display.interface: tui)
    sparkii -c                     Resume the most recent session
    sparkii -c "my project"        Resume a session by name (latest in lineage)
    sparkii --resume <session_id>  Resume a specific session by ID
    sparkii --resume latest        Resume the most recent session (same as -c)
    sparkii --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    sparkii setup                  Run setup wizard
    sparkii logout                 Clear stored authentication
    sparkii auth add <provider>    Add a pooled credential
    sparkii auth list              List pooled credentials
    sparkii auth remove <p> <t>    Remove pooled credential by index, id, or label
    sparkii auth reset <provider>  Clear exhaustion status for a provider
    sparkii model                  Select default model
    sparkii fallback [list]        Show fallback provider chain
    sparkii fallback add           Add a fallback provider (same picker as `sparkii model`)
    sparkii fallback remove        Remove a fallback provider from the chain
    sparkii config                 View configuration
    sparkii config edit            Edit config in $EDITOR
    sparkii config set model gpt-4 Set a config value
    sparkii gateway                Run messaging gateway
    sparkii -s sparkii-agent-dev,github-auth
    sparkii -w                     Start in isolated git worktree
    sparkii gateway install        Install gateway background service
    sparkii sessions list          List past sessions
    sparkii sessions browse        Interactive session picker
    sparkii sessions rename ID T   Rename/title a session
    sparkii logs                   View agent.log (last 50 lines)
    sparkii logs -f                Follow agent.log in real time
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="sparkii",
        description="Sparkii Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `sparkii chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SPARKII_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `sparkii setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SPARKII_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.sparkii/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Sparkii Agent",
    )
    ...
    # top-level parser: when the user writes `sparkii -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/sparkii_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SPARKII_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.sparkii/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
