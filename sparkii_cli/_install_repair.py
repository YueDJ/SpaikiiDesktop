...
- ``sparkii_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``sparkii_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``sparkii_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``sparkii_cli.main`` imports
``managed_uv``, ``sparkii_constants``, and friends only in its late path; the
...
:func:`sparkii_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from sparkii_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (sparkii_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``sparkii acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # sparkii_constants is stdlib-only, so the canonical layout helpers are safe
...
def _quarantine_running_sparkii_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Rename live sparkii*.exe shims aside so the installer can rewrite them.
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    moved = _quarantine_running_sparkii_exe(scripts_dir) if scripts_dir else []
    try:
        ...
        # the quarantined shims renamed aside and `sparkii` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``sparkii*.exe`` shims on Windows so they can be replaced
