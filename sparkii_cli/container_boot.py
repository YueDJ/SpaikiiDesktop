...
``$SPARKII_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-sparkii-setup
(the stage2 hook) has chowned the volume and seeded $SPARKII_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$SPARKII_HOME``,
    not under ``profiles/``). The dispatcher in ``sparkii_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``sparkii gateway start`` (no ``-p``) targets. Without it,
    bare ``sparkii gateway start`` inside the container would land on
    ...
    ``$SPARKII_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    # ``sparkii gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `sparkii profile create` (config.yaml
            # is not — that comes later via `sparkii setup`). Use it as the
            ...
            # via ``sparkii -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("SPARKII_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the sparkii args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/sparkii/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/sparkii/docker/main-wrapper.sh
    ...
    ``sparkii`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``sparkii <subcommand>``, so an
    explicit leading ``sparkii`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `sparkii <subcommand>`; peel an explicit sparkii.
    if args and Path(args[0]).name == "sparkii":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``sparkii dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted SPARKII_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `sparkii -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit sparkii
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the sparkii user) won't hit EACCES. See
...
def main() -> int:
    ...
    # bind-mounted SPARKII_HOME, both race to flock() the same s6-log lock
