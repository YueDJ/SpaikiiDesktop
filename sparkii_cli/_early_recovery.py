"""Dependency-light venv recovery that runs BEFORE sparkii_cli.main's imports.
...
a corrupted venv.  ``sparkii_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``sparkii_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Sparkii already vendors
    (``~/.sparkii/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".sparkii" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``sparkii acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``sparkii_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``sparkii_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``sparkii update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `sparkii update`: CLI and Desktop retries preserve
...
# every `sparkii` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``sparkii update`` (or the self-lock preflight, #83569) left the
    ...
    ``sparkii_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``sparkii acp`` included).
    ...
    """
    try:
        from sparkii_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; sparkii will keep working from "
