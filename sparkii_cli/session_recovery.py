"""Offline, non-destructive recovery for a damaged Sparkii session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``sparkii_cli.sqlite_safe_read`` exists to
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="sparkii-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Sparkii process using this profile and retry. "
                "This includes the interactive `sparkii` CLI session this "
                ...
                "Sparkii process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `sparkii` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from sparkii_cli.session_lost_and_found import (
