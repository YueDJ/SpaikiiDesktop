...
def create_task(
    ...
    *,
    ...
    if project_id:
        from sparkii_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from sparkii_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from sparkii_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from sparkii_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from sparkii_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
