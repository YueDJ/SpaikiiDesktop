"""
Sparkii Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sparkii/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Sparkii PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing sparkii-agent or sparkii PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Sparkii Agent" comment and following line
                if '# Sparkii Agent' in line or '# sparkii-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('sparkii' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing sparkii
                if 'sparkii' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Sparkii-owned file, and
...
def remove_wrapper_script():
    """Remove the sparkii wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "sparkii",
        Path.home() / ".local" / "bin" / "sparkii-acp",
        Path.home() / ".local" / "bin" / "sparkii-agent",
        Path("/usr/local/bin/sparkii"),
        Path("/usr/local/bin/sparkii-acp"),
        Path("/usr/local/bin/sparkii-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains sparkii_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'sparkii_cli' in content or 'sparkii-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``sparkii`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Sparkii home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sparkii gateway run`` processes
    ...
    try:
        from sparkii_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sparkii_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SPARKII_HOME`` and ``SPARKII_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\sparkii\git\cmd``, ``%LOCALAPPDATA%\sparkii\git\bin``,
#      ``%LOCALAPPDATA%\sparkii\git\usr\bin``, ``%LOCALAPPDATA%\sparkii\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\sparkii\git\`` and Node to
#      ``%LOCALAPPDATA%\sparkii\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``sparkii dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\sparkii\gateway-service\`` and sometimes
...
def remove_hermes_env_vars_windows() -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SPARKII_HOME", "SPARKII_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SPARKII_HOME directory.

    We shell out to ``sparkii -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SPARKII_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's SPARKII_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``sparkii uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SPARKII_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from sparkii_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ Sparkii Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Sparkii agent is still installed. Run 'sparkii' to use the CLI,")
    print("or 'sparkii uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sparkii/ (configs, data, logs)
    ...
    # full wipe (code + ~/.sparkii data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Sparkii Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their SPARKII_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Sparkii data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Sparkii code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Sparkii PATH entries from shell configs / Windows User PATH")
    print("  • Sparkii wrapper scripts and Sparkii-managed node/npm/npx symlinks")
...
def _perform_uninstall(
    *,
    ...
    ``sparkii`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SPARKII_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_env:
            ...
            log_info("No Sparkii-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing sparkii command...")
    ...
    if removed_node_links:
        ...
        log_info("No Sparkii-managed node/npm/npx symlinks found")
    ...
    # 5. Optionally remove ~/.sparkii/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SPARKII_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://sparkii-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://sparkii-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Sparkii Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m sparkii_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import sparkii_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``sparkii_constants`` + ``sparkii_cli.colors``
    (and lazily ``sparkii_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m sparkii_cli.uninstall")
    parser.add_argument(
        "--mode",
