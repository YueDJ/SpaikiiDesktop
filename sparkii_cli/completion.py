"""Shell completion script generation for sparkii CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_sparkii_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Sparkii Agent bash completion
# Add to ~/.bashrc:
#   eval "$(sparkii completion bash)"

_sparkii_profiles() {{
    local profiles_dir="$HOME/.sparkii/profiles"
...
_sparkii_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sparkii_profiles)" -- "$cur"))
        return
    fi
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef sparkii
# Sparkii Agent zsh completion
# Add to ~/.zshrc:
...
_sparkii_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.sparkii/profiles" ]]; then
        profiles+=($HOME/.sparkii/profiles/*(N/:t))
    fi
...
_sparkii() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_sparkii_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'sparkii command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Sparkii Agent fish completion",
        "# Add to your config:",
        "#   sparkii completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __sparkii_profiles",
        "    echo default",
        "    if test -d $HOME/.sparkii/profiles",
        "        for d in $HOME/.sparkii/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c sparkii -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c sparkii -f -s p -l profile"
        " -d 'Profile name' -xa '(__sparkii_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__sparkii_profiles)' -d 'Profile name'"
                )
