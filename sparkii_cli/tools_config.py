...
from sparkii_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from sparkii_cli.nous_account import format_nous_portal_entitlement_message
...
from sparkii_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from sparkii_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from sparkii_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from sparkii_cli.plugins import get_plugin_toolset_keys_nowait
...
from sparkii_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    ...
    A binary selected by ``SPARKII_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Sparkii' runtime contract."""
    import time
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    override = os.environ.get("SPARKII_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    SPARKII_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Sparkii' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Sparkii' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `sparkii update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Sparkii cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("SPARKII_CUA_DRIVER_CMD", "").strip():
            _print_info(
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: sparkii computer-use doctor")
            return False
    if ok and before:
        try:
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Sparkii-managed ``$SPARKII_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Sparkii installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Sparkii-managed install)
        ...
        # (PATH -> Homebrew/Sparkii-managed node -> npx) rather than a bare
        # shutil.which — Sparkii-managed-Node-only setups resolve agent-browser
        ...
        # Sparkii-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Sparkii-managed-Node-only setups resolve npx only through the
...
def _get_platform_tools(
    ...
    *,
    ...
    # explicit config like ``[sparkii-cli, a2a]`` must survive filtering just
    ...
    # JSON-array string (e.g. "['memory']") from `sparkii config set` or a
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from sparkii_constants import sparkii_home_key

    scope = sparkii_home_key()
...
# Python dependencies installed explicitly through ``sparkii tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``sparkii tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from sparkii_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``sparkii tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from sparkii_cli.nous_subscription import _has_agent_browser
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `sparkii model`).
    if managed_feature:
        from sparkii_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
