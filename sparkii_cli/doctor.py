"""
Doctor command for sparkii CLI.

Diagnoses issues with Sparkii Agent setup.
"""
...
_DHH = display_sparkii_home()  # user-facing display path (e.g. ~/.sparkii or ~/.sparkii/profiles/coder)
...
from sparkii_constants import is_termux as _is_termux
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``sparkii_cli.sqlite_safe_read``).
...
def _format_db_size(db_path: Path) -> str:
    ...
    from sparkii_cli.backup import _format_size
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.sparkii/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SPARKII_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from sparkii_cli.auth import get_minimax_oauth_auth_status
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'sparkii sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. SPARKII_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("SPARKII_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("SPARKII_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches sparkii_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``sparkii --version`` reporting a stale version while
    ...
    """
    try:
        from sparkii_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != sparkii_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'sparkii update', or set "
            "sparkii_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-sparkii and dashboard static services are up
    ...
    for static in ("main-sparkii", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `sparkii profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `sparkii doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from sparkii_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the SPARKII_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("SPARKII_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via SPARKII_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `sparkii`.
    os.environ.setdefault("SPARKII_INTERACTIVE", "1")

    # Handle `sparkii doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.sparkii/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`sparkii doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from sparkii_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
    ...
    # Detect drift between pyproject.toml and sparkii_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.sparkii/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        # matches sparkii_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'sparkii setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'sparkii setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'sparkii setup' to create one")
                issues.append("Run 'sparkii setup' to create .env")
    
    # Check ~/.sparkii/config.yaml (primary) or project cli-config.yaml (fallback)
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'sparkii config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from sparkii_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.sparkii/.env or run 'sparkii setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'sparkii setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'sparkii config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'sparkii setup' to migrate config")
                else:
                    issues.append("Run 'sparkii doctor --fix' or 'sparkii setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'sparkii doctor --fix'")
        except Exception:
            pass

        # Detect stale SPARKII_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives SPARKII_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("SPARKII_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"SPARKII_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `sparkii setup` run)",
                )
                if should_fix:
                    if remove_env_value("SPARKII_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale SPARKII_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove SPARKII_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the SPARKII_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale SPARKII_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'sparkii doctor --fix'"
                    )
        except Exception:
            pass
    ...
    if state_db_path.exists():
        try:
            ...
            from sparkii_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'sparkii doctor --fix' "
                        "(or 'sparkii sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from sparkii_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'sparkii doctor --fix' "
                        "(or 'sparkii sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            from sparkii_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'sparkii sessions optimize-storage' "
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'sparkii doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "sparkii"
        ...
        _cmd_link = _cmd_link_dir / "sparkii"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(sparkii not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/sparkii → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/sparkii points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/sparkii → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/sparkii — run 'sparkii doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/sparkii exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/sparkii not found",
                    "(sparkii command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/sparkii → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/sparkii symlink — run 'sparkii doctor --fix'")
    ...
    try:
        from sparkii_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'sparkii-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `sparkii update` does, so a session's first browser call
            ...
            # `sparkii update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `sparkii doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable SPARKII_HOME mirror
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'sparkii config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _SPARKII_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``sparkii doctor``.
    ...
    # never the legitimate source for `sparkii doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'sparkii setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = SPARKII_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: sparkii memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: sparkii memory setup",
                    "No Honcho API key — run 'sparkii memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run sparkii memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: sparkii memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: sparkii memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from sparkii_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "sparkii -p" in content:
                            _m = _re.search(r"sparkii -p (\S+)", content)
    ...
    # `sparkii doctor --live` (real network calls; bounded + read-only).
    try:
        from sparkii_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'sparkii doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
