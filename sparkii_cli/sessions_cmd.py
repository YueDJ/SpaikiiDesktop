"""``sparkii sessions`` command — extracted from ``sparkii_cli/main.py``.
...
Helpers that stay in ``sparkii_cli.main`` (``get_sparkii_home``,
...
``sparkii_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_sparkii_home():
    return _m().get_sparkii_home()
...
#: Default age floor for `sparkii sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`sparkii sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from sparkii_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    sparkii sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from sparkii_cli.session_recovery import (
    ...
    if action == "import":
        from sparkii_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from sparkii_state import workspace_key as _ws_key
        ...
        from sparkii_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from sparkii_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_sparkii_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from sparkii_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_sparkii_home() / "session-exports"
        ...
        # `sparkii sessions prune`: no time window and no filters at all
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `sparkii sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: sparkii sessions pin <session_id>"
            )
            return
        ...
        # Launch sparkii --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`sparkii sessions optimize` later to reclaim freed space.)")
