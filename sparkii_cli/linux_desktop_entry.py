"""Install and remove the Linux desktop entry (``sparkii.desktop``).

``sparkii desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "sparkii.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``sparkii.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    ...
    if bin_path:
        ...
        argv = [str(Path(sys.executable).resolve()), "-m", "sparkii_cli.main", "desktop"]
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "sparkii"
