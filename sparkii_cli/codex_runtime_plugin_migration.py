"""Migrate Sparkii' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Sparkii' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Sparkii mcp_servers.<n>.command/args/env  → codex stdio transport
  Sparkii mcp_servers.<n>.url/headers       → codex streamable_http transport
  Sparkii mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Sparkii mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Sparkii-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Sparkii' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Sparkii config.")
...
# Sparkii keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Sparkii' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Sparkii key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SPARKII_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Sparkii)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Sparkii' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Sparkii' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Sparkii
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="sparkii-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SPARKII_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed sparkii-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.sparkii`` symlink that happens to live under ``/private/var/folders``
...
def _build_sparkii_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Sparkii' own
    ...
    so a sparkii installed under /opt/, /usr/local/, or a venv all work.
    SPARKII_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SPARKII_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_sparkii_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SPARKII_HOME its launcher sets
    ...
    # — that would override the launcher's SPARKII_HOME and pin the subprocess
    ...
    env["SPARKII_QUIET"] = "1"
    env["SPARKII_REDACT_SECRETS"] = env.get("SPARKII_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_sparkii_tools: bool = True,
) -> MigrationReport:
    """Translate Sparkii mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.
    ...
    Args:
        ...
        expose_sparkii_tools: when True (default), register Sparkii' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Sparkii for tools
    ...
    # Inject Sparkii' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Sparkii for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/sparkii_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_sparkii_tools:
        translated["sparkii-tools"] = _build_sparkii_tools_mcp_entry()
        if "sparkii-tools" not in report.migrated:
            report.migrated.append("sparkii-tools")
