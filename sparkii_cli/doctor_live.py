"""``sparkii doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from sparkii_cli.doctor import SPARKII_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (SPARKII_HOME / "node" / "bin",
                          SPARKII_HOME / "node",
                          SPARKII_HOME / "node_modules" / ".bin"):
    ...
    # sparkii_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``sparkii mcp test``.
    """
