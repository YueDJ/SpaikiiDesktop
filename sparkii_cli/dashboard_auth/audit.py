...
Profile-aware location: ``$SPARKII_HOME/logs/dashboard-auth.log``.
...
from ``sparkii_constants`` or other sparkii_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SPARKII_HOME/logs/dashboard-auth.log``.

    Uses ``sparkii_constants.get_sparkii_home()`` (a leaf module — no import
    cycle) so profile overrides and the native-Windows ``%LOCALAPPDATA%``
    fallback are honored.
    """
    from sparkii_constants import get_sparkii_home

    return get_sparkii_home() / "logs" / "dashboard-auth.log"
