...
from sparkii_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from sparkii_cli.banner import check_for_updates
...
# Update pipeline extracted to sparkii_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (sparkii_cli.main._cmd_update_impl,
# sparkii_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from sparkii_cli.update_cmd import (  # noqa: F401
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the SPARKII_DESKTOP_DISABLE_GPU env var the Electron app
...
# Dashboard process-hygiene helpers extracted to sparkii_cli/dashboard_procs.py
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from sparkii_constants import venv_bin_dir
...
def _quarantine_running_hermes_exe(
    ...
    3. Print a clear warning naming the most likely culprit (running
       Sparkii Desktop / gateway / REPL) and pointing to ``--force``.
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sparkii.exe.old.*`` left by prior updates.
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from sparkii_cli.profiles import rename_profile
