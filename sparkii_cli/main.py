#!/usr/bin/env python3
"""
Sparkii CLI - Main entry point.

Usage:
    sparkii                     # Interactive chat (default)
    sparkii chat                # Interactive chat
    sparkii gateway             # Run gateway in foreground
    sparkii gateway start       # Start gateway as service
    sparkii gateway stop        # Stop gateway service
    sparkii gateway status      # Show gateway status
    sparkii gateway install     # Install gateway service
    sparkii gateway uninstall   # Uninstall gateway service
    sparkii setup               # Interactive setup wizard
    sparkii logout              # Clear stored authentication
    sparkii status              # Show status of all components
    sparkii cron                # Manage cron jobs
    sparkii cron list           # List cron jobs
    sparkii cron status         # Check if cron scheduler is running
    sparkii doctor              # Check configuration and dependencies
    sparkii honcho setup                    # Configure Honcho AI memory integration
    sparkii honcho status                   # Show Honcho config and connection status
    sparkii honcho sessions                 # List directory → session name mappings
    sparkii honcho map <name>               # Map current directory to a session name
    sparkii honcho peer                     # Show peer names and dialectic settings
    sparkii honcho peer --user NAME         # Set user peer name
    sparkii honcho peer --ai NAME           # Set AI peer name
    sparkii honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sparkii honcho mode                     # Show current memory mode
    sparkii honcho mode [hybrid|honcho|local]  # Set memory mode
    sparkii honcho tokens                   # Show token budget settings
    sparkii honcho tokens --context N       # Set session.context() token cap
    sparkii honcho tokens --dialectic N     # Set dialectic result char cap
    sparkii honcho identity                 # Show AI peer identity representation
    sparkii honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sparkii honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sparkii + Honcho
    sparkii version             Show version
    sparkii update              Update to latest version
    sparkii uninstall           Uninstall Sparkii Agent
    sparkii acp                 Run as an ACP server for editor integration
    sparkii sessions browse     Interactive session picker with search

    sparkii claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: sparkii_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``sparkii_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``sparkii update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``sparkii_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, sparkii crashes on import and the user can't run
# ``sparkii update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# Two lines of inline path math so ``python sparkii_cli/main.py`` (script
# mode — sys.path[0] is sparkii_cli/, not the repo root) can import the
# canonical helpers; everything else lives in sparkii_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from sparkii_cli import _startup_fast  # noqa: E402
...
# package directory, so if IT can't import, nothing else in sparkii_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from sparkii_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from sparkii_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'sparkii' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``sparkii tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``sparkii.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("sparkii")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"sparkii", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"sparkii")
        # Windows: the .exe name is already ``sparkii.exe`` — nothing to do.
    except Exception:
        pass
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SPARKII_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".sparkii", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SPARKII_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``sparkii … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SPARKII_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SPARKII_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SPARKII_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`sparkii --tui … >log`, CI capture):
...
def _try_ultrafast_version() -> bool:
    """Handle ``sparkii --version`` before config/logging imports."""
...
from sparkii_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from sparkii_cli.subcommands.cron import build_cron_parser
from sparkii_cli.subcommands.sync import build_sync_parser
from sparkii_cli.subcommands.gateway import build_gateway_parser
from sparkii_cli.subcommands.profile import build_profile_parser
from sparkii_cli.subcommands.model import build_model_parser
from sparkii_cli.subcommands.setup import build_setup_parser

from sparkii_cli.subcommands.whatsapp import build_whatsapp_parser
from sparkii_cli.subcommands.slack import build_slack_parser
from sparkii_cli.subcommands.login import build_login_parser
from sparkii_cli.subcommands.logout import build_logout_parser
from sparkii_cli.subcommands.auth import build_auth_parser
from sparkii_cli.subcommands.status import build_status_parser
from sparkii_cli.subcommands.pause import build_pause_parser
from sparkii_cli.subcommands.webhook import build_webhook_parser
from sparkii_cli.subcommands.hooks import build_hooks_parser
from sparkii_cli.subcommands.doctor import build_doctor_parser
from sparkii_cli.subcommands.verify import build_verify_parser
from sparkii_cli.subcommands.security import build_security_parser
from sparkii_cli.subcommands.approvals import build_approvals_parser
from sparkii_cli.subcommands.dump import build_dump_parser
from sparkii_cli.subcommands.debug import build_debug_parser
from sparkii_cli.subcommands.backup import build_backup_parser
from sparkii_cli.subcommands.import_cmd import build_import_cmd_parser
from sparkii_cli.subcommands.import_agent import build_import_agent_parser
from sparkii_cli.subcommands.config import build_config_parser
from sparkii_cli.subcommands.skin import build_skin_parser
from sparkii_cli.subcommands.console import build_console_parser
from sparkii_cli.subcommands.version import build_version_parser
from sparkii_cli.subcommands.update import build_update_parser
from sparkii_cli.subcommands.uninstall import build_uninstall_parser
from sparkii_cli.subcommands.dashboard import build_dashboard_parser
from sparkii_cli.subcommands.gui import build_gui_parser
from sparkii_cli.subcommands.logs import build_logs_parser
from sparkii_cli.subcommands.prompt_size import build_prompt_size_parser
from sparkii_cli.subcommands.memory import build_memory_parser
from sparkii_cli.subcommands.acp import build_acp_parser
from sparkii_cli.subcommands.tools import build_tools_parser
from sparkii_cli.subcommands.insights import build_insights_parser
from sparkii_cli.subcommands.monitoring import build_monitoring_parser
from sparkii_cli.subcommands.skills import build_skills_parser
from sparkii_cli.subcommands.pairing import build_pairing_parser
from sparkii_cli.subcommands.plugins import build_plugins_parser
from sparkii_cli.subcommands.mcp import build_mcp_parser
from sparkii_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (sparkii tools, sparkii setup, sparkii model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sparkii {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any sparkii module import.
#
# Many modules cache SPARKII_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SPARKII_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.sparkii/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SPARKII_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `sparkii mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Sparkii' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo sparkii -p <name>` against the invoking user's home.
        ...
        candidate = home / ".sparkii" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`sparkii chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors sparkii_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SPARKII_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.sparkii/profiles/coder or
    # /opt/data/profiles/coder).  If SPARKII_HOME points to the sparkii root
    # instead (e.g. systemd hardcodes SPARKII_HOME=/root/.sparkii), we must
    ...
    # run-script as SPARKII_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``sparkii gateway run`` to mean
    # "the root SPARKII_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set SPARKII_HOME
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent sparkii from starting
            print(
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "SPARKII_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SPARKII_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `sparkii` subcommands
...
try:
    from sparkii_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from sparkii_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if sparkii_constants not importable yet
...
# Provider model-selection wizard flows extracted to sparkii_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (sparkii_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_sparkii_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SPARKII_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SPARKII_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    auth_file = get_sparkii_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _session_browse_picker(sessions: list, session_db=None) -> Optional[str]:
    ...
    def _delete_session(session_id: str) -> bool:
        if session_db is None:
            return False
        try:
            sessions_dir = get_sparkii_home() / "sessions"
        except Exception:
            sessions_dir = None
        try:
            return bool(
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``sparkii -c`` from repo A continues repo A's last session rather than the
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sparkii {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sparkii {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'sparkii sessions list' to see available sessions, or "
            ...
            try:
                from sparkii_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  sparkii --tui --resume {target}")
    if title:
        print(f'  sparkii --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sparkii/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@sparkii" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/sparkii-ink/package.json",
    "packages/sparkii-ink/index.js",
    "packages/sparkii-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SPARKII_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SPARKII_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SPARKII_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SPARKII_SKIP_NODE_BOOTSTRAP"):
        return
...
def _find_bundled_tui(sparkii_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if sparkii_cli_dir is None:
        sparkii_cli_dir = Path(__file__).parent
    bundled = sparkii_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``sparkii update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SPARKII_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Sparkii checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `sparkii update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Sparkii checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `sparkii --tui`\n"
        "If the checkout is still inconsistent, run `sparkii update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SPARKII_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SPARKII_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $SPARKII_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Sparkii installed,
    ...
    ext_dir = os.environ.get("SPARKII_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SPARKII_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SPARKII_TUI_DIR (e.g. `unset SPARKII_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # sparkii_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_sparkii_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Sparkii-managed
            ...
            from sparkii_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @sparkii/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @sparkii/ink resolves through
        # packages/sparkii-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "sparkii-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sparkii_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("SPARKII_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["SPARKII_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("SPARKII_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["SPARKII_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("SPARKII_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["SPARKII_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["SPARKII_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a sparkii process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sparkii-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SPARKII_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SPARKII_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["SPARKII_MODEL"] = model
        env["SPARKII_INFERENCE_MODEL"] = model
    if provider:
        env["SPARKII_TUI_PROVIDER"] = provider
        env["SPARKII_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SPARKII_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SPARKII_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SPARKII_TUI_SKILLS"] = value
    if query:
        env["SPARKII_TUI_QUERY"] = query
    if image:
        env["SPARKII_TUI_IMAGE"] = image
    if checkpoints:
        env["SPARKII_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SPARKII_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SPARKII_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SPARKII_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SPARKII_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SPARKII_ACCEPT_HOOKS"] = "1"
    ...
    # SPARKII_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `sparkii --tui` try to
    ...
    # still set SPARKII_TUI_RESUME themselves.
    env.pop("SPARKII_TUI_RESUME", None)
    if resume_session_id:
        env["SPARKII_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `sparkii update` so
    ...
    if code == 42:
        from sparkii_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SPARKII_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``sparkii kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``sparkii kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SPARKII_KANBAN_BOARD"):
        return
    try:
        from sparkii_cli.kanban_db import get_current_board

        os.environ["SPARKII_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.sparkii/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Sparkii — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Sparkii still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``sparkii … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'sparkii sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: sparkii --resume {_imported_id})")
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'sparkii doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Sparkii isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  sparkii setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'sparkii setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sparkii_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    # rare launch right after `sparkii update` changed a bundled skill.
    ...
    if getattr(args, "yolo", False):
        os.environ["SPARKII_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.sparkii/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SPARKII_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SPARKII_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SPARKII_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from sparkii_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from sparkii_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Sparkii?")
        print()
    ...
    # has no creds.json.  Every subsequent `sparkii gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: sparkii gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `sparkii gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sparkii gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Sparkii Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sparkii gateway")
        ...
        print()
        print("  Or install as a service: sparkii gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sparkii whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``sparkii whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``sparkii_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from sparkii_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from sparkii_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sparkii model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("SPARKII_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sparkii model' for "
                "available providers, or run 'sparkii doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in sparkii_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``sparkii model``
    ...
    # a leftover OPENAI_BASE_URL in ~/.sparkii/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sparkii/.env if the active provider is not 'custom'.
...
#
# Sparkii uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `sparkii model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sparkii model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`sparkii_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Sparkii only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from sparkii_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Sparkii URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `sparkii_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `sparkii` invocation — including
# fast paths like `sparkii --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on sparkii_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `sparkii` invocation, including fast paths like `sparkii --version`
...
def _self():
    ...
    keeps monkeypatches on sparkii_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``sparkii setup`` / ``sparkii model``.
    ...
    recover from a malformed paste without editing ``~/.sparkii/.env`` by hand.
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `sparkii setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from sparkii_constants import display_sparkii_home as _dhh_fn

            print(
                f"    Sparkii will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Sparkii CLI with a provider."""
    from sparkii_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from sparkii_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sparkii_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from sparkii_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from sparkii_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.sparkii/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `sparkii sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set SPARKII_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "SPARKII_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="sparkii sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="sparkii sync now")
...
def cmd_webhook(args):
    ...
    from sparkii_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``sparkii slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: sparkii slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sparkii slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from sparkii_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sparkii_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from sparkii_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from sparkii_cli.hooks import hooks_command
...
def cmd_verify(args):
    ...
    from sparkii_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `sparkii security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from sparkii_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `sparkii approvals <subcmd>`."""
    from sparkii_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from sparkii_cli.dump import run_dump
...
def cmd_skin(args):
    ...
    from sparkii_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Sparkii home directory to a zip file."""
    if getattr(args, "quick", False):
        from sparkii_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from sparkii_cli.backup import run_backup
...
def cmd_uninstall(args):
    """Uninstall Sparkii Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from sparkii_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from sparkii_cli.uninstall import run_gui_uninstall
    ...
    from sparkii_cli.uninstall import run_uninstall
...
# Update pipeline lives in sparkii_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on sparkii_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in SPARKII_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``sparkii update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``sparkii update``
    ...
    ``sparkii`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``sparkii update``
    ...
    ``sparkii_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "sparkii_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $SPARKII_HOME."""
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``sparkii`` launcher present but ``sparkii_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the sparkii venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — sparkii venv python3, no nix-shell overhead
    ...
    # a sparkii venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sparkii update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Sparkii-managed install, or a freshly provisioned managed npm when
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "sparkii_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sparkii web``).
    ...
    # build path usable on a stock `py -m sparkii_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_sparkii_node_path())
    ...
    # as `sparkii update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (sparkii web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``sparkii update`` can unconditionally call ``sparkii desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``sparkii desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SPARKII_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SPARKII_HOME."""
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Sparkii.app/Contents/MacOS/Sparkii → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``sparkii desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``sparkii update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Sparkii.app/Contents/MacOS/Sparkii"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Sparkii.exe",
            release_dir / "win-ia32-unpacked" / "Sparkii.exe",
            release_dir / "win-arm64-unpacked" / "Sparkii.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "sparkii",
            release_dir / "linux-unpacked" / "Sparkii",
            release_dir / "linux-arm64-unpacked" / "sparkii",
            release_dir / "linux-arm64-unpacked" / "Sparkii",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Sparkii.exe
...
# Sparkii.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Sparkii.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Sparkii.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 sparkii-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Sparkii.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Sparkii.exe from backup.")
        print("    Your existing version was kept and still works. Run `sparkii desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `sparkii desktop --force-build` to rebuild, or re-run the Sparkii")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Sparkii'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Sparkii.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Sparkii.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Sparkii.exe: Access is
    ...
    an unrelated "Sparkii" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Sparkii.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Sparkii is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Sparkii.app/Contents/MacOS/Sparkii  ->  app bundle = .../Sparkii.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``sparkii desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Sparkii Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Sparkii Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `sparkii desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the SPARKII_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Sparkii Desktop (Linux only, best-effort).
    ...
    shell. ``sparkii uninstall --gui`` removes it.
    """
    try:
        from sparkii_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from sparkii_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "cwd", None):
        ...
        env["SPARKII_DESKTOP_CWD"] = os.getcwd()
    ...
    # `SPARKII_DESKTOP_DISABLE_GPU=... sparkii desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "SPARKII_DESKTOP_DISABLE_GPU" not in os.environ:
        env["SPARKII_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `SPARKII_DESKTOP_PASSWORD_STORE=... sparkii desktop` keeps working.
    if sys.platform == "linux" and "SPARKII_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["SPARKII_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  sparkii gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Sparkii-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → sparkii-setup →
            ...
            if not source_mode:
                ...
                # holds Sparkii.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Sparkii.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Sparkii.exe, close any")
                    print("  running Sparkii desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> sparkii desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Sparkii is
                ...
                # success on a Sparkii.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Sparkii shows up
    ...
    if source_mode:
        print("→ Launching Sparkii Desktop from source build...")
    ...
    print(f"→ Launching packaged Sparkii Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in sparkii_cli/dashboard_procs.py
...
# sparkii_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "sparkii dashboard",
            "sparkii_cli.main dashboard",
            "sparkii_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "sparkii serve",
            "sparkii_cli.main serve",
            "sparkii_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "sparkii-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Sparkii installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``sparkii-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/sparkii-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but sparkii launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``sparkii update``.
    ...
    log_path = get_sparkii_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `sparkii update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $SPARKII_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``sparkii update``.
    ...
    protocol stream (``sparkii acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `sparkii update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``sparkii.exe`` launch always has the launcher as an
    ...
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from sparkii.exe; applying package-only first aid, "
    ...
    try:
        from sparkii_cli import _install_repair as _ir
        ...
        if self_locked:
            print(
                "  Sparkii is still running from the launcher that needs "
                "replacing. Close other Sparkii windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\sparkii.exe`` is a launcher that runs the interpreter with
    ...
    ``sparkii ...`` command holds its own shim, and an editable install run
    ...
    ``sparkii.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _sparkii_exe_shims(scripts_dir)}
    if not shims:
        return None
...
_UPDATE_REEXEC_ENV = "SPARKII_UPDATE_REEXEC"
...
def _reexec_update_off_windows_shim() -> bool:
    ...
    cmd = [str(python_exe), "-m", "sparkii_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from sparkii_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``sparkii update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from sparkii_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_sparkii_exe(
    ...
    when ``sparkii update`` runs, ``sparkii.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``sparkii.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next sparkii invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Sparkii Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Sparkii Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``sparkii.exe`` re-runs itself under the venv Python before
    ...
    for shim in _sparkii_exe_shims(scripts_dir):
        ...
        print(
            "    Close Sparkii Desktop, exit other `sparkii` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `sparkii update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Sparkii versions queued for our shims.

    Sparkii used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _sparkii_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``sparkii.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``sparkii.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``sparkii`` drops off PATH. This wraps
    ...
    try:
        ...
        # quarantined aside and `sparkii` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sparkii.exe.old.*`` and stale reboot renames left by prior updates.

    Called early on every sparkii invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``sparkii.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``sparkii.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `sparkii update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``sparkii update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``sparkii.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``sparkii.exe`` shim is locked during ``sparkii update``, or when uv/distlib
    skips a launcher write. The symptom is ``sparkii-agent.exe`` and
    ``sparkii-acp.exe`` present but ``sparkii.exe`` missing, so ``sparkii`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `sparkii update --force` after "
            "closing other sparkii processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m sparkii_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``sparkii update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``sparkii.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``sparkii`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `sparkii update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `sparkii update --force` after closing other sparkii processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sparkii update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sparkii/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sparkii update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sparkii update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sparkii/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sparkii update --gateway``) the update is already
...
def cmd_update(args):
    """Update Sparkii Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Sparkii Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `sparkii update --branch=<x>` would actually pull.
    ...
    # Windows: an update launched through venv\Scripts\sparkii.exe holds that
    ...
    from sparkii_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sparkii -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `sparkii profile` — show current profile status
        from sparkii_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_sparkii_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → sparkii -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.sparkii)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sparkii profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sparkii -p {name} chat")
                else:
        ...
        from sparkii_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from sparkii_constants import get_sparkii_home as _hh
        ...
        from sparkii_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `sparkii profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → sparkii -p {name}  ({wrapper})")
        print()
        ...
        from sparkii_cli.profiles import profile_exists, validate_alias_name
        ...
        from sparkii_cli.profiles import normalize_profile_name, rename_profile
        ...
        from sparkii_cli.profiles import export_profile
        ...
        from sparkii_cli.profiles import import_profile
        ...
        import tempfile
        from sparkii_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  sparkii -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      sparkii -p {plan.manifest.name} chat")
        ...
        from sparkii_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `sparkii profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"sparkii -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from sparkii_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from sparkii_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No sparkii dashboard processes running.")
        return 0

    print(f"{len(live)} sparkii dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``sparkii dashboard register`` for OAuth.
    ...
    try:
        from sparkii_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `sparkii dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    sparkii dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "SPARKII_DASHBOARD_OAUTH_CLIENT_ID into ~/.sparkii/.env for you.\n"
            "  Docs: https://sparkii-agent.nousresearch.com/docs/"
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from sparkii_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.sparkii/desktop-ssh: a
    # literal "~/.sparkii/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of SPARKII_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_sparkii_home(): a non-default sticky profile (or any SPARKII_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_sparkii_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".sparkii" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``SPARKII_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``sparkii dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sparkii dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sparkii update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with sparkii serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with SPARKII_DESKTOP=1 plus
    # SPARKII_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # SPARKII_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `sparkii dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited SPARKII_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # SPARKII_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (SPARKII_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets SPARKII_SERVE_HEADLESS below.
    if os.environ.get("SPARKII_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("SPARKII_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("SPARKII_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("SPARKII_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → SPARKII_HOME points into
    ...
    if (
        ...
        and os.environ.get("SPARKII_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "sparkii_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: SPARKII_HOME is explicitly pinned to the
        ...
        # SPARKII_HOME.  We must resolve the root explicitly instead of just
        # dropping SPARKII_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV SPARKII_HOME=/opt/data`), so an unset
    ...
    # the same logs directory as every other Sparkii surface.
    try:
        from sparkii_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["SPARKII_SERVE_HEADLESS"] = "1"
    elif "SPARKII_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SPARKII_WEB_DIST"])
            if "SPARKII_WEB_DIST" in os.environ
            else PROJECT_ROOT / "sparkii_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom SPARKII_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "SPARKII_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # SPARKII_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["SPARKII_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ SPARKII_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset SPARKII_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads SPARKII_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["SPARKII_WEB_DIST"] = str(_dist_root)
    ...
    from sparkii_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from sparkii_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from sparkii_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from sparkii_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from sparkii_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Sparkii log files."""
    from sparkii_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Sparkii command console."""
    from sparkii_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from sparkii_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``sparkii -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``sparkii_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``sparkii -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``sparkii`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``sparkii <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``sparkii photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SPARKII_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that SPARKII_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from sparkii_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["SPARKII_SAFE_MODE"] = "1"
    os.environ["SPARKII_IGNORE_USER_CONFIG"] = "1"
    os.environ["SPARKII_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    ``sparkii`` / ``sparkii -w -s foo --yolo`` / ``sparkii chat`` don't need the
    ...
    """
    if os.environ.get("SPARKII_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("SPARKII_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SPARKII_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SPARKII_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SPARKII_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `sparkii --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from sparkii_constants import get_sparkii_home, display_sparkii_home

        mem_dir = get_sparkii_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_sparkii_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_sparkii_home()}/memories/\n")
    else:
        from sparkii_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Sparkii Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from sparkii_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from sparkii_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: sparkii-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from sparkii_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from sparkii_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``sparkii skills trust [path]`` / ``sparkii skills untrust [path]``.
...
def cmd_pairing(args):
    from sparkii_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from sparkii_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from sparkii_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from sparkii_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``sparkii-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``SPARKII_AGENT`` is the Sparkii-specific marker. setdefault: never
    clobber an outer harness (e.g. Sparkii running inside another agent's
    terminal).
    """
...
def main():
    """Main entry point for sparkii CLI."""
    # Cosmetic: make the process show up as 'sparkii' instead of 'python3.11'
    ...
    # If the checkout changed since the last launch (sparkii update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``sparkii update``
    ...
    # ``sparkii skills install update``) merely defers recovery one launch;
    # under-matching (missing ``sparkii -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in sparkii_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from sparkii_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from sparkii_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sparkii-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sparkii model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.sparkii/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://sparkii-agent.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from sparkii_cli import secrets_cli as _secrets_cli
    from sparkii_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `sparkii proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sparkii-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from sparkii_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``sparkii proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from sparkii_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in sparkii_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in sparkii_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in sparkii_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `sparkii whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in sparkii_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # logout command  (parser built in sparkii_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in sparkii_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in sparkii_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in sparkii_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in sparkii_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in sparkii_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Sparkii gateways)
    # =========================================================================
    from sparkii_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from sparkii_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in sparkii_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in sparkii_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in sparkii_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in sparkii_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in sparkii_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in sparkii_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in sparkii_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in sparkii_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.sparkii/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo sparkii uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from sparkii_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in sparkii_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: sparkii_cli/subcommands/import_agent.py)
    # =========================================================================
    ...
    # =========================================================================
    # config command  (parser built in sparkii_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in sparkii_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in sparkii_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in sparkii_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in sparkii_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from sparkii_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in sparkii_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``sparkii --help``, ``sparkii version``, ``sparkii logs``,
    ...
    try:
        from sparkii_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Sparkii shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from sparkii_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from sparkii_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in sparkii_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in sparkii_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `sparkii computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `sparkii tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `sparkii computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Sparkii app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from sparkii_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("SPARKII_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from sparkii_cli.tools_config import _cua_driver_env
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by SPARKII_CUA_DRIVER_CMD, or unset "
                            "the override and run: sparkii computer-use install --upgrade"
                        )
                    else:
                        print("    Run: sparkii computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: sparkii computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: sparkii computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: sparkii computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: sparkii computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: sparkii computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in sparkii_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <sparkii home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Sparkii",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Sparkii session store "
            "so it can be resumed with 'sparkii --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in sparkii_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on sparkii_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in sparkii_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in sparkii_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in sparkii_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in sparkii_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in sparkii_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in sparkii_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in sparkii_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in sparkii_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Sparkii-Setup.exe success screen tells users to
    # run `sparkii desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in sparkii_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in sparkii_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in sparkii_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``sparkii -c Pokemon Agent Dev`` → ``sparkii -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'sparkii -c model' where 'model' is consumed as the
    ...
    # --yolo: set SPARKII_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (sparkii hooks list, cron
    ...
    # ``sparkii egress start`` refusing when credential_source=bitwarden
