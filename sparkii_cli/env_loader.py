...
def _known_sparkii_env_keys() -> set[str]:
    ...
    """
    from sparkii_cli.config import _EXTRA_ENV_KEYS
    from sparkii_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``sparkii_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from sparkii_cli.config import _sanitize_env_lines
    except ImportError:
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``sparkii_cli.config.apply_terminal_config_to_env`` — the
