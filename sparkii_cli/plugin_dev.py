"""Runtime-backed validation behind ``sparkii plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary SPARKII_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="sparkii-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "SPARKII_BUNDLED_PLUGINS": str(bundled),
                "SPARKII_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "sparkii_plugins" or name.startswith("sparkii_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Sparkii discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "sparkii_plugins" or name.startswith("sparkii_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_sparkii_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".sparkii" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from sparkii_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Sparkii supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Sparkii never auto-installs plugin dependencies; "
    ...
    if schema:
        from sparkii_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Sparkii' real scanner and registration path."""
    try:
