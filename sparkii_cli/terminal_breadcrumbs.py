"""Per-terminal session breadcrumbs for ``sparkii -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SPARKII_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``sparkii -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_sparkii_home() / "terminal-sessions"
