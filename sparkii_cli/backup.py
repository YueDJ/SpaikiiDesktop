"""
Backup and import commands for sparkii CLI.

`sparkii backup` creates a zip archive of the entire ~/.sparkii/ directory
(excluding the sparkii-agent repo and transient files).

`sparkii import` restores from a backup zip, overlaying onto the current
SPARKII_HOME root.
"""
...
# ``sparkii-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/sparkii-agent/``
...
# SPARKII_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "sparkii-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under SPARKII_HOME) —
...
# File names that ``sparkii import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SPARKII_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SPARKII_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to sparkii root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``sparkii-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/sparkii-agent/`` — must be preserved.
        if part == "sparkii-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SPARKII_HOME.
...
# Block 4: moved to the core package; keep the name re-exported here for
# surface-side callers and patch targets.
from core.sqlite_util import is_zeroed_sqlite_file  # noqa: F401
...
# to the user as a hung `sparkii update` (#70553 follow-up). Sessions databases
...
def _run_backup_locked(args, sparkii_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"sparkii-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"sparkii-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_sparkii_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(sparkii_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "sparkii-agent" and not is_root)
        ]
    ...
    # outside SPARKII_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_sparkii_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: sparkii import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a sparkii home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.sparkii/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a sparkii dir name
        if prefix in {".sparkii", "sparkii"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``sparkii import``, which is the
    ...
    so without the chown a ``sudo sparkii import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``sparkii import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Sparkii' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_sparkii_home()}")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SPARKII_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_sparkii_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # sparkii_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: sparkii profile list  (after installing sparkii)")
        ...
        try:
            from sparkii_cli.gateway import ensure_gateway_service, _is_service_running
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and sparkii backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SPARKII_HOME).
...
# — `sparkii update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SPARKII_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SPARKII_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: sparkii snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside SPARKII_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    """CLI entry point for sparkii backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_sparkii_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    but writes to ``<SPARKII_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `sparkii claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    ``sparkii claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``sparkii import <archive>``.  Writes to
    ``<SPARKII_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
