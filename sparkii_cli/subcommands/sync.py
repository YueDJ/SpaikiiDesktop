"""``sparkii sync`` subcommand parser — Skill Sync.

Cloned from ``sparkii_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    sparkii sync status                 show gate/opt-in/head state
    sparkii sync pull                   pull and materialize opted-in skills
    sparkii sync push                   push opted-in skills
    sparkii sync now                    reconcile: pull then push
    sparkii sync enable <skill>         opt a skill into sync
    sparkii sync disable <skill>        opt a skill out of sync
    sparkii sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    sparkii sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  sparkii sync status            what is synced, and from where\n"
            "  sparkii sync enable my-skill   include a skill in your sync\n"
            "  sparkii sync now               pull, then push\n"
            "  sparkii sync propose my-skill  share a skill with your team\n"
        ),
