"""``sparkii dashboard`` / ``sparkii serve`` subcommand parsers.
...
``sparkii_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    """
    parser.add_argument(
        "--port", type=int, default=9219, help="Port (default 9219, 0 for auto-assign by OS)"
    )
    parser.add_argument(
    ...
    # `sparkii dashboard` / `sparkii serve` cmdlines and SIGTERM them directly —
    # the same path `sparkii update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Sparkii web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Sparkii web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Sparkii Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Sparkii desktop app shells (<= 0.15.x) spawn the
    # backend as `sparkii dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Sparkii couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Sparkii Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Sparkii backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Sparkii backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports SPARKII_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `sparkii dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.sparkii/.env.
    # Nested subparser so bare `sparkii dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "SPARKII_DASHBOARD_OAUTH_CLIENT_ID into ~/.sparkii/.env, and prints "
            "how to engage the login gate. Requires being logged in (sparkii setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://sparkii.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via SPARKII_DASHBOARD_PORTAL_URL. Mainly for "
