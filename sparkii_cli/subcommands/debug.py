"""``sparkii debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Sparkii Agent. Use 'sparkii debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sparkii debug share              Upload debug report (asks for confirmation)
    sparkii debug share --yes        Skip confirmation (for scripts/CI)
    sparkii debug share --lines 500  Include more log lines
    sparkii debug share --expire 30  Keep paste for 30 days
    sparkii debug share --local      Print report locally (no upload)
    sparkii debug share --no-redact  Disable upload-time secret redaction
    sparkii debug share --nous       Upload to Nous-internal storage (private)
    sparkii debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sparkii debug share'",
    )
