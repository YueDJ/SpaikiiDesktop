"""``sparkii login`` subcommand parser.

Extracted verbatim from ``sparkii_cli/main.py:main()`` (god-file Phase 2).
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``sparkii login`` was removed in favor of ``sparkii auth`` / ``sparkii model``
    (the runtime handler in ``sparkii_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``sparkii login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``sparkii --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``sparkii login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``sparkii model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `sparkii auth` to manage credentials, "
            "`sparkii model` to select a provider, or `sparkii setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `sparkii model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: sparkii-cli)"
    )
