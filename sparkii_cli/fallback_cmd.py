"""
sparkii fallback — manage the fallback provider chain.
...
Subcommands:
  sparkii fallback [list]   Show the current fallback chain (default when no subcommand)
  sparkii fallback add      Pick provider + model via the same picker as `sparkii model`,
                           then append the selection to the chain
  sparkii fallback remove   Pick an entry to delete from the chain
  sparkii fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.sparkii/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `sparkii model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  sparkii fallback add")
        print()
        return
    ...
    print("  Docs: https://sparkii-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `sparkii model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `sparkii fallback list` to view, or `sparkii fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``sparkii fallback [subcommand]``."""
