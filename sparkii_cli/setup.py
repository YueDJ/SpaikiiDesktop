"""
Interactive setup wizard for Sparkii Agent.
...
_DOCS_BASE = "https://sparkii-agent.nousresearch.com/docs"
...
from sparkii_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Sparkii Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Sparkii using environment variables or config commands:")
    print_info("  sparkii config set model.provider custom")
    print_info("  sparkii config set model.base_url http://localhost:8080/v1")
    print_info("  sparkii config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'sparkii setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``SPARKII_NONINTERACTIVE=1`` (see ``sparkii_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("SPARKII_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``SPARKII_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from sparkii_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from sparkii_cli.auth import resolve_provider
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'sparkii setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'sparkii setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'sparkii setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'sparkii setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('sparkii setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('sparkii setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('sparkii setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('sparkii setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('sparkii setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('sparkii config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('sparkii config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('sparkii config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('sparkii', Colors.GREEN)}              Start chatting")
    print(f"   {color('sparkii gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('sparkii doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `sparkii tools` and `sparkii setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``sparkii model``)
    ...
    provider added to ``sparkii model`` is automatically available here.
    ...
    # Delegate to the shared sparkii model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: sparkii model")
    ...
    # on demand via `sparkii auth add`, `sparkii setup` vision, and
    # `sparkii setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from sparkii_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``sparkii model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from sparkii_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Sparkii auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via sparkii setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'sparkii setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Sparkii runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `sparkii setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `sparkii setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `sparkii egress setup` then `sparkii egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `sparkii egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'sparkii-agent[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Sparkii installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sparkii-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SPARKII_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SPARKII_MAX_ITERATIONS")
    ...
    print_info("  Run `sparkii setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SPARKII_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.sparkii/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from sparkii_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_sparkii_home(Path(get_sparkii_home()))
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Sparkii delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Sparkii to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from sparkii_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  sparkii config edit")
    print_info("   Open config in your editor:  sparkii config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from sparkii_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Sparkii from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'sparkii setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     sparkii config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: sparkii gateway install --system")
                print_info("  Or run in foreground:  sparkii gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   sparkii gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `sparkii setup tools` and `sparkii tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Sparkii profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``sparkii_cli.auth`` — lists every supported
    ...
    """
    try:
        from sparkii_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        from sparkii_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_sparkii.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_sparkii migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_sparkii", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Sparkii.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Sparkii to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Sparkii at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Sparkii at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Sparkii at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Sparkii at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Sparkii equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Sparkii config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Sparkii.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Sparkii's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Sparkii can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: sparkii claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: sparkii claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Sparkii configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Sparkii (use sparkii claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      sparkii setup           — full or quick (auto-detected)
      sparkii setup model     — just model/provider
      sparkii setup tts       — just text-to-speech
      sparkii setup terminal  — just terminal backend
      sparkii setup gateway   — just messaging platforms
      sparkii setup tools     — just tool configuration
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Sparkii Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Sparkii Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Sparkii Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Sparkii configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'sparkii setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Sparkii?",
            [
    ...
    # Tune later with `sparkii setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'sparkii setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    sparkii setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``sparkii tools`` (which
    ...
    try:
        ...
        from sparkii_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``sparkii setup agent`` /
    ``sparkii config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `sparkii update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        sparkii tools")
        print_info("  Seed skills:         sparkii skills opt-in --sync")
        print_info("  Add MCP servers:     sparkii mcp add")
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`sparkii update` runs from re-injecting them. Opt back in any")
            print_info("time with `sparkii skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `sparkii tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `sparkii plugins list` / `sparkii plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `sparkii plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `sparkii mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `sparkii mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   sparkii tools")
    print_info("  Seed skills:         sparkii skills opt-in --sync")
    print_info("  Add MCP servers:     sparkii mcp add")
    print_info("  Tune agent settings: sparkii setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'sparkii setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Sparkii to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'sparkii setup gateway'.")
