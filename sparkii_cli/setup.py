...
def setup_gateway(config: dict):
    ...
    # via `sparkii import` or `sparkii setup gateway`). Gating this on
...
def run_setup_wizard(args):
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from sparkii_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _run_first_time_quick_setup(config: dict, sparkii_home, is_existing: bool):
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `sparkii import` from another machine).
        from sparkii_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
