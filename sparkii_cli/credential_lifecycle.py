"""Unified provider-credential lifecycle across every store Sparkii reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.sparkii/.env``                     — the canonical secret store
    2. ``~/.sparkii/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.sparkii/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `sparkii auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``sparkii auth add``.
    """
    from sparkii_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from sparkii_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from sparkii_cli.config import load_env, remove_env_value
