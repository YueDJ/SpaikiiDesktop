...
on Japanese locales, etc.).  Sparkii's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``sparkii_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Sparkii is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SPARKII_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SPARKII_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Sparkii-managed Git install directories so
    ...
    # already-running shells don't see the change — which means sparkii
    ...
    # install without Sparkii-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Sparkii's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Sparkii's
    ``sparkii config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Sparkii (User env var in Windows Settings, or
    export in a PowerShell profile) and Sparkii picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Sparkii-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\sparkii\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``sparkii`` from right after install) retain their old PATH.

    Any subprocess Sparkii spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Sparkii-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "sparkii", "git", "cmd"),
        os.path.join(local_appdata, "sparkii", "git", "bin"),
        os.path.join(local_appdata, "sparkii", "git", "usr", "bin"),
        # Sparkii venv Scripts directory — host of the sparkii.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes sparkii, but harmless to include.
        os.path.join(local_appdata, "sparkii", "sparkii-agent", "venv", "Scripts"),
