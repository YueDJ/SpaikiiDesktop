"""Per-provider model-selection wizard flows for ``sparkii setup`` / ``sparkii model``.

Extracted from ``sparkii_cli/main.py`` as part of the god-file decomposition
campaign (``~/.sparkii/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from sparkii_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``sparkii_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from sparkii_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``sparkii_cli.main`` at import time -> no import cycle.
"""
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from sparkii_cli.auth import _resolve_api_key_provider_secret
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sparkii_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.sparkii/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `sparkii setup` from scratch. OpenRouter
    ...
    from sparkii_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from sparkii_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `sparkii moa configure <name>` first.")
        return
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `sparkii model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Sparkii will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Sparkii will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sparkii_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from sparkii_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from sparkii_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from sparkii_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from sparkii_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from sparkii_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from sparkii_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `sparkii auth`)
    from sparkii_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    ...
    if selected:
        ...
        # Save the API key env var name so sparkii knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.sparkii/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``sparkii_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from sparkii_cli.main import _prompt_provider_choice
    from sparkii_cli.auth import ZAI_ENDPOINTS
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Sparkii typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Sparkii, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from sparkii_cli.main import _run_anthropic_oauth_flow
    from sparkii_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'sparkii model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
