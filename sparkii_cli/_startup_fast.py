...
``sparkii_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SPARKII_HOME.
    """
...
def print_fast_version_info() -> None:
    from sparkii_cli import __release_date__, __version__

    print(f"Sparkii Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'sparkii version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``sparkii --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the SPARKII_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("SPARKII_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
