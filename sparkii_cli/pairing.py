...
Usage:
    sparkii pairing list              # Show all pending + approved users
    sparkii pairing approve <platform> <request-id|code>  # Approve a pairing request
    sparkii pairing revoke <platform> <user_id> # Revoke user access
    sparkii pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle sparkii pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: sparkii pairing {list|approve|revoke|clear-pending}")
        print("Run 'sparkii pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: sparkii pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.sparkii/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'sparkii pairing list' to see pending requests.\n")
