...
change is that references to helpers/constants that STAY in ``sparkii_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``sparkii_cli.main`` reference — so existing call sites and test monkeypatches
that target ``sparkii_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``sparkii_cli.main``.
...
Imports are one-way: ``sparkii_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``sparkii_cli.main`` reference.

    Lets callers keep patching ``sparkii_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``sparkii_cli.main`` -> ``sparkii_cli.update_cmd`` stays one-way
    at import time.
    """
    from sparkii_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``sparkii update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``sparkii_cli.dashboard_procs`` so that post-update dashboard cleanup
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``sparkii`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="sparkii-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``sparkii update`` can be driven by a
    ...
    """
    from sparkii_constants import FIRST_PARTY_MODULE_ROOTS
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sparkii update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sparkii update`.
    ...
    print("  Preview now:  sparkii curator run --dry-run")
    print("  Pause it:     sparkii curator pause")
    print(
        "  Docs:         https://sparkii-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `sparkii update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `sparkii sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    sparkii sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  sparkii sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``sparkii update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``sparkii update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: sparkii curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from sparkii_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    sparkii dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``sparkii_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.sparkii-update-staging"
    backup = f"{dst}.sparkii-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `sparkii update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``sparkii_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``sparkii update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && sparkii update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SPARKII_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== sparkii-update completed {action_id} ===")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Sparkii Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `sparkii update "
            f"--branch {branch}`, or update against main with `sparkii update`."
        )
        _m().sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="sparkii-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"sparkii-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for sparkii-agent itself
        ...
        # GitHub ZIPs extract to sparkii-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sparkii-agent-{branch}")
        ...
        print(
            "  Re-run `sparkii update` to retry; if the agent won't start, "
            "reinstall from https://sparkii-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from sparkii_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `sparkii update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: sparkii skills list-modified  "
    ...
    try:
        from sparkii_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from sparkii_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_sparkii_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_sparkii_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sparkii-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes sparkii completely unrunnable (SyntaxError on import).
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sparkii_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sparkii_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``sparkii update``, every profile is now current.
    """
    homes = []
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another sparkii.exe is running:"]
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `sparkii update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``sparkii tools``."""
    try:
        from sparkii_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Sparkii Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``sparkii tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Sparkii Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Sparkii Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``sparkii update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `sparkii update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `sparkii update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from sparkii_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from sparkii_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SPARKII_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `sparkii update` is exactly the step
...
def _npm_lockfile_changed(sparkii_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `sparkii
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    sparkii update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `sparkii update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `sparkii update`.")
            failed = []
            if any(
    ...
    # `sparkii update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `sparkii update`.")
        return list(labels)
    ...
    # `sparkii update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.sparkii/logs/update.log`` only, never the terminal.

    During ``sparkii update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``sparkii update --check``: fetch and report without installing.
    ...
    from sparkii_cli.gitlock import clear_stale_git_locks
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sparkii update`` without requiring a reinstall.  The
    ...
    ``sparkii`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/sparkii.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v sparkii'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/sparkii, code at /usr/local/lib/sparkii-agent).
    fhs_link = Path("/usr/local/bin/sparkii")
    ...
    path_comment = (
        "# Sparkii Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``sparkii-acp`` launcher next to the ``sparkii`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``sparkii update`` without a reinstall.  ACP hosts
    ...
    No-op on Windows (install.ps1 copies ``sparkii.exe`` + ``sparkii-acp.exe``
    ...
    so ``sparkii-acp.exe`` already resolves) and wherever a ``sparkii-acp`` is
    already present next to the ``sparkii`` command.  Unwritable directories
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            continue
        print(f"  ✓ Installed sparkii-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SPARKII_HOME under
      ``backups/`` (restorable via ``sparkii import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from sparkii_cli.backup import (
    ...
    # Render path using display_sparkii_home so the user sees ~/.sparkii/...
    try:
        from sparkii_constants import get_sparkii_home, display_sparkii_home

        home = get_sparkii_home()
        try:
            display_path = f"{display_sparkii_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  sparkii import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``sparkii update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``sparkii update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run sparkii from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.sparkii-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".sparkii-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The sparkii.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m sparkii_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``sparkii
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m sparkii_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "sparkii_cli.main" in cmdline_low:
        ...
        # the `-m sparkii_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``sparkii_cli.main`` used to import
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `sparkii` launch will complete it")
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``sparkii update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from sparkii_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning sparkii-setup, and the update-in-progress marker
    ...
    managed-runtime descendants (the ``.sparkii-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "sparkii_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.sparkii-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    sparkii.exe concurrent-instance guard does not see them. They still import
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m sparkii_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: sparkii gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``sparkii gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from sparkii_cli import gateway_windows
        from sparkii_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``sparkii-gateway*.service``/``sparkii-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("sparkii-serve")`` alone would also accept the
        # unrelated ``sparkii-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "sparkii-gateway.service"
            or unit.startswith("sparkii-gateway-")
            or unit == "sparkii-serve.service"
            or unit.startswith("sparkii-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``sparkii-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``sparkii-serve*`` units (#83438) don't, so sending them
    ...
    (``sparkii-gateway-helper`` is fine — profile units are
    ``sparkii-gateway-<profile>`` — but ``sparkii-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "sparkii-gateway" or svc_name.startswith("sparkii-gateway-")
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``sparkii_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``sparkii_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``sparkii update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``sparkii-setup.exe``) executes
    ``$SPARKII_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_sparkii_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``sparkii update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``sparkii update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `sparkii desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "sparkii_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Sparkii-managed Node on PATH: when
    # `sparkii update` runs inside the desktop updater chain (Desktop →
    # sparkii-setup → sparkii update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `sparkii desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Sparkii Agent...")
    print()

    # On Windows, abort early if another sparkii.exe is holding the venv shim
    ...
    # (most commonly the Desktop app's `sparkii serve` backend) will keep .pyd
    ...
    # the sparkii.exe shim guard above, but its lock probe only checks the shim
    ...
    # process by design (a CLI `sparkii update` IS the venv python), and an
    ...
    try:
        ...
        # orphan merge-conflict markers in sparkii_cli/config.py bricked
        # every user who ran ``sparkii update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``sparkii
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``sparkii update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``sparkii update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``sparkii doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && sparkii update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_sparkii_home added to sparkii_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``sparkii`` launch finishes the install
        ...
        # symbols in sparkii_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `sparkii` again "
        ...
        if not import_ok:
            ...
            print("    Run `sparkii update` again — if it persists, reinstall:")
            print("    https://sparkii-agent.nousresearch.com")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_sparkii_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.sparkii/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SPARKII_HOME so
        # it is not affected by sync_skills()'s module-level SPARKII_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SPARKII_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from sparkii_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'sparkii config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'sparkii config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: sparkii config migrate")
            else:
                print()
                print("Skipped. Run 'sparkii config migrate' later to configure.")
        else:
        ...
        # check `sparkii curator status`. Self-stamps after printing so it
        ...
        try:
            ...
            logger.debug("sparkii-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``sparkii update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `sparkii update`
                # must stay fast; `sparkii computer-use install --upgrade`
        ...
        # When running as ``sparkii update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... sparkii-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... sparkii-gateway*)
            ...
            try:
                from sparkii_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all sparkii-gateway* units (default + profiles) plus
            # sparkii-serve* units (the Desktop app's backend, #83438).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: sparkii gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # sparkii-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `sparkii update` restart it automatically, allow\n"
                        ...
                        # path in `sparkii gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sparkii gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sparkii -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_sparkii_home() / ".update_exit_code"
                    try:
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_sparkii_units():
                ...
                print()
                print("  These pre-rename units (sparkii.service) fight the current")
                print("  sparkii-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    sparkii gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes sparkii-serve*,
        ...
        print("  sparkii model              # Select provider and model")
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``sparkii_cli.gateway`` no longer imports,
