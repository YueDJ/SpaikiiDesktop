...
def _update_node_dependencies() -> list[str]:
    ...
    # need desktop during `sparkii update`, so we install root-only first
    ...
    # hung. The chatty npm-deprecation noise during `sparkii update` comes from
...
def _ensure_acp_launcher() -> None:
    ...
    ``sparkii-acp.exe`` already resolves) and wherever a ``sparkii-acp`` is
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``sparkii-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("sparkii-gateway"):
            continue
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # build.  ``sparkii desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``sparkii desktop`` shouldn't be forced into a full
        # Electron build by ``sparkii update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `sparkii desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "sparkii_cli.main", "desktop", "--build-only"]
                ...
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `sparkii desktop` to retry)")
        ...
        try:
            ...
            # Discover all sparkii-gateway* units (default + profiles)
