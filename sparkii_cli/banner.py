...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `sparkii ... > file`) raises
...
from sparkii_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/sparkii-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/sparkii-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/nousresearch/sparkii-agent/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "sparkii-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `sparkii update`,
...
def check_for_updates() -> Optional[int]:
    ...
    # SPARKII_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    if embedded_rev:
        ...
        # $SPARKII_HOME/sparkii-agent/ may be a stale copy from --clone-all;
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``sparkii_cli/build_info.py``) and return it as a frozen
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/sparkii-agent/releases/tag"
...
def format_banner_version_label() -> str:
    ...
    base = f"Sparkii Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built sparkii; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_sparkii_home() / "cache" / "banner_snapshot.json"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SPARKII_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SPARKII_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or sparkii setup[/]"
            )
        else:
    ...
    if os.getenv("SPARKII_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from sparkii_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SPARKII_AGENT_LOGO
        console.print(_logo)
        console.print()
