"""Windows ConPTY bridge for the `sparkii dashboard` chat tab.

Drop-in counterpart to ``sparkii_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``sparkii_cli.web_server`` consumes: ``spawn``, ``read``,
