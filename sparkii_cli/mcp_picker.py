"""MCP picker — interactive `sparkii mcp picker` (also the default `sparkii mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``sparkii mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `sparkii plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Sparkii session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from sparkii_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `sparkii mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: sparkii mcp install <name>    Picker: sparkii mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Sparkii is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Sparkii — run `sparkii update` "
...
def show_catalog() -> None:
    """`sparkii mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`sparkii mcp picker` (and default `sparkii mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`sparkii mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `sparkii mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
