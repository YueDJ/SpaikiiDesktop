"""Cross-process mutual exclusion for in-flight Sparkii updates.
...
once — so a dashboard-spawned ``sparkii update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SPARKII_HOME>/.sparkii-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``sparkii update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Sparkii is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``sparkii-setup`` binary under ``~/.sparkii`` is only refreshed by a full
  installer run (``copy_self_to_sparkii_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Sparkii is still running") on every
  GUI update forever, with no Sparkii process actually running.
"""
...
MARKER_NAME = ".sparkii-update-in-progress"

# Set by an orchestrating updater (the Tauri `sparkii-setup --update` flow) to
# its own pid before spawning `sparkii update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SPARKII_UPDATE_HANDOFF_PID"
...
# to show "Sparkii is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Sparkii home (never the context-local profile override):
    the Rust updater resolves ``$SPARKII_HOME`` or the platform default, and the
    ...
    return get_process_sparkii_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``sparkii update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``sparkii-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Sparkii update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `sparkii update` as a stage): we run under ITS claim
