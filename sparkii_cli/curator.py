"""Curator — background skill maintenance (engine + `sparkii curator` CLI).
...
This module used to be split between ``agent/curator.py`` (engine) and a thin
CLI shell; the engine was moved here so the product layer owns it. It renders
a status table, triggers a run, pauses/resumes, pins/unpins skills, and
persists curator state in ``.curator_state``.

Strict invariants (preserved from the original engine):
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`sparkii curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `sparkii curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`sparkii curator status` and run `sparkii curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `sparkii curator adopt <name>`, "
          "or all with `sparkii curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`sparkii curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.sparkii/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `sparkii curator rollback <id>`; "
        "whole-tree snapshots remain available via `sparkii curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `sparkii curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`sparkii curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.sparkii/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from sparkii_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.sparkii/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.sparkii/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `sparkii curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`sparkii curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by sparkii_cli.main fallthrough)."""
...
def _state_file() -> Path:
    return get_sparkii_home() / "skills" / ".curator_state"
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `sparkii update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``sparkii curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``sparkii update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``sparkii curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `sparkii curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Sparkii' background skill CURATOR. This is an "
    ...
    "into ~/.sparkii/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: sparkii-config-*, sparkii-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.sparkii/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.sparkii/logs/curator/``)
    ...
    authored skill data in ``~/.sparkii/skills/``.

    ``ensure_sparkii_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_sparkii_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every sparkii install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: sparkii curator status
        keep an umbrella stable: sparkii curator pin document-tools
    ...
    lines.append("full report: sparkii curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: sparkii curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.sparkii/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.sparkii/skills/.archive/` for "
            "safety and can be restored via `sparkii curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.sparkii/skills/.archive/`. "
            "Restore any via `sparkii curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `sparkii curator restore <name>`")
    lines.append("- All archives live under `~/.sparkii/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``sparkii curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `sparkii curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `sparkii update` only if "
        ...
        # recorded in state so `sparkii curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``sparkii model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `sparkii model` → auxiliary
