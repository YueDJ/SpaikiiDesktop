"""
Dump command for sparkii CLI.

Outputs a compact, plain-text summary of the user's Sparkii setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.sparkii/.env.
    ...
    exports. ``sparkii debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.sparkii_build_sha`` by the Dockerfile's
    ``SPARKII_GIT_SHA`` build-arg (see ``sparkii_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``sparkii dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from sparkii_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(sparkii_home: Path) -> int:
...
def _cron_summary(sparkii_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sparkii_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sparkii_home:      {display_sparkii_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.sparkii/.env: a managed
        ...
        # A credential added via `sparkii auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `sparkii auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["sparkii-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sparkii_home)}")
    lines.append(f"  skills:             {_count_skills(sparkii_home)}")
