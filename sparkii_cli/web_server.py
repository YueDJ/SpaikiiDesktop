...
try:
    ...
    # running `sparkii dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["SPARKII_WEB_DIST"]) if "SPARKII_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``sparkii gateway run`` — but the
    desktop app spawns a ``sparkii dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same SPARKII_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``sparkii_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from sparkii_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `sparkii update` current before the
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `sparkii update` kept 500ing every
    ...
    # Import sparkii_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (SPARKII_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `sparkii
...
app = FastAPI(title="Sparkii Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from sparkii_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# SPARKII_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("SPARKII_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Sparkii-Session-Token"
...
#
# This list is defined in ``sparkii_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from sparkii_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Sparkii-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``sparkii-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from sparkii_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from sparkii_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from sparkii_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `sparkii egress setup` "
            "and `sparkii egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Sparkii (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during sparkii update. "
...
from sparkii_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Sparkii provider slug — fine.
    ...
    1. Vendor-name → Sparkii-provider mapping: when the provider string is
       not a known Sparkii provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Sparkii
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from sparkii_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Sparkii process is using it.
    """
    from sparkii_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "SPARKII_DASHBOARD_FILES_ROOT"
...
# SPARKII_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to SPARKII_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Sparkii credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no SPARKII_HOME resolution, so it blocks these trees wherever
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``sparkii update``.
    ...
    a container — e.g. the sparkii-webui image sharing the Sparkii source tree),
    the dashboard's ``sparkii update`` button is the correct update path and
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Sparkii inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``sparkii:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.sparkii-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``sparkii_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from sparkii_cli import web_git as _web_git  # noqa: E402
...
from sparkii_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from sparkii_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from sparkii_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Sparkii home
# (get_default_sparkii_root()) — NOT the profile-scoped SPARKII_HOME — so every
...
def _read_or_create_install_id() -> Optional[str]:
    """Read (or mint + persist) the install id under the root Sparkii home.
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors sparkii_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_sparkii_home() at call time
    ...
    try:
        ...
        # SPARKII_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of sparkii_cli.gateway blocks the
        ...
        # and which providers are registered so ``sparkii status`` and the
        ...
        try:
            from sparkii_cli.auth import get_nous_session_validity
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_sparkii_home())
            )
        ...
        try:
            ...
            status["memory"] = await asyncio.get_running_loop().run_in_executor(
                ...
                functools.partial(
                    ...
                    profile_dir if profile_dir else get_sparkii_home(),
                ),
            )
        except Exception:
        ...
        # disk-exhaustion incidents). One statvfs call on SPARKII_HOME's
        ...
        try:
            from sparkii_state import SessionDB as _SDB
            from sparkii_constants import get_sparkii_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Sparkii Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the sparkii home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_sparkii_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from sparkii_cli.auth import get_nous_auth_status_local
...
# streamed to a per-action log file under ``~/.sparkii/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_sparkii_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "sparkii-update": "sparkii-update.log",
...
def _spawn_sparkii_action(
    ...
    *,
    ...
    """Spawn ``sparkii <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``sparkii_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "sparkii_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _SPARKII_GATEWAY=1. Inheriting it makes a spawned `sparkii gateway restart`
    ...
    action_env = {**os.environ, "SPARKII_NONINTERACTIVE": "1"}
    action_env.pop("_SPARKII_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("SPARKII_ACTION_ID")
    if action_id:
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["sparkii", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``sparkii gateway restart``, reusing an in-flight restart.
    ...
    proc = _spawn_sparkii_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``sparkii gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``SPARKII_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_sparkii():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Sparkii updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("sparkii-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "sparkii-update",
    ...
    existing = _ACTION_PROCS.get("sparkii-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("sparkii-update")
        if action_id:
    ...
    try:
        ...
        _log.exception("Failed to spawn sparkii update")
...
async def check_sparkii_update(force: bool = False):
    ...
    ``POST /api/sparkii/update`` actually runs ``sparkii update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Sparkii version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Sparkii is managed by Termux APT; run `pkg upgrade sparkii-agent`."
        )
        return payload
    ...
    try:
        ...
        if force:
            try:
                (get_sparkii_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.sparkii/config.yaml`` under ``tts.``.
    """
...
from sparkii_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from sparkii_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from sparkii_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from sparkii_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Sparkii supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_sparkii_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/sparkii is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (SPARKII_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_sparkii_home()))
            return
...
async def get_egress_status():
    ...
    from sparkii_cli.proxy_cli import format_status_text
...
# in sparkii_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from sparkii_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `sparkii model` does so GUI onboarding lands on a
    ...
    try:
        ...
        from sparkii_cli.models import pick_silent_default_model
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.sparkii/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from sparkii_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (sparkii_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from sparkii_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``sparkii model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from sparkii_cli.main import _auto_provider_name, _save_custom_provider
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different SPARKII_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_sparkii_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``sparkii model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `sparkii model` provider — would be
        ...
        # Vertex — a `sparkii model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `sparkii model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Sparkii from Telegram DMs, groups, and topics.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Sparkii to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Sparkii in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Sparkii through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Sparkii via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Sparkii through an IMAP/SMTP mailbox.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Sparkii to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Sparkii inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Sparkii to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Sparkii through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Sparkii to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Sparkii to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Sparkii.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Sparkii from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Sparkii over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Sparkii through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Sparkii over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Sparkii to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Sparkii as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Sparkii via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Sparkii Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in sparkii gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in sparkii gateway setup",
...
# sparkii_cli/setup_hidden_env.py. Shared with the `sparkii setup gateway`
# wizard so the surfaces ask for the same things.
from sparkii_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # SPARKII_HOME contextvar override (#56986 / #69143), so the profile's
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.sparkii-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"SparkiiDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Sparkii Agent").strip() or "Sparkii Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Sparkii appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the SPARKII_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized SPARKII_HOME)
...
# the canonical ``sparkii auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.sparkii/.anthropic_oauth.json`` — Sparkii-managed PKCE flow (what
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Sparkii even
    when they also have a separate Sparkii-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Sparkii never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "sparkii auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "sparkii auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "sparkii auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "sparkii auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "sparkii auth add xai-oauth",
        "docs_url": "https://sparkii-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "sparkii auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from sparkii_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Sparkii, so the disconnect
    ...
    user sees exactly what executes, and Sparkii then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Sparkii — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``sparkii model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `sparkii model` universe (zero-edit for new plugins).
    try:
        from sparkii_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"sparkii auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `sparkii model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Sparkii-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from sparkii_cli.auth import clear_provider_auth
            ...
            try:
                from sparkii_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.sparkii/.anthropic_oauth.json AND credential pool
...
# Guarded so sparkii web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Sparkii file AND credential pool.
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "sparkii-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("SPARKII_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from sparkii_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``sparkii auth add minimax-oauth``.
    """
    from sparkii_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `sparkii auth add xai-oauth`: first credential may become
            ...
            # prior ``sparkii auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``sparkii model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Sparkii "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from sparkii_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `sparkii update` until the first message forced a writable open.
    """
    from sparkii_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from sparkii_state import SessionDB, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``sparkii serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `sparkii sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_sparkii_home()
...
async def get_logs(
    ...
    from sparkii_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_sparkii_home() / "logs" / log_name
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (SPARKII_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.sparkii``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    SPARKII_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SPARKII_HOME)."""
    from sparkii_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_sparkii_home_override(str(home))
    try:
...
from sparkii_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from sparkii_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/sparkii_cli/
        # test_config_read_guard.py). The SPARKII_HOME override scopes
...
async def _forward_cron_fire_to_gateway(
    ...
    → internal_port 9219), but cron execution belongs to the GATEWAY process:
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``sparkii gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (sparkii_cli.mcp_config), so
# servers managed here show up under `sparkii mcp list` and vice versa.  Secrets
...
from sparkii_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from sparkii_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from sparkii_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
    ...
    try:
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = SparkiiTokenStorage(flow.server_name)
...
#
# Wraps the same JSON store the CLI uses (sparkii_cli.webhook); the webhook
...
async def list_webhooks():
    import sparkii_cli.webhook as wh
...
# spawn the real `sparkii gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from sparkii_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `sparkii auth remove`) can seed again.
        # Mirrors the `sparkii auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``sparkii auth remove`` uses: each source cleans up its
...
def _dashboard_backup_dir() -> Path:
    return get_sparkii_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"sparkii-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def list_hooks():
    ...
    def _run():
        ...
        try:
            from sparkii_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_sparkii_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``sparkii`` subprocess, and
...
from sparkii_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from sparkii_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_sparkii_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "sparkii setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local SPARKII_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local SPARKII_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the SPARKII_HOME override. Returns the
    number of skills newly disabled.
    """
    from sparkii_constants import set_sparkii_home_override, reset_sparkii_home_override
    from sparkii_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_sparkii_home()`` at call
       time — the context-local override from ``set_sparkii_home_override``
    ...
    retargeted to the *current* ``get_sparkii_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a SPARKII_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_sparkii_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_sparkii_home()`` at call time
...
from sparkii_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from sparkii_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Sparkii,
...
#
# The endpoint spawns the same ``sparkii --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from sparkii_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from sparkii_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from sparkii_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from sparkii_cli.dashboard_auth.audit import AuditEvent, audit_log
        from sparkii_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``sparkii --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SPARKII_TUI_RESUME`` env var —
    matching what ``sparkii_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``SPARKII_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``SPARKII_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``SPARKII_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``SPARKII_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_sparkii_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``sparkii -p <name>`` performs. The in-process
    ``SPARKII_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from sparkii_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Sparkii TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides SPARKII_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("SPARKII_TUI_DISABLE_MOUSE", "1")
    env.setdefault("SPARKII_TUI_INLINE", "1")
    ...
    env["SPARKII_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["SPARKII_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["SPARKII_TUI_RESUME"] = resume

    if sidecar_url:
        env["SPARKII_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile SPARKII_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["SPARKII_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``SPARKII_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("SPARKII_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from sparkii_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="sparkii-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Sparkii Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Sparkii CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="sparkii-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from sparkii_cli.console_engine import SparkiiConsoleEngine

        engine = SparkiiConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Sparkii Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Sparkii inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("SPARKII_TUI_RESUME") or raw_resume
...
# SPARKII_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`sparkii_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from sparkii_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Sparkii Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="sparkii-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    """
    # `sparkii serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("SPARKII_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (sparkii serve): web UI disabled — use "
            "`sparkii dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/sparkii``)
        ...
        ``__SPARKII_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__SPARKII_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SPARKII_BASE_PATH__="{prefix}";'
                f"window.__SPARKII_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__SPARKII_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__SPARKII_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SPARKII_BASE_PATH__="{prefix}";'
                f"window.__SPARKII_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``SPARKII_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Sparkii Teal canvas, then
    ...
    # under ``/sparkii`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Sparkii backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Sparkii Teal",         "description": "Classic dark teal — the canonical Sparkii look"},
    {"name": "default-large", "label": "Sparkii Teal (Large)", "description": "Sparkii Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.sparkii/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.sparkii/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_sparkii_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``SPARKII_HOME``.
    """
    themes_dir = get_process_sparkii_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.sparkii/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as sparkii_cli.plugins):
    1. User plugins:    ~/.sparkii/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.sparkii/plugins/  (only if SPARKII_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from sparkii_cli.plugins import get_bundled_plugins_dir
    ...
    # SPARKII_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``SPARKII_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the sparkii root (``~/.sparkii/plugins``). Scan the default
    # root as well (``get_default_sparkii_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``SPARKII_HOME`` unchanged when it *is* the root), mirroring how
    # ``sparkii_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from sparkii_constants import get_default_sparkii_root

    user_plugin_roots = [get_process_sparkii_home() / "plugins"]
    root_plugins = get_default_sparkii_root() / "plugins"
    ...
    # ``sparkii_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("SPARKII_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".sparkii" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from sparkii_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_sparkii_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"sparkii auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from sparkii_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from sparkii_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sparkii_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sparkii_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.sparkii/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.sparkii/plugins/ if you trust it)",
...
from sparkii_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``SPARKII_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("SPARKII_DESKTOP_READY_FILE")
    if not target:
        return
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``SPARKII_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("SPARKII_PARENT_PID")
    start_marker = os.environ.get("SPARKII_PARENT_START_MARKER")
    nonce = os.environ.get("SPARKII_PARENT_NONCE")
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``SPARKII_SERVE_HEADLESS``), so
    ...
    try:
        from sparkii_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the sparkii-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing SPARKII_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Sparkii Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `sparkii dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`sparkii plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # SPARKII_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "SPARKII_BACKEND_READY" if headless else "SPARKII_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  Sparkii backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Sparkii Web UI → http://{host}:{actual_port}")
