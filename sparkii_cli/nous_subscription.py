...
# coverage category (sparkii_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `sparkii tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``sparkii status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Sparkii-managed Node dirs (Windows installer / POSIX $SPARKII_HOME/node)
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("SPARKII_LOCAL_STT_COMMAND"):
        return True
    try:
    ...
    *,
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("SPARKII_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("SPARKII_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from sparkii_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`sparkii tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``sparkii tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``sparkii model``'s Nous
    login, this:
