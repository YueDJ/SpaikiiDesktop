"""Slash command definitions and autocomplete for the Sparkii CLI.
...
class CommandDef:
    ...
    # Names a key in ``sparkii_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``sparkii_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Sparkii config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Sparkii works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Sparkii' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Sparkii is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sparkii/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Sparkii Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Sparkii Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sparkii_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/sparkii`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Sparkii ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Sparkii
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``sparkii skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sparkii skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sparkii skills list`` and
...
# unrelated command landed. These claim their slots right after /sparkii,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /sparkii <command>).
...
# native slot, the alias spelling stays reachable via /sparkii reset).
...
# ``/sparkii <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/sparkii" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /sparkii topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /sparkii moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /sparkii debug on Slack.
#   - egress: Docker-only proxy status; reachable as /sparkii egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /sparkii init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /sparkii version on
...
#   - diff: git working-tree diff; reached via /sparkii diff on Slack so it
...
#     /sparkii update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /sparkii heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /sparkii refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /sparkii pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /sparkii whoami on Slack.
...
#     /sparkii platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_SPARKII_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/sparkii <verb>`` subcommand.
    ...
    can still reach them via ``/sparkii <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/sparkii`` is always reserved as the first entry so the
    legacy ``/sparkii <subcommand>`` form keeps working for anything that
    ...
    # Reserve /sparkii as the catch-all top-level command.
    entries.append(("sparkii", "Talk to Sparkii or run a subcommand", "[subcommand] [args]"))
    seen.add("sparkii")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SPARKII_ONLY:
            # Intentionally Slack-via-/sparkii only (see _SLACK_VIA_SPARKII_ONLY).
            return
    ...
    # everything except /sparkii, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://sparkii-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /sparkii handler.

    Maps both canonical names and aliases so /sparkii bg do stuff works
    the same as /sparkii background do stuff.

    Plugin-registered slash commands are included so ``/sparkii <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from sparkii_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via sparkii_cli.personality."""
        try:
