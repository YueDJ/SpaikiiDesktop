"""
Gateway subcommand for sparkii CLI.

Handles: sparkii gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_sparkii_home is imported lazily at call sites to avoid ImportError
# when sparkii_constants is cached from a pre-update version during `sparkii update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "sparkii CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `sparkii update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``sparkii gateway status`` from falsely counting the ``sparkii`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``sparkii gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_sparkii_home().resolve())
    # Forward slashes on both sides of the SPARKII_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. SPARKII_HOME
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``sparkii update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``sparkii update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Sparkii profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Sparkii profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sparkii_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m sparkii_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``sparkii update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Sparkii's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose SPARKII_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``sparkii update`` users
    # with no running gateway until they re-invoke ``sparkii gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (SPARKII_HOME,
    ...
    if sys.platform == "win32":
        try:
            from sparkii_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / SPARKII_HOME) the windowless
        # base interpreter needs to import sparkii_cli.  Empty on POSIX, where
...
def _sparkii_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``SPARKII_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("SPARKII_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_sparkii_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SPARKII_HOME``.

    Under ``sudo``, ``SPARKII_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_sparkii_home` falls back to ``/root/.sparkii`` — the wrong
    profile. The unit file pins ``SPARKII_HOME`` for the actual gateway
    ...
    unit_home = (_sparkii_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("SPARKII_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SPARKII_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SPARKII_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}sparkii gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}sparkii gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from sparkii_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: sparkii gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `sparkii gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``sparkii gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from sparkii_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (sparkii_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Sparkii_Gateway /
            # Sparkii_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `sparkii gateway install` deployment.
            from sparkii_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Sparkii_Gateway"
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SPARKII_HOME-scoped).
...
from sparkii_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Sparkii Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``sparkii gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SPARKII_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SPARKII_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from sparkii_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "sparkii-gateway"
SERVICE_DESCRIPTION = "Sparkii Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SPARKII_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SPARKII_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SPARKII_HOME paths
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SPARKII_HOME.

    Default ``~/.sparkii`` returns ``sparkii-gateway`` (backward compatible).
    Profile ``~/.sparkii/profiles/coder`` returns ``sparkii-gateway-coder``.
    Any other SPARKII_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Sparkii installs that predate the
# sparkii-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (sparkii-gateway-*.service) and unrelated third-party
# "sparkii" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("sparkii.service",)
...
def _find_legacy_sparkii_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``sparkii.service`` before the rename to
    ``sparkii-gateway.service``). When both a legacy unit and the current
    ``sparkii-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``sparkii-gateway-coder.service`` and unrelated third-party
      ``sparkii-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``sparkii.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Sparkii gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current sparkii-gateway service and")
...
def remove_legacy_sparkii_units(
    ...
    print()
    print("Legacy Sparkii gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: sparkii gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo sparkii gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sparkii gateway uninstall")
    print_info("    sudo sparkii gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: sparkii gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``sparkii setup`` (end of wizard)
    and ``sparkii import``: if this host supports a service manager and no
    ...
    ``sparkii gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   sparkii gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: sparkii gateway")
        return False
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from sparkii_cli import gateway_windows
        ...
        print_info("  You can retry manually: sparkii gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Sparkii often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.sparkii`` → ``ai.sparkii.gateway.plist`` (backward compatible).
    Profile ``~/.sparkii/profiles/coder`` → ``ai.sparkii.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.sparkii.gateway-{suffix}" if suffix else "ai.sparkii.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # sparkii_constants cached from before this symbol existed. See
            # _reload_sparkii_constants() in sparkii_cli/managed_uv.py.
            from sparkii_cli.managed_uv import _reload_sparkii_constants

            venv_python_path = _reload_sparkii_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.sparkii/sparkii-agent  -> /home/alice/.sparkii/sparkii-agent
      /opt/sparkii                 -> /opt/sparkii  (kept as-is)
...
def _sparkii_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SPARKII_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_sparkii_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.sparkii                    → /home/alice/.sparkii
    ...
    # different SPARKII_HOME into the generated service unit.
    current_default = Path.home() / ".sparkii"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m sparkii_cli.main``, so module
    ...
    dir, or a clone that ``sparkii update`` later relocates/removes — the path
    ...
    ``SPARKII_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if SPARKII_HOME
...
def _append_node_dir_for_service(
    ...
    The Sparkii-managed Node under ``$SPARKII_HOME/node`` goes first when it
    ...
    *sparkii_root* is the Sparkii home the unit will run against. System units
    ...
    managed_node_present = sparkii_managed_node_tree_present(sparkii_root)
    for directory in iter_sparkii_node_dirs(sparkii_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target Sparkii home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Sparkii home is known — probing here would stat the calling
...
Group={group_name}
ExecStart={python_path} -m sparkii_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m sparkii_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="SPARKII_HOME={sparkii_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned SPARKII_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo sparkii gateway … --system``, SPARKII_HOME is often stripped
    # and falls back to ``/root/.sparkii``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / SPARKII_HOME comparisons
    ...
    _sync_sparkii_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir SPARKII_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved SPARKII_HOME
    ...
    exported a throwaway ``SPARKII_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``sparkii gateway restart``
    with ``SPARKII_HOME=/tmp/sparkii-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="SPARKII_HOME=..."`` lines and launchd
    ``<key>SPARKII_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'SPARKII_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>SPARKII_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp SPARKII_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: SPARKII_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported SPARKII_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single SPARKII_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # SPARKII_HOME into the unit (manual E2E homes like /tmp/sparkii-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Sparkii install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo sparkii gateway uninstall --system")
    print_info("         sparkii gateway install")
    print_info("         sparkii gateway start")
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("SPARKII_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (sparkii.service from pre-rename installs)
    # before installing the new sparkii-gateway.service. If both remain, they
    ...
    # Existing system units already pin SPARKII_HOME; adopt it before any
    ...
    # ``sudo sparkii gateway install --system --force`` would bake /root/.sparkii
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}sparkii gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}sparkii gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}sparkii gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # SPARKII_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `sparkii gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `sparkii gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}sparkii gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}sparkii gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}sparkii gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sparkii gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.sparkii.gateway-{suffix}" if suffix else "ai.sparkii.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``sparkii gateway start/stop/status`` call).
...
#      background process (the `nohup sparkii gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_sparkii_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_sparkii_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m sparkii_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active SPARKII_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "sparkii_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``sparkii update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup sparkii gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from sparkii_constants import display_sparkii_home as _dhh
    ...
    print(
        f"  Try manually: nohup sparkii gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `sparkii gateway start`)
    ...
    nofile_block = ""
    try:
        from sparkii_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.sparkii/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See sparkii-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_sparkii_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Sparkii install"
    )
    return True
...
def launchd_stop():
    ...
    # `sparkii gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `sparkii update`
...
def launchd_status(deep: bool = False):
    ...
    # Sparkii PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Sparkii PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Sparkii install")
    else:
        print("⚠ Service definition is stale relative to the current Sparkii install")
        print("  Run: sparkii gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: sparkii gateway stop")
            else:
    ...
    if deep:
        log_file = get_sparkii_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``SPARKII_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``sparkii gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        from sparkii_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``sparkii gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``sparkii gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other SPARKII_HOME roots.
    """
    ...
    print("  Use 'sparkii gateway restart' to replace it,")
    print("  or 'sparkii gateway stop' first.")
    print("  Or use 'sparkii gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SPARKII_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Sparkii gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'sparkii' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/sparkii/docker/entrypoint.sh before the Sparkii command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SPARKII_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set SPARKII_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `sparkii gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SPARKII_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("SPARKII_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `sparkii gateway restart` which already
    ...
    # `sparkii gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Sparkii Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # SPARKII_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SPARKII_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``SPARKII_GATEWAY_MAX_STARTS`` /
    # ``SPARKII_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("SPARKII_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("SPARKII_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``sparkii gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. sparkii) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Sparkii delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Sparkii connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: sparkii pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Sparkii will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``sparkii setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run sparkii under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.sparkii/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_sparkii_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from sparkii_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from sparkii_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'sparkii pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: sparkii pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'sparkii gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Sparkii will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Sparkii will store the returned account_id/token in ~/.sparkii/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `sparkii gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_sparkii_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `sparkii pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `sparkii pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "SparkiiAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``sparkii_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.sparkii/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.sparkii/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: sparkii gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: sparkii gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: sparkii gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo sparkii gateway install --system"
                        )
                    print_info("  Or run in foreground:  sparkii gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: sparkii gateway run")
                print_info(
                    "  For persistence:   tmux new -s sparkii 'sparkii gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: sparkii gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup sparkii gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'sparkii gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`sparkii_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from sparkii_cli.service_manager import (
    ...
    if profile is None:
        ...
        # SPARKII_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``sparkii gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``sparkii gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``sparkii gateway run`` are unaffected.
      2. ``SPARKII_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``sparkii gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``SPARKII_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("SPARKII_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("SPARKII_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${SPARKII_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if SPARKII_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or SPARKII_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-sparkii service's
    # pattern in docker/s6-rc.d/main-sparkii/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: sparkii gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: sparkii gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s sparkii 'sparkii gateway run'"
                )
                print()
            ...
            print()
            print(
                "  sparkii gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s sparkii 'sparkii gateway run'         # persistent via tmux"
            )
            print(
                "  nohup sparkii gateway run > ~/.sparkii/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from sparkii_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  sparkii profile create <name>     # creates the s6 service slot")
                print("  sparkii -p <name> gateway start   # bring it up via s6")
                print("  sparkii status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: sparkii gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: sparkii gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: sparkii gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  sparkii profile delete <name>     # tears down the s6 service slot")
                print("  sparkii -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `sparkii -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: sparkii gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_SPARKII_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `sparkii gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_SPARKII_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `sparkii gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: sparkii gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sparkii gateway install")
                else:
                    print("To install as a service:")
                    print("  sparkii gateway install")
                    print("  sudo sparkii gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  sparkii gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup sparkii gateway run > ~/.sparkii/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s sparkii 'sparkii gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  sparkii gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  sparkii gateway install  # Install as user service")
                    print(
                        "  sudo sparkii gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Sparkii gateway unit files from
        # pre-rename installs (e.g. sparkii.service). Profile units and
        # unrelated third-party services are never touched.
