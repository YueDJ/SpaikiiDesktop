"""Dashboard process-hygiene helpers — extracted from ``sparkii_cli/main.py``.
...
``sparkii_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``sparkii_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``sparkii_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``sparkii dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sparkii update`` replaces files on disk, the running
    ...
    ``sparkii-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Sparkii Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``sparkii serve`` as
    ...
    environment variable ``SPARKII_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`sparkii serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "sparkii.*dashboard"`) keeps us consistent
            # with `sparkii_cli.gateway._scan_gateway_pids` and avoids the
...
def _sparkii_home_for_pid(pid: int) -> str | None:
    """Best-effort ``SPARKII_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("SPARKII_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"SPARKII_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Sparkii Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``sparkii update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], sparkii_home: str | None = None
) -> str:
    """Stable owner key: ``SPARKII_HOME`` when known, else ``--profile`` / ``-p``.

    ``SPARKII_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.sparkii`` roots) keep a resolved ``home:`` key so
    ...
    if sparkii_home:
        try:
            home_path = Path(sparkii_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(sparkii_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(sparkii_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``sparkii update``.

    Each candidate is ``(pid, argv, sparkii_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``SPARKII_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``SPARKII_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``sparkii update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, sparkii_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, sparkii_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``sparkii dashboard`` / ``sparkii serve`` processes.

    Called at the end of ``sparkii update`` (default ``reason``) and also
    from ``sparkii dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``sparkii update`` path), a detected
    ``sparkii-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``sparkii-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``sparkii update``'s systemd
    fleet-restart loop, which restarts ``sparkii-serve*`` units before this
    ...
    # When the Sparkii Desktop Electron app spawns this dashboard as a
    # backend child, it sets SPARKII_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SPARKII_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `sparkii update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot SPARKII_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _sparkii_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. sparkii update's
    ...
    #    remote backend (sparkii serve) under Restart=on-failure never comes
...
def _detect_concurrent_sparkii_instances(
    ...
    The Sparkii Desktop Electron app spawns ``sparkii.EXE`` as a backend child,
    so during ``sparkii update`` the user-invoked process and the desktop's
    ...
    shims (``sparkii.exe`` / ``sparkii-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``sparkii update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``sparkii.exe``) is a separate process from the Python
    ...
    # setuptools-generated sparkii.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``sparkii update``
    ...
    #      sparkii.exe sitting *under* a non-Sparkii parent (e.g. a Sparkii
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        sparkii serve --host 127.0.0.1 --port 0
        sparkii serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "sparkii" not in cmd and "sparkii_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (SPARKII_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("SPARKII_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``sparkii serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``sparkii serve`` on a Mac
...
def _sparkii_home_dir() -> Path:
    """Resolved Sparkii home (SPARKII_HOME override or ~/.sparkii)."""
    override = os.environ.get("SPARKII_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "sparkiiPath", "sparkiiHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated SPARKII_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    root = base_dir if base_dir is not None else (
        _sparkii_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local sparkii serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``sparkii serve`` backends with no parent.
    ...
    running under SPARKII_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never SPARKII_DESKTOP_CHILD_PID entries
