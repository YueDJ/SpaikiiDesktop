...
configured for "cli" in `sparkii tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (SPARKII_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `sparkii chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"sparkii -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "sparkii -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"sparkii -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "sparkii -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "sparkii -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to SPARKII_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("SPARKII_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "sparkii -z: --provider requires --model (or SPARKII_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"sparkii -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("sparkii -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``sparkii -z`` / oneshot mode.

    Oneshot bypasses ``SparkiiCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    env_model = os.getenv("SPARKII_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from sparkii_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("SPARKII_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # SparkiiCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                SPARKII_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via SPARKII_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via SPARKII_YOLO_MODE=1
