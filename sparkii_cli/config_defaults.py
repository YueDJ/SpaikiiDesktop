"""Default configuration data for Sparkii Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from sparkii_cli/config.py. Must not import from sparkii_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["sparkii-cli"],
    # SQLite journal mode used by every Sparkii database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Sparkii server processes.
    ...
    "session": {
        # Per-terminal `sparkii -c`: each CLI session drops a breadcrumb file
        # under $SPARKII_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # budget. CLI one-shot equivalent: `sparkii chat --run-budget N`.
        ...
        # an interactive `sparkii gateway restart` must never block for hours
        ...
        # the Sparkii-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Sparkii
        ...
        # identity slot (SOUL.md). Empty by default. The SPARKII_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Sparkii adds a coding operating brief
        ...
        # `sparkii status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SPARKII_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             SPARKII_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SPARKII_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Sparkii auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Sparkii sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Sparkii
        ...
        # bundled Sparkii image, which drops to the `sparkii` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Sparkii sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Sparkii should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``sparkii chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sparkii/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: sparkii sweeps the checkpoint base at startup
        ...
        # ``sparkii checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .sparkii.md, or .cursorrules before Sparkii applies
    ...
    # Single-query (``sparkii -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Sparkii truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Sparkii' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); sparkii =
                                      # Sparkii' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Sparkii' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Sparkii does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``sparkii kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``sparkii kanban decompose`` and the kanban
        ...
        # ``sparkii profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `sparkii model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `sparkii` (and `sparkii chat`) launches by default:
        ...
        # REPL and `--tui` (or SPARKII_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `sparkii --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `sparkii -c` muscle memory.  Default off so existing
        # users aren't surprised.  SPARKII_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `sparkii --tui` drops a one-time hint
        ...
        # model — see sparkii_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to SPARKII_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/sparkii`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `sparkii pets`. Disabled until
        ...
        "pet": {
            ...
            # get_sparkii_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``SPARKII_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SPARKII_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SPARKII_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SPARKII_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SPARKII_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/sparkii``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Sparkii appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sparkii/cache/piper-voices/
    ...
    # "Hey Sparkii" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey sparkii",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_sparkii" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sparkii/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.sparkii/cache/delegation/
    ...
    # response. If not, Sparkii feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Sparkii auto-pauses the goal and
    ...
    "moa": {
        ...
        # <sparkii_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.sparkii/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.sparkii/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``sparkii skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``sparkii skills trust`` / ``sparkii skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${SPARKII_SKILL_DIR} and ${SPARKII_SESSION_ID} in SKILL.md
        ...
        # (`sparkii skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.sparkii/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.sparkii/.curator_backups/blobs/. Enables `sparkii curator ledger`
        # and single-mutation `sparkii curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `sparkii curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `sparkii curator run --consolidate` overrides
        ...
        # normally restored on every `sparkii update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `sparkii curator purge` deletes
        ...
        # skipped), snapshot ~/.sparkii/skills/ into
        # ~/.sparkii/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sparkii curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for sparkii-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Sparkii Agent*" header.
    ...
    # dangerous command. -q runs export SPARKII_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SPARKII_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Sparkii' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.sparkii/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SPARKII_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .sparkii config) always require human
        ...
        # longer trigger the startup banner. Add via `sparkii doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``sparkii_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Sparkii to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SPARKII_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SPARKII_CRON_MAX_PARALLEL env var.
        ...
        # SPARKII_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SPARKII_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via SPARKII_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `sparkii -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `sparkii` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `sparkii kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     sparkii-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Sparkii tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.sparkii/logs/.
    ...
    # update model picker lists without shipping a sparkii-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://sparkii-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `sparkii model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Sparkii provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal SPARKII_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.sparkii/sessions/sessions.json entirely.
        ...
        # the SPARKII_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.sparkii.gateway` that defenses 1-2 don't
        ...
        # `sparkii update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SPARKII_GATEWAY_MAX_STARTS`` / ``SPARKII_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.sparkii/.env,
        ...
        # behavior: files must live under the Sparkii cache, under
        ...
        # user. Bridged to SPARKII_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Sparkii
        # cache (~/.sparkii/cache/{documents,images,audio,video,screenshots})
        ...
        # to SPARKII_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SPARKII_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SPARKII_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.sparkii/state.db.
    ...
    "sessions": {
        ...
        # the default of ``sparkii sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.sparkii/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `sparkii sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `sparkii update` prints a one-line notice with
        ...
        # ~/.sparkii/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SPARKII_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SPARKII_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `sparkii sessions export`. Checked
    ...
    # ``sparkii doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `sparkii doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``sparkii update`` behaviour.
    "updates": {
        ...
        #     DBs) into <SPARKII_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``sparkii backup``-style zip
        #     of SPARKII_HOME into <SPARKII_HOME>/backups/, restorable with
        #     ``sparkii import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `sparkii update` does with uncommitted local changes to the
        ...
        # When `sparkii update` finds the source checkout parked on a feature
    ...
    "lsp": {
        ...
        #              ``<SPARKII_HOME>/lsp/bin/`` on first use.
    ...
    # `sparkii tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.sparkii/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.sparkii/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.sparkii/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Sparkii may use this encrypted
            ...
            # ~/.sparkii/bin/ on first use.  When False you must install
            ...
            # `sparkii secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.sparkii/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Sparkii disables it for our users unless they
        # explicitly opt in here. When false (default), Sparkii sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Sparkii also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Sparkii computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Sparkii passes the
    ...
    #
    # Configure with `sparkii egress setup`.  Disabled by default — the rest of
    # Sparkii works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.sparkii/bin/ on
    ...
    # Sparkii Desktop (Electron app) launch options. These only affect
    # `sparkii desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SPARKII_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the SPARKII_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Sparkii mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sparkii model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Sparkii auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Sparkii' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Sparkii, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @sparkii:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SPARKII_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #sparkii)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sparkii-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sparkii-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Sparkii API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Sparkii API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Sparkii API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SPARKII_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SPARKII_TOOL_PROGRESS_MODE for backward
    ...
    # SPARKII_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "SPARKII_PREFILL_MESSAGES_FILE": {
    ...
    "SPARKII_EPHEMERAL_SYSTEM_PROMPT": {
