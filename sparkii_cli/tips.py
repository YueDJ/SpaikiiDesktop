...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or sparkii -c.",
    ...
    "Ctrl+Z suspends Sparkii to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "sparkii -c resumes your most recent CLI session. sparkii -c \"project name\" resumes by title.",
    "sparkii -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "sparkii -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "sparkii chat -t web,terminal enables only specific toolsets for a focused session.",
    "sparkii chat -s github-pr-workflow preloads a skill at launch.",
    "sparkii chat -q \"query\" runs a single non-interactive query and exits.",
    "sparkii chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "sparkii chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "sparkii --yolo bypasses all dangerous command approval prompts for the entire session.",
    "sparkii chat --source telegram tags the session for filtering in sparkii sessions list.",
    "sparkii -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "sparkii doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "sparkii dump outputs a compact setup summary — great for bug reports.",
    "sparkii config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "sparkii config edit opens config.yaml in your default editor.",
    "sparkii config check scans for missing or stale configuration options.",
    "sparkii sessions browse opens an interactive session picker with search.",
    "sparkii sessions stats shows session counts by platform and database size.",
    "sparkii sessions prune --older-than 30 cleans up old sessions.",
    "sparkii skills search react --source skills-sh searches the skills.sh public directory.",
    "sparkii skills check scans installed hub skills for upstream updates.",
    "sparkii skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "sparkii skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "sparkii mcp add github --command npx adds MCP servers from the command line.",
    "sparkii mcp serve runs Sparkii itself as an MCP server for other agents.",
    "sparkii auth add lets you add multiple API keys for credential pool rotation.",
    "sparkii completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "sparkii logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "sparkii backup creates a zip backup of your entire Sparkii home directory.",
    "sparkii profile create coder creates an isolated profile that becomes its own command.",
    "sparkii profile create work --clone copies your current config and keys to a new profile.",
    "sparkii update syncs new bundled skills to ALL profiles automatically.",
    "sparkii gateway install sets up Sparkii as a system service (systemd/launchd).",
    "sparkii memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "sparkii webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: sparkii tools disables unused tools, sparkii skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "sparkii models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as sparkii -w).",
    ...
    "SPARKII_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Sparkii tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'sparkii profile create coder' creates the 'coder' command.",
    "sparkii profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Sparkii prints a resume command with session ID and stats.",
    "sparkii sessions export backup.jsonl exports all sessions for backup or analysis.",
    "sparkii -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "sparkii skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .sparkii/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: sparkii cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Sparkii runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "sparkii gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Sparkii via a local macOS server.",
    ...
    "Context files (.sparkii.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Sparkii loads project context from .sparkii.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "sparkii mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "sparkii mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "sparkii mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — sparkii tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.sparkii/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "sparkii chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "sparkii plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "sparkii acp runs Sparkii as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SPARKII_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "sparkii auth add nous or sparkii auth add openai-codex sets up OAuth-based providers.",
    ...
    "sparkii status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.sparkii/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Sparkii your own.",
    ...
    "Before a gateway session reset, Sparkii auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.sparkii/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Sparkii state via SPARKII_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SPARKII_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SPARKII_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.sparkii/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'sparkii profile create ls' would be rejected.",
    "sparkii profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "sparkii claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SPARKII_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via sparkii tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.sparkii/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.sparkii/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Sparkii auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Sparkii config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Sparkii is working.',
    ...
    '/reload reloads ~/.sparkii/.env into the running session — pick up new API keys without restarting.',
    ...
    "SPARKII_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.sparkii/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.sparkii/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'sparkii curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "sparkii curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'sparkii curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'sparkii auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SPARKII_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.sparkii/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.sparkii/skills/ so drop-in skills appear without restarting the session.',
    ...
    'sparkii -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'sparkii chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'sparkii chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'sparkii chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "sparkii chat --source tool tags programmatic chats so they don't clutter sparkii sessions list.",
    'sparkii dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'sparkii sessions rename <ID> "new title" renames any past session; sparkii sessions delete <ID> removes one.',
    'sparkii import restores a full Sparkii backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'sparkii fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'sparkii pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'sparkii setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'sparkii status --deep runs the full health sweep across every component; plain sparkii status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SPARKII_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SPARKII_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.sparkii/plugins/ — trust-gated by design.',
    "SPARKII_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SPARKII_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SPARKII_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SPARKII_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SPARKII_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SPARKII_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SPARKII_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SPARKII_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SPARKII_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SPARKII_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SPARKII_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Sparkii block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from sparkii sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.sparkii/dashboard-plugins/.',
]
