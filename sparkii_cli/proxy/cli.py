"""CLI handlers for the ``sparkii proxy`` subcommand."""
...
from sparkii_cli.proxy.adapters import ADAPTERS, get_adapter
from sparkii_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "sparkii proxy requires aiohttp. Run `sparkii setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"sparkii auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Sparkii proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Sparkii proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: sparkii proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``sparkii proxy <subcommand>``."""
    ...
    print(
        "sparkii proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  sparkii proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
