...
# xAI's public API is OpenAI-compatible for the endpoints Sparkii commonly
# uses. The Responses endpoint is included because Sparkii' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Sparkii-managed OAuth credentials."""

    auth_hint = "sparkii auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`sparkii auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`sparkii auth reset xai-oauth` or re-authenticate with "
                    "`sparkii auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `sparkii auth add xai-oauth --type oauth`."
            )
