...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "sparkii_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("SPARKII_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("SPARKII_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("SPARKII_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["SPARKII_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["SPARKII_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["SPARKII_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("SPARKII_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("SPARKII_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("SPARKII_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    from sparkii_cli.gateway import _profile_suffix
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``SPARKII_HOME`` whenever it
    ...
    source checkout only if ``SPARKII_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SPARKII_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m sparkii_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``sparkii gateway run``.
    ...
    # if someone imports sparkii_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_sparkii_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_sparkii_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_sparkii_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "sparkii_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_sparkii_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_sparkii_home_path(entry) for entry in extra_pythonpath]]
    )
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_sparkii_home_path(get_python_path())
    )
    ...
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_sparkii_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (SPARKII_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m sparkii_cli.main
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m sparkii_cli.main gateway run`` directly — NOT
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("SPARKII_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("SPARKII_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from sparkii_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `sparkii -p <profile> gateway install` must be safe.
    ...
    from sparkii_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"sparkii {profile_arg} gateway start" if profile_arg else "sparkii gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: sparkii gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: sparkii gateway start")
        _print_next_steps()
        return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: sparkii gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: sparkii gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `sparkii gateway status`."
        )
