...
Pure logic: walks a Sparkii config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `sparkii doctor` and a future `sparkii migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Sparkii config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Sparkii config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `sparkii migrate xai` reports nothing to migrate
