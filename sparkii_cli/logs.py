...
Usage examples::
    ...
    sparkii logs                    # last 50 lines of agent.log
    sparkii logs -f                 # follow agent.log in real time
    sparkii logs errors             # last 50 lines of errors.log
    sparkii logs gateway -n 100    # last 100 lines of gateway.log
    sparkii logs gui -f            # follow gui.log (dashboard/pty/ws)
    sparkii logs desktop -f        # follow desktop.log (Electron app boot/backend)
    sparkii logs --level WARNING    # only WARNING+ lines
    sparkii logs --session abc123   # filter by session ID substring
    sparkii logs --component tools  # only tool-related lines
    sparkii logs --since 1h         # lines from the last hour
    sparkii logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_sparkii_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sparkii_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sparkii_home()}/logs/")
        return

    print(f"Log files in {display_sparkii_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sparkii chat' to generate logs)")
