# Sparkii Agent - Development Guide

Instructions for AI coding assistants and developers working on the sparkii-agent codebase.
...
Read the balance right: Sparkii ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`sparkii tools`, `sparkii setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `SPARKII_HOME`. Mocks hide integration bugs.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `sparkii tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.sparkii/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `sparkii <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `sparkii webhook`, `sparkii cron`, `sparkii tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.sparkii/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Sparkii Cloud. Only the first two are spawned
by us and carry `SPARKII_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_SPARKII_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `SPARKII_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`sparkii --tui` against that same backend) is
...
`$HOME/.sparkii/sparkii-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── cli.py                # SparkiiCLI class — interactive CLI orchestrator (~11k LOC)
├── sparkii_state.py       # SessionDB — SQLite session store (FTS5 search)
├── sparkii_constants.py   # get_sparkii_home(), display_sparkii_home() — profile-aware paths
├── sparkii_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── sparkii_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── sparkii-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `sparkii --tui`
...
**User config:** `~/.sparkii/config.yaml` (settings), `~/.sparkii/.env` (API keys only).
**Logs:** `~/.sparkii/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_sparkii_home()`.
Browse with `sparkii logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Sparkii: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`sparkii_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `SparkiiCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.sparkii/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`sparkii_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/sparkii` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `sparkii_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `SparkiiCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `sparkii --tui` or `SPARKII_TUI=1`.
...
npm run dev       # watch mode (rebuilds sparkii-ink + tsx --watch)
npm start         # production
npm run build     # full build (sparkii-ink + tsc)
...
### TUI in the Dashboard (`sparkii dashboard` → `/chat`)

The dashboard embeds the real `sparkii --tui` — **not** a rewrite.  See `sparkii_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `sparkii_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `sparkii --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `sparkii --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@sparkii/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `sparkii serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `SPARKII_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `sparkii` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `sparkii --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Sparkii built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Sparkii core.
Use the plugin route instead: create `~/.sparkii/plugins/<name>/plugin.yaml`
and `~/.sparkii/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Sparkii tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_SPARKII_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_SPARKII_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_sparkii_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `SPARKII_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_sparkii_home()` for the base directory — never `Path.home() / ".sparkii"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `sparkii_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `sparkii_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `sparkii tools`, `sparkii setup`, most CLI subcommands | `sparkii_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`sparkii_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
sparkii_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.sparkii/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Sparkii gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `sparkii_cli/skin_engine.py`:
...
Users create `~/.sparkii/skins/<name>.yaml`:
...
Sparkii has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.sparkii/plugins/` and pip-installed entry points.

### General plugins (`sparkii_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.sparkii/plugins/`, `./.sparkii/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `sparkii` at startup so
  `sparkii <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$SPARKII_HOME/plugins/`, `./.sparkii/plugins/` (opt-in via
`SPARKII_ENABLE_PROJECT_PLUGINS`), and `sparkii_agent.memory_providers` entry
...
it at argparse setup time and wires it into `sparkii <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `sparkii --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `sparkii_cli/main.py`, etc.).
...
into `~/.sparkii/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `sparkii memory setup` / `post_setup()` without
...
`~/.sparkii/plugins/` (or via pip entry points). They register through
...
2. User: `$SPARKII_HOME/plugins/model-providers/<name>/`
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `sparkii skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.sparkii.tags`, `metadata.sparkii.category`,
`metadata.sparkii.related_skills`, `metadata.sparkii.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.sparkii.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Sparkii tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Sparkii Agent" is the secondary collaborator. If the
   contributor's commit shows "Sparkii Agent" as author (because they
   used Sparkii to draft the skill), replace it with their actual name
...
lives in the `sparkii-agent-dev` skill at
...
`"messaging"`); `_SPARKII_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `sparkii tools` (the curses UI) or the
...
Sparkii returns a delegation id immediately and the result re-enters the
...
go to `~/.sparkii/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `sparkii_cli/curator.py` wires `sparkii curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.sparkii/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `sparkii cron <verb>`
...
- File lock at `~/.sparkii/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `sparkii kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `sparkii_cli/kanban.py` wires `sparkii kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`sparkii-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `SPARKII_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Sparkii-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `SPARKII_BACKGROUND_NOTIFICATIONS` env var):
...
Sparkii supports **profiles** — multiple fully isolated instances, each with its own
`SPARKII_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `sparkii_cli/main.py` sets
`SPARKII_HOME` before any module imports. All `get_sparkii_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_sparkii_home()` for all SPARKII_HOME paths.** Import from `sparkii_constants`.
   NEVER hardcode `~/.sparkii` or `Path.home() / ".sparkii"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".sparkii" / "config.yaml"
   ```

2. **Use `display_sparkii_home()` for user-facing messages.** Import from `sparkii_constants`.
   This returns `~/.sparkii` for default or `~/.sparkii/profiles/<name>` for profiles.
   ```python
   # GOOD
   from sparkii_constants import display_sparkii_home
   print(f"Config saved to {display_sparkii_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.sparkii/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_sparkii_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_sparkii_home()`,
   not `Path.home() / ".sparkii"`.

4. **Tests that mock `Path.home()` must also set `SPARKII_HOME`** — since code now uses
   `get_sparkii_home()` (reads env var), not `Path.home() / ".sparkii"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"SPARKII_HOME": str(tmp_path / ".sparkii")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not SPARKII_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".sparkii" / "profiles"`, NOT `get_sparkii_home() / "profiles"`.
   This is intentional — it lets `sparkii -p coder profile list` see all profiles regardless
...
### DO NOT hardcode `~/.sparkii` paths
Use `get_sparkii_home()` from `sparkii_constants` for code paths. Use `display_sparkii_home()`
for user-facing print/log messages. Hardcoding `~/.sparkii` breaks profiles — each profile
has its own `SPARKII_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `sparkii_cli/curses_ui.py`. See `sparkii_cli/tools_config.py` for an example.
...
with actual imports (not mocks) against a temp `SPARKII_HOME`.

### Tests must not write to `~/.sparkii/`
The `_isolate_sparkii_home` autouse fixture in `tests/conftest.py` redirects `SPARKII_HOME` to a temp dir. Never hardcode `~/.sparkii/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_sparkii_home()` resolve within the temp dir.
Use the pattern from `tests/sparkii_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `SPARKII_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.sparkii/` | Your real config+auth.json                  | Temp dir per test                         |
...
Sparkii supports Linux, macOS and native Windows, and plenty of its behaviour
