...
conversation and the click lands in another (NousResearch/sparkii-agent#88200).
...
def home(tmp_path, monkeypatch):
    """Temp SPARKII_HOME with the default profile plus one named profile."""
    h = tmp_path / ".sparkii"
    (h / "profiles" / "ops").mkdir(parents=True)
    monkeypatch.setenv("SPARKII_HOME", str(h))
    return h
