...
(``SPARKII_DISABLE_LAZY_INSTALLS=1``) and lazy installs are redirected to a
writable durable target (``SPARKII_LAZY_INSTALL_TARGET``). honcho/hindsight
...
class TestSupermemoryEnsureCalled:
    def test_client_construction_calls_ensure(self, monkeypatch):
        ...
        _SupermemoryClient(api_key="k", timeout=5.0, container_tag="sparkii")
...
#
# This is the exact hosted-Fly condition: SPARKII_DISABLE_LAZY_INSTALLS=1 seals
# the venv, but SPARKII_LAZY_INSTALL_TARGET redirects installs to a writable
...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("SPARKII_LAZY_INSTALL_TARGET")
