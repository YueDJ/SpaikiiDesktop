...
the legacy ``SPARKII_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "sparkii-agent-main"
...
assert (PR_DIR / "tools" / "transcription_tools.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a sparkii-agent checkout"
)
...
# Isolated SPARKII_HOME so the config write is hermetic.
...
for name in list(sys.modules):
    if (name.startswith("tools.")
            ...
            or name.startswith("sparkii_cli.")):
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'sparkii-agent-main' checkout pinned to "
