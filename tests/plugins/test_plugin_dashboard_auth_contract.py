...
plugin SDK (``window.__SPARKII_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
...
_FORBIDDEN = "__SPARKII_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + sparkii-achievements are bundled today; assert at least one is
