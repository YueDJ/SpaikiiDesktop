...
Run from the PR worktree:
    ...
    cd ~/.sparkii/sparkii-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.sparkii/sparkii-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.sparkii/sparkii-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a sparkii-agent checkout"
)
...
# Isolated SPARKII_HOME for the config write.
