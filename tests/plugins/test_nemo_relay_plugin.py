...
""",
    ...
    monkeypatch.setenv("SPARKII_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    return plugins_toml
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("sparkii.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    assert package["metrics"][0]["name"] == "sparkii.model_route.count"
    assert package["metrics"][0]["value"] == 1
    assert (tmp_path / "atif" / "sparkii-atif-s1.json").exists()
