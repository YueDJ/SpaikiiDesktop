...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SPARKII_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", sparkii_home=str(tmp_path / ".sparkii"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", sparkii_home=str(sparkii_home))
        return p
