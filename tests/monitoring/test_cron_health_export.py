...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("sparkii.gateway.up"), _M("sparkii.gateway.active_agents"),
        _M("sparkii.gateway.busy"), _M("sparkii.gateway.drainable"),
        _M("sparkii.gateway.restart_requested"),
        _M("sparkii.platform.up"), _M("sparkii.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("sparkii.cron.scheduler.heartbeat_age_seconds"),
        _M("sparkii.cron.scheduler.last_success_age_seconds"),
        _M("sparkii.cron.scheduler.catch_up_occurrences"),
        _M("sparkii.cron.jobs.enabled"), _M("sparkii.cron.jobs.running"),
        _M("sparkii.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Sparkii Agent-owned Relay transport health" in text
