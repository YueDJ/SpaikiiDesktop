...
def test_required_frontmatter_fields(p):
    ...
    sparkii = (fm.get("metadata") or {}).get("sparkii") or {}
    if not (sparkii.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.sparkii.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    sparkii = (fm.get("metadata") or {}).get("sparkii") or {}
    dangling = [
        rs for rs in (sparkii.get("related_skills") or []) if rs not in _all_names()
    ]
