...
def test_frontmatter_required_fields():
    ...
    sparkii = fm["metadata"]["sparkii"]
    assert sparkii["tags"]
    assert "related_skills" in sparkii
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Sparkii Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["sparkii"]["related_skills"]:
        hits = (
