...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``sparkii_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_SPARKII_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_SPARKII_CONFIG
...
def test_readable_config_keeps_every_pre_existing_key(tmp_path: Path):
    ...
    assert merged["model"] == "sparkii-4-405b"
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.sparkii/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: sparkii-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "sparkii-4-405b" in real.read_text(encoding="utf-8")
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_SPARKII_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # sparkii_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.sparkii/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "sparkii_config_mod"}
