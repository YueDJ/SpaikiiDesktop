...
A one-shot ``sparkii cron run <job_id>`` used to background-dispatch the run
...
1. ``sparkii_cli.cron._job_action("run", ...)`` declares the channel stateless
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``sparkii cron run`` leaves behind: a row stuck in
...
class TestOneShotCliRunIsSynchronous:
    ...
    def test_background_dispatch_refused_when_channel_stateless(self, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_SESSION_KEY", "inherited-gateway-session")
