...
resolve_runtime_provider against a temp SPARKII_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        assert "sparkii cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "host running sparkii" in delivered
        assert "sparkii cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "sparkii cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("sparkii_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
