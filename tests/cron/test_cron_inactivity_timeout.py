...
- Unlimited timeout (SPARKII_CRON_TIMEOUT=0)
- Backward compat: SPARKII_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """SPARKII_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """SPARKII_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("SPARKII_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_sparkii_time_importable(self):
        """sparkii_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _sparkii_now
        assert callable(_sparkii_now)
