...
def cron_store(tmp_path, monkeypatch):
    ...
    (sparkii_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))
    monkeypatch.setattr(J, "SPARKII_DIR", sparkii_home)
    monkeypatch.setattr(J, "CRON_DIR", sparkii_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", sparkii_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", sparkii_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_sparkii_now", lambda: now):
            due = J.get_due_jobs()
