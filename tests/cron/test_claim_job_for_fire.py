...
These exercise the real store against a temp SPARKII_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated SPARKII_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_sparkii_home() reads the env live.
    yield tmp_path
