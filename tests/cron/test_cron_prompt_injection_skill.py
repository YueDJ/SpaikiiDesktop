...
def cron_env(tmp_path, monkeypatch):
    """Isolated SPARKII_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `SPARKII_HOME` alone doesn't reach it. We also patch the
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_skill_with_env_exfil_command_in_prose_is_allowed(self, cron_env):
        ...
        ``cat ~/.sparkii/.env``") must NOT be blocked. This was a real
        false positive in the bundled `sparkii-agent-dev` skill that
        ...
        prompt = scheduler._build_job_prompt(job)
        assert prompt is not None
        assert "cat ~/.sparkii/.env" in prompt
...
class TestScriptOutputNotStrictScanned:
    ...
    Live incident: the ``sparkii-triage`` cron was blocked every 5 minutes
