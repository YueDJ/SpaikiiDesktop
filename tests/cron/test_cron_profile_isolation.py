...
profile's SPARKII_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.sparkii/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SPARKII_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SPARKII_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SPARKII_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import sparkii_constants

    monkeypatch.setattr(
        sparkii_constants, "_get_platform_default_sparkii_home", lambda: root
    )
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    try:
        assert jobs.SPARKII_DIR.resolve() == profile_home.resolve()
        assert (
