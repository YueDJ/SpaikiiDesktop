...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - worker logs are captured to SPARKII_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "SPARKII_KANBAN_TASK": task.id,
            "SPARKII_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="sparkii_e2e_")
    ...
    shim_path = os.path.join(shim_dir, "sparkii")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m sparkii_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/sparkii_e2e_*/worker_*.log
    import glob
