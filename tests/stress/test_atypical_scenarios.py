...
normal tests assume away:
  ...
  - Filesystem: SPARKII_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own SPARKII_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"sparkii_atyp_{name}_")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "SPARKII_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "sparkii_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["SPARKII_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE sparkii_home: {resolved}")
...
@scenario("sparkii_home_with_spaces")
def _(home, kb):
    """SPARKII_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="sparkii with spaces ")
    os.environ["SPARKII_HOME"] = weird
    ...
    try:
        ...
        print(f"  SPARKII_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("sparkii_home_with_unicode")
def _(home, kb):
    """SPARKII_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/sparkii_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  SPARKII_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("sparkii_home_via_symlink")
def _(home, kb):
    """SPARKII_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="sparkii_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["SPARKII_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same SPARKII_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(sparkii_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from sparkii_cli import web_server as ws  # noqa
    except Exception:
        pass
