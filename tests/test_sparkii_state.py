...
class TestConnectionLifecycle:
    def test_failed_writable_open_does_not_leak_tracked_connection(
        ...
        opened = []
        real_connect = sparkii_state._connect_tracked_db
        ...
        monkeypatch.setattr(sparkii_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(
            SessionDB,
            "_init_schema",
    ...
    def test_failed_wal_read_open_does_not_leak_tracked_connection(
        ...
        from sparkii_cli import sqlite_safe_read

        db_path = tmp_path / "state.db"
        db = SessionDB(db_path=db_path)
        opened = []
        real_connect = sparkii_state._connect_tracked_db
        real_pragmas = sparkii_state.apply_database_pragmas
        ...
        monkeypatch.setattr(sparkii_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(sparkii_state, "apply_database_pragmas", fail_pragmas)
        ...
        try:
            ...
            monkeypatch.setattr(
                sparkii_state, "apply_database_pragmas", real_pragmas
            )
            for conn in opened:
                try:
                    conn.close()
...
class TestReconcileColumnsErrorHandling:
    ...
    def test_duplicate_column_race_stays_quiet(self, tmp_path, caplog):
        ...
        try:
            ...
            with caplog.at_level(logging.WARNING, logger="sparkii_state"):
                stale._reconcile_columns(cursor)
        finally:
            conn.close()
        assert not [
...
class TestListSessionsRich:
    ...
    def test_rich_list_keeps_legacy_reset_children_visible(self, db, end_reason):
        from sparkii_state_common import _ephemeral_child_sql
    ...
    # tests/sparkii_state/test_resolve_resume_session_id.py
