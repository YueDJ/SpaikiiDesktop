"""Tests for the secret-source tracking in ``sparkii_cli.env_loader``.

These cover the small public surface that lets `sparkii model` / `sparkii setup`
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    runs exactly once per SPARKII_HOME per process.
    """
    ...
    # cli.py, sparkii_cli/main.py, run_agent.py, trajectory_compressor.py,
