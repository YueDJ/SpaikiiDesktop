...
minimal ``SparkiiCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def sparkii_home(tmp_path, monkeypatch):
    ...
    # Bust the goal module's DB cache so it re-resolves SPARKII_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal SparkiiCLI stub with an active goal wired in."""
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, sparkii_home):
        ...
        src = inspect.getsource(SparkiiCLI.chat)
