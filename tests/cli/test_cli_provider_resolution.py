...
def test_sparkii_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    shell = cli.SparkiiCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "sparkii_cli.main._save_custom_provider",
...
def test_auto_provider_name_localhost():
    from sparkii_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from sparkii_cli.main import _save_custom_provider
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    _save_custom_provider(
        ...
        api_key="sk-secret",
        name="Ollama",
        key_env="SPARKII_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "SPARKII_CUSTOM_LOCALHOST_11434_API_KEY"
