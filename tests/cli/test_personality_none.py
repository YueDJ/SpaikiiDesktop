...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._sparkii_home", tmp_path), \
             patch("sparkii_constants.display_sparkii_home", return_value="~/.sparkii/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.sparkii/profiles/coder/config.yaml`"
