...
SessionDB.session_gateway_runtime (sparkii_state.py) — the round trip that
makes `sparkii --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SparkiiCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SparkiiCLI)
