...
#
# Installs a real, earlier Sparkii the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `sparkii`.
#
...
# Keep sandbox state out of the default .sparkii-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".sparkii-sandbox-e2e-$ROUTE"
export SPARKII_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/sparkii/.sparkii/sparkii-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/sparkii-agent.git"
...
LOG_DIR="${SPARKII_E2E_LOG_DIR:-$(mktemp -d -t sparkii-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED sparkii accept FLAG on `sparkii update`?
#
...
# that silently rots. `sparkii update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "sparkii update --help 2>&1" | grep -qF -- "$flag"
}
...
require_hermes_works() {
  ...
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "sparkii runs $when"
}

# ── install the earlier Sparkii ─────────────────────────────────────────────
...
case "$ROUTE" in
  update)
    ...
    # sparkii which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="sparkii update --yes"
    else
      update_cmd="sparkii update </dev/null"
    fi
