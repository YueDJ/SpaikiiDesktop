"""Tests for sparkii_constants module."""
...
class TestGetDefaultSparkiiRoot:
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SPARKII_HOME=<root>/profiles/<name>),
    ...
    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; SPARKII_HOME / home changes invalidate.

        get_default_sparkii_root() resolves SPARKII_HOME against the native
        ...
        (native home, SPARKII_HOME) compared for free each call.
        """
        # SPARKII_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_sparkii_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            sparkii_constants, "_default_sparkii_root_memo", None, raising=False
        )

        first = get_default_sparkii_root()
        ...
        # SPARKII_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("SPARKII_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_sparkii_root() == docker_root
        assert resolve_calls["n"] > before, (
            "SPARKII_HOME change must force a fresh resolution"
        )
...
class TestGetProcessSparkiiHome:
    """Tests for get_process_sparkii_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_sparkii_home() == home
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Sparkii-managed npm/node wrappers."""
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = sparkii_constants._SPARKII_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(sparkii_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(sparkii_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from sparkii_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".sparkii"
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert sparkii_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sparkii_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert sparkii_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert sparkii_constants.windows_path_to_wsl("/home/alex") is None
        assert sparkii_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(sparkii_constants, "is_wsl", lambda: True)
        assert sparkii_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sparkii_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert sparkii_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(sparkii_constants.sys, "platform", "darwin")
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
        assert sparkii_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert sparkii_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("SPARKII_HOME", str(home))
        monkeypatch.setenv(
            "SPARKII_NODE_TARGET_MAJOR",
            str(sparkii_constants._SPARKII_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(sparkii_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            sparkii_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            sparkii_constants,
        ...
        monkeypatch.setattr(
            sparkii_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(sparkii_constants._SPARKII_NODE_TARGET_MAJOR)
        ...
        result = sparkii_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sparkii_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_sparkii_managed_node() is False
        assert sparkii_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_sparkii_managed_node() is False
        assert sparkii_constants._managed_node_heal_attempted is True
