...
def _restore_tool_modules():
    original_sparkii_home = os.environ.get("SPARKII_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_sparkii_home is None:
            os.environ.pop("SPARKII_HOME", None)
        else:
            os.environ["SPARKII_HOME"] = original_sparkii_home
        _reset_modules(("tools", "sparkii_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="sparkii-agent", create_if_missing=create_if_missing)
