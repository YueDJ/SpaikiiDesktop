...
UDS socket lifecycle, sparkii_tools generation, timeout enforcement,
...
class TestSparkiiToolsGeneration(unittest.TestCase):
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_sparkii_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/sparkii_exec_", mkdir_cmd)
        self.assertIn("SPARKII_RPC_DIR=/data/data/com.termux/files/usr/tmp/sparkii_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/sparkii_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/sparkii_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """SPARKII_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"SPARKII_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import sparkii_constants; print(sparkii_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("sparkii_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from sparkii_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from sparkii_tools import json_parse
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from sparkii_tools import retry
def always_fail():
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_sparkii_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("SPARKII_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["SPARKII_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("SPARKII_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching SPARKII_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated sparkii_tools module reads SPARKII_RPC_TOKEN and sends it."""
        src = generate_sparkii_tools_module(["terminal"], transport="uds")
        self.assertIn("SPARKII_RPC_TOKEN", src)
