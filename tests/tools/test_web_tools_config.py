...
class TestBackendSelection:
    ...
    ``sparkii tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from sparkii_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.sparkii/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.sparkii/.env), not bare os.getenv. Parametrized over the four
