...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SPARKII_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SPARKII_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_sparkii_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SPARKII_HOME": "/h", "SPARKII_PROFILE": "p",
        "SPARKII_CONFIG": "/c.yaml", "SPARKII_ENV": "/e",
        "SPARKII_DELEGATED_CHILD_CONTEXT": "1",
        # other SPARKII_* → dropped (broad prefix removed)
        "SPARKII_BASE_URL": "https://x", "SPARKII_INTERACTIVE": "1",
        "SPARKII_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "SPARKII_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SPARKII_BASE_URL", "SPARKII_INTERACTIVE", "SPARKII_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SPARKII_* vars."""
    import logging
