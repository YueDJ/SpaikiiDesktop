...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_SESSION_KEY", session_key)
        monkeypatch.setenv("SPARKII_EXEC_ASK", "1")
        monkeypatch.delenv("SPARKII_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("sparkii-verify-", "sparkii-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "sparkii-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/sparkii-verify-example.py",
            "rm -f /tmp/sparkii-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../sparkii-verify-example.py",
            "rm -f /tmp/a/../../tmp/sparkii-verify-example.py",
            "rm -f /var/tmp/sparkii-verify-example.py",
            "rm -f /tmp/sparkii-verify-*",
            "rm -f /tmp/sparkii-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/sparkii-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/sparkii-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\sparkii-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\sparkii-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\sparkii-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SPARKII_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.sparkii/.env",
            "echo x | tee $SPARKII_HOME/.env",
            'echo x | tee "$SPARKII_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestHermesConfigWriteProtection:
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.sparkii/config.yaml",
            "echo '  mode: off' >> ~/.sparkii/config.yaml",
            "echo x | tee ~/.sparkii/config.yaml",
            "echo x | tee $SPARKII_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.sparkii/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.sparkii/config.yaml/.env must require approval — the
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("SPARKII_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.sparkii/sparkii-agent && source venv/bin/activate && python -m sparkii_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m sparkii_cli.main gateway run --replace &",
            "nohup python -m sparkii_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart sparkii-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep sparkii) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "sparkii.*gateway")',
            "kill -9 `pgrep sparkii`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof sparkii)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof sparkii_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof sparkii`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_against_sparkii_label_detected(self):
        for cmd in (
            "launchctl stop ai.sparkii.gateway",
            "launchctl kickstart -k system/ai.sparkii.gateway",
            "launchctl bootout system/ai.sparkii.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.sparkii.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("SPARKII_GATEWAY_SESSION", "SPARKII_CRON_SESSION",
                      "SPARKII_YOLO_MODE",
                      "SPARKII_SESSION_KEY", "SPARKII_INTERACTIVE")
        }
        os.environ.pop("SPARKII_YOLO_MODE", None)
        os.environ.pop("SPARKII_INTERACTIVE", None)
        # SPARKII_CRON_SESSION takes priority over SPARKII_GATEWAY_SESSION in
        ...
        os.environ.pop("SPARKII_CRON_SESSION", None)
        os.environ["SPARKII_GATEWAY_SESSION"] = "1"
        os.environ["SPARKII_SESSION_KEY"] = self.SESSION_KEY
