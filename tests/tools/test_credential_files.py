...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        sparkii_home.mkdir()
        (sparkii_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(sparkii_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.sparkii/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        (sparkii_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.sparkii/skills"


    def test_custom_container_base(self, tmp_path):
        sparkii_home = tmp_path / ".sparkii"
        (sparkii_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SPARKII_HOME": str(sparkii_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.sparkii")

        assert mounts[0]["container_path"] == "/home/user/.sparkii/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.sparkii/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.sparkii/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SPARKII_HOME."""
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path / ".sparkii"))
        (tmp_path / ".sparkii").mkdir()

        # Create a sensitive file one level above sparkii_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside sparkii_home
    ...
    def test_nested_subdir_inside_sparkii_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SPARKII_HOME must be allowed."""
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        subdir = sparkii_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SPARKII_HOME pointing outside must be rejected."""
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

        # Create a sensitive file outside sparkii_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside sparkii_home pointing outside
        symlink = sparkii_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SPARKII_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, sparkii_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SPARKII_HOME."""
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))
        ...
        sensitive.write_text("{}")
        self._write_config(sparkii_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(sparkii_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SPARKII_HOME via config must still mount."""
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

        (sparkii_home / "oauth.json").write_text("{}")
        self._write_config(sparkii_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        (sparkii_home / "cache" / "documents").mkdir(parents=True)
        (sparkii_home / "cache" / "audio").mkdir(parents=True)
        (sparkii_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.sparkii/cache/documents" in paths
        assert "/root/.sparkii/cache/audio" in paths
        assert "/root/.sparkii/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_sparkii_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        # Use legacy dir name with content — get_sparkii_dir prefers
        # populated old over new.
        legacy_doc = sparkii_home / "document_cache"
        legacy_img = sparkii_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(sparkii_home / "document_cache") in host_paths
        assert str(sparkii_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.sparkii/cache/documents" in container_paths
        assert "/root/.sparkii/cache/images" in container_paths
    ...
    def test_empty_sparkii_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.sparkii/attachments" in container_paths
        assert "/root/.sparkii/images" in container_paths
        assert "/root/.sparkii/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``SPARKII_HOME/images``, not
        ...
        """
        sparkii_home = tmp_path / ".sparkii"
        (sparkii_home / "images").mkdir(parents=True)
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.sparkii/images" in by_container
        assert by_container["/root/.sparkii/images"] == str(sparkii_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        sparkii_home = tmp_path / ".sparkii"
        (sparkii_home / "images").mkdir(parents=True)
        upload = sparkii_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.sparkii/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        sparkii_home = tmp_path / ".sparkii"
        img_dir = sparkii_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.sparkii/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(sparkii_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.sparkii/cache/images/x.png"
        assert (sparkii_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Sparkii cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        sparkii_home = tmp_path / ".sparkii"
        (sparkii_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))
        return str(sparkii_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_sparkii(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.sparkii/attachments/drop.zip"

    def test_ssh_maps_to_tilde_sparkii(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.sparkii/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        sparkii_home = tmp_path / ".sparkii"
        doc_dir = sparkii_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — SPARKII_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping SPARKII_HOME, but every master
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.sparkii/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.sparkii/google_token.json" in paths
        assert "/root/.sparkii/.env" not in paths
