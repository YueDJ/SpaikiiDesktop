...
Older images made /opt/sparkii/ui-tui and /opt/sparkii/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SPARKII_TUI_DIR=/opt/sparkii/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R sparkii:sparkii /opt/sparkii/ui-tui" not in text
    assert "chown -R sparkii:sparkii /opt/sparkii/node_modules" not in text
