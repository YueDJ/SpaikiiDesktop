...
# cwd. Without an explicit PYTHONPATH the child resolves ``sparkii_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SPARKII_PROFILE", "parent-worker")
    ...
    monkeypatch.setenv("SPARKII_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with SPARKII_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("SPARKII_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("SPARKII_KANBAN_RUN_ID", "123")
    home = tmp_path / ".sparkii"
    home.mkdir()
    monkeypatch.setenv("SPARKII_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote SPARKII_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".sparkii"
    home.mkdir()
    ...
    monkeypatch.setenv("SPARKII_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("SPARKII_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("SPARKII_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("SPARKII_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("SPARKII_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("SPARKII_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("SPARKII_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("SPARKII_DELEGATED_CHILD_CONTEXT") is None
    assert env["SPARKII_HOME"] == str(home)
    ...
    assert "SPARKII_KANBAN_CLAIM_LOCK" not in env
...
def test_delegate_child_kanban_cli_cannot_delete_parent_board(
    ...
    code = (
        "from sparkii_cli import kanban; "
        "import argparse; "
