...
Covers:
- :class:`SparkiiTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SparkiiMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SPARKII_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SparkiiTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SparkiiTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
        storage = SparkiiTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SparkiiMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SparkiiTokenStorage, **context_attrs):
    ...
    provider = _SPARKII_PROVIDER_CLS.__new__(_SPARKII_PROVIDER_CLS)
    provider._sparkii_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
        storage = SparkiiTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SPARKII_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
        storage = SparkiiTokenStorage("flow-srv")
        ...
        with patch.object(
            _SPARKII_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
