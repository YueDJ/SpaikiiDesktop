...
``core.config.get_env_value``) and not only from ``os.environ`` —
...
class TestDotenvFallbackPerProvider:
    ...
    ``core.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``core.config.load_env`` to simulate ``~/.sparkii/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while core.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            # os.environ is empty.
            from core.config import get_env_value as live_get
