...
``sparkii_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``SPARKII_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # SPARKII_* handling (#27303): only the operational allowlist passes;
        # every other SPARKII_* is dropped (the broad prefix was removed).
        "SPARKII_HOME": "/home/alice/.sparkii",        # allowlisted → kept
        "SPARKII_PROFILE": "default",                 # allowlisted → kept
        "SPARKII_INTERACTIVE": "1",                   # not allowlisted → dropped
        "SPARKII_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "SPARKII_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``sparkii_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``sparkii_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_sparkii_tools_module
        import tempfile, ast
        stub = generate_sparkii_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_sparkii_tools_module(["terminal"], transport="uds")
...
# let the child import sparkii_tools, a user script that printed non-ASCII
