...
try:
    ...
    pytest.skip("sparkii-agent tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/sparkii-agent")
...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic SPARKII_HOME fixture runs), so on a dev machine whose real
        ...
        SPARKII_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo Sparkii session (or any shell exporting
        SPARKII_YOLO_MODE=1) freezes True and every guard auto-approves.
