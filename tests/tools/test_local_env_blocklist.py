...
Verifies that Sparkii-managed provider, tool, and gateway env vars are
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.sparkii/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Sparkii inference secret
        ...
        models`` run inside a Sparkii terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _SPARKII_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Sparkii-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "SPARKII_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _SPARKII_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_SPARKII_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_SPARKII_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_SPARKII_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_SPARKII_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Sparkii venv path instead of the project's own ``.venv`` —
    silently clobbering the Sparkii environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Sparkii venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.sparkii/sparkii-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/sparkii",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "sparkii-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Sparkii-owned entry stripping (#74817).

    The Desktop Electron app injects the Sparkii repo root and the Sparkii
    ...
    ``_strip_sparkii_owned_pythonpath`` surgically removes only the
    entries Sparkii itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Sparkii-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _sparkii_repo_root
        cannot silently pass), duplicate Sparkii entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Sparkii ordering with an
        empty component preserved.
        """
        from tools.environments.local import _strip_sparkii_owned_pythonpath
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Sparkii-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Sparkii venv
        ...
        off-by-one in _sparkii_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Sparkii-owned entries.
        ...
        Sparkii-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Sparkii venv site-packages entries are stripped
        by the same Sparkii-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_sparkii_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        sparkii_win = f"C:\\\\Users\\\\u\\\\.sparkii\\\\sparkii-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([sparkii_win, user_win]),
        }
        ...
        # the Sparkii-owned check cannot match a backslash path here.
        assert sparkii_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_sparkii_owned_paths_stripped(self):
        """On Windows, a Sparkii venv site-packages entry written with
        backslashes is stripped by the same Sparkii-owned check, while a
        ...
        _strip_sparkii_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert sparkii_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Sparkii venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Sparkii runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "sparkii-agent"
        ...
        monkeypatch.setattr(local, "_sparkii_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_sparkii_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local._strip_sparkii_owned_pythonpath(env)
    ...
    def test_builders_strip_sparkii_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        Sparkii venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.sparkii_subprocess_env()
    ...
    def test_scrub_child_env_strips_sparkii_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Sparkii venv
        site-packages entries should be stripped when
        _strip_sparkii_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_sparkii_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_sparkii_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (Sparkii repo root + Sparkii
        ...
        * inherited Sparkii venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        sparkii_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_tool._uses_sparkii_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [sparkii_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(sparkii_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Sparkii venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from sparkii_cli.gateway_windows import _preserve_sparkii_home_path
        ...
        monkeypatch.setenv("SPARKII_HOME", str(configured_home))

        launcher_entry = Path(_preserve_sparkii_home_path(physical_root))
        aliases = local._build_sparkii_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "sparkii-agent"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_sparkii_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with SPARKII_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home SPARKII_HOME through resolve_profile_env() before the
        ...
        monkeypatch.setenv("SPARKII_HOME", str(configured_home))
        lexical_root = configured_home / "sparkii-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) SPARKII_HOME.
        aliases = local._build_sparkii_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\sparkii\\sparkii-agent -> C:\\...\\sparkii-agent) while the
        ...
        # repo-level link: <configured-home>/sparkii-agent -> physical repo
        try:
            _make_directory_link(configured_home / "sparkii-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "sparkii-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "sparkii-agent"
        ...
        for lookalike in (configured_home / "sparkii-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/sparkii-agent.
        ...
        try:
            _make_directory_link(configured_root / "sparkii-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "sparkii-agent"
        aliases = local._build_sparkii_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "sparkii-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._sparkii_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Sparkii runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Sparkii venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "sparkii_subprocess_env":
                result = local_mod.sparkii_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/sparkii-venv",
            "VIRTUAL_ENV": "/opt/sparkii-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/sparkii-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/sparkii-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_SPARKII_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Sparkii (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _SPARKII_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _SPARKII_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Sparkii-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _SPARKII_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Sparkii, and blocklisting
        ...
        leaked_block = general_chain & _SPARKII_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_SPARKII_PROVIDER_ENV_BLOCKLIST)

    def test_claude_code_oauth_token_is_inheritable(self):
        """CLAUDE_CODE_OAUTH_TOKEN is owned by the user's Claude Code install
        (subscription OAuth), not a Sparkii inference credential. Stripping it
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _SPARKII_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _SPARKII_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_sparkii_bin_injection(self):
        ...
        from tools.environments import local as local_mod
        saved = local_mod._SPARKII_BIN_DIR
        local_mod._SPARKII_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._SPARKII_BIN_DIR = saved
...
class TestSparkiiBinDirOnPath:
    """The sparkii install dir is reachable in the terminal subshell PATH.
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._SPARKII_BIN_DIR = local_mod._SENTINEL
    ...
    def test_make_run_env_injects_sparkii_bin_dir(self):
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._SPARKII_BIN_DIR = "/opt/sparkii/bin"
        with patch.dict(
            os.environ,
        ...
        assert entries[0] == "/opt/sparkii/bin"
        assert "/usr/bin" in entries


class TestSparkiiInternalDynamicSecrets:
    """Dynamically-named Sparkii secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_SPARKII_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _SPARKII_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _SPARKII_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _SPARKII_PROVIDER_ENV_BLOCKLIST
