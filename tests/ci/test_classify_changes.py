...
def _lanes(python=False, scan=False, deps=False, uv_lock=False, installer=False, mcp_catalog=False, docker_meta=False, ci_review=False, python_prod=None, nix=None, docker=None) -> dict[str, bool]:
    ...
    # python_prod. Pass either explicitly to override.
    _python_prod = python if python_prod is None else python_prod
    _product = _python_prod
    return {
        "python": python,
...
CASES = {
    ...
    "docs → no uv_lock": (
        ["docs/developer-guide/plugins/index.md"],
        _lanes(),
    ),
    ...
    "nix module → nix only": (["nix/python.nix"], _lanes(nix=True)),
    ...
    "product python → nix": (["core/config.py"], _lanes(python=True, scan=True)),
