...
# Must match the root the guard itself computes.  Hardcoding ``~/.sparkii``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\sparkii``: the paths under test were then *correctly*
...
REAL_ROOT = sparkii_state._real_platform_state_root()
