"""Tests for the top-level `./sparkii` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sparkii` should use `sparkii_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "sparkii"
    called = []

    fake_main_module = types.ModuleType("sparkii_cli.main")

    def fake_main():
        called.append("sparkii_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sparkii_cli.main", fake_main_module)
    ...
    assert called == ["sparkii_cli.main"]
