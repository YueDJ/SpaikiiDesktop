...
from sparkii_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import sparkii_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "sparkii_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("sparkii_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_sparkii_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Sparkii Node dir even when
