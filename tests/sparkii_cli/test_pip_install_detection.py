...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("core.config.get_managed_system", return_value=None), \
         patch("core.config.get_sparkii_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("core.config.get_sparkii_home", return_value=home):
        from core.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("core.config.get_managed_system", return_value=None), \
         patch("core.config.get_sparkii_home", return_value=tmp_path), \
