...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``sparkii.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("SPARKII_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Sparkii.app" / "Contents" / "MacOS" / "Sparkii"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Sparkii.exe"
    else:
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("sparkii_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sparkii_cli.main._register_linux_desktop_entry"), \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Sparkii-managed
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Sparkii.app/Contents/MacOS/Sparkii in place. Re-downloading Electron can't
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Sparkii.app with the pieces the signer must find."""
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Sparkii.app component, silently skipping every .node/.dylib/crashpad
...
def _clear_keychain_env(monkeypatch):
    for var in (
        ...
        "SPARKII_DESKTOP_PASSWORD_STORE",
    ):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("sparkii_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("sparkii_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("sparkii_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    assert launch_env["SPARKII_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SPARKII_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("SPARKII_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SPARKII_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "SPARKII_DESKTOP_PASSWORD_STORE" not in launch_env
