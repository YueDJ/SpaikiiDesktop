...
the SAME provider universe as ``sparkii model`` (the CLI/TUI picker).
...
def test_every_sparkii_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `sparkii model` universe."""
    ...
    assert not missing, (
        "providers shown in `sparkii model` but not configurable in the desktop "
