...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/sparkii", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/sparkii", ["/usr/bin/sparkii", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        sparkii).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\sparkii.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\sparkii.exe", "chat"]]
...
class TestResolveSparkiiBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a sparkii.exe so the test doesn't
