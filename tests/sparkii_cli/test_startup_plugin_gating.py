...
This saves 500-650ms on ``sparkii --help``, ``sparkii version``,
``sparkii logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``sparkii --help`` in-process and parse the subcommand block.
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``sparkii photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain sparkii-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level sparkii <name> CLI command via PluginContext.
