"""Tests for the Sparkii plugin system (sparkii_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<sparkii_home>/plugins/``; we derive
    ``<sparkii_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Sparkii home for this
    plugin isn't necessarily the current ``SPARKII_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``SPARKII_HOME``/``set_sparkii_home_override()`` per-profile afterwards
...
class TestPluginLoading:
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``sparkii_agent.plugins`` entry point but exposing
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $SPARKII_HOME/plugins/.
        sparkii_home = tmp_path / "sparkii_test"
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_sparkii_home_override(self, tmp_path):
        """set_sparkii_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``SPARKII_HOME`` via a
        context-local ContextVar, which — per
        ``sparkii_constants.set_sparkii_home_override`` — deliberately does
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('SPARKII_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['SPARKII_HOME'] itself (simulating
        # a real plugin like sparkii-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_sparkii_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_sparkii_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``sparkii_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``sparkii_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `sparkii_plugins.stateful_plugin.state`.
...
class TestPluginDebugLogging:
    """SPARKII_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from SPARKII_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """SPARKII_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """SPARKII_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".sparkii" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARKII_HOME", str(prof))
