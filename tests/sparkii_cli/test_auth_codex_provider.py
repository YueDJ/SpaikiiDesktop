"""Tests for Codex auth — tokens stored in Sparkii auth store (~/.sparkii/auth.json)."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``sparkii auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``sparkii auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `sparkii auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``sparkii auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``sparkii auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Sparkii store
    assert not (codex_home / "auth.json").exists()

    # Sparkii auth store should have the tokens
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run sparkii auth".
    """
    from sparkii_cli.auth import (
    ...
    assert "sparkii auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from sparkii_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from sparkii_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch sparkii_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("sparkii_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
