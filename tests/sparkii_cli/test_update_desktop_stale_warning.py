...
#88251: ``sparkii update`` treated a failed desktop pack as non-fatal, printed
...
from sparkii_cli.update_cmd import (
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sparkii_constants.with_sparkii_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "sparkii_constants.display_sparkii_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_sparkii_home", lambda: tmp_path)
