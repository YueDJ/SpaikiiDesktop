...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.sparkii`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from sparkii_cli import web_server

    monkeypatch.setenv(
        "SPARKII_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """SPARKII_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from sparkii_cli import web_server

    monkeypatch.setenv("SPARKII_HOME", str(isolated_profiles["default"]))
