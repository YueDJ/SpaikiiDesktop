...
``venv\\Scripts\\sparkii.exe`` is a launcher that runs the interpreter with the
...
is holding, which Windows refuses — so ``sparkii update`` re-runs itself under
...
SHIM_NAMES = ["sparkii.exe", "sparkii-agent.exe", "sparkii-acp.exe", "sparkii-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["sparkii", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "sparkii.exe" / "__main__.py")])
    assert cli_main._windows_shim_in_process_chain() == venv / "sparkii.exe"


def test_detects_shim_from_main_module_spec_origin(venv, monkeypatch):
    fake_main = types.SimpleNamespace(
        __file__=None,
        __spec__=types.SimpleNamespace(origin=str(venv / "sparkii.exe")),
    )
...
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    ...
    _fake_psutil(monkeypatch, [str(venv / "sparkii.exe")])
    assert cli_main._windows_shim_in_process_chain() == venv / "sparkii.exe"


def test_ignores_sparkii_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "sparkii.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "sparkii.exe")])
...
def test_no_shim_off_windows(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "sparkii.exe"), "update"])
    assert cli_main._windows_shim_in_process_chain() is None
...
def test_reexec_runs_same_args_under_venv_python(venv, monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", [str(venv / "sparkii.exe"), "update", "--yes"])
    calls = _capture_popen(monkeypatch)

    assert cli_main._reexec_update_off_windows_shim() is True
    cmd, env, kwargs = calls[0]
    assert cmd == [
        str(venv / "python.exe"), "-m", "sparkii_cli.main", "update", "--yes",
    ]
...
def test_reexec_falls_through_when_venv_python_is_missing(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_update_off_windows_shim() is False
    assert "-m sparkii_cli.main update" in capsys.readouterr().out
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\sparkii\venv\Scripts\sparkii.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\sparkii\venv\Scripts\sparkii.exe",
        r"!\??\C:\sparkii\venv\Scripts\sparkii.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\sparkii\venv\Scripts\sparkii.exe")]
    entries = [
        r"\??\C:\sparkii\venv\Scripts\sparkii.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = cli_main._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries
