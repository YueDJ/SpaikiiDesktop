...
from sparkii_cli.console_engine import SparkiiConsoleEngine, run_console_repl
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    result = SparkiiConsoleEngine().execute(
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_sparkii_home):
    ...
    assert code == 0
    assert "Sparkii Console" in stdout.getvalue()
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_sparkii_home, monkeypatch
):
    ...
    result = SparkiiConsoleEngine().execute("checkpoints prune", confirmed=True)
