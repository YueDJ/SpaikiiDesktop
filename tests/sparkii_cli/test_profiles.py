...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SPARKII_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.sparkii/profiles)
    * SPARKII_HOME  -> tmp_path/.sparkii  (so get_sparkii_home() agrees)
    * Creates the bare-minimum ~/.sparkii directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_sparkii_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".sparkii"
...
class TestNoSkillsOptOut:
    """Tests for `sparkii profile create --no-skills` and the opt-out marker."""
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `sparkii update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".sparkii" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_backend_scan_matches_shebang_exec_of_sparkii_shim(self, profile_env, monkeypatch):
        """A `sparkii` console-script shim spawned directly (e.g. Electron's
        findOnPath('sparkii') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "sparkii" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "sparkii".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/sparkii", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/sparkii", "--profile", "other", "serve"]),
            # Non-sparkii script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_sparkii_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "sparkii" (e.g.
        sparkii-notes.py, sparkii-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "sparkii" -- only the actual known
        console-script entry points (sparkii, sparkii-agent, sparkii-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/sparkii-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/sparkii-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (sparkii-agent,
        sparkii-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "sparkii"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/sparkii-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/sparkii-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".sparkii" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".sparkii" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom SPARKII_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("SPARKII_HOME", str(custom))
        # With Docker-aware roots, a custom SPARKII_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "exec /opt/sparkii/bin/sparkii -p mybot" in content
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".sparkii" / "profiles" / "oldname"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultSparkiiHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_sparkii_home()."""



    def test_default_sparkii_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_sparkii_home() returns SPARKII_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARKII_HOME", str(docker_home))
        home = _get_default_sparkii_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under SPARKII_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SPARKII_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="sparkii gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with SPARKII_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("SPARKII_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_sparkii_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured SPARKII_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-sparkii"
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("SPARKII_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No SPARKII_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("SPARKII_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_sparkii_home()
