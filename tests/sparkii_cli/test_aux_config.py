"""Tests for the auxiliary-model configuration UI in ``sparkii model``.
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "Sparkii-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · Sparkii-4.5"
