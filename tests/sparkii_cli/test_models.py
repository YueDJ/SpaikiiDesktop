...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("core.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("core.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("core.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        sparkii-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("core.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("core.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from core.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from core.models import _find_openrouter_slug
        with patch("core.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from core.models import _format_price_per_mtok
...
class TestLocalOllamaModelDiscovery:
    def test_provider_model_ids_uses_ollama_api_tags_from_provider_config(self):
        ...
        from core.models import provider_model_ids
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from core.models import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("core.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from core.models import _get_ollama_base_url, should_use_ollama_native_catalog

        with patch("core.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "core.models.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from core.models import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("core.models.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("core.models.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("core.models.fetch_ollama_local_models") as fetch_local, patch(
    ...
    def test_native_ollama_catalog_uses_configured_key_env(self, monkeypatch):
        from core.models import _get_ollama_request_headers
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from core.models import _credential_fingerprint
        ...
        ), patch("core.models.probe_ollama_local_models") as probe_ollama:
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("core.config.load_config", return_value={"providers": {}}), patch(
            "core.models.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "core.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("core.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        try:
            with patch("core.config.load_config", return_value=config), patch(
                "core.config.save_config"
            ), patch("sparkii_cli.auth._save_model_choice"), patch(
                "sparkii_cli.auth.deactivate_provider"
            ), patch("sparkii_cli.main._save_custom_provider"), patch(
                "sparkii_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_uses_ollama_api_tags_for_ollama_provider(self):
        ...
        from core.models import validate_requested_model
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("core.models.probe_ollama_local_models") as probe_ollama, patch(
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        try:
            with patch(
                ...
                "core.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_ollama_root_matching_is_case_insensitive_for_hostnames(self):
        from core.models import _same_ollama_native_root
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from core.models import _get_ollama_base_url, _root_for_ollama_native_api
