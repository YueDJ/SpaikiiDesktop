...
class TestConfigYamlRouting:
    ...
    def test_cron_script_timeout_is_recognized(self, _isolated_sparkii_home, capsys):
        ...
        assert "script_timeout_seconds: 600" in _read_config(_isolated_sparkii_home)
...
class TestCronModelDriftConfigWarning:
    ...
    def test_warning_names_the_user_owned_cli_pin_path(
        ...
        assert "sparkii cron edit <job_id> --provider <provider> --model <model>" in warning
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from core.config import redact_config_value
...
class TestSchemaValidation:
    ...
    a post-write notice warns that Sparkii may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_sparkii_home, capsys):
        ...
        set_config_value("desktop.macos_signing_identity", "Sparkii Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_sparkii_home))
        assert saved["desktop"]["macos_signing_identity"] == "Sparkii Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from core.config import _validate_config_key
...
class TestMalformedYAMLConfigPreservation:
    ...
    def test_unset_config_value_refuses_broken_yaml(self, _isolated_sparkii_home, capsys):
        ...
        from core.config import unset_config_value
