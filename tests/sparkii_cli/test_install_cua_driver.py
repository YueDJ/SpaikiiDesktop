...
* Be supported-platform-only — no-op silently elsewhere so ``sparkii update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Sparkii
...
class TestRequireConfirmedUpdate:
    """`sparkii update` passes require_confirmed_update=True: the full
    ...
    Explicit `sparkii computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `sparkii computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_despite_indeterminate_check(self):
        """Sparkii' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `sparkii update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `sparkii update`.
    """
...
class TestCuaVersionSummary:
    """`sparkii computer-use status` prints one line, whatever the binary says.

    A binary chosen by SPARKII_CUA_DRIVER_CMD is under no obligation to answer
