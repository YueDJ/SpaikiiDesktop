...
``sparkii*.exe`` shims aside (``sparkii.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``sparkii`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("sparkii", "sparkii-agent", "sparkii-acp", "sparkii-gateway"):
...
# ---------------------------------------------------------------------------
# sparkii_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "sparkii.exe" in names, "sparkii.exe must be restored after a no-op install"
    assert "sparkii-acp.exe" in names
    assert "sparkii-gateway.exe" in names
    assert (scripts / "sparkii.exe").read_bytes() == b"MZ-old-sparkii"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "sparkii.exe").read_bytes() == b"MZ-new-sparkii"
...
# ---------------------------------------------------------------------------
# sparkii_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "sparkii.exe" in names, "sparkii.exe must be restored after a no-op recovery install"
    assert (scripts / "sparkii.exe").read_bytes() == b"MZ-old-sparkii"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "sparkii.exe").write_bytes(b"MZ-new-sparkii")
