"""Tests for sparkii_cli/goals.py — persistent cross-turn goals."""
...
def sparkii_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves SPARKII_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from core.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, sparkii_home):
        ...
        mgr.set("port goal command to sparkii")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to sparkii" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from sparkii_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, sparkii_home):
        ...
        from core import goals
        from core.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from core.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, sparkii_home):
        from core.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestParseContract:
    ...
    def test_inline_fields_parsed(self):
        from core.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from core.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from core.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, sparkii_home):
        from core.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from core.goals import GoalManager as GM2
