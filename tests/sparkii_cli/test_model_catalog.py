"""Tests for core.model_catalog — remote manifest fetch + cache + fallback."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from core.model_catalog import _validate_manifest
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        from core.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in core.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        import importlib
        from core import model_catalog
        from core.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from core.model_switch import list_picker_providers
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("core.models.check_nous_free_tier", return_value=False), patch(
                "core.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "core.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
