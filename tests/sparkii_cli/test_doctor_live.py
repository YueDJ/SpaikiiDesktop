"""Tests for ``sparkii doctor --live`` — opt-in bounded real-call tool-backend probes.
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `sparkii doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("sparkii_cli.doctor.SPARKII_HOME", tmp_path / "home")
        monkeypatch.setattr("sparkii_cli.doctor.PROJECT_ROOT", tmp_path / "root")
