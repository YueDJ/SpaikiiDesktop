...
import pytest
from core.model_switch import list_authenticated_providers, switch_model
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("core.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("core.model_switch.resolve_alias", return_value=None), \
         ...
         patch("core.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("core.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("core.models.fetch_api_models", _fake_fetch)
