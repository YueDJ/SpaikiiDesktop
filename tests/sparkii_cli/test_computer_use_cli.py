...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "sparkii_cli.main", "computer-use", *args],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("sparkii_cli.main")
    monkeypatch.setattr(sys, "argv", ["sparkii", "computer-use", *args])
...
def test_computer_use_status_returns_zero_for_compatible_driver(
    ...
    monkeypatch.delenv("SPARKII_CUA_DRIVER_CMD", raising=False)
...
def test_computer_use_status_returns_nonzero_for_incompatible_standard_driver(
    ...
    assert "Run: sparkii computer-use install" in output
...
def test_computer_use_status_returns_nonzero_for_incompatible_custom_driver(
    ...
    assert _invoke(monkeypatch, "status") == 1
    output = capsys.readouterr().out
    assert "custom binary from SPARKII_CUA_DRIVER_CMD" in output
