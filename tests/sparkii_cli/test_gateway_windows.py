"""Tests for sparkii_cli.gateway_windows."""
...
import sparkii_cli.setup as setup
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "sparkii_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Sparkii\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Sparkii\venv\Scripts\python.exe"
