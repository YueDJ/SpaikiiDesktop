"""Tests for sparkii_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `sparkii setup` and `sparkii import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("sparkii_constants.is_container", lambda: container)
