...
``~/.sparkii/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(sparkii_home):
...
def test_plain_line_save_and_remove_still_work(sparkii_home):
    ...
    env_text = sparkii_home.joinpath(".env").read_text(encoding="utf-8")
