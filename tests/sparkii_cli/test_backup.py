...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew sparkii_cli.backup's datetime forward instead of sleeping.
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Sparkii backup."""
        from sparkii_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Sparkii configuration has been restored." in out
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from sparkii_cli.backup import run_backup, run_import
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from sparkii_cli.backup import _detect_prefix
...
class TestImportAtomicWrites:
    """`sparkii import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the SPARKII_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (sparkii_home / "config.yaml").write_text(original)
        monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sparkii_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(sparkii_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside SPARKII_HOME."""
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        store = sparkii_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = sparkii_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        sparkii_home = tmp_path / ".sparkii"
        sparkii_home.mkdir()
        target = sparkii_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo sparkii import`` onto a user-owned (or Docker/NAS volume-owned)
        SPARKII_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "sparkii_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``sparkii import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from sparkii_cli.backup import _safe_copy_db
...
class TestPreMigrationBackup:
    ...
    def test_restorable_with_sparkii_import(self, sparkii_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Sparkii
