...
def _run_sparkii(home: Path, *args: str, marker: bool = False) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["SPARKII_HOME"] = str(home)
    env["SPARKII_KANBAN_HOME"] = str(home)
    for name in (
    ...
    if marker:
        env["SPARKII_DELEGATED_CHILD_CONTEXT"] = "1"
    else:
        env.pop("SPARKII_DELEGATED_CHILD_CONTEXT", None)
...
def test_delegated_child_kanban_cli_refusal_returns_nonzero_exit_status(tmp_path):
    ...
    created = _run_sparkii(home, "kanban", "create", "exit status probe", "--json")
    assert created.returncode == 0, created.stderr
    task_id = json.loads(created.stdout)["id"]

    refused = _run_sparkii(
        home,
        "kanban",
        "comment",
        task_id,
