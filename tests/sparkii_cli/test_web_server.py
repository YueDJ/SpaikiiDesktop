...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Sparkii vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestWebServerEndpoints:
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `sparkii update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(sparkii_state, "SessionDB", boom)
    ...
    def test_update_sparkii_returns_apt_guidance_without_spawning(self, monkeypatch):
        import sparkii_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn sparkii update")
        ...
        assert data["update_command"] == "pkg upgrade sparkii-agent"
        assert spawned is False

        check = self.client.get("/api/sparkii/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade sparkii-agent"
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Sparkii provider — keep the user's aggregator instead of writing a
...
class TestStatusInstallId:
    ...
    Sparkii home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_sparkii_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """SPARKII_HOME=<root> and SPARKII_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import sparkii_cli.web_server as ws

        root = tmp_path / "sparkii-root"
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets SPARKII_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the sparkii root's plugins/ directory."""
        root = tmp_path / "sparkii-root"
