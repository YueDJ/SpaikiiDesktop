...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("sparkii_cli._subprocess_compat.bounded_probe_run") as mock_probe:
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("sparkii_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: sparkii update's systemd fleet-restart loop may
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"sparkii-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["sparkii", "dashboard", "--port", "8300"]
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("sparkii_cli.dashboard_procs._sparkii_home_for_pid", return_value=None), \
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from sparkii_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.sparkii/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["sparkii", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from sparkii_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "sparkii_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "sparkii_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["sparkii", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["sparkii", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["sparkii", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_sparkii_home(self):
        from sparkii_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/sparkii-home-a"
        a = ["sparkii", "dashboard", "--port", "8300"]
        b = ["sparkii", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, home),
            (2, b, home),
        ])
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.sparkii/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["sparkii", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["sparkii", "dashboard", "--port", "8301"]
        home = "/home/u/.sparkii"
        out = _filter_dashboard_respawn_candidates([
            (1, a, home),
    ...
    def test_distinct_dot_sparkii_homes_do_not_share_cap(self):
        from sparkii_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["sparkii", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``sparkii update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``sparkii_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from sparkii_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("sparkii_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        try:
            ...
            stale = sys.modules["sparkii_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["sparkii_cli._subprocess_compat"])
            importlib.reload(sys.modules["sparkii_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "sparkii_cli._subprocess_compat" in reloaded
        assert "sparkii_cli.dashboard_procs" in reloaded
