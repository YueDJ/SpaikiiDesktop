"""Tests for sparkii_cli.security_advisories.
...
import sparkii_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect SPARKII_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full sparkii_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "sparkii_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
