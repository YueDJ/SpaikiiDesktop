...
class TestMinorLineFallForward:
    ...
    `sparkii update` with no path to a fixed runtime.
    """
...
class TestVenvPythonUpdateBoundary:
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        expected = Path("/opt/sparkii/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/sparkii/venv/bin/python")
        assert _venv_python(Path("/opt/sparkii/venv")) == expected
