...
def _patch_gateway_discovery():
    ...
    """
    with patch("sparkii_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("sparkii_cli.gateway.supports_systemd_services", return_value=False), \
         patch("sparkii_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `sparkii update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".sparkii"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(sparkii_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            sparkii_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("SPARKII_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("SPARKII_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/sparkii-agent silently misses updates.
        """
        from sparkii_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/sparkii-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sparkii_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "sparkii_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("sparkii_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sparkii_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "sparkii_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sparkii_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "sparkii_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sparkii_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "sparkii_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        import sparkii_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.sparkii"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.sparkii/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.sparkii/profiles/work"))
...
class TestCmdUpdateBranchFlag:
    """``sparkii update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``sparkii update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``sparkii update --branch=<non-main>`` must refuse on the ZIP fallback path.
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "Sparkii.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "sparkii_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        project_root = tmp_path / "sparkii-agent"
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("sparkii-agent-main/apps/desktop/package.json", "{}")
