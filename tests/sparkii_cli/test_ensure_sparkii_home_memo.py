"""ensure_sparkii_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".sparkii"
    second = tmp_path / "b" / ".sparkii"

    monkeypatch.setenv("SPARKII_HOME", str(first))
    cfg.ensure_sparkii_home()

    # Profile switch: SPARKII_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SPARKII_HOME", str(second))
    cfg.ensure_sparkii_home()
