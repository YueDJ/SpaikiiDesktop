"""Dashboard Sparkii Console websocket tests."""
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from sparkii_cli.console_engine import ConsoleResult, SparkiiConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(SparkiiConsoleEngine, "execute", slow_execute)
