...
from sparkii_cli import doctor as doctor_mod
from sparkii_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `sparkii doctor` crashed on Windows
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("SPARKII_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_sparkii_home(self, tmp_path, provider=""):
        """Create a minimal SPARKII_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_sparkii_home(tmp_path, provider)
        ...
        try:
            from sparkii_cli import auth as _auth_mod
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (sparkii_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".sparkii"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`sparkii doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp SPARKII_HOME.

        ``run_doctor`` reads the module-level ``SPARKII_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("SPARKII_HOME")`` alone
        leaves doctor probing the REAL ~/.sparkii. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from sparkii_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("sparkii_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "sparkii_cli.doctor", raising=False)
        from sparkii_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale SPARKII_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.delenv("SPARKII_MAX_ITERATIONS", raising=False)
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_sparkii_tool_progress_warning_says_unsupported_since_floor(self):
        """SPARKII_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"SPARKII_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["SPARKII_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"SPARKII_TOOL_PROGRESS_MODE": "all"})
        )
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"SPARKII_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: SPARKII_TOOL_PROGRESS_MODE" in out
