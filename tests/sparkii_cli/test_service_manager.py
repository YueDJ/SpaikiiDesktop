"""Tests for sparkii_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `sparkii_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import sparkii_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("sparkii_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "sparkii_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
# unprivileged sparkii user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from sparkii_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``sparkii gateway run``, an agent action, the Open WebUI helper) holds
    the per-SPARKII_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root SPARKII_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `sparkii gateway run --replace` (no -p flag).
    assert "sparkii gateway run --replace" in default_text
    assert "sparkii -p default" not in default_text
...
#
# `sparkii gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_sparkii_without_chown(
    ...
    sparkii). Restartable log/run must not pathname-chown or pathname-rm a
    sparkii-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on sparkii-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid sparkii mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid sparkii rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside SPARKII_HOME the way
    # a real sparkii uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'SPARKII_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that sparkii cannot unlink a foreign
        ...
        'if [ -n "$SPARKII_TEST_DROPPED" ]; then\n'
        "  exit 0\n"
        "fi\n"
        f'exec {real_rm} "$@"\n',
        encoding="utf-8",
