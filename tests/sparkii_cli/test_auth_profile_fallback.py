...
When ``SPARKII_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.sparkii/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.sparkii            (has its own auth.json fixture)
    * Profile     -> tmp_path/.sparkii/profiles/coder   (active, SPARKII_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Sparkii is not
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    import sparkii_cli.auth as auth
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SPARKII_HOME != ~/.sparkii, no profiles)."""
