...
Before the fix, ``sparkii whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``sparkii gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    sparkii = home / ".sparkii"
    sparkii.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("SPARKII_HOME", str(sparkii))
...
def _env_value(sparkii_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from sparkii_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("sparkii_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``sparkii whatsapp`` with an existing paired session and
