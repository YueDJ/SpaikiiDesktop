"""Tests for the AI_AGENT / SPARKII_AGENT harness-attribution env vars.
...
Sparkii-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Sparkii' id in the public agent-harness
registry (``sparkii-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Sparkii process env, and survives the
cross-session leak guard that strips ``SPARKII_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__SPARKII_CWD_testsession0__"
        env._snapshot_path = "/tmp/sparkii-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'SPARKII_AGENT="${SPARKII_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT SPARKII=$SPARKII_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "SPARKII_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} SPARKII=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", SPARKII_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi SPARKII=false" in out.stdout
