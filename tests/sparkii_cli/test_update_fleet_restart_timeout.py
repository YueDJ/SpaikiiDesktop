...
On hosts with many profile-backed ``sparkii-gateway*.service`` units,
``sparkii update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "sparkii-gateway-xiaomo1",
            "sparkii-gateway-xiaomo2",
            "sparkii-gateway-xiaomo3",
            "sparkii-gateway-xiaomo4",
            "sparkii-gateway-xiaomo5",
            "sparkii-gateway-xiaomo6",
            "sparkii-gateway-xiaomo7",
            "sparkii-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "sparkii-gateway-xiaomo5":
        ...
        assert failed == ["sparkii-gateway-xiaomo5"]
        assert restarted == [
            "sparkii-gateway-xiaomo1",
            "sparkii-gateway-xiaomo2",
            "sparkii-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "sparkii-gateway-xiaomo5"]
        ]
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "sparkii-gateway-coder.service loaded active running",
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["sparkii-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["sparkii-gateway-xiaomo5", "sparkii-gateway-xiaomo6", "sparkii-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("sparkii-gateway-xiaomo5") == 1
        assert "sparkii-gateway-xiaomo6" in out
