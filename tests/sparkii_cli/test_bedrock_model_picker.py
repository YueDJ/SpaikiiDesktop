...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SPARKII_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SPARKII_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SPARKII_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from sparkii_cli.providers import SPARKII_OVERLAYS
        assert "bedrock" in SPARKII_OVERLAYS
