...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"sparkii-agent[{extra}]" in spec
        ]
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `sparkii update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `sparkii update` downgrades the "
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``sparkii update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `sparkii update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `sparkii update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `sparkii update` will churn/downgrade the "
