...
path via ``sparkii chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SPARKII_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SPARKII_HOME at a temp dir and turn moa.save_traces on."""
    sparkii_home = tmp_path / ".sparkii"
    sparkii_home.mkdir()
    monkeypatch.setenv("SPARKII_HOME", str(sparkii_home))

    # save_moa_turn reads config via sparkii_cli.config.load_config; stub it to
    ...
    return sparkii_home / "moa-traces"
