...
to a generic "Your API key was rejected... run sparkii setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "sparkii setup".
    assert "sparkii portal" in source
