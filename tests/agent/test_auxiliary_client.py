...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp SPARKII_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "nous-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "nous", "model": "sparkii-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "nous",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "sparkii-4", "https://inference-api.nousresearch.com/v1", "max_tokens"),
        ],
    )
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``sparkii auth status`` already reports these as logged in; compression
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``sparkii model``, no per-task override is
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("sparkii_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Sparkii auth / credential pool."""
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Sparkii owns the retry/timeout budget (issue #54465). The SDK
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Sparkii reasoning.

    MoA slot reasoning is carried through call_llm as a Sparkii
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/YueDJ/SpaikiiDesktop/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/YueDJ/SpaikiiDesktop/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "SparkiiAgent"
