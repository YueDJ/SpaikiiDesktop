...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"sparkii-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "sparkii-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "sparkii-agent-dev"
...
# `sparkii curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `sparkii curator pin <umbrella>` in the rename block so
