...
def backup_env(monkeypatch, tmp_path):
    """Isolate SPARKII_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_sparkii_home picks up the env var fresh.
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SPARKII_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SPARKII_DIR picks up the tmp HOME."""
    import sparkii_constants
    importlib.reload(sparkii_constants)
