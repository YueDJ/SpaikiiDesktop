...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SPARKII_HOME under tmp_path."""
    monkeypatch.setenv("SPARKII_HOME", str(tmp_path / ".sparkii"))
    (tmp_path / ".sparkii").mkdir()

    # Force the constants/image cache helpers to re-read SPARKII_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("sparkii_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_sparkii_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SPARKII_HOME — gateway cleanup
