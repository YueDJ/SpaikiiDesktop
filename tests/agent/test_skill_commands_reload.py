...
def hermes_home(monkeypatch):
    ...
    we monkeypatch the module-level ``SPARKII_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="sparkii-reload-skills-")
    monkeypatch.setenv("SPARKII_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "SPARKII_HOME", home, raising=False)
