...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_sparkii_home``,
    ``get_default_sparkii_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``SPARKII_HOME`` and no resolver mocks.
    """

    def test_real_sparkii_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Sparkii profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """SPARKII_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".sparkii"
        ...
        assert "Active Sparkii profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "SPARKII_AGENT_HELP_GUIDANCE", "HELP")
    ...
    with (
        ...
        patch("sparkii_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("sparkii_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under SPARKII_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from sparkii_cli import config as _cfgmod
