...
for the credential stores while leaving arbitrary ``SPARKII_HOME`` files
...
def test_arbitrary_sparkii_home_file_not_blocked(fake_home):
    """Non-credential files inside SPARKII_HOME stay readable."""
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    project.mkdir()
    # auth.json outside SPARKII_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside SPARKII_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, SPARKII_HOME = <root>/profiles/<name>, but
