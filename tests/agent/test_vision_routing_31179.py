...
def isolated_home(monkeypatch):
    """Temp SPARKII_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="sparkii_test_31179_")
    sparkii_home = os.path.join(test_home, ".sparkii")
    os.makedirs(sparkii_home)
...
def _fresh_modules():
    """Drop cached sparkii modules so each test reloads against current env."""
    _drop_reload_targets()
