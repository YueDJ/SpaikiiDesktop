"""Regression tests for NousResearch/sparkii-agent#2765.
...
service that didn't inherit ``~/.sparkii/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "sparkii memory status" in msg
