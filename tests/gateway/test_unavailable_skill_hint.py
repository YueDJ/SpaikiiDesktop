...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`sparkii_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + SPARKII_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `sparkii skills config` writes).
    monkeypatch.setattr(
    ...
    assert "sparkii skills config" in msg
