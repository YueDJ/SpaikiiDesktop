...
Guards against the 60-vs-500 bug where a stale `.env SPARKII_MAX_ITERATIONS=60`
...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "SPARKII_AGENT_TIMEOUT",
            "SPARKII_AGENT_TIMEOUT_WARNING",
            "SPARKII_TURN_LEASE_TIMEOUT",
            "SPARKII_SESSION_STALL_TIMEOUT",
            "SPARKII_GATEWAY_BUSY_INPUT_MODE",
            "SPARKII_GATEWAY_BUSY_TEXT_MODE",
...
def test_env_platform_connect_timeout_wins_over_config(hermes_home: Path) -> None:
    ...
    assert env.get("SPARKII_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
