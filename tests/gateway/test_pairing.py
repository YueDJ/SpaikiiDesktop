...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_sparkii_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the sparkii ROOT, not the current
        # SPARKII_HOME — the current home may itself be a profile, and nesting
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `sparkii -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Sparkii home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``sparkii pairing`` CLI continues to work without a profile context."""
        from sparkii_constants import get_sparkii_home
        monkeypatch.setattr("sparkii_constants.get_sparkii_home", lambda: tmp_path)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Sparkii layout."""
        monkeypatch.setenv("SPARKII_HOME", str(tmp_path))
