...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Sparkii"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^sparkii"]}
        )
        assert adapter._message_matches_mention_patterns("sparkii please help") is True
        assert adapter._message_matches_mention_patterns("please sparkii help") is False
