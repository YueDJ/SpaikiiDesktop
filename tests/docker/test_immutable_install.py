...
Build the real image and verify at runtime:
  ...
  1. /opt/sparkii is not writable by the sparkii user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and SPARKII_DISABLE_LAZY_INSTALLS are set
  3. /opt/sparkii/.install_method contains "docker" (code-scoped stamp)
  4. $SPARKII_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $SPARKII_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_sparkii(
    built_image: str, container_name: str,
) -> None:
    """The sparkii user must not be able to modify /opt/sparkii.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/sparkii as the sparkii user
        "touch /opt/sparkii/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sparkii user can write to /opt/sparkii (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/sparkii/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sparkii user can write to /opt/sparkii/.venv: {r.stdout}"
    )


def test_sparkii_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    SPARKII_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$SPARKII_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"SPARKII_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $SPARKII_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $SPARKII_HOME was not healed on boot: "
        f"{r.stdout}"
    )
