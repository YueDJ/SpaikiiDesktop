"""Runtime smoke test for Docker $SPARKII_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the sparkii user after container boot.
...
registered later runs its log service as the dropped sparkii user and
...
def test_logs_gateways_seeded_and_sparkii_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by sparkii after boot."""
    ...
    assert "logs=sparkii" in r.stdout, (
        f"logs/ not owned by sparkii: {r.stdout}"
    )
    assert "gateways=sparkii" in r.stdout, (
        f"logs/gateways/ not owned by sparkii: {r.stdout}"
    )
