...
Override the image with ``SPARKII_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SPARKII_TEST_IMAGE", "sparkii-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SPARKII_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SPARKII_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged sparkii user
# ---------------------------------------------------------------------------
#
# Background: every Sparkii runtime path inside the container drops to UID
# 10000 (the ``sparkii`` user) via ``s6-setuidgid sparkii``. ``docker exec``
...
# fails (PermissionError swallowed) for sparkii, so a test that ran as root
...
# helpers below run every probe under ``-u sparkii`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: sparkii).
