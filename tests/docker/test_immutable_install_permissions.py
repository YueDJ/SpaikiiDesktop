...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$SPARKII_HOME" = "/opt/data" && '
        'test "$SPARKII_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_sparkii_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/sparkii/agent/message_sanitization.py")
        try:
