...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "sparkii"
        assert config.workspace_id == "sparkii"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"sparkii": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"sparkii": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"sparkii": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="sparkii-agent"
        ):
            result = config.resolve_session_name("/home/user/sparkii-agent/subdir")
        assert result == "sparkii-agent"
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: SPARKII_HOME points at ~/.sparkii/profiles/<name>, so
        # _get_default_sparkii_home() must resolve back to ~/.sparkii — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".sparkii"
        ...
        assert _get_default_sparkii_home() == default_home
...
class TestResolveActiveHost:
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"SPARKII_HONCHO_HOST": "sparkii.coder"}):
            assert resolve_active_host() == "sparkii.coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"sparkii": {"enabled": True, "aiPeer": "sparkii"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (sparkii.work)
        ...
        cfg = HonchoClientConfig(
            ...
            raw={"hosts": {"sparkii.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_sparkii_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SPARKII_MANAGED_DIR", str(managed_dir))
