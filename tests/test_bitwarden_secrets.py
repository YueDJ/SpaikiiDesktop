...
binary download are exercised manually by `sparkii secrets bitwarden
...
def sparkii_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_sparkii_home; clear if needed.
    import sparkii_constants
    if hasattr(sparkii_constants, "_SPARKII_HOME_CACHE"):
        sparkii_constants._SPARKII_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from core.env_loader import _apply_external_secret_sources
