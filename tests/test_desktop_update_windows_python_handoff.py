...
`scripts/desktop-update/windows.ps1` drives `sparkii update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\sparkii.exe` console-script launcher. On Windows that launcher is
a real process that keeps `sparkii.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `sparkii.exe` the launcher still has
...
Start Menu shortcut points at a `Sparkii.exe` that no longer exists.

Driving the update as `python.exe -m sparkii_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-SparkiiStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$sparkiiExe` shim. `sparkii.exe` may still be *named* in the file for the
...
def test_invoke_sparkii_step_calls_drive_python_not_the_shim() -> None:
    source = _read()

    invocations = re.findall(r"Invoke-SparkiiStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-SparkiiStep call in "
    ...
    assert not offenders, (
        "Every Invoke-SparkiiStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the sparkii.exe shim. Driving the update "
        "through the shim keeps sparkii.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "sparkii_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m sparkii_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "sparkii_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m sparkii_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_sparkii_exe_shim() -> None:
    source = _read()

    assert "Invoke-SparkiiStep $sparkiiExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the sparkii.exe shim (`Invoke-SparkiiStep $sparkiiExe`). That is the "
