"""Shared fixtures for the sparkii-agent test suite.
...
2. **Isolated SPARKII_HOME.** SPARKII_HOME points to a per-test tempdir so
   code reading ``~/.sparkii/*`` via ``get_sparkii_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".sparkii"`` instead of the canonical
   ``get_sparkii_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SPARKII_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SPARKII_HOME before ANY test module is imported ──────────────────
# `sparkii_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_sparkii_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.sparkii/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SPARKII_HOME, but fixtures run
...
# the REAL Sparkii root — capture it BEFORE the sandbox rewires SPARKII_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.sparkii (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SPARKII_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SPARKII_HOME = os.environ.get("SPARKII_HOME", "")


def _sparkii_home_points_at_production(value: str) -> bool:
    """True when a pre-set SPARKII_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SPARKII_HOME=~/.sparkii``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``sparkii_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.sparkii`` — the escape vector that landed pytest
...
if _sparkii_home_points_at_production(os.environ.get("SPARKII_HOME", "")):
    _SESSION_SPARKII_HOME = tempfile.mkdtemp(prefix="sparkii-test-home-")
    os.environ["SPARKII_HOME"] = _SESSION_SPARKII_HOME
    atexit.register(shutil.rmtree, _SESSION_SPARKII_HOME, True)
...
# like a real CLI") — which used to disarm sparkii_state's live-DB guard in
# the child at the same moment the child lost the SPARKII_HOME redirect.
# SPARKII_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# sparkii_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SPARKII_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SPARKII_TEST_ISOLATION"] = os.environ.get("SPARKII_HOME", "") or "1"

#: SPARKII_HOME as it stood when conftest was imported - i.e. before any test
...
SPARKII_HOME_AT_CONFTEST_IMPORT = os.environ.get("SPARKII_HOME", "")
...
# SPARKII_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SPARKII_BEHAVIORAL_VARS = frozenset({
    ...
    # any test run launched FROM a Sparkii agent session inherits them and
    # sparkii_constants home-resolution helpers prefer them over monkeypatched
    ...
    "SPARKII_MAX_ITERATIONS",
    "SPARKII_SESSION_PLATFORM",
    "SPARKII_SESSION_CHAT_ID",
    "SPARKII_SESSION_CHAT_NAME",
    "SPARKII_SESSION_CHAT_TYPE",
    "SPARKII_SESSION_THREAD_ID",
    "SPARKII_SESSION_SOURCE",
    "SPARKII_SESSION_KEY",
    "SPARKII_GATEWAY_SESSION",
    "SPARKII_CRON_SESSION",
    "_SPARKII_GATEWAY",
    "SPARKII_PLATFORM",
    "SPARKII_MODEL",
    "SPARKII_INFERENCE_MODEL",
    "SPARKII_INFERENCE_PROVIDER",
    "SPARKII_TUI_PROVIDER",
    "SPARKII_MANAGED",
    "SPARKII_MANAGED_DIR",
    "SPARKII_DEV",
    "SPARKII_CONTAINER",
    ...
    "SPARKII_TIMEZONE",
    "SPARKII_REDACT_SECRETS",
    "SPARKII_BACKGROUND_NOTIFICATIONS",
    "SPARKII_EXEC_ASK",
    "SPARKII_HOME_MODE",
    "SPARKII_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.sparkii/kanban.db instead of the per-test SPARKII_HOME.
    "SPARKII_KANBAN_DB",
    "SPARKII_KANBAN_BOARD",
    "SPARKII_KANBAN_HOME",
    "SPARKII_KANBAN_WORKSPACES_ROOT",
    "SPARKII_KANBAN_LOGS_ROOT",
    "SPARKII_KANBAN_TASK",
    "SPARKII_KANBAN_WORKSPACE",
    "SPARKII_KANBAN_RUN_ID",
    "SPARKII_KANBAN_CLAIM_LOCK",
    "SPARKII_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "SPARKII_DELEGATED_CHILD_CONTEXT",
    "SPARKII_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "sparkii" host.
    "SPARKII_HONCHO_HOST",
    ...
    "SPARKII_DASHBOARD_OAUTH_CLIENT_ID",
    "SPARKII_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SPARKII_HOME to per-test tempdirs so code that
    reads ``~/.sparkii/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SPARKII_* vars that could change test semantics.
    for name in _SPARKII_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SPARKII_HONCHO_HOST", "sparkii")

    # 3. Redirect SPARKII_HOME to a per-test tempdir. Code that reads
    #    ``~/.sparkii/*`` via ``get_sparkii_home()`` now gets the tempdir.
    #
    ...
    (fake_sparkii_home / "memories").mkdir()
    (fake_sparkii_home / "skills").mkdir()
    monkeypatch.setenv("SPARKII_HOME", str(fake_sparkii_home))
    ...
    # sparkii_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SPARKII_TEST_ISOLATION", str(fake_sparkii_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SPARKII_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. sparkii_state computes ``DEFAULT_DB_PATH = get_sparkii_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from sparkii_state import ...``) that
    ...
    sparkii_state_mod = sys.modules.get("sparkii_state")
    if sparkii_state_mod is not None and hasattr(sparkii_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            sparkii_state_mod, "DEFAULT_DB_PATH", fake_sparkii_home / "state.db"
        )
    ...
    #    ~/.sparkii/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import core.plugins as _plugins_mod
        ...
        # previous test's tmp_path SPARKII_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_sparkii_home(_hermetic_environment):
...
# invocation), kanban writes silently pollute the real ~/.sparkii. This autouse
...
# allow-list because test-level fixtures legitimately move SPARKII_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SPARKII_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SPARKII_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Sparkii root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SPARKII_HOME and not _sparkii_home_points_at_production(
        _PRE_SANDBOX_SPARKII_HOME
    ):
        # SPARKII_HOME was genuinely set to a CUSTOM root before the sandbox
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.sparkii. See #69283."
        )
...
# ``sparkii_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Sparkii root. This fixture wires the
# test-side knobs:
...
#     installs where SPARKII_HOME is not ~/.sparkii) into the guard's
...
# so subprocess children that import sparkii_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("sparkii_state")
    ...
    if _PRE_SANDBOX_SPARKII_HOME and not _sparkii_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SPARKII_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
# approvals from one test's session into another's.
    ...
    # A leaked context-local Sparkii home override redirects every later
    # ``get_sparkii_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from sparkii_constants import get_sparkii_home_override, set_sparkii_home_override

        if get_sparkii_home_override() is not None:
            set_sparkii_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal sparkii config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``sparkii-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> sparkii-gateway``
...
def _wal_is_usable() -> bool:
    """True when Sparkii will actually put a database into WAL mode here.

    Sparkii refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Sparkii can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Sparkii managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``sparkii_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_sparkii_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_sparkii_home`` fixture
    redirects ``SPARKII_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.sparkii/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``sparkii_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SPARKII_VOICE_TTS"] = "1"``. The
...
# OS gating
#
# Sparkii runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.sparkii``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Sparkii falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Sparkii uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart sparkii-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting sparkii/python patterns are also blocked.
    """
    ...
    def _matches_sparkii_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SPARKII_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_sparkii_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch sparkii-themed patterns + a
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live sparkii-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting sparkii/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `sparkii update` (or the
        ...
        if "update" in low and (
            # sparkii update / sparkii update --gateway / setsid bash -c ... sparkii update
            ("sparkii" in low and "update" in low.split())
            or
            # python -m sparkii_cli.main update --gateway
            ("sparkii_cli" in low and "update" in low.split())
            or
            # venv/bin/sparkii update  (absolute path variant used in tests)
            (".venv/bin/sparkii" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`sparkii update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SPARKII_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    if hasattr(_tts_tool, "stream_tts_to_speaker"):
        monkeypatch.setattr(_tts_tool, "stream_tts_to_speaker", _blocked_speak_text)
    if hasattr(_voice_mode, "play_beep"):
        monkeypatch.setattr(_voice_mode, "play_beep", _blocked_speak_text)
    if hasattr(_voice_mode, "_play_int16_via_tempfile"):
        monkeypatch.setattr(
            _voice_mode, "_play_int16_via_tempfile", _blocked_play_audio_file
        )
