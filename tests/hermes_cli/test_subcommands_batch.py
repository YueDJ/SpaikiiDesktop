...
def test_config_get_unset_subcommands_parse():
    """`sparkii config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="sparkii")
...
# ── deprecated `sparkii login` fails gracefully, not with argparse error ────
#
# `sparkii login` is a removed command; its handler (`login_command` in
# `sparkii_cli/auth.py`) prints a deprecation notice pointing at `sparkii auth` /
# `sparkii model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `sparkii --help`.
