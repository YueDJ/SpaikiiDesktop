...
from sparkii_cli import uninstall
...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_sparkii_home", lambda: sparkii_home)
    monkeypatch.setattr(uninstall, "_is_default_sparkii_home", lambda home: False)
    ...
    assert str(sparkii_home) in output
    assert project_root.exists()
    assert sparkii_home.exists()
