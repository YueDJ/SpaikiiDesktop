"""Tests for sparkii_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("sparkii_cli.skills_config.save_config")
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SPARKII_SESSION_PLATFORM should be used when SPARKII_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_sparkii_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SPARKII_PLATFORM should win over SPARKII_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from sparkii_cli.skills_config import _get_categories
        skills = [
