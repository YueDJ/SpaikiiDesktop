...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.sparkii/sparkii-agent/utils.py)
...
2. ``sparkii update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``sparkii_cli/main.py`` (the ``sparkii update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``sparkii_constants`` after the
