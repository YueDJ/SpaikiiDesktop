...
def _make_agent():
    sparkii_home = Path(tempfile.mkdtemp(prefix="sparkii-test-home-"))
    (sparkii_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._sparkii_home", sparkii_home),
