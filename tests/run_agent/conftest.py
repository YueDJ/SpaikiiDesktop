...
# Note: the product-layer background-review provider is registered by the
# frontend surfaces (CLI/gateway/TUI) via ``set_background_review_provider``
# at import time. The core test suite runs without a surface, so no provider
# is registered here — tests that exercise the review fork drive the worker
# directly instead.
