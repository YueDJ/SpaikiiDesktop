...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    sparkii_state = ModuleType("sparkii_state")
    sparkii_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "sparkii_state", sparkii_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Sparkii"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Sparkii"
