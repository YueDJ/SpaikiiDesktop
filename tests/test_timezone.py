"""
Tests for timezone support (sparkii_time module + integration points).
...
def _reset_sparkii_time_cache():
    """Reset the sparkii_time module cache (replacement for removed reset_cache)."""
    sparkii_time._cached_tz = None
    sparkii_time._cached_tz_name = None
    sparkii_time._cache_resolved = False
...
class TestHermesTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["SPARKII_TIMEZONE"] = "Asia/Kolkata"
        result = sparkii_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["SPARKII_TIMEZONE"] = "UTC"
        result = sparkii_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SPARKII_TIMEZONE"] = "Europe/London"
        tz = sparkii_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SPARKII_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SPARKII_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SPARKII_TIMEZONE=" + os.environ.get("SPARKII_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SPARKII_TIMEZONE=NOT_SET" in result["output"], (
            "SPARKII_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SPARKII_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SPARKII_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SPARKII_TIMEZONE"] = "US/Eastern"
        _reset_sparkii_time_cache()
