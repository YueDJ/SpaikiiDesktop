...
Uses a throwaway SPARKII_HOME so it never touches ~/.sparkii. No real names.
"""
...
def _fresh_home():
    """Point SPARKII_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_sparkii_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SPARKII_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SPARKII_HOME is set.
...
# a different get_sparkii_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SPARKII_HOME (throwaway): {HOME}")
