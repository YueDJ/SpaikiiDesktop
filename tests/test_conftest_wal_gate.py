"""The conftest WAL gate must agree with sparkii_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``sparkii_state``. That is deliberate: importing
``sparkii_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.sparkii`` before the per-test ``SPARKII_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_sparkii_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and sparkii_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_sparkii_state_at_collection():
    """The gate must stay import-free of sparkii_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.sparkii, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``sparkii_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "sparkii_state" or name.startswith("sparkii_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported sparkii_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.sparkii during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("sparkii_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["sparkii_state"] = saved
    assert not blocked
