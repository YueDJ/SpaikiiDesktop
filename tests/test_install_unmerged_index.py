...
The ``sparkii update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m sparkii_cli.main gateway run`` — image name
    ``pythonw``, not ``sparkii.exe`` — so the ``taskkill /IM sparkii.exe`` guard
    ...
    # The sparkii.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM sparkii.exe' in text
