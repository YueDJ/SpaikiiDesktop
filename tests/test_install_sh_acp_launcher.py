"""`setup_path()` must also install a `sparkii-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `sparkii-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Sparkii as not installed. `setup_path()` therefore
writes a `sparkii-acp` launcher next to the `sparkii` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/sparkii-acp".*?'
    r'log_success "Installed sparkii-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the sparkii-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    return command_link_dir / "sparkii-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `sparkii-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "sparkii-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/sparkii-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/sparkii-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# sparkii-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

SPARKII_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/sparkii-agent".*?'
    r'log_success "Installed sparkii-agent launcher[^\n]*\n)',
    re.S,
)


def _extract_sparkii_agent_shim_block() -> str:
    match = SPARKII_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
        "could not locate the sparkii-agent launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)


def _run_sparkii_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted sparkii-agent block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: sparkii-agent is NOT installed by this block (handled
    ...
    script = (
        ...
        f"command_link_display_dir={command_link_dir}\n"
        f"USE_VENV={use_venv}\n"
        "log_success(){ :; }\n" + _extract_sparkii_agent_shim_block()
    )
    result = subprocess.run(
        ["bash", "-c", script],
        capture_output=True,
    ...
    assert result.returncode == 0, (
        f"sparkii-agent shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "sparkii-agent"


def test_venv_install_writes_executable_sparkii_agent_launcher(tmp_path):
    """venv install must write a user-executable sparkii-agent launcher."""
    shim = _run_sparkii_agent_block(tmp_path, "true")
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "sparkii-agent must dispatch to run_agent.py"


def test_sparkii_agent_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove sparkii-agent alongside
    sparkii and sparkii-acp."""
    from sparkii_cli.uninstall import remove_wrapper_script

    # Simulate a sparkii-agent wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "sparkii-agent"
    ...
    assert local_shim in removed, "local sparkii-agent wrapper must be removed"
