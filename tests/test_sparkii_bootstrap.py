"""Tests for sparkii_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Sparkii entry point
(sparkii, sparkii-agent, sparkii-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Sparkii entry point imports sparkii_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sparkii_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sparkii_bootstrap", None)
    import sparkii_bootstrap  # noqa: WPS433
    return sparkii_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Sparkii CLI
...
class TestEntryPointsImportBootstrap:
    """Every Sparkii entry point must import sparkii_bootstrap as its
    ...
    # Entry points that invoke Sparkii as a process.  Each one must
    # import sparkii_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sparkii_cli/main.py",   # sparkii CLI (console_script)
        "run_agent.py",          # sparkii-agent (console_script)
        "acp_adapter/entry.py",  # sparkii-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sparkii_bootstrap' and that
        ...
        half-finished ``sparkii update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``sparkii_bootstrap`` as a top-level module) leaves sparkii
        ...
        """
        # Resolve relative to the sparkii-agent repo root.  Tests live
        # at tests/test_sparkii_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # sparkii start even when sparkii_bootstrap hasn't been
        ...
        assert first_import_name == "sparkii_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sparkii_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sparkii_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Sparkii's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SPARKII_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SPARKII_PYTHON_SRC_ROOT"] = env
            elif "SPARKII_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SPARKII_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/sparkii")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SPARKII_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SPARKII_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/sparkii", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Sparkii root.  The guard must relocate Sparkii to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/sparkii"])
        assert result[0] == "/opt/sparkii"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Sparkii root.
        assert result.index("/opt/sparkii") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SPARKII_PYTHON_SRC_ROOT"] = "/env/sparkii"
            hb.harden_import_path()
            assert sys.path[0] == "/env/sparkii"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SPARKII_PYTHON_SRC_ROOT", None)
