# Moved from sparkii_cli/model_switch.py during the Block 4 split; the CLI module is now a shim over this file.
...
- ``sparkii_cli.providers``        -- canonical provider identity + overlays
- ``sparkii_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Sparkii wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Sparkii.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Sparkii versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``sparkii model`` wizard — not a
    ...
    while ``sparkii model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Sparkii itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Sparkii itself discovered (``models_discovered: true``
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``sparkii config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from core.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``sparkii_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sparkii model' for available providers, or define it "
        ...
        from core.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from core.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from core.runtime_provider import _auto_detect_local_model
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from core.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from core.models import should_use_ollama_native_catalog
    ...
    # sparkii_cli.runtime_provider.resolve_runtime_provider; without the strip,
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from core.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Sparkii provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from core.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from core.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from core.providers import SPARKII_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from core.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Sparkii-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for sparkii_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if _canonical != sparkii_id:
            continue
        if sparkii_id.lower() in seen:
            continue
        if sparkii_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from core.credentials import is_runtime_provider_routable
        ...
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(sparkii_id)
            seen.add(sparkii_id.lower())

    # --- Section 2: Sparkii-only providers (SPARKII_OVERLAYS) ---
    ...
    for pid, overlay in SPARKII_OVERLAYS.items():
        ...
        if sparkii_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or sparkii_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or sparkii_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(sparkii_slug)
            seen.add(pid.lower())
            seen.add(sparkii_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from sparkii_cli/models.py (OPENROUTER_MODELS,
    ...
    # Build curated model lists keyed by sparkii provider ID
    ...
    if "ollama-cloud" not in curated:
        from core.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from core.models import fetch_lmstudio_models
    ...
    # in PROVIDER_TO_MODELS_DEV or SPARKII_OVERLAYS (keeps /model in sync
    # with `sparkii model`).
    try:
        from core.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``sparkii model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``sparkii model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from core.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``sparkii model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from core.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`sparkii_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from core.models import fetch_openrouter_models
