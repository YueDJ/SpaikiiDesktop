# Moved from sparkii_cli/kanban_specify.py during the Block 4 split; the CLI module is now a shim over this file.

"""Kanban triage specifier — flesh out a one-liner into a real spec.

Used by ``sparkii kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``sparkii_cli/goals.py`` — same aux
...
SPARKII_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("SPARKII_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Sparkii Agent board.
...
def _profile_author() -> str:
    """Mirror of ``sparkii_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("SPARKII_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=SPARKII_KANBAN_SPECIFY_MAX_TOKENS,
