...
``core.config.migrate_config``. Each step is a function
...
``core.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``core.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``core.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``core.config`` (e.g.
``patch("core.config.read_raw_config", ...)``) keep working, because
...
def _cfg():
    """Return the live ``core.config`` module (lazy, cycle-free)."""
