"""
Configuration management for Sparkii Agent.

Config files are stored in ~/.sparkii/ for easy access:
- ~/.sparkii/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sparkii/.env         - API keys and secrets

This module provides:
- sparkii config          - Show current configuration
- sparkii config edit     - Open config in editor
- sparkii config get      - Print a resolved configuration value
- sparkii config set      - Set a specific value
- sparkii config unset    - Remove a user configuration value
- sparkii config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``sparkii config set`` (which rewrites
    ...
    ``config.yaml`` in place: sparkii never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.sparkii/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``sparkii logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  sparkii config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Sparkii makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Sparkii itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``sparkii update``, the TUI build.
...
#   on every plugin install / ``sparkii update``.
...
# * ``SPARKII_HOME`` / ``SPARKII_PROFILE`` / ``SPARKII_CONFIG`` /
#   ``SPARKII_ENV`` — Sparkii runtime location flags. Writing these into
...
#
# IMPORTANT: ``SPARKII_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SPARKII_LANGFUSE_PUBLIC_KEY,
# SPARKII_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Sparkii runtime location — never via dashboard env writer.
    # NOT a SPARKII_* blanket: integration credentials (SPARKII_GEMINI_*,
    # SPARKII_LANGFUSE_*, SPARKII_SPOTIFY_*, ...) ARE allowed.
    "SPARKII_HOME", "SPARKII_PROFILE", "SPARKII_CONFIG", "SPARKII_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Sparkii runtime location "
            "(SPARKII_HOME, SPARKII_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.sparkii/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # SPARKII_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `sparkii plugins enable
    # observability/langfuse` or `sparkii tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SPARKII_LANGFUSE_ENV",
    "SPARKII_LANGFUSE_RELEASE",
    "SPARKII_LANGFUSE_SAMPLE_RATE",
    "SPARKII_LANGFUSE_MAX_CHARS",
    "SPARKII_LANGFUSE_CAPTURE",
    "SPARKII_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "SPARKII_ACP_AUTH_METHOD",
    "SPARKII_ACP_AUTO_APPROVE",
    "SPARKII_COPILOT_ACP_COMMAND",
    "SPARKII_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from core.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set SPARKII_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SPARKII_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_sparkii_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # SPARKII_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Sparkii is running in package-manager-managed mode.

    Two signals: the SPARKII_MANAGED env var (set by the systemd service),
    or a .managed marker file in SPARKII_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Sparkii through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``sparkii_cli/`` — i.e. the git checkout for source
    installs, ``/opt/sparkii`` inside the published image. It is a property of
    the running interpreter, NOT of ``$SPARKII_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Sparkii was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$SPARKII_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SPARKII_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.sparkii``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SPARKII_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.sparkii:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SPARKII_HOME``, a
    ...
    ``docker`` and ``sparkii update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SPARKII_HOME.
    try:
    ...
    #    host install whose shared $SPARKII_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``sparkii update``.
    try:
        method = (
            (get_sparkii_home() / ".install_method")
    ...
    # detect Nix installs that don't set SPARKII_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``sparkii_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SPARKII_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/sparkii`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "apt":
        ...
        return "pkg upgrade sparkii-agent"
...
# Long-form text for ``sparkii update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# sparkii_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Sparkii, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``sparkii update`` inside Docker.
...
def format_managed_message(action: str = "modify this Sparkii installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Sparkii installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Sparkii."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    """
    if os.environ.get("SPARKII_DEV") == "1":
        return None
    ...
    container_mode_file = get_sparkii_home() / ".container-mode"
...
# Re-export from sparkii_constants — canonical definition lives there.
from sparkii_constants import get_sparkii_home, get_process_sparkii_home  # noqa: F811,E402
...
def _resolve_sparkii_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    ownership. The entrypoint chowns the top-level SPARKII_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SPARKII_UID", "").strip()
    gid_str = os.environ.get("SPARKII_GID", "").strip()
    try:
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the sparkii group can
    share state with the gateway service.

    The mode can be overridden via the SPARKII_HOME_MODE environment variable
    (e.g. SPARKII_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SPARKII_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SPARKII_UID``/``SPARKII_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SPARKII_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Sparkii runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SPARKII_CONTAINER") or os.environ.get("SPARKII_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SPARKII_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SPARKII_HOME, upgrading legacy empty templates.
...
# ensure_sparkii_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SPARKII_HOME_ENSURED: set = set()


def ensure_sparkii_home():
    """Ensure ~/.sparkii directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``SPARKII_HOME`` we only re-run
    ...
    ``get_sparkii_home()`` and therefore re-run for the new path.
    """
    home = get_sparkii_home()
    key = str(home)

    if key in _SPARKII_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``sparkii profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SPARKII_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_sparkii_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _SPARKII_HOME_ENSURED.add(key)


def _ensure_sparkii_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SPARKII_HOME {home} does not exist."
        )
...
from core.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `sparkii config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.sparkii.config`` entries, then checks
    ...
    try:
        ...
        # should never break `sparkii update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Sparkii' own config writer has historically emitted it
    ...
    # Sparkii (``_save_discovered_models_to_config``), not hand-curated.
    # Older Sparkii versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Sparkii versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sparkii_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (sparkii_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (sparkii_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (sparkii_cli/setup.py)
    "known_plugin_toolsets", # written/read by sparkii_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sparkii setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Sparkii won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, sparkii send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sparkii doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_sparkii_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "sparkii update / sparkii -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ sparkii config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in core.config_migrations as a
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: sparkii config set <key> <value>")
    ...
    # `sparkii update`, but only the version bump is persisted.
    ...
    # metadata.sparkii.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.sparkii/.env); "
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``SparkiiCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``sparkii config set``
    ...
    backends) — while display paths (``sparkii status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sparkii_home / "config.yaml"`` local variable
...
from core.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401
...
def render_personality_prompt(value: Any) -> str:
    ...
    from core.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    """
    from core.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sparkii/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_sparkii_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.sparkii/config.yaml.
    ...
    (which change ``SPARKII_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Sparkii host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/sparkii/config.yaml invalidates the
...
#   openai-codex (OAuth — sparkii auth) — OpenAI Codex
#   nous         (OAuth — sparkii auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.sparkii/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Sparkii writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sparkii/.env.
    ...
    interactive menu render (`sparkii tools`, `sparkii setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `sparkii tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sparkii/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sparkii/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``SPARKII_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"SPARKII_CUSTOM_{slug}_API_KEY" if slug else "SPARKII_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sparkii/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.sparkii/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Sparkii — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Sparkii vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.sparkii/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.sparkii/.env`` over ``os.environ``.

    Used for Sparkii-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Sparkii Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    for _anthropic_var in ("ANTHROPIC_API_KEY", "ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN"):
        _anthropic_value = get_env_value_prefer_dotenv(_anthropic_var) or ""
        if _anthropic_value:
            anthropic_value = _anthropic_value
            break
    ...
    # Warn on stale SPARKII_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SPARKII_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SPARKII_MAX_ITERATIONS={_env_ghost} "
                f"(run 'sparkii doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  sparkii config edit     # Edit config file", Colors.DIM))
    print(color("  sparkii config set <key> <value>", Colors.DIM))
    print(color("  sparkii setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``SPARKII_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("SPARKII_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `sparkii cron list`, then pin the "
        "intended values with `sparkii cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (sparkii_cli/plugins_cmd.py, sparkii_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `sparkii config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``sparkii config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``sparkii config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``sparkii config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (sparkii config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        if value.lower() in {'true', 'yes', 'on'}:
            ...
            #   sparkii config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   sparkii config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``sparkii config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``sparkii config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    sparkii config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    sparkii config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `sparkii config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_sparkii_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `sparkii config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `sparkii config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: sparkii config get <key> [--json]")
            print()
            print("Examples:")
            print("  sparkii config get model")
            print("  sparkii config get terminal.backend")
            print("  sparkii config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: sparkii config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  sparkii config set model anthropic/claude-sonnet-4")
            print("  sparkii config set terminal.backend docker")
            print("  sparkii config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: sparkii config unset <key>")
            print()
            print("Examples:")
            print("  sparkii config unset model")
            print("  sparkii config unset terminal.backend")
            print("  sparkii config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sparkii config migrate' to add them")
        ...
        print("  sparkii config           Show current configuration")
        print("  sparkii config edit      Open config in editor")
        print("  sparkii config get <key>          Print a resolved config value")
        print("  sparkii config set <key> <value>   Set a config value")
        print("  sparkii config unset <key>        Remove a config value")
        print("  sparkii config check     Check for missing/outdated config")
        print("  sparkii config migrate   Update config with new options")
        print("  sparkii config path      Show config file path")
        print("  sparkii config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sparkii config`` UI so users
