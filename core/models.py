# Moved from sparkii_cli/models.py during the Block 4 split; the CLI module is now a shim over this file.

"""
Canonical model catalogs and lightweight validation helpers.

Add, remove, or reorder entries here — both `sparkii setup` and
`sparkii` provider-selection will pick up the change automatically.
"""
...
_SPARKII_USER_AGENT = f"sparkii-cli/{_SPARKII_VERSION}"
...
# $SPARKII_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Sparkii-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SPARKII_HOME/models_dev_cache.json directly (no network). Falls
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Sparkii that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Sparkii that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    # The Nous portal account module was removed during the foundation trim;
    # default to paid so free-tier gating never blocks users.
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_sparkii_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SPARKII_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  sparkii model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `sparkii model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Sparkii provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `sparkii model` / setup wizard /
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from core.model_catalog import get_default_model_from_cache
...
# ``sparkii model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in sparkii_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``sparkii auth add openai-codex`` without
    ``sparkii model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``sparkii model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sparkii-agent is tool-calling-first — every provider path assumes the model
...
# sparkii_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from core.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sparkii-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Sparkii lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from core.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from sparkii_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return sparkii-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; sparkii's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    # Nous runtime env override was removed with the Nous product trim.
    env_base = None

    api_key = ""
    creds_base = ""
    # Live Nous runtime credential resolution was removed with the product
    # trim; only the static default base remains.
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from core.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``sparkii model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from core.credentials import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _SPARKII_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from core.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Sparkii should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``sparkii auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.sparkii/.env``.
    ...
    try:
        from core.credentials import read_credential_pool
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Sparkii release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Sparkii release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Live Nous Portal /models fetching was removed with the product trim;
        # fall through to the curated manifest below.
        pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["nous"] snapshot — so newly
        # added Portal models still surface without a Sparkii release.
...
#
# Cache strategy:
#   - One JSON file at $SPARKII_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SPARKII_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `sparkii config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SPARKII_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Sparkii' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Sparkii' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SPARKII_USER_AGENT}
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=sparkii"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `sparkii moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Sparkii will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Sparkii will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Sparkii cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Sparkii actually ships.  (#46850)
            #
