# Moved from sparkii_cli/goals.py during the Block 4 split; the CLI module is now a shim over this file.

"""Persistent session goals — the Ralph loop for Sparkii.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Sparkii feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.SparkiiCLI`` or the gateway
...
def _bootstrap_session_db(home: str, done: threading.Event) -> None:
    ...
    try:
        ...
        # SPARKII_HOME). It then caches the wrong profile's DB under this
        # profile's key.
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current SPARKII_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``sparkii_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.sparkii/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``sparkii -p <profile> chat -q "work kanban task <id>"``). The worker's
