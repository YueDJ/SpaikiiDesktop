"""Direct NeMo Relay integration for Sparkii shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Sparkii core Relay host."""
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Sparkii task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Sparkii's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Sparkii retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Sparkii shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Sparkii shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Sparkii shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_tool_call(
        ...
        try:
            ...
            logger.warning(
                "Sparkii shared-metrics tool call close failed",
                exc_info=True,
            )
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Sparkii shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Sparkii shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Sparkii shared metrics operation failed", exc_info=True)
            return None
...
def enabled() -> bool:
    ...
    try:
        ...
        logger.debug("Unable to read Sparkii shared-metrics policy", exc_info=True)
        value = False
    else:
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Sparkii lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Sparkii shared metrics hook failed: %s", hook_name, exc_info=True
        )


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Sparkii's runtime registry."""
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Sparkii execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Sparkii shared metrics initialization failed", exc_info=True)
...
# Defensive: in test embeddings the coordinator may be a stub without the
# initializer registry; a metrics subscriber must never crash module import.
try:
    relay_runtime.SESSION_COORDINATOR.register_session_initializer(
        SUBSCRIBER_NAME,
        _prepare_core_session,
    )
except Exception:  # pragma: no cover - embedding-specific
    logger.debug(
        "shared-metrics session initializer registration skipped",
        exc_info=True,
    )
