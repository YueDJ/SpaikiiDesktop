# Moved from sparkii_cli/managed_uv.py during the Block 4 split; the CLI module is now a shim over this file.
...
_RUNTIME_DIR_NAME = ".sparkii-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``sparkii update`` runs
    the call site from the *old*, already-imported ``sparkii_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``sparkii update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under SPARKII_HOME
    caches the last successful self-update time.
    """
    try:
        from sparkii_constants import get_sparkii_home

        stamp = get_sparkii_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``sparkii update`` so the managed copy stays current.
...
def _reload_sparkii_constants():
    """Re-execute ``sparkii_constants`` from disk and return the fresh module.

    ``sparkii update`` imports ``sparkii_constants`` from the OLD checkout,
    ...
    return importlib.reload(sparkii_constants)
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `sparkii update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("sparkii_cli.main")
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``sparkii update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``sparkii update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$SPARKII_HOME/bin/`` instead of ``~/.local/bin/``.
    """
