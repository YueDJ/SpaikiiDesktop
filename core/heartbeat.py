# Moved from sparkii_cli/heartbeat.py during the Block 4 split; the CLI module is now a shim over this file.
...
remains ``sparkii cron`` / the ``cronjob`` tool, which runs in isolated
...
def _get_session_db() -> Optional[Any]:
    # Reuse the goals module's per-SPARKII_HOME cached SessionDB so both
