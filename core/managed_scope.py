...
This is DISTINCT from ``core.config.is_managed()`` / ``SPARKII_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through core.config.load_config) to honor managed
    scope. Mirrors core.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from core.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
