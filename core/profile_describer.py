# Moved from sparkii_cli/profile_describer.py during the Block 4 split; the CLI module is now a shim over this file.

"""Profile describer — auto-generate ``description`` for a profile.

Used by ``sparkii profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``sparkii_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Sparkii Agent kanban board.
...
Rules:
  ...
  - Never write "Sparkii Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_sparkii_home())
        else:
