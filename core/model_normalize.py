# Moved from sparkii_cli/model_normalize.py during the Block 4 split; the CLI module is now a shim over this file.
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  Sparkii revisions folded every non-reasoner input into
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Sparkii' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Sparkii provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``sparkii_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from core.models import normalize_copilot_model_id
