# Moved from sparkii_cli/kanban_db.py during the Block 4 split; the CLI module is now a shim over this file.
...
``<root>`` is the **shared Sparkii root** (the parent of any active
...
``sparkii -p <profile>`` joins the same board as the dispatcher that
...
the profile's Sparkii home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SPARKII_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SPARKII_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``sparkii kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.sparkii``. In Docker / custom
deployments where ``SPARKII_HOME`` points outside ``~/.sparkii`` (e.g.
``/opt/sparkii``), ``<root>`` is ``SPARKII_HOME``. Legacy env-var
overrides still work:

* ``SPARKII_KANBAN_DB`` — pin the database file path directly.
* ``SPARKII_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SPARKII_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SPARKII_KANBAN_DB``,
``SPARKII_KANBAN_WORKSPACES_ROOT``, and ``SPARKII_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/sparkii-kanban-v1-spec.pdf`` for the full
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``sparkii_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from sparkii_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("SPARKII_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active SPARKII_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from core.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from core.lifecycle import has_hook
...
# ``SPARKII_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``SPARKII_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("SPARKII_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``SPARKII_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("SPARKII_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``SPARKII_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "SPARKII_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "sparkii_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``sparkii-agent``
...
def kanban_home() -> Path:
    """Return the shared Sparkii root that anchors the kanban board.
    ...
    profile's ``SPARKII_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``sparkii kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SPARKII_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``sparkii kanban boards
    ...
    env = os.environ.get("SPARKII_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``sparkii kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SPARKII_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SPARKII_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SPARKII_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("SPARKII_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``SPARKII_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("SPARKII_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``sparkii kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``sparkii_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``SPARKII_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (sparkii_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``SPARKII_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("SPARKII_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``sparkii gateway run --replace`` /
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see sparkii_cli.sqlite_safe_read), so it must only run once the board has
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Sparkii treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``sparkii kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SPARKII_KANBAN_DB`` env → ``SPARKII_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `sparkii kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        from sparkii_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See sparkii_state._WAL_INCOMPAT_MARKERS for detection logic.
                from sparkii_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``sparkii kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``SPARKII_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from core.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``sparkii --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from core import projects_db as _pdb
    ...
    # `sparkii --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``sparkii kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `sparkii kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``sparkii kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``sparkii kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``sparkii kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``sparkii kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``sparkii kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``SPARKII_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Sparkii' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Sparkii-managed storage. A board ``default_workdir`` pointing at a
...
def _cleanup_worktree_workspace(
    ...
    """
    try:
        from core.git_worktree import _worktree_has_unpushed_commits, _worktree_is_dirty
    except Exception:
...
#      via ``sparkii kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Sparkii checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Sparkii treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Sparkii creates/reuses that linked worktree. With
      no ``workspace_path``, Sparkii anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Sparkii uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``SPARKII_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Sparkii
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``sparkii kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``sparkii kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Sparkii profile.
    ...
    if not rows:
        return False
    try:
        from core.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Sparkii ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Sparkii
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``sparkii kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``SPARKII_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from core.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from core.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Sparkii profile.
        # `_default_spawn` invokes ``sparkii -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Sparkii
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `sparkii kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Sparkii bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from core.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_sparkii_argv() -> list[str]:
    ...
    1. ``$SPARKII_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("sparkii")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\sparkii.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m sparkii_cli.main`` — fallback for setups where
       Sparkii is launched from a venv and the ``sparkii`` shim is not on
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``sparkii -p <profile> chat -q ...`` subprocess.
    ...
    ``SPARKII_KANBAN_DB`` / ``SPARKII_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject SPARKII_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `sparkii -p <name>` the
    # _apply_profile_override() runs *before* sparkii_constants is imported.
    # If SPARKII_HOME is absent from the child's env, get_sparkii_home() falls
    # back to Path.home() / ".sparkii" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from core.profiles import resolve_profile_env
    try:
        env["SPARKII_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via SPARKII_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SPARKII_HOME never had profiles created.
        pass
    if task.tenant:
        env["SPARKII_TENANT"] = task.tenant
    env["SPARKII_KANBAN_TASK"] = task.id
    env["SPARKII_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `sparkii kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["SPARKII_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["SPARKII_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["SPARKII_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SPARKII_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["SPARKII_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["SPARKII_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    env["SPARKII_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["SPARKII_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SPARKII_KANBAN_BOARD"] = resolved_board
    # SPARKII_PROFILE is the author the kanban_comment tool defaults to.
    # `sparkii -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SPARKII_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited SPARKII_TUI=1
    ...
    # older sparkii builds on PATH that predate the flag's precedence.
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("SPARKII_HOME"))
    if worker_toolsets:
    ...
    # `sparkii kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`sparkii` executable not found on PATH. "
            "Install Sparkii Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``sparkii kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``sparkii kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `sparkii kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled SPARKII_PROFILE values like "sparkii-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Sparkii root exists

    Reads profile paths directly so this module has no import dependency on
    ``sparkii_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from sparkii_constants import get_default_sparkii_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``sparkii kanban assignees`` for the terminal.
