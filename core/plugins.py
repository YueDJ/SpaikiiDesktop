"""
Sparkii Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with sparkii-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.sparkii/plugins/<name>/``
3. **Project plugins** – ``./.sparkii/plugins/<name>/`` (opt-in via
   ``SPARKII_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``sparkii_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
# Injectable gateway platform-registry bridge
# ---------------------------------------------------------------------------
#
# The plugin loader is core-owned and must not import ``gateway``.  The
# gateway registers its ``PlatformRegistry`` singleton here at import time
# (see ``gateway/platform_registry.py``); in core-only processes the bridge
# stays ``None`` and platform registration is a safe no-op.
_platform_registry_provider = None


def set_platform_registry_provider(registry) -> None:
    """Install the gateway's platform registry for plugin platform adapters."""
    global _platform_registry_provider
    _platform_registry_provider = registry


def _get_platform_registry():
    """Return the installed platform registry, or ``None`` in core-only runs."""
    return _platform_registry_provider


def get_platform_registry():
    """Public accessor for the installed platform registry (``None`` if unset).

    Kernel consumers (``toolsets.py``, ``agent/system_prompt.py``) use this
    instead of importing ``gateway.platform_registry`` directly; the gateway
    registers its registry via :func:`set_platform_registry_provider`.
    """
    return _platform_registry_provider


# ---------------------------------------------------------------------------
# Injectable surface bridges
# ---------------------------------------------------------------------------
#
# The plugin loader is core-owned and must not import surface packages.  Each
# surface integration below (platform actions, profiles, command resolution,
# dashboard auth, built-in auxiliary tasks, approval transports, agent
# plugins, lifecycle observability) is registered by its owning surface module
# at import time.  In core-only processes the bridges stay ``None`` and the
# corresponding feature degrades gracefully.

_platform_actions_provider = None
_profile_name_provider = None
_command_resolver_provider = None
_dashboard_auth_provider = None
_builtin_aux_tasks_provider = None
_approval_transport_provider = None
_agent_plugin_provider = None


def set_platform_actions_provider(factory) -> None:
    """Install the ``PlatformActions`` factory (sparkii_cli/platform_actions)."""
    global _platform_actions_provider
    _platform_actions_provider = factory


def _get_platform_actions_provider():
    return _platform_actions_provider


def set_profile_name_provider(fn) -> None:
    """Install ``get_active_profile_name`` (sparkii_cli/profiles)."""
    global _profile_name_provider
    _profile_name_provider = fn


def _get_profile_name_provider():
    return _profile_name_provider


def set_command_resolver_provider(fn) -> None:
    """Install ``resolve_command`` (sparkii_cli/commands)."""
    global _command_resolver_provider
    _command_resolver_provider = fn


def _get_command_resolver_provider():
    return _command_resolver_provider


def set_dashboard_auth_provider(provider) -> None:
    """Install the dashboard-auth provider namespace.

    ``provider`` must expose ``DashboardAuthProvider`` (the base class),
    ``register_provider``, ``snapshot_registration`` and
    ``restore_registration`` (sparkii_cli/dashboard_auth).
    """
    global _dashboard_auth_provider
    _dashboard_auth_provider = provider


def _get_dashboard_auth_provider():
    return _dashboard_auth_provider


def set_builtin_aux_tasks_provider(fn) -> None:
    """Install a callable returning ``_AUX_TASKS`` (sparkii_cli/main)."""
    global _builtin_aux_tasks_provider
    _builtin_aux_tasks_provider = fn


def _get_builtin_aux_tasks_provider():
    return _builtin_aux_tasks_provider


def set_approval_transport_provider(cls) -> None:
    """Install ``RegisteredApprovalTransport`` (sparkii_cli/approval_transport)."""
    global _approval_transport_provider
    _approval_transport_provider = cls


def _get_approval_transport_provider():
    return _approval_transport_provider


def set_agent_plugin_provider(namespace) -> None:
    """Install the agent-plugin helper namespace (sparkii_cli/agent_plugins).

    ``namespace`` must expose ``discover_mcp``, ``read_manifest`` and
    ``load`` callables.
    """
    global _agent_plugin_provider
    _agent_plugin_provider = namespace


def _get_agent_plugin_provider():
    return _agent_plugin_provider


_lifecycle_observer = None


def set_lifecycle_observer(observer) -> None:
    """Install a first-party lifecycle observer namespace.

    The namespace must expose ``observe(hook_name, **kwargs)`` and
    ``handles(hook_name)`` (see ``sparkii_cli/observability``).  The observer
    fires before plugin callbacks so first-party observability features keep
    working when core hook call sites import this module directly.
    """
    global _lifecycle_observer
    _lifecycle_observer = observer


def _get_lifecycle_observer():
    return _lifecycle_observer


# Middleware contract constants (core-owned; sparkii_cli/middleware re-imports).
OBSERVER_SCHEMA_VERSION = "sparkii.observer.v1"
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``SPARKII_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SPARKII_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SPARKII_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.sparkii/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SPARKII_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SPARKII_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Sparkii loggers.
    """
    ...
    logger.debug(
        "SPARKII_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Sparkii' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by sparkii_cli.kanban_db when a task
    ...
    # separate `sparkii -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `sparkii kanban dispatch`),
    ...
    # ``sparkii kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: sparkii_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINT_CAPABILITIES_GROUP = "sparkii_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``sparkii_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- sparkii-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- sparkii-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- sparkii-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``sparkii:<event>``.
SPARKII_EVENT_NAMESPACE = "sparkii"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Sparkii home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "sparkii", "depends",
}

# Highest manifest schema version this Sparkii understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Sparkii "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `sparkii plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.sparkii/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``sparkii plugins list``. For a flat plugin at
    ...
    # ``sparkii_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Sparkii never
    ...
    # (``sparkii plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Sparkii paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_sparkii_home() / "plugin-data" / self._data_namespace
...
class _UnavailablePlatformActions:
    """Core-only fallback for ``PluginContext.platform_actions``.

    Returned when no surface has installed the ``PlatformActions`` provider
    (no CLI/gateway in the process).  Every verb returns a structured error
    dict instead of raising, mirroring the live facade's contract.
    """

    def __init__(self, plugin_id: str) -> None:
        self.plugin_id = plugin_id

    def _unavailable(self, verb: str, **kwargs: Any) -> Dict[str, Any]:
        return {
            "ok": False,
            "error": (
                f"platform action '{verb}' unavailable: no platform actions "
                "provider installed (core-only process)"
            ),
            "verb": verb,
        }

    async def add_reaction(self, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        return self._unavailable("add_reaction", **kwargs)

    async def set_thread_title(self, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        return self._unavailable("set_thread_title", **kwargs)
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Sparkii config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            _factory = _get_platform_actions_provider()
            if _factory is None:
                logger.warning(
                    "Plugin %s requested platform actions but no platform "
                    "actions provider is installed (core-only process); "
                    "returning an unavailable facade.",
                    self.manifest.name,
                )
                self._platform_actions = _UnavailablePlatformActions(self.plugin_id)
            else:
                self._platform_actions = _factory(self.plugin_id)
    ...
    def profile_name(self) -> str:
        """Return the active Sparkii profile name (e.g. ``"default"``).

        Derived from ``SPARKII_HOME`` via
        :func:`sparkii_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.sparkii/profiles/<name>``, or ``"custom"`` when
        ``SPARKII_HOME`` points somewhere unrecognized.
        """
        try:
            _profile_fn = _get_profile_name_provider()
            if _profile_fn is None:
                return "default"
            return _profile_fn()
        except Exception:
            return "default"
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Sparkii core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``sparkii honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``sparkii <subcommand>``
        ...
        try:
            _resolve_command = _get_command_resolver_provider()
            if _resolve_command is not None and _resolve_command(clean) is not None:
                logger.warning(
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`sparkii_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        _dash = _get_dashboard_auth_provider()
        if _dash is None:
            logger.warning(
                "Plugin '%s' tried to register a dashboard-auth provider "
                "but no dashboard-auth provider is installed (core-only "
                "process); ignoring.",
                self.manifest.name,
            )
            return
        DashboardAuthProvider = _dash.DashboardAuthProvider
        register_provider = _dash.register_provider
        snapshot_registration = _dash.snapshot_registration
        restore_registration = _dash.restore_registration
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        NOTE ON TIMING: ``load_sparkii_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_sparkii_dotenv``) so the first process sees them (#64177).
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        if platform_registry is None:
            logger.debug(
                "Plugin %s tried to register platform %r but no platform "
                "registry is installed (core-only process); ignoring.",
                self.manifest.name,
                name,
            )
            return None
        from core.plugin_platform import PlatformEntry
    ...
    def register_slack_action_handler(
        ...
        Sparkii' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``sparkii model → Configure auxiliary models`` picker
        ...
        _aux_tasks_provider = _get_builtin_aux_tasks_provider()
        _builtin_aux_tasks = (
            _aux_tasks_provider() if _aux_tasks_provider is not None else ()
        )
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``sparkii:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``sparkii:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, SPARKII_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{SPARKII_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``sparkii:<event>``
    ...
    def register_skill(
        ...
        ``~/.sparkii/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or sparkii_home_key()
        ...
        # by resolved sparkii home). The ledger is therefore keyed per manager
        # — i.e. per (sparkii_home, plugin_id) — and every release/restore
        ...
        # contributed, so `sparkii plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("SPARKII_SAFE_MODE"):
                logger.info("SPARKII_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_sparkii_dotenv() already ran at import time. Re-pull so the
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with sparkii and must
            ...
            # all ~20 of them added several seconds to every `sparkii`
            # invocation — including plain `sparkii chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Sparkii ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `sparkii plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        _RegisteredApprovalTransport = _get_approval_transport_provider()
        if _RegisteredApprovalTransport is None:
            # Late fallback: core.approval_transport registers itself at import,
            # but if this manager was imported before that module was reached,
            # resolve the class directly (no surface dependency).
            try:
                from core.approval_transport import RegisteredApprovalTransport as _RAT

                _RegisteredApprovalTransport = _RAT
            except Exception:
                _RegisteredApprovalTransport = None
        if _RegisteredApprovalTransport is None:
            raise RuntimeError(
                "approval transports require the CLI/gateway surface "
                "(no approval-transport provider installed)"
            )
        ...
        self._approval_transports[clean] = _RegisteredApprovalTransport(
            name=clean,
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_sparkii_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_sparkii_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.sparkii/plugins/)
        user_dir = get_sparkii_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.sparkii/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("SPARKII_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".sparkii" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SPARKII_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                _agent_plugins = _get_agent_plugin_provider()
                if _agent_plugins is not None and _agent_plugins.discover_mcp(
                    Path(manifest.path),
                    get_sparkii_home()
                    / "plugin-data"
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    ...
                    if _agent_plugins is None:
                        continue
                    data, diagnostics = _agent_plugins.read_manifest(child)
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Sparkii process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``sparkii_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``sparkii plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`sparkii plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``sparkii tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `sparkii plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Sparkii validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Sparkii does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `sparkii plugins list` would
    ...
    def _load_portable_plugin(
        ...
        try:
            ...
            if _agent_plugins is None:
                logger.warning(
                    "Agent Plugin '%s' cannot be loaded: no agent-plugin "
                    "provider installed (core-only process).",
                    lookup_key,
                )
                return
            package = _agent_plugins.load(
                Path(manifest.path),
                get_sparkii_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Sparkii home (profile switch reusing a slug
        # like "sparkii-lcm") or from an earlier force=True reload in the
...
# Keyed cache: resolved Sparkii home -> PluginManager. Sparkii supports
# multiple profiles via different SPARKII_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_sparkii_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# sparkii_constants.set_sparkii_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_sparkii_home() / "plugins"`` and some
    plugins (notably context engines such as sparkii-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Sparkii home (env var *or* the context-local
    ``set_sparkii_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Sparkii home instead of
    being one process-wide singleton.
    """
    try:
        return get_sparkii_home().expanduser().resolve()
    except Exception:
        return get_sparkii_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``sparkii_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``sparkii_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Sparkii profile/home.
    ...
    profile switch — via ``SPARKII_HOME`` or the context-local
    ``set_sparkii_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=sparkii_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_sparkii_home() / "cache" / "plugin_toolset_keys.json"
...
def invoke_hook(hook_name: str, **kwargs: Any) -> List[Any]:
    ...
    """
    observer = _get_lifecycle_observer()
    if observer is not None:
        try:
            observer.observe(hook_name, **kwargs)
        except Exception:
            logger.warning(
                "First-party lifecycle observer failed for %s",
                hook_name,
                exc_info=True,
            )
...
def has_hook(hook_name: str) -> bool:
    ...
    if observer is not None:
        try:
            handles = getattr(observer, "handles", None)
            if callable(handles) and handles(hook_name):
                return True
        except Exception:
            pass
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="sparkii-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``sparkii:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``sparkii tools`` TUI so plugin-provided toolsets appear
