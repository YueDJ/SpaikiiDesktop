...
the tool registry or launching Sparkii (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``sparkii-cli`` to the non-existent ``sparkii``. ``resolve_toolset('sparkii')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``sparkii-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``sparkii`` instead of ``sparkii-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `sparkii tools` to reconfigure."
        )
    return warnings
