# Moved from sparkii_cli/profiles.py during the Block 4 split; the CLI module is now a shim over this file.

"""
Profile management for multiple isolated Sparkii instances.

Each profile is a fully independent SPARKII_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.sparkii/profiles/<name>/`` by default.

The "default" profile is ``~/.sparkii`` itself — backward compatible,
zero migration needed.

Usage::

    sparkii profile create coder          # fresh profile + bundled skills
    sparkii profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sparkii profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sparkii -p coder chat                 # or via flag
    sparkii profile use coder             # set as sticky default
    sparkii profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See sparkii_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.sparkii``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   sparkii-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `sparkii backup` archives
...
# Marker file written by `sparkii profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `sparkii
...
# `sparkii skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Sparkii infrastructure directories that only the default profile
         (``~/.sparkii``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_sparkii_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sparkii) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sparkii-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SPARKII_HOME equals the
...
# into the export. The set below identifies the *known Sparkii profile
# artifacts* at the root of SPARKII_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``sparkii_constants``.
...
_RESERVED_NAMES = frozenset({
    "sparkii", "default", "test", "tmp", "root", "sudo",
})

# Sparkii subcommands that cannot be used as profile names/aliases
_SPARKII_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the sparkii root, NOT to the current SPARKII_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SPARKII_HOME points outside
    ``~/.sparkii``, profiles live under ``SPARKII_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_sparkii_home() / "profiles"


def _get_default_sparkii_home() -> Path:
    """Return the default (pre-profile) SPARKII_HOME path.

    In standard deployments this is ``~/.sparkii``.
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sparkii_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Sparkii installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SPARKII_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_sparkii_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, sparkii subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SPARKII_SUBCOMMANDS:
        return f"'{canon}' conflicts with a sparkii subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Sparkii. If we
    ...
    try:
        ...
        # not be migrated. The next `sparkii doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``sparkii -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``sparkii profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``sparkii -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``sparkii profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SPARKII_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Sparkii config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sparkii_home()
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SPARKII_HOME. When
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_sparkii_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``sparkii update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sparkii)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sparkii/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `sparkii -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Sparkii profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `sparkii update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`sparkii profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Sparkii (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `sparkii profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `sparkii -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SPARKII_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``sparkii profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SPARKII_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_sparkii_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Sparkii *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Sparkii backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SPARKII_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `sparkii -p <canon> profile
    ...
    # something (e.g. Electron's `findOnPath('sparkii')` resolution) spawns the
    ...
    # interpreter, not "sparkii", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _SPARKII_CONSOLE_SCRIPT_NAMES = frozenset({"sparkii", "sparkii-agent", "sparkii-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Sparkii process: either an entrypoint marker in argv, a
            # resolved executable named `sparkii`, or a python interpreter
            # directly exec'ing a `sparkii`-named console-script shim (argv[0]
            ...
            if not is_sparkii and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: sparkii, sparkii-agent,
                # sparkii-acp) rather than a bare `startswith("sparkii")` --
                ...
                # "sparkii-notes.py" or "sparkii-unrelated-tool" as the shim,
                ...
                is_sparkii = script_stem in _SPARKII_CONSOLE_SCRIPT_NAMES
            if not is_sparkii:
                continue
            ...
            # ...or by SPARKII_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SPARKII_HOME", "")
...
def _stop_profile_backends(canon: str, profile_dir: Path) -> None:
    ...
    try:
        from core.process_utils import _pid_exists, terminate_pid as _terminate_pid
    except Exception:
        return
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sparkii).\n"
            "To remove everything, use: sparkii uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_sparkii_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SPARKII_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    """
    try:
        from core.gateway_service import get_gateway_service_context
        ns = get_gateway_service_context()
        if ns is None or ns.detect_service_manager() != "s6":
            return  # host path — silent, no registration needed
        mgr = ns.get_service_manager()
    except RuntimeError:
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SPARKII_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SPARKII_HOME")
    try:
        os.environ["SPARKII_HOME"] = str(profile_dir)
        from core.gateway_service import get_gateway_service_context
        _svc = get_gateway_service_context()
        if _svc is None:
            return
        get_service_name = _svc.get_service_name
        get_launchd_plist_path = _svc.get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SPARKII_HOME"] = old_home
        elif "SPARKII_HOME" in os.environ:
            del os.environ["SPARKII_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        from core.process_utils import terminate_pid as _terminate_pid
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sparkii/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: sparkii profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SPARKII_HOME.

    Returns ``"default"`` if SPARKII_HOME is not set or points to ``~/.sparkii``.
    Returns the profile name if SPARKII_HOME points into ``~/.sparkii/profiles/<name>``.
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SPARKII_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Sparkii file the
      user may have lying alongside SPARKII_HOME (#58394).
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.sparkii itself — its parent is ~/ and its
        # directory name is ".sparkii", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sparkii profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sparkii itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sparkii). "
            "Specify a different name: sparkii profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="sparkii_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_sparkii_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SPARKII_HOME path string.

    Called early in the CLI entry point, before any sparkii modules
    are imported, to set the SPARKII_HOME environment variable.

    When SPARKII_HOME is already set, the configured spelling IS the
