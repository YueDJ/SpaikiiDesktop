# Moved from sparkii_cli/copilot_auth.py during the Block 4 split; the CLI module is now a shim over this file.
...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `sparkii model` to authenticate via OAuth\n"
...
def _jwt_disk_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_sparkii_home()) / _JWT_DISK_FILENAME
    except Exception:
        return None
