"""Response filtering helpers (moved from gateway during the Block 4 split).

These helpers decide whether a completed agent turn should be delivered to the
chat.  The cron scheduler (core) and the gateway webhook lane both use the
autonomous silence matcher, so the canonical implementation lives in core and
``gateway/response_filters.py`` re-exports it — the two lanes cannot drift.
"""
