# Moved from sparkii_cli/providers.py during the Block 4 split; the CLI module is now a shim over this file.

"""
Single source of truth for provider identity in Sparkii Agent.
...
2. **Sparkii overlays** — transport type, auth patterns, aggregator flags,
...
# -- Sparkii overlay ----------------------------------------------------------
# Sparkii-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SparkiiOverlay:
    """Sparkii-specific provider metadata layered on top of models.dev."""
...
SPARKII_OVERLAYS: Dict[str, SparkiiOverlay] = {
    "moa": SparkiiOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "sparkii", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Sparkii overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Sparkii overlay
    ...
    overlay = SPARKII_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + sparkii extra
    ...
    if overlay is not None:
        # Sparkii-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SPARKII_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Sparkii overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in SPARKII_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Sparkii provider IDs must win over LOSSY alias collapsing.
