# Moved from sparkii_cli/loops.py during the Block 4 split; the CLI module is now a shim over this file.
...
Design notes / invariants (same contract as ``sparkii_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.SparkiiCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from core.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from core.goals import _warn_dropped_write
