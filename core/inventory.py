# Moved from sparkii_cli/inventory.py during the Block 4 split; the CLI module is now a shim over this file.
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``sparkii model`` CLI picker. Adds network calls
    ...
    try:
        from core.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``sparkii_cli/moa_cmd.py``.
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    """
    from core.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`sparkii_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from core.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from core.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `sparkii model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Sparkii.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `sparkii model` to configure ({auth_type})"
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`sparkii_cli.model_switch.list_picker_providers`) so the
