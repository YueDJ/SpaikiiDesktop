# Moved from sparkii_cli/approval_transport.py during the Block 4 split; the CLI module is now a shim over this file.
...
# Block 4: register the transport-registry class with the core plugin loader so
# ``PluginContext.register_approval_transport`` can reach it without an import
# cycle.  (This wiring was missing after the Block 3 core split; without it
# plugin-owned approval transports always raised "no provider installed".)
try:
    from core.plugins import set_approval_transport_provider

    set_approval_transport_provider(RegisteredApprovalTransport)
