"""Pre-execution ACP edit approval contract (core-owned).

Extracted from ``acp_adapter/edit_approval.py`` during the Block 4 repo split so
the tool dispatcher (``model_tools.py``) can enforce edit approval without
importing the ACP frontend package.  ACP-specific protocol wiring — building
``ToolCallUpdate`` payloads and bridging to ``request_permission`` — stays in
``acp_adapter/edit_approval.py``, which re-exports the names below.
