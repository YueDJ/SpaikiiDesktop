# Moved from sparkii_cli/runtime_provider.py during the Block 4 split; the CLI module is now a shim over this file.
...
from core.credentials import ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, AuthError, format_auth_error, resolve_provider, resolve_api_key_provider_credentials, has_usable_secret, is_actual_local_base_url, normalize_actual_base_url
...
from core.providers import custom_provider_aliases, custom_provider_slug
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from core.credentials import resolve_provider
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from core.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Sparkii' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "anthropic":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from core.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from core.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("SPARKII_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/sparkii_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'sparkii doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``SPARKII_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("SPARKII_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'sparkii model' or "
    ...
    *,
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Sparkii canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Sparkii-compatible importers).
