# Moved from sparkii_cli/tools_config.py during the Block 4 split; the CLI module is now a shim over this file.

"""
Unified tool configuration for Sparkii Agent.

`sparkii tools` and `sparkii setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.sparkii/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``sparkii tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`sparkii_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
#
# Block 4: rich CLI output lives in the frontend; core uses plain-print
# fallbacks so this module has no frontend dependency.  The interactive
# configurator menus are provided through registered hooks (see below).
...
def _print_warning(*args, **kwargs):
    print("WARNING:", *args)
...
# Interactive configurator hooks (registered by the frontend at import time;
# core-only processes fall back to graceful degradation).
_curses_ui_provider = None
_plugin_enable_store_provider = None
_setup_helpers_provider = None


def set_curses_ui_provider(provider) -> None:
    """Register the curses menu namespace (sparkii_cli/curses_ui)."""
    global _curses_ui_provider
    _curses_ui_provider = provider


def set_plugin_enable_store_provider(provider) -> None:
    """Register the plugin enable/disable store (sparkii_cli/plugins_cmd)."""
    global _plugin_enable_store_provider
    _plugin_enable_store_provider = provider


def set_setup_helpers_provider(provider) -> None:
    """Register the setup-wizard helpers (sparkii_cli/setup)."""
    global _setup_helpers_provider
    _setup_helpers_provider = provider


def get_curses_ui():
    """Return the registered curses-UI namespace, or ``None``."""
    provider = _curses_ui_provider
    if provider is None:
        return None
    try:
        return provider()
    except Exception:  # noqa: BLE001 - surface absence must not crash core
        return None


def get_plugin_enable_store():
    """Return the registered plugin enable-store namespace, or ``None``."""
    provider = _plugin_enable_store_provider
    if provider is None:
        return None
    try:
        return provider()
...
def get_setup_helpers():
    """Return the registered setup-wizard helpers, or ``None``."""
    provider = _setup_helpers_provider
    if provider is None:
        return None
    try:
        return provider()
...
# They're still in _SPARKII_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `sparkii tools` → Video Generation, which walks
...
# Platform-scoped toolsets: only appear in the `sparkii tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from core.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from core.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``sparkii tools`` checklist actually offers
    for ``platform``.
    ...
    so ``sparkii tools`` never claims to add or remove a toolset the user was
...
from core.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Sparkii routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set SPARKII_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SPARKII_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SPARKII_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SPARKII_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SPARKII_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SPARKII_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `sparkii model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("SPARKII_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Sparkii telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $SPARKII_HOME/bin is never on PATH, so a bare which()
    # misses the uv Sparkii installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``sparkii update`` and
      by ``sparkii computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``sparkii update`` sets
    ...
    a further ~600s wait on Windows). ``sparkii computer-use install
    ...
    ``sparkii update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `sparkii update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `sparkii computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `sparkii update` (require_confirmed_update=True) keeps the
    ...
    # `sparkii computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: sparkii computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `sparkii update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `sparkii update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.sparkii/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Sparkii process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $SPARKII_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Sparkii installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_sparkii_home()}/sparkii-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in sparkii-agent directory)")
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.sparkii/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.sparkii/config.yaml")
        ...
        try:
            from core.tools_config import get_plugin_enable_store
            store = get_plugin_enable_store()
            if store is None:
                raise RuntimeError("plugin enable store unavailable")
            enabled = store.get_enabled_set()
            if "observability/langfuse" in enabled or "langfuse" in enabled:
                ...
                store.save_enabled_set(enabled)
                _print_success("    Plugin observability/langfuse enabled")
        except Exception as exc:
            _print_warning(f"    Could not enable plugin automatically: {exc}")
            _print_info("    Run manually: sparkii plugins enable observability/langfuse")
        _print_info("    Restart Sparkii for tracing to take effect.")
        _print_info("    Verify: sparkii plugins list")
        ...
        # The xAI Grok OAuth provider was removed in the Phase 0 foundation
        # trim; only the explicit XAI_API_KEY path remains.
        ...
        try:
            from core.tools_config import get_setup_helpers
            helpers = get_setup_helpers()
            if helpers is None:
                raise RuntimeError("setup helpers unavailable")
            from core.config import save_env_value
        except Exception as exc:
            _print_warning(f"    Could not load setup helpers: {exc}")
            _print_info("    Run later: sparkii auth add xai-oauth   (or set XAI_API_KEY)")
            return

        idx = helpers.prompt_choice(
            "    How do you want xAI to authenticate?",
            choices=[
                "Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                "Paste an xAI API key (console.x.ai)",
                "Skip — configure later via `sparkii auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if helpers.run_xai_oauth_login_from_setup():
                _print_success(
                ...
                _print_warning(
                    ...
                    "Run later: sparkii auth add xai-oauth"
                )
        elif idx == 1:
            api_key = helpers.prompt("    xAI API key", password=True)
            if api_key:
                save_env_value("XAI_API_KEY", api_key)
                _print_success("    XAI_API_KEY saved")
            else:
                _print_warning(
                    "    No API key provided. Run later: sparkii auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``sparkii tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``sparkii tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: sparkii tools post-setup <key>")
        return 2
...
#:
#: Saving ``sparkii tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[sparkii-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``sparkii tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"sparkii-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (sparkii-acp, sparkii-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # "sparkii-cli" (which include all _SPARKII_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "sparkii-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``sparkii tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `sparkii tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"sparkii-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("sparkii-")}
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by sparkii tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `sparkii` instead of
    # `sparkii-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `sparkii update`/`sparkii doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `sparkii tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `sparkii tools` is the user's opt-in to reconfigure tools, so treat
...
def _prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    return ui.curses_radiolist(question, choices, selected=default, cancel_returns=default)
...
def _visible_providers(
    ...
    *,
    ...
    Nous-managed Tool Gateway rows (``managed_nous_feature`` /
    ``requires_nous_auth``) were removed with the Nous product line and are
    never shown.
    """
    visible = []
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `sparkii tools` gets a silent no-op
...
def provider_readiness_status(
    ...
    *,
    ...
    if provider.get("requires_nous_auth") or managed_feature:
        # Nous-managed rows were removed with the Nous product line; treat
        # them as needing auth so nothing activates.
        return "needs_auth"
...
def _is_provider_active(
    ...
    *,
    ...
    managed_feature = provider.get("managed_nous_feature")
    if managed_feature:
        # Nous-managed rows were removed with the Nous product line.
        return False
...
# Kept in sync with the dashboard selects (sparkii_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # Nous-managed rows (managed_nous_feature / requires_nous_auth) were
    # removed with the Nous product line; nothing to gate here.
    if managed_feature or provider.get("requires_nous_auth"):
        _print_warning("  Not available — the Nous Tool Gateway was removed.")
        return
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``sparkii model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``sparkii model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `sparkii model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `sparkii model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `sparkii tools` and `sparkii setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Sparkii Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://sparkii-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from sparkii_constants import display_sparkii_home
    print(color(f"  Tool configuration saved to {display_sparkii_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'sparkii' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    ui = get_curses_ui()
    if ui is None:
        _print_warning("Interactive MCP tools checklist unavailable (no curses UI).")
        return
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, sparkii mcp
