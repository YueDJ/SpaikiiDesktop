...
# node:26 image instead (Sparkii pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/sparkii is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/sparkii/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when sparkii
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# sparkii process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('sparkii')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main sparkii process, the dashboard,
...
# 'Sparkii WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via SPARKII_UID at runtime
RUN useradd -u 10000 -m -d /opt/data sparkii
...
#
# ui-tui/packages/sparkii-ink/ is copied IN FULL (not just its manifests)
...
# lock on the @sparkii/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Sparkii when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Sparkii production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/sparkii/.venv at first
...
# gives the non-root sparkii user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link sparkii-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/sparkii are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the sparkii user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/sparkii/bin && \
    cp /opt/sparkii/docker/sparkii-exec-shim.sh /opt/sparkii/bin/sparkii && \
    chmod 0755 /opt/sparkii/bin/sparkii && \
    printf 'docker\n' > /opt/sparkii/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $SPARKII_HOME. $SPARKII_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``sparkii update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the sparkii user via
# `s6-setuidgid sparkii` in its run script. If SPARKII_UID is unset, services
# run as the default sparkii user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `sparkii dump` reports
...
# Static services declared at build time: main-sparkii + dashboard.
...
# `exec sparkii`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SPARKII_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/sparkii/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-sparkii-setup && \
    chmod +x /etc/cont-init.d/01-sparkii-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SPARKII_WEB_DIST=/opt/sparkii/sparkii_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/sparkii/ui-tui/dist/entry.js`) and skip the
...
ENV SPARKII_TUI_DIR=/opt/sparkii/ui-tui
ENV SPARKII_HOME=/opt/data
ENV SPARKII_WRITE_SAFE_ROOT=/opt/data
ENV SPARKII_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/sparkii (root-owned, read-only) so a runtime
...
# is seeded + chowned to the sparkii user by docker/stage2-hook.sh and lives
...
ENV SPARKII_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> sparkii ...` they default to root, and any file the
# command writes under $SPARKII_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/sparkii/bin/sparkii, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid sparkii`
# when invoked as root. Non-root callers (supervised processes,
# `--user sparkii`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/sparkii/.venv/bin/sparkii). See the shim source for
# the opt-out env var (SPARKII_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/sparkii-exec-shim.sh /opt/sparkii/bin/sparkii
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/sparkii/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> sparkii ...` and any
# subprocess that doesn't activate the venv first still find sparkii.
#
# /opt/sparkii/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/sparkii/bin:/opt/sparkii/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. sparkii
# subcommand vs. no-args), drops to the sparkii user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/sparkii/docker/entrypoint-dispatch.sh" ]
CMD [ ]
