...
# `s6-setuidgid hermes` in its run script. If SPARKII_UID is unset, services
...
# /opt/hermes/.sparkii_build_sha at build time, and have
# sparkii_cli/build_info.py read it at runtime.  Both `hermes dump` and
...
RUN if [ -n "${SPARKII_GIT_SHA}" ]; then \
        printf '%s\n' "${SPARKII_GIT_SHA}" > /opt/hermes/.sparkii_build_sha; \
    fi
...
# ---------- Runtime ----------
ENV SPARKII_WEB_DIST=/opt/hermes/sparkii_cli/web_dist
...
ENV SPARKII_TUI_DIR=/opt/hermes/ui-tui
