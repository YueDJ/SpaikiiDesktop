#!/bin/sh
# shellcheck shell=sh
# /opt/sparkii/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching SPARKII_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `sparkii` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `sparkii <args>` (subcommand passthrough)
#
# Drop to sparkii via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${SPARKII_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${SPARKII_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export SPARKII_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset SPARKII_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid sparkii "$@"; exec "$@"; }
...
# an arbitrary non-root, non-sparkii UID, the bootstrap was skipped and the
# baked image dirs (owned by the sparkii build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sparkii)" ]; then
    cat >&2 <<EOF
[sparkii] ERROR: container started with --user $cur_uid (an arbitrary, non-sparkii UID) — not supported.
...
# to the sparkii user's home before dropping privileges so libraries that
...
_sparkii_orig_cwd="${SPARKII_ORIG_CWD:-$PWD}"
...
# the user's command so `sparkii chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_sparkii_orig_cwd"
...
# Sparkii subcommand pass-through.
drop sparkii "$@"
