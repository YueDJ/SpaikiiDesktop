...
#
# Wired into the image as /etc/cont-init.d/01-sparkii-setup by the
...
SPARKII_HOME="${SPARKII_HOME:-/opt/data}"
...
# starts non-root. The baked install tree under /opt/sparkii is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SPARKII_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SPARKII_UID/SPARKII_GID — or the PUID/PGID aliases — which the
...
# `id -u sparkii` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# sparkii UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sparkii)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-sparkii UID).
...
and the baked /opt/sparkii install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SPARKII_UID=\$(id -u) -e SPARKII_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the sparkii user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SPARKII_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid sparkii mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SPARKII_HOME paths whose parents only
# root can create (e.g. `SPARKII_HOME=/home/sparkii/.sparkii` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SPARKII_UID/SPARKII_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SPARKII_UID/
# SPARKII_GID still win when both are set.  See #15290, salvages #25872.
SPARKII_UID="${SPARKII_UID:-${PUID:-}}"
SPARKII_GID="${SPARKII_GID:-${PGID:-}}"

if [ -n "${SPARKII_UID:-}" ] && validate_uid_gid "$SPARKII_UID" && [ "$SPARKII_UID" != "$(id -u sparkii)" ]; then
    echo "[stage2] Changing sparkii UID to $SPARKII_UID"
    usermod -u "$SPARKII_UID" sparkii
fi
if [ -n "${SPARKII_GID:-}" ] && validate_uid_gid "$SPARKII_GID" && [ "$SPARKII_GID" != "$(id -g sparkii)" ]; then
    echo "[stage2] Changing sparkii GID to $SPARKII_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SPARKII_GID" sparkii 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised sparkii user (UID 10000)
...
# sparkii process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes sparkii. Idempotent across container
...
#   - sparkii is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G sparkii 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] sparkii already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" sparkii 2>/dev/null; then
        echo "[stage2] Added sparkii to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group sparkii failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SPARKII_UID is remapped or the top-level $SPARKII_HOME isn't owned by
# the runtime sparkii UID, restore ownership to sparkii — but ONLY for the
# directories sparkii actually writes to. The full $SPARKII_HOME may be a
...
chown_sparkii_tree() {
    ...
    chown -R sparkii:sparkii "$target" 2>/dev/null || \
...
if [ "$needs_chown" = true ]; then
    ...
    #
    # Top-level $SPARKII_HOME: chown the directory itself (not its contents)
    # so sparkii can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SPARKII_HOME"; then
        :
    else
        chown sparkii:sparkii "$SPARKII_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SPARKII_HOME failed (rootless container?) — continuing"
    fi
    # Sparkii-owned subdirs: recursive chown is safe here because these are
...
# sparkii user. Hosted/container instances keep mutable state under
# $SPARKII_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SPARKII_DISABLE_LAZY_INSTALLS=1. Keeping /opt/sparkii root-owned and
...
# $SPARKII_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SPARKII_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to sparkii
...
# Always reset ownership of $SPARKII_HOME/profiles to sparkii on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> sparkii …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as sparkii and walks
...
# (jobs.json) must stay readable by the unprivileged sparkii runtime even
...
# top-level $SPARKII_HOME is mis-owned, so a warm volume with sparkii-owned
# SPARKII_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid sparkii mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as sparkii.
if [ -d "$SPARKII_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SPARKII_HOME/logs/gateways"; then
        :
    else
        chown sparkii:sparkii "$SPARKII_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# sparkii pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged sparkii gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SPARKII_HOME is
...
# The targeted data-volume chown above only covers sparkii-owned
# *subdirectories*; loose state files living directly under $SPARKII_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> sparkii …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged sparkii runtime then hits
...
# sweep so host-owned files in a bind-mounted $SPARKII_HOME are never
...
# entries of sparkii_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SPARKII_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SPARKII_HOME/$f"; then
            :
        else
            chown sparkii:sparkii "$SPARKII_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the sparkii runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SPARKII_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SPARKII_HOME/config.yaml"; then
        :
    else
        chown sparkii:sparkii "$SPARKII_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SPARKII_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as sparkii user ---
# Run as sparkii via s6-setuidgid so dirs end up owned correctly (matters
...
as_sparkii mkdir -p \
    ...
    "$SPARKII_HOME/logs" \
    "$SPARKII_HOME/logs/gateways" \
    "$SPARKII_HOME/hooks" \
    "$SPARKII_HOME/memories" \
    "$SPARKII_HOME/skills" \
    "$SPARKII_HOME/skins" \
    "$SPARKII_HOME/plans" \
    "$SPARKII_HOME/workspace" \
    "$SPARKII_HOME/home" \
    "$SPARKII_HOME/pairing" \
    "$SPARKII_HOME/platforms/pairing" \
    "$SPARKII_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/sparkii/.install_method (see Dockerfile), NOT written here into
# $SPARKII_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SPARKII_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.sparkii:/opt/data) and sometimes shared with a
...
# 'sparkii update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SPARKII_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/sparkii stamp, so deleting the data-dir copy is safe).
if [ -f "$SPARKII_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SPARKII_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SPARKII_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SPARKII_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
...
if [ -f "$SPARKII_HOME/.env" ] && ! grep -q '^API_SERVER_KEY=..*' "$SPARKII_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SPARKII_HOME/.env"; then
        :
    else
        ...
        if [ -n "$_gen_key" ]; then
            ...
            sed -i '/^API_SERVER_KEY=$/d' "$SPARKII_HOME/.env" 2>/dev/null || true
            printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SPARKII_HOME/.env"
...
if [ -f "$SPARKII_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SPARKII_HOME/.env"; then
        :
    else
        chown sparkii:sparkii "$SPARKII_HOME/.env" 2>/dev/null || true
        chmod 600 "$SPARKII_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $SPARKII_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `sparkii update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SPARKII_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SPARKII_HOME/config.yaml" ]; then
    s6-setuidgid sparkii "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SPARKII_HOME/auth.json" ] && [ -n "${SPARKII_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SPARKII_HOME/auth.json"; then
        :
    else
        printf '%s' "$SPARKII_AUTH_JSON_BOOTSTRAP" > "$SPARKII_HOME/auth.json"
        chown sparkii:sparkii "$SPARKII_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SPARKII_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SPARKII_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SPARKII_HOME/auth.json" ] && [ -n "${SPARKII_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SPARKII_HOME/auth.json"; then
        :
    else
        s6-setuidgid sparkii "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SPARKII_HOME/auth.json" \
...
# can set SPARKII_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SPARKII_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SPARKII_HOME/gateway_state.json" ] && \
        [ "${SPARKII_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SPARKII_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SPARKII_HOME/gateway_state.json"
        chown sparkii:sparkii "$SPARKII_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SPARKII_HOME/gateway_state.json"
    fi
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/sparkii/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Sparkii spawns for the
...
# main-wrapper.sh propagates it into the supervised ``sparkii`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-sparkii, dashboard, etc.).
