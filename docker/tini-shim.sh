...
# Sparkii WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SPARKII_TINI_SHIM_TARGET overrides the `/init` path (and
# SPARKII_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SPARKII_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SPARKII_TINI_SHIM_WRAPPER:-/opt/sparkii/docker/main-wrapper.sh}"
