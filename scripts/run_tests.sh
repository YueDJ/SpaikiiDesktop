#!/usr/bin/env bash
# Canonical test runner for sparkii-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.sparkii/...)
#
# Usage:
...
# (SPARKII_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.sparkii/sparkii-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.sparkii/sparkii-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${SPARKII_PYTHON:-}" ] && [ -x "$SPARKII_PYTHON" ] \
    && "$SPARKII_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: SPARKII_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `sparkii` binary rather
  # than the devShell hook.
  PYTHON="$SPARKII_PYTHON"
  echo "▶ no local venv — using Nix dev venv via SPARKII_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and SPARKII_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.sparkii/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.sparkii"
...
#
#   * SPARKII_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * SPARKII_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     SPARKII_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# SPARKII_RUN_SLOW_PET_TESTS / SPARKII_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no SPARKII_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in SPARKII_TEST_IMAGE SPARKII_TEST_WORKERS SPARKII_TEST_PATHS \
  SPARKII_TEST_FILE_TIMEOUT SPARKII_TEST_FILE_RETRIES SPARKII_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${SPARKII_RUN_SLOW_PET_TESTS:+SPARKII_RUN_SLOW_PET_TESTS="$SPARKII_RUN_SLOW_PET_TESTS"} \
  ${SPARKII_E2E_BROWSER:+SPARKII_E2E_BROWSER="$SPARKII_E2E_BROWSER"} \
