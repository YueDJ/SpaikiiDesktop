# ============================================================================
# Sparkii Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable Sparkii.exe.
    #
    # Why opt-in:
    #   * Sparkii-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Sparkii.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Sparkii.exe
    ...
    #     `sparkii desktop` already builds on demand.
...
# SPARKII_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[sparkii] $Message")
}
...
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SPARKII_HOME) { "$env:SPARKII_HOME\sparkii-agent" } else { "$env:LOCALAPPDATA\sparkii\sparkii-agent" }
    )
}
...
function Write-Banner {
    ...
    Write-Host "|             * Sparkii Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Uv {
    ...
    try {
        ...
        # (sparkii_cli/managed_uv.py looks only at $SparkiiHome\bin\uv.exe).
...
# Put the Sparkii-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone sparkii-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Sparkii-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Sparkii-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("sparkii-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SPARKII_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Sparkii-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Sparkii setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Sparkii uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\sparkii\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Sparkii needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Sparkii copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\sparkii\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SPARKII_GIT_BASH_PATH`` (User scope) so Sparkii can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Sparkii features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Sparkii needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SPARKII_GIT_BASH_PATH`` (User env scope) so Sparkii can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SPARKII_GIT_BASH_PATH", $candidate, "User")
            $env:SPARKII_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SPARKII_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Sparkii may not find Git Bash."
    Write-Info "If needed, set SPARKII_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Sparkii requires Node >=26)"
    }
    ...
    Write-Info "Installing Sparkii-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\sparkii-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SparkiiHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SparkiiHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SparkiiHome\node") {
                    try {
                        Rename-Item "$SparkiiHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Sparkii-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\sparkii-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `sparkii
                    # update` path (#4735).
                    ...
                    $stashName = "sparkii-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. sparkii-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``sparkii update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Sparkii behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running sparkii processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\sparkii-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `sparkii update` (see the notes at the shared clone-path
    ...
    # next `sparkii update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Sparkii-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running sparkii process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running sparkii processes before recreating venv..."
            ...
            # The launcher CLI (sparkii.exe) plus its child tree.
            & taskkill /F /T /IM sparkii.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM sparkii.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m sparkii_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not sparkii.exe.
            ...
            # Sparkii children re-exec through .sparkii-runtime, so killing only
            ...
            # user (see sparkii_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Sparkii " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `sparkii update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Sparkii (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `sparkii.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install sparkii-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and sparkii.exe broken with
    ...
    # here, not 30 seconds later when the user runs `sparkii`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Sparkii processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Sparkii processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Sparkii and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register sparkii.exe in dist-info/RECORD but fail to
        ...
        # `sparkii` command while sparkii-agent.exe / sparkii-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m sparkii_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `sparkii dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\sparkii_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `sparkii dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `sparkii dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: sparkii_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        # Expose ONLY the sparkii launchers on PATH -- never the whole
        ...
        # (#83797): unrelated projects start resolving python to Sparkii'
        # runtime interpreter. A dedicated bin dir with copies of the
        # launcher exes keeps `sparkii` globally available without
        # shadowing anything. (Launcher exes embed the venv interpreter
        # path, so they work from any location and survive updates.)
        $sparkiiBin = "$InstallDir\bin"
        New-Item -ItemType Directory -Force -Path $sparkiiBin | Out-Null
        foreach ($launcher in @("sparkii.exe", "sparkii-acp.exe")) {
            $src = "$InstallDir\venv\Scripts\$launcher"
            if (Test-Path $src) {
                Copy-Item -Force $src "$sparkiiBin\$launcher"
            }
        }
    }
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept sparkii via $sparkiiBin)"
    }
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.sparkii-bootstrap-complete which tells the Sparkii
    ...
    # Sparkii-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `sparkii update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".sparkii-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and Sparkii's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Sparkii-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'sparkii setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\sparkii-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\sparkii-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\sparkii-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `sparkii tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SparkiiHome "bin"
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Sparkii' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'sparkii tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `sparkii
...
# paths still lazy-install via install_cua_driver() (sparkii_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `sparkii computer-use install` runs. Bounded
        ...
        # Sparkii' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'sparkii tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: sparkii computer-use install"
            }
        } else {
...
# the final `electron` -> `Sparkii` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Sparkii.exe. Only called from
    ...
    # enforces the build floor (Node >=26) and prepends the Sparkii-managed
    ...
    # The --dir mode produces an unpacked Sparkii.exe in
    ...
    # are NOT trying to work around). The Sparkii icon + product name are
    # stamped onto Sparkii.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\sparkii-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Sparkii.exe",
        "$desktopDir\release\win-arm64-unpacked\Sparkii.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Sparkii.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Sparkii icon + identity are stamped onto Sparkii.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, sparkii-agent#38216).
    ...
    #    Sparkii.exe. We deliberately do NOT point them at `sparkii desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Sparkii.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Sparkii.lnk')
        )
        ...
        # with the Sparkii icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.sparkii/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'sparkii setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m sparkii_cli.main setup
    } else {
        python -m sparkii_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'sparkii whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: sparkii gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: sparkii gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sparkii gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   sparkii              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sparkii setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sparkii config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sparkii config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sparkii gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sparkii update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Sparkii repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Sparkii-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Sparkii to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://sparkii-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
