...
# Python-driven tool session (e.g. Sparkii terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/sparkii-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/sparkii-agent.git"
SPARKII_HOME="${SPARKII_HOME:-$HOME/.sparkii}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SPARKII_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SPARKII_INSTALL_DIR"
...
#   code at /usr/local/lib/sparkii-agent, command at /usr/local/bin/sparkii,
#   data still at /root/.sparkii (SPARKII_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SPARKII_HOME/.no-bundled-skills so future"
            echo "                   'sparkii update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Sparkii.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.sparkii/sparkii-agent"
            echo "                   default (root, Linux): /usr/local/lib/sparkii-agent"
            echo "  --sparkii-home PATH  Data directory (default: ~/.sparkii, or \$SPARKII_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Sparkii installs the code under"
            echo "  /usr/local/lib/sparkii-agent and links the command into"
            echo "  /usr/local/bin/sparkii (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SPARKII_HOME"
            echo "  (default /root/.sparkii).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SPARKII_HOME/sparkii-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `sparkii update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Sparkii-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Sparkii Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install sparkii command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `sparkii` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SPARKII_HOME/sparkii-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SPARKII_HOME/sparkii-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sparkii-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SPARKII_HOME/sparkii-agent — then preserve it)
#
# Always no-op when the user set --dir or $SPARKII_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SPARKII_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SPARKII_HOME/sparkii-agent/.git" ]; then
            INSTALL_DIR="$SPARKII_HOME/sparkii-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sparkii-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sparkii-agent"
        ...
        # /usr/local/bin/sparkii wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/sparkii"
        log_info "  Data:    $SPARKII_HOME (unchanged)"
...
# Point a Sparkii-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SPARKII_HOME/node/bin — which is NOT on
...
# Sparkii's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Sparkii-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SPARKII_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SPARKII_HOME/node/etc"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://sparkii-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Sparkii owns its own uv at $SPARKII_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sparkii_cli/managed_uv.py) looks in the same
    # place, so install.sh and `sparkii update` stay in sync.
    local _managed_uv="$SPARKII_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SPARKII_HOME/bin ..."
    mkdir -p "$SPARKII_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/sparkii-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sparkii-uv-installer.$$.sh")"
    ...
    # directly into $SPARKII_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SPARKII_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Sparkii-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Sparkii-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Sparkii-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Sparkii-managed Node from a previous run over a too-old system one.
    if [ -x "$SPARKII_HOME/node/bin/node" ] && [ -x "$SPARKII_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SPARKII_HOME/node/bin/node" --version)"; then
        export PATH="$SPARKII_HOME/node/bin:$PATH"
        log_success "Node.js $("$SPARKII_HOME/node/bin/node" --version) found (Sparkii-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Sparkii-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Sparkii requires Node >=26) — installing Sparkii-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.sparkii/node/..."
    ...
    # Place into ~/.sparkii/node/ and symlink binaries into the same bin dir
    # the sparkii command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SPARKII_HOME/node"
    mkdir -p "$SPARKII_HOME"
    mv "$extracted_dir" "$SPARKII_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SPARKII_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SPARKII_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SPARKII_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SPARKII_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.sparkii/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Sparkii install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Sparkii Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `sparkii update` path
                # (#4735).
                ...
                local stash_name
                stash_name="sparkii-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``sparkii update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Sparkii behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"sparkii-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up sparkii command..."

    if [ "$USE_VENV" = true ]; then
        SPARKII_BIN="$INSTALL_DIR/venv/bin/python"
        SPARKII_ENTRYPOINT="$INSTALL_DIR/sparkii"
    else
        SPARKII_BIN="$(which sparkii 2>/dev/null || echo "")"
        if [ -z "$SPARKII_BIN" ]; then
            log_warn "sparkii not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SPARKII_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SPARKII_ENTRYPOINT" ]; }; then
        log_warn "Sparkii launcher prerequisites not found"
    ...
    # Create a user-facing shim for the sparkii command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SPARKII_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SPARKII_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/sparkii"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/sparkii" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SPARKII_BIN" "$SPARKII_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/sparkii" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/sparkii"
    log_success "Installed sparkii launcher → $command_link_display_dir/sparkii"

    # Also expose `sparkii-agent`. The `sparkii-agent` console script declared in
    ...
    rm -f "$command_link_dir/sparkii-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/sparkii-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SPARKII_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/sparkii-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SPARKII_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/sparkii-agent"
    log_success "Installed sparkii-agent launcher → $command_link_display_dir/sparkii-agent"

    # Also expose `sparkii-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `sparkii-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Sparkii as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/sparkii-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/sparkii-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SPARKII_BIN" "$SPARKII_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/sparkii-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SPARKII_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/sparkii-acp"
    log_success "Installed sparkii-acp launcher → $command_link_display_dir/sparkii-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sparkii command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sparkii' \
                >/dev/null 2>&1; then
        ...
        log_info "sparkii not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Sparkii Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Sparkii Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Sparkii Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sparkii works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.sparkii directory structure (config at top level, code in subdir)
    mkdir -p "$SPARKII_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.sparkii/.env (top level, easy to find)
    if [ ! -f "$SPARKII_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SPARKII_HOME/.env"
            log_success "Created ~/.sparkii/.env from template"
        else
            touch "$SPARKII_HOME/.env"
            log_success "Created ~/.sparkii/.env"
        fi
    else
        log_info "~/.sparkii/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SPARKII_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.sparkii/config.yaml (top level, easy to find)
    if [ ! -f "$SPARKII_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SPARKII_HOME/config.yaml"
            log_success "Created ~/.sparkii/config.yaml from template"
        fi
    else
        log_info "~/.sparkii/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in sparkii_cli/default_soul.py — the
    ...
    if [ ! -f "$SPARKII_HOME/SOUL.md" ]; then
        cat > "$SPARKII_HOME/SOUL.md" << 'SOUL_EOF'
You are Sparkii Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.sparkii/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.sparkii/"

    # Seed bundled skills into ~/.sparkii/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `sparkii update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'sparkii update'." \
            > "$SPARKII_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SPARKII_HOME/.no-bundled-skills"
        log_info "  Future 'sparkii update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.sparkii/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.sparkii/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SPARKII_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SPARKII_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.sparkii/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$SPARKII_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Sparkii Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Sparkii will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Sparkii Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `sparkii update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `sparkii tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SPARKII_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'sparkii tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `sparkii tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `sparkii computer-use install` on their own).
    ...
    # Same upstream installer `sparkii computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Sparkii install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Sparkii'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'sparkii tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: sparkii computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sparkii setup' after install."
        return 0
    fi
    ...
    # Run sparkii setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m sparkii_cli.main setup < /dev/tty
    else
        python -m sparkii_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Sparkii to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SPARKII_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'sparkii whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sparkii gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SPARKII_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SPARKII_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: sparkii gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sparkii gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SPARKII_CMD gateway > "$SPARKII_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.sparkii/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: sparkii gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: sparkii gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.sparkii-bootstrap-complete, which tells the Sparkii
    ...
    local marker_path="$INSTALL_DIR/.sparkii-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SPARKII_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SPARKII_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SPARKII_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}sparkii${NC}              Start chatting"
    echo -e "   ${GREEN}sparkii setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sparkii config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sparkii config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sparkii gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sparkii update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sparkii' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sparkii' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sparkii' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SPARKII_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SPARKII_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SPARKII_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `sparkii update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SPARKII_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Sparkii` rename
...
# _purge_electron_build_cache() used by `sparkii desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Sparkii-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `sparkii desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Sparkii" ]; then
            app="$desktop_dir/release/linux-unpacked/Sparkii"
        elif [ -x "$desktop_dir/release/linux-unpacked/sparkii" ]; then
            app="$desktop_dir/release/linux-unpacked/sparkii"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Sparkii.app" \
            "$desktop_dir/release/mac/Sparkii.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `sparkii desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SPARKII_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SPARKII_HOME="$SPARKII_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from sparkii_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `sparkii update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Sparkii-managed Node
            # provisioned during prerequisites/node-deps (at $SPARKII_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SPARKII_HOME.
    # $SPARKII_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'sparkii update' on this host install.
