...
# Configuration
REPO_URL_SSH="git@github.com:YueDJ/SparkiiDesktop.git"
REPO_URL_HTTPS="https://github.com/YueDJ/SparkiiDesktop.git"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/YueDJ/SparkiiDesktop/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$SPARKII_HOME/node/bin/node" ] && node_satisfies_build "$("$SPARKII_HOME/node/bin/node" --version)"; then
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (sparkii --tui may not work)"
        }
...
ensure_browser() {
    ...
    local ab_bin="$SPARKII_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
