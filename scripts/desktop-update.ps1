...
$SparkiiHome = Split-Path -Parent $InstallRoot
...
function Show-ProgressWindow {
    ...
    try {
        ...
        $form.Text = "Sparkii Update"
        $form.Size = New-Object System.Drawing.Size(720, 420)
        $form.StartPosition = "CenterScreen"
        $form.ControlBox = $false
        $form.TopMost = $true
        $label = New-Object System.Windows.Forms.Label
        $label.Text = "Updating Sparkii -- do not close this window. Sparkii restarts automatically when the update finishes."
        $label.Dock = "Top"
        $label.Height = 34
...
function Invoke-StreamedSparkii([string]$Exe, [string[]]$SparkiiArgs, [string]$Tag) {
    ...
    $outFile = Join-Path $env:TEMP ("sparkii-handoff-{0}-{1}.out" -f $Tag, $PID)
    $errFile = Join-Path $env:TEMP ("sparkii-handoff-{0}-{1}.err" -f $Tag, $PID)
    ...
    # drained at the end (sparkii update is stdout-dominant).
...
try {
    ...
    $sparkiiExe = Join-Path $InstallRoot "venv\Scripts\sparkii.exe"
    if (-not (Test-Path -LiteralPath $sparkiiExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $sparkiiExe is missing. The install needs repair (run the Sparkii installer or `sparkii doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = $res.Code
        $finalMsg = "sparkii update failed (exit $($res.Code)). See logs\desktop-update-handoff.log."
    }
    exit $finalCode
} finally {
