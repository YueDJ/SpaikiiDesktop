...
try {
    ...
    $hermesExe = Join-Path $InstallRoot "venv\Scripts\sparkii.exe"
    if (-not (Test-Path -LiteralPath $hermesExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $hermesExe is missing. The install needs repair (run the Sparkii installer or `sparkii doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
