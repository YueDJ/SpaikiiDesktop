"""Run a real Sparkii CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_sparkii_executable(sparkii_repo: Path) -> Path:
    ...
    raise SystemExit(
        "Sparkii executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Sparkii source checkout containing .venv/bin/sparkii",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["sparkii.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['sparkii.client.active']}"
        )
    [model] = by_name["sparkii.model_route.count"]
    expected_model = {
        "name": "sparkii.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['sparkii.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["sparkii.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['sparkii.task_run.started']}"
        )
    [terminal] = by_name["sparkii.task_run.finished"]
    ...
    [tool] = by_name["sparkii.tool_call.count"]
    ...
    lifecycle = by_name["sparkii.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["sparkii.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Sparkii development environment requires jsonschema"
        ) from exc
    ...
    if metrics["sparkii.client.active"] != [
        {
            "name": "sparkii.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['sparkii.client.active']}"
        )
    [model] = metrics["sparkii.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['sparkii.model_route.count']}"
        )
    [terminal] = metrics["sparkii.task_run.finished"]
    ...
    [tool] = metrics["sparkii.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["sparkii.skill.lifecycle.count"]
    ...
    loads = metrics["sparkii.skill.load.count"]
    if {
        (
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="sparkii-relay-shared-metrics-"))
    home = root / "sparkii-home"
    ...
    (root / "sparkii.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "sparkii.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Sparkii exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Sparkii model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Sparkii did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Sparkii did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from sparkii_cli.observability import relay_shared_metrics",
    ...
    print("Sparkii -> NeMo Relay shared-metrics smoke test passed")
