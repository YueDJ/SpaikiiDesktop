...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Sparkii\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (sparkii-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "sparkii@nousresearch.com",
    "sparkii-audit@example.com",
    "nousbot@nousresearch.com",
    "sparkii@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "NousResearch/sparkii-agent",
                "--state", "merged",
