...
batch (tracker: [#77056](https://github.com/NousResearch/sparkii-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Sparkii home, same model, same tasks, same reps.
...
1. Create a dedicated Sparkii home with credentials for the models under test:
   ...
   SPARKII_HOME=$ABEVAL_HOME sparkii plugins enable observability/nemo_relay
   ```
