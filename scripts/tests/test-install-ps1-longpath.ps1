...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[sparkii] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[sparkii`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[sparkii`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SparkiiHome = $(if ($paths) { $paths.hermes_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\sparkii\sparkii-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-sparkii'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-sparkii') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
