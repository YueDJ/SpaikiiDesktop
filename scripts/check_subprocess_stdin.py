...
When Sparkii runs in TUI mode, the gateway child process communicates with
...
# ``get_sparkii_home() / "plugins"`` (user) and ``./.sparkii/plugins/`` (project,
# gated behind ``SPARKII_ENABLE_PROJECT_PLUGINS``) — see
...
# this script runs as a standalone subprocess — ``sparkii_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import sparkii_constants (this script
    ...
    # plugins in get_sparkii_home()/plugins/ and project plugins in
    # ./.sparkii/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_sparkii_home() / "plugins"]
    if os.environ.get("SPARKII_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".sparkii" / "plugins")
