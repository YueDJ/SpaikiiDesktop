...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sparkii_home / "auth.json")
    ...
    (sparkii_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return sparkii_home
...
def reset_module_state():
    """Drop cached modules so the new SPARKII_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "sparkii_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SPARKII_HOME"] = str(home)
...
def main():
    ...
    # Restore original SPARKII_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SPARKII_HOME"] = ORIGINAL_HOME
    else:
