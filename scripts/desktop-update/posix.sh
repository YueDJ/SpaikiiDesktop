...
#
# The whole job: wait for the Desktop to exit, run `sparkii update`, tell the
...
# the SPARKII_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (SPARKII_HOME/sparkii-agent)
...
SPARKII_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
SPARKII_HOME="${SPARKII_HOME:-${TMPDIR:-/tmp}}"
MARKER="$SPARKII_HOME/.sparkii-update-in-progress"
LOG_DIR="$SPARKII_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$SPARKII_HOME/.sparkii-update-result.json"
STATUS="${TMPDIR:-/tmp}/sparkii-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Sparkii update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Sparkii update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Sparkii update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Sparkii update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `sparkii update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/sparkii-update-ui-$$" \
...
linux_gate() {
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Sparkii to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Sparkii.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Sparkii.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Sparkii (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Sparkii also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Sparkii to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${SPARKII_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${SPARKII_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `sparkii update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Sparkii while the
...
# so the shim server and the later `sparkii update` subprocess both inherit
...
# update_lock.py (the `sparkii update` child adopts it via process ancestry).
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Sparkii window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Sparkii fully and try again."
...
SPARKII_BIN="$INSTALL_ROOT/venv/bin/sparkii"
[ -x "$SPARKII_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $SPARKII_BIN is missing. The install needs repair (run the Sparkii installer or sparkii doctor)."; log "$FINAL_MSG"; exit 3; }

# Run FROM the install root: `sparkii update` resolves the tree it mutates
...
log "running: sparkii update --yes --gateway --branch $BRANCH"
publish_stage "Updating code and dependencies"
OUT="$("$SPARKII_BIN" update --yes --gateway --branch "$BRANCH" 2>&1)"; CODE=$?
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "sparkii update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Sparkii windows") is not retryable.
...
# Truthful completion: `sparkii update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside sparkii update; retrying build"
  publish_stage "Rebuilding Desktop"
  "$SPARKII_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run sparkii desktop --force-build from a terminal to retry."
    exit 6
  }
fi

if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run sparkii debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
