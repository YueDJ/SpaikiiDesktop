...
# (%SPARKII_HOME%\sparkii-setup.exe). That binary has no self-update path --
# copy_self_to_sparkii_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `sparkii update` refreshes
...
#     -InstallRoot <path>   repo checkout (SPARKII_HOME\sparkii-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Sparkii.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .sparkii-update-in-progress in place (tests)
#
...
# .sparkii-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim SPARKII_HOME\.sparkii-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately). sparkii_cli/update_lock.py's ancestry rule lets our
# `sparkii update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Sparkii.exe pid.
try {
    Add-Type -Namespace SparkiiHandoff -Name Win32 -MemberDefinition @'
...
$SparkiiHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $SparkiiHome ".sparkii-update-in-progress"
$LogDir = Join-Path $SparkiiHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $SparkiiHome ".sparkii-update-result.json"
$script:Ui = $null
$script:UiStage = "Sparkii will open once done."   # until the first gate; matches ui.html
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-SparkiiStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("sparkii-update-ui-{0}" -f $PID)
                $edgeArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [SparkiiHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `sparkii debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"sparkii debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [SparkiiHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Sparkii themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Sparkii.exe before writing the replacement,
    ...
    # DO NOT spawn Sparkii.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [SparkiiHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [SparkiiHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [SparkiiHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
...
function Invoke-SparkiiStep([string]$Exe, [string[]]$SparkiiArgs, [string]$Tag) {
    ...
    # afterwards, where `sparkii debug share` picks it up.
    ...
    $psi.Arguments = ($SparkiiArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # sparkii update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; SPARKII_SELFTEST_FAIL=1 exercises the
# error state, SPARKII_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:SPARKII_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:SPARKII_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:SPARKII_SELFTEST_FAIL) {
...
try {
    ...
    Publish-UiProgress "Waiting for Sparkii to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Sparkii window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Sparkii fully and try again."
    ...
    Publish-UiProgress "Preparing Sparkii files"
    $shim = Join-Path $InstallRoot "venv\Scripts\sparkii.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Sparkii install open (venv\Scripts\sparkii.exe locked after 20s). Nothing was changed. Close other Sparkii windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the sparkii.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\sparkii.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_sparkii_exe must first rename the running sparkii.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that sparkii.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Sparkii window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Sparkii.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m sparkii_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Sparkii installer or `sparkii doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "sparkii_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    Write-HandoffLog ("running: python " + ($updateArgs -join " "))
    Publish-UiProgress "Updating code and dependencies"
    $res = Invoke-SparkiiStep $pythonExe $updateArgs "update"
    ...
    # `sparkii update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        ...
        $rebuild = Invoke-SparkiiStep $pythonExe @("-m", "sparkii_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `sparkii desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `sparkii debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Sparkii is back",
    ...
    if ($finalCode -ne 0) {
        ...
        Publish-UiProgress "Opening Sparkii"
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Sparkii to finish (it could not restart itself)."
