...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SPARKII_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `sparkii`
        # invocation -- including plain `sparkii chat`, which never touches any
        ...
        # `sparkii setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``sparkii setup``/``gateway status``, channel
...
# Register the singleton with the plugin loader so plugin-registered platform
# adapters (``PluginContext.register_platform`` / deferred platform loaders)
# reach the gateway without the plugin system importing ``gateway``.
try:
    from sparkii_cli.plugins import set_platform_registry_provider

    set_platform_registry_provider(platform_registry)
except Exception:  # pragma: no cover - defensive; plugins is always present
    pass
