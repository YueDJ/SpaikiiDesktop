...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<sparkii_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SPARKII_HOME."""
    from sparkii_constants import get_sparkii_home

    flush_dir = get_sparkii_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<sparkii_home>/pending_messages/``), so a runtime cap rotation no
