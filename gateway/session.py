...
# ``SPARKII_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``SPARKII_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("SPARKII_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in sparkii_state, request-dump
...
def build_session_context_prompt(
    ...
    *,
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_sparkii_home()}/cron/output/)"
    )
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.sparkii/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.sparkii/state.db "
                "and are shown by `sparkii sessions list` and `/sessions`. "
