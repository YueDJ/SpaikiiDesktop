...
2. An event-loop heartbeat file at ``<SPARKII_HOME>/state/gateway.heartbeat`` so
...
def _process_sparkii_home() -> Path:
    """SPARKII_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<SPARKII_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_sparkii_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from sparkii_logging import drain_log_queue
