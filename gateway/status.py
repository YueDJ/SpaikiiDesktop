...
The PID file lives at ``{SPARKII_HOME}/gateway.pid``.  SPARKII_HOME defaults to
``~/.sparkii`` but can be overridden via the environment variable.  This means
separate SPARKII_HOME directories naturally get separate PID files — a property
...
from sparkii_constants import get_sparkii_home, _get_platform_default_sparkii_home
from typing import Any, Callable, NamedTuple, Optional
from utils import atomic_json_write

# Block 4: process helpers moved to the core package; keep re-exported here so
# surface-side callers and patch targets keep working.  The canonical
# implementations live in core/process_utils.py.
from core.process_utils import (  # noqa: F401
    _get_process_start_time,
    _pid_exists,
    get_process_start_time,
    set_profile_gateway_liveness_provider,
    set_gateway_running_pid_provider,
    terminate_pid,
)
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_sparkii_home() / "gateway-starts.log"
...
def _get_process_sparkii_home() -> Path:
    """Return the process-level SPARKII_HOME, skipping context-local overrides.
    ...
    ``get_sparkii_home()`` honors ``_SPARKII_HOME_OVERRIDE`` contextvar used for
...
def _canonical_sparkii_home(path: Path | str) -> Path:
    """Return a stable absolute SPARKII_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_sparkii_home(left: Path | str, right: Path | str) -> bool:
    """Compare SPARKII_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_sparkii_home(left))) == os.path.normcase(
        str(_canonical_sparkii_home(right))
    )


# Mirrors sparkii_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (sparkii_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a SPARKII_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.sparkii/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_sparkii_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_sparkii_home(canonical, get_default_sparkii_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_sparkii_home(canonical, _get_platform_default_sparkii_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    SPARKII_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``sparkii_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SPARKII_HOME."""
    home = _get_process_sparkii_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SPARKII_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "sparkii" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Sparkii' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Sparkii gateway lifecycle subcommand from a command line.
    ...
    test also matched ``sparkii_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\sparkii-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Sparkii's
    ...
    for token in tokens:
        ...
        if basename in ("sparkii-gateway", "sparkii-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "sparkii_cli.main" in joined
        or "sparkii_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("sparkii", "sparkii.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `sparkii gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Sparkii-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Sparkii gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SPARKII_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.sparkii`` or ``$SPARKII_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``sparkii_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SPARKII_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a SPARKII_HOME= value on the argv
    ...
    # a non-matching explicit SPARKII_HOME= on the argv. SPARKII_HOME is usually
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `sparkii status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SPARKII_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``sparkii_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SPARKII_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SPARKII_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from sparkii_home.
    profile = _profile_label_for_home(_get_process_sparkii_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``sparkii.service`` + ``sparkii-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SPARKII_HOME --replace while
    ...
    our_home = _get_process_sparkii_home()
    target_home = record.get("target_sparkii_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_sparkii_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_sparkii_home")
        if replacer_home is not None and not _same_sparkii_home(
    ...
    # ``sparkii gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SPARKII_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_sparkii_home(
            target_home or _get_process_sparkii_home()
        )
        ...
        record = {
            ...
            "target_sparkii_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_sparkii_home": str(
                _canonical_sparkii_home(_get_process_sparkii_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SPARKII_HOME's gateway PID record, and the live
    ...
    if not Path(raw_home).expanduser().is_absolute():
        return None
    target_home = _canonical_sparkii_home(raw_home)
...
# Block 4: register the authoritative running-PID probe with the core package
# so kernel modules (tools/process_registry.py) can ask "is this PID the live
# gateway" without importing the gateway package.  Cleanup is disabled here —
# process_registry's supervisor check must not mutate gateway runtime state.
set_gateway_running_pid_provider(lambda: get_running_pid(cleanup_stale=False))


def _profile_gateway_liveness(profile_dir) -> bool:
    """Two-branch profile gateway liveness used by core profiles checks."""
    try:
        if (
...
set_profile_gateway_liveness_provider(_profile_gateway_liveness)
