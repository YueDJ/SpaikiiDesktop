...
# Audio file extensions Sparkii recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Sparkii but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Sparkii' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Sparkii-created Telegram private-chat
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``SPARKII_SESSION_PLATFORM`` contextvar — that contextvar is
...
# Block 4: image/audio/document cache helpers moved to the core package; keep
# them re-exported here so gateway adapters and existing patch targets keep
# working.  The canonical implementations live in core/media_cache.py.
from core.media_cache import (  # noqa: F401
    AUDIO_CACHE_DIR,
...
if TYPE_CHECKING:
    from sparkii_cli.display import ToolPreview
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SparkiiAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
MEDIA_DELIVERY_STRICT_ENV = "SPARKII_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SPARKII_HOME / "image_cache",
    _SPARKII_HOME / "audio_cache",
    _SPARKII_HOME / "video_cache",
    _SPARKII_HOME / "document_cache",
    _SPARKII_HOME / "browser_screenshots",
    ...
    _SPARKII_HOME / "cache" / "images",
    _SPARKII_HOME / "cache" / "audio",
    _SPARKII_HOME / "cache" / "videos",
    _SPARKII_HOME / "cache" / "documents",
    _SPARKII_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Sparkii root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* SPARKII_HOME's cache, so a gateway running at
    the root (e.g. ``SPARKII_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when SPARKII_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _SPARKII_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("SPARKII_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _SPARKII_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Sparkii profile and shared Sparkii root both contain control
    ...
    # SPARKII_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.sparkii
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Sparkii
    secrets (``~/.sparkii/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Sparkii cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.sparkii/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Sparkii cache dirs (``/root/.sparkii/...``), the default
    ...
    # In-process gateways (Desktop backend, `sparkii serve`) may not have
    ...
    # are longer prefixes, so /root/.sparkii/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.sparkii/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.sparkii"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``SPARKII_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Sparkii-managed cache, under an operator-allowlisted root
    (``SPARKII_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Sparkii root (~/.sparkii/.env,
    ...
    # ``MEDIA:~/.sparkii/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?sparkii\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+sparkii[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SPARKII_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Sparkii commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-SPARKII_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("SPARKII_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SPARKII_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SPARKII_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Sparkii sends
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        if preview:
            from sparkii_cli.display import prepare_tool_preview
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-SPARKII_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Sparkii home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Sparkii home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.sparkii/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from sparkii_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SPARKII_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SPARKII_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SPARKII_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SPARKII_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SPARKII_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SPARKII_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                    ...
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # SPARKII_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile SPARKII_HOME isolation.
        """
