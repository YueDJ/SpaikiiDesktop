...
def _account_dir(sparkii_home: str) -> Path:
    path = Path(sparkii_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(sparkii_home: str, account_id: str) -> Path:
    return _account_dir(sparkii_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(sparkii_home, account_id)
...
def load_weixin_account(sparkii_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, sparkii_home: str):
        self._root = _account_dir(sparkii_home)
...
def _sync_buf_path(sparkii_home: str, account_id: str) -> Path:
    return _account_dir(sparkii_home) / f"{account_id}.sync.json"


def _load_sync_buf(sparkii_home: str, account_id: str) -> str:
    path = _sync_buf_path(sparkii_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(sparkii_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(sparkii_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._token_store = ContextTokenStore(sparkii_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(sparkii_home, self._account_id)
            if persisted:
