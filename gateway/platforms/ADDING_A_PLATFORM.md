...
There are two ways to add a platform to the Sparkii gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sparkii/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Sparkii code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sparkii gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `sparkii_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Sparkii core.
...
## 12. Status Display (`sparkii_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sparkii_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sparkii_cli/ toolsets.py \
  --include="*.py" -l | sort -u
