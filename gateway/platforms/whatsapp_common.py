...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Sparkii Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SPARKII_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/sparkii), this function
    mirrors the bridge source to a writable SPARKII_HOME location and returns that
    ...
    # Try SPARKII_HOME location first
    sparkii_home = get_sparkii_home()
    sparkii_home_bridge = sparkii_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SPARKII_HOME if needed
    if sparkii_home_bridge.exists():
        return sparkii_home_bridge

    # Mirror the bridge source to SPARKII_HOME
    try:
        sparkii_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            sparkii_home_bridge,
            dirs_exist_ok=False,
