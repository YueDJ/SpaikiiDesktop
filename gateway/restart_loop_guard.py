...
Defenses 1 and 2 (the ``_SPARKII_GATEWAY`` guard on ``sparkii gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.sparkii.gateway")``,
...
State lives in ``<SPARKII_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_sparkii_home() / "gateway" / "restart_loop.json"
