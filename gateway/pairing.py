...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{SPARKII_HOME}/pairing`` while newer code/docs may
    write ``{SPARKII_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own SPARKII_HOME using the same legacy/consolidated layout rules
    as ``sparkii -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current SPARKII_HOME.
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``sparkii -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `sparkii` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> sparkii pairing approve ...` as root, "
                    "re-run with `docker exec -u sparkii <container> ...` and "
                    "chown the existing file to the sparkii user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``sparkii
