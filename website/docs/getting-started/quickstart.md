...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Sparkii Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Sparkii setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Sparkii Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Sparkii Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Sparkii for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Sparkii working on my machine | `sparkii setup` | Run a real chat and verify it responds |
| I already know my provider | `sparkii model` | Save the config, then start chatting |
| I want a bot or always-on setup | `sparkii gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `sparkii model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `sparkii model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Sparkii cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Sparkii Agent
### With the Sparkii Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Sparkii Desktop installer](https://sparkii-agent.nousresearch.com/) from our website and run it.

### Without Sparkii Desktop:
For a command-line only install without Sparkii Desktop, run:
...
The single most important setup step. Use `sparkii model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `sparkii setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `sparkii update`. Re-enable anything later with `sparkii tools`, seed skills with `sparkii skills opt-in --sync`, or tune settings with `sparkii setup agent`.
:::
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `sparkii_cli/models.py` may change between releases) | `sparkii model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `sparkii model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `sparkii model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `sparkii model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `sparkii model` (requires `copilot` CLI + `copilot login`) |
...
Sparkii Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `sparkii model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Sparkii separates secrets from normal config:

- **Secrets and tokens** → `~/.sparkii/.env`
- **Non-secret settings** → `~/.sparkii/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
sparkii config set model anthropic/claude-opus-4.6
sparkii config set terminal.backend docker
sparkii config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
sparkii            # classic CLI
sparkii --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Sparkii ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `sparkii` vs `sparkii --tui`.
:::
...
```bash
sparkii --continue    # Resume the most recent session
sparkii -c            # Short form
```
...
```bash
sparkii gateway setup    # Interactive platform configuration
```
...
- `sparkii tools` — tune tool access per platform
- `sparkii skills` — browse and install reusable workflows
...
```bash
sparkii config set terminal.backend docker    # Docker isolation
sparkii config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `sparkii egress setup && sparkii egress start`; `sparkii setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Sparkii install directory (the curl installer placed it at
# ~/.sparkii/sparkii-agent on Linux/macOS or %LOCALAPPDATA%\sparkii\sparkii-agent on Windows):
cd ~/.sparkii/sparkii-agent
...
Skills are on-demand instruction documents that teach Sparkii how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Sparkii ships with a catalog of bundled skills already installed in `~/.sparkii/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
sparkii skills browse                      # list everything available
sparkii skills search kubernetes           # find skills by keyword
sparkii skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `sparkii skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Sparkii ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.sparkii/sparkii-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Sparkii opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `sparkii model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `sparkii gateway setup` and check `sparkii gateway status` |
| `sparkii --continue` can't find old session | Switched profiles or session never saved | Check `sparkii sessions list` and confirm you're in the right profile |
...
| `sparkii doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `sparkii model`
3. `sparkii setup`
4. `sparkii sessions list`
5. `sparkii --continue`
6. `sparkii gateway status`
...
| Command | Description |
|---------|-------------|
| `sparkii` | Start chatting |
| `sparkii model` | Choose your LLM provider and model |
| `sparkii tools` | Configure which tools are enabled per platform |
| `sparkii setup` | Full setup wizard (configures everything at once) |
| `sparkii doctor` | Diagnose issues |
| `sparkii update` | Update to latest version |
| `sparkii gateway` | Start the messaging gateway |
| `sparkii --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-sparkii-to-another-machine)** — `sparkii backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
