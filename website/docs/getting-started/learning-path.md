...
sidebar_position: 3
title: 'Learning Path'
description: 'Choose your learning path through the Sparkii Agent documentation based on your experience level and goals.'
---

# Learning Path

Sparkii Agent can do a lot — CLI assistant, Telegram/Discord bot, task automation, RL training, and more. This page helps you figure out where to start and what to read based on your experience level and what you're trying to accomplish.

:::tip Start Here
If you haven't installed Sparkii Agent yet, begin with the [Installation guide](/getting-started/installation) and then run through the [Quickstart](/getting-started/quickstart). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
First-time users almost always want `sparkii setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Sparkii Agent can do.
...
Use Sparkii Agent as an interactive terminal assistant for writing, reviewing, and running code.
...
Extend Sparkii Agent with your own tools and reusable skill packages.

1. [Plugins](/user-guide/features/plugins)
2. [Build a Sparkii Plugin](/developer-guide/plugins)
...
page is for built-in Sparkii core development, not the usual user/custom-tool path.
:::

### "I want to train models"

Use reinforcement learning to fine-tune model behavior with Sparkii Agent's RL training pipeline (powered by [Atropos](https://github.com/NousResearch/atropos)).
...
:::tip
RL training works best when you already understand the basics of how Sparkii Agent handles conversations and tool calls. Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Sparkii Agent into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Sparkii tools programmatically | [Code Execution](/user-guide/features/code-execution) |
