...
A `venv.stale.*` or `venv.failed.*` directory can remain when another process still owns a file handle. Close Sparkii Desktop, gateways, and Python processes using the install, then retry the install/update; parked directories are cleaned up best-effort after a successful recreation.
...
Take your setup with you before removing anything: `sparkii backup` captures the entire `~/.sparkii` directory including credentials, while `sparkii profile export` packs a single profile with credentials excluded by design (so an export alone is not a full backup). See [`sparkii backup` vs `sparkii profile export`](/reference/faq#sparkii-backup-vs-sparkii-profile-export).
:::
