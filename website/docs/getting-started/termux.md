...
description: "Run Sparkii Agent directly on an Android phone with Termux"
---

# Sparkii on Android with Termux
...
Sparkii Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Sparkii from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Sparkii installs are marked with install method `apt`. Sparkii therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade sparkii-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Sparkii runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Sparkii now ships a Termux-aware installer path:
...
- links `sparkii` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `sparkii` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/sparkii" "$PREFIX/bin/sparkii"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `sparkii` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.sparkii/.env`.
...
### `sparkii doctor` says ripgrep or Node is missing
