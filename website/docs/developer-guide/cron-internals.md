...
description: "How Sparkii stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `sparkii_cli/cron.py` | CLI `sparkii cron` subcommands |
...
Jobs are stored in `~/.sparkii/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `sparkii cron` commands are run or during active CLI sessions.
...
```python
# ~/.sparkii/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.sparkii/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `sparkii cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `sparkii cron` CLI provides direct job management:

```bash
sparkii cron list                    # Show all jobs
sparkii cron create                  # Interactive job creation (alias: add)
sparkii cron edit <job_id>           # Edit job configuration
sparkii cron pause <job_id>          # Pause a running job
sparkii cron resume <job_id>         # Resume a paused job
sparkii cron run <job_id>            # Trigger immediate execution
sparkii cron remove <job_id>         # Delete a job
```
