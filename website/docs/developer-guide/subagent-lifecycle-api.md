...
Plugins can launch and supervise fresh Sparkii child sessions without importing
...
active agent turn fails closed with `No active Sparkii parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Sparkii can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Sparkii's existing unsafe-tool block remains enforced.
