...
Sparkii Agent uses a SQLite database (`~/.sparkii/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `sparkii_state.py`


## Architecture Overview

```
~/.sparkii/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `sparkii_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `sparkii_state.py` for the exact SQL.
...
Multiple sparkii processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.sparkii/state.db
...
Default path: `~/.sparkii/state.db`

This is derived from `sparkii_constants.get_sparkii_home()` which resolves to
`~/.sparkii/` by default, or the value of `SPARKII_HOME` environment variable.
