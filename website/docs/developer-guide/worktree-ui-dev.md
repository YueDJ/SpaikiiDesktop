...
| Variable | Meaning |
|----------|---------|
| `SPARKII_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SPARKII_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SPARKII_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Sparkii itself — they're private to these helpers. The variables Sparkii *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `sparkii --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:
...
hgui() {
  ...
  ( cd "$desktop"
    ...
    SPARKII_DESKTOP_SPARKII_ROOT="$root" \
    SPARKII_DESKTOP_PYTHON="$SPARKII_MAIN_CHECKOUT/.venv/bin/python" \
    SPARKII_DESKTOP_IGNORE_EXISTING=1 \
    SPARKII_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SPARKII_DESKTOP_SPARKII_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `sparkii`. |
| `SPARKII_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SPARKII_DESKTOP_IGNORE_EXISTING` | Ignores any `sparkii` on `PATH` so it can't shadow the worktree. |
| `SPARKII_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
_sparkii_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'sparkii_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
