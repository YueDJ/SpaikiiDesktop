...
title: "Desktop Plugin SDK (@sparkii/plugin-sdk)"
description: "Extend the native Sparkii Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Sparkii Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SparkiiPlugin`.
It imports one module — `@sparkii/plugin-sdk` — and gets everything: the app's
...
`$SPARKII_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Sparkii. This page is the **native
desktop app** (`sparkii desktop`) SDK — the `@sparkii/plugin-sdk` module and
`$SPARKII_HOME/desktop-plugins/`. The **web dashboard** (`sparkii dashboard`) has
its own, unrelated plugin system on `window.__SPARKII_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Sparkii Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SPARKII_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SPARKII_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SparkiiPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SPARKII_HOME/desktop-plugins/hello/plugin.js` (that's `~/.sparkii/...`
by default, or `~/.sparkii/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.sparkii/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@sparkii/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Sparkii' }),
      jsx('div', {
...
The only importable specifiers are `@sparkii/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SparkiiPlugin`:

```ts
interface SparkiiPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `sparkii.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@sparkii/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@sparkii/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@sparkii/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@sparkii/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@sparkii/plugin-sdk'
...
`window.sparkii.send('get-price eth')` (or a declarative
`<button data-sparkii-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@sparkii/plugin-sdk'
...
for requests; `targetProfile` is the backend Sparkii profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Sparkii
...
ctx.os.notify({
  ...
  // Body click → focus Sparkii + navigate. Same vocabulary as OS deep links:
  activate: 'sparkii://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'sparkii://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `sparkii://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `sparkii://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@sparkii/plugin-sdk'
...
```javascript
import { queryClient } from '@sparkii/plugin-sdk'
...
a package into `~/.sparkii/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.sparkii/plugins` is not reachable as a filesystem —
...
the `sparkii://` scheme — a plain anchor on your website or README:
...
`sparkii-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Sparkii plugin and declare it in a
`manifest.json`:
...
sparkii-agent codebase directly (`sparkii_state`, `sparkii_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.sparkii/`)
...
(`sparkii.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SparkiiPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@sparkii/plugin-sdk` alias.
2. It's still lint-fenced to `@sparkii/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@sparkii/plugin-sdk`, `react`,
...
| Plugin contract | `SparkiiPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SparkiiOpenTarget`, `Contribution` |
...
### Agents: the `sparkii-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`sparkii-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SPARKII_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `sparkii logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@sparkii/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.sparkii/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.sparkii/logs/errors.log` for
