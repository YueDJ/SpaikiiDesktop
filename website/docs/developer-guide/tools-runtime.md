...
Sparkii tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `sparkii_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
