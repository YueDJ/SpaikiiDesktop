...
title: "Build a Sparkii Plugin"
description: "Step-by-step guide to building a complete Sparkii plugin with tools, hooks, data files, and skills"
---

# Build a Sparkii Plugin

This guide walks through building a complete Sparkii plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Sparkii has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SPARKII_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.sparkii/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `sparkii skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Sparkii can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Sparkii already owns. It does not replace native `plugin.yaml` plus
...
```bash
sparkii plugins install owner/repository --no-enable
sparkii plugins list
sparkii plugins enable <plugin-name>
```
...
Sparkii validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Sparkii.
...
records it as Published. Sparkii keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Sparkii does not expose a
...
continue to work after a normal Sparkii upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Sparkii inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Sparkii releases can therefore load a plugin whose manifest contains
...
Sparkii enforces this contract with frozen external-plugin fixtures discovered
from an isolated `SPARKII_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.sparkii/plugins/calculator
cd ~/.sparkii/plugins/calculator
```

### Validate with Plugin Doctor

`sparkii plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Sparkii itself. It reports invalid hook names, callbacks that do
...
```bash
sparkii plugins doctor . --ci
```

Doctor uses a temporary `SPARKII_HOME`, restores plugin registration state after
...
This tells Sparkii: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `sparkii_cli/plugin_capabilities.py` for the
...
`sparkii_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`sparkii_agent.plugins` entry point:

```toml
[project.entry-points."sparkii_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."sparkii_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Sparkii reads these from installed metadata without importing your code, so
`sparkii plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Sparkii understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Sparkii validates them, and `sparkii plugins install` / `sparkii plugins doctor` surface missing ones with a `pip install` hint, but Sparkii **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Sparkii' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/NousResearch/sparkii-agent/issues/64165) and
[#15220](https://github.com/NousResearch/sparkii-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Sparkii may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `sparkii my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Sparkii continues fine
...
Use plugin-relative config keys for user-visible behavior. Sparkii resolves them
...
`<SPARKII_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `SPARKII_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
SPARKII_PLUGINS_DEBUG=1 sparkii plugins list
```
...
The same logs are always written to `~/.sparkii/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
sparkii logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `sparkii plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.sparkii/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `sparkii plugins update` / `remove` git-pull or delete it — your
...
# <sparkii home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.sparkii/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.sparkii/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `sparkii plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Sparkii into installing arbitrary packages — only specs Sparkii itself ships are eligible |
...
auditable in `~/.sparkii/logs/agent.log`. Plugins load after built-in
...
does not ship with Sparkii core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `sparkii plugins list`
...
loader, the plugin is disabled and Sparkii continues. The gate exists
...
You normally never edit this key by hand. `sparkii plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Sparkii injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: sparkii-agent"}
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $SPARKII_HOME/hook_outputs
```
...
- **The system prompt is Sparkii's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `sparkii_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Sparkii sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`sparkii.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Sparkii still loads on an older one.

### Register CLI commands

Plugins can add their own `sparkii <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for sparkii my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: sparkii my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for sparkii my-plugin."""
...
After registration, users can run `sparkii my-plugin status`, `sparkii my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `sparkii name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `sparkii chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `SPARKII_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `sparkii <subcommand>` (e.g. `sparkii kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "sparkii kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Sparkii from a hook.
...
Sparkii has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.sparkii/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `sparkii gateway status` without SDK instantiation.
...
Sparkii also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Sparkii without any Python plugin. Declare them in `~/.sparkii/config.yaml`:
...
Sparkii connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.sparkii/hooks/<name>/`:

```yaml
# ~/.sparkii/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.sparkii/hooks/long-task-alert/handler.py
...
sparkii skills search my-workflow --source myorg/skills-repo
sparkii skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install sparkii-plugin-calculator
# Plugin auto-discovered on next sparkii startup
```
...
```python
# Wrong — will break if Sparkii passes extra context
def handler(args):
    ...
