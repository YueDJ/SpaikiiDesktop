...
sidebar_position: 1
title: "Architecture"
description: "Sparkii Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Sparkii Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ sparkii_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SparkiiCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── sparkii_state.py           # SQLite session/state database with FTS5
├── sparkii_constants.py       # SPARKII_HOME, profile-aware paths
...
│
├── sparkii_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `sparkii` subcommands (large file)
...
│   ├── skills_config.py      # sparkii skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # sparkii tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # sparkii gateway start/stop
│
...
```text
User input → SparkiiCLI.process_input()
...
Three discovery sources: `~/.sparkii/plugins/` (user), `.sparkii/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `sparkii plugins` or `config.yaml`.
...
Exposes Sparkii as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sparkii -p <name>`) gets its own SPARKII_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
