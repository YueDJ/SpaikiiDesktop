...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Sparkii already has applies. The plugin never sees a
...
sparkii-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Sparkii provider id (e.g. "openrouter")
...
using Sparkii built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Sparkii provider
        # (must be one Sparkii already knows about — same names as
        # `sparkii model` and config.yaml model.provider).
        ...
        # plugin sends — Sparkii does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.sparkii/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Sparkii' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Sparkii subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/sparkii-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/sparkii-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/sparkii-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/sparkii-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
