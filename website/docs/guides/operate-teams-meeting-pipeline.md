...
```bash
sparkii teams-pipeline validate
```
...
```bash
sparkii teams-pipeline token-health
sparkii teams-pipeline token-health --force-refresh
```
...
```bash
sparkii teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
sparkii teams-pipeline maintain-subscriptions
sparkii teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Sparkii cron (recommended if you already run the Sparkii gateway)

Sparkii ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.sparkii/scripts/`. First create the script:

```bash
mkdir -p ~/.sparkii/scripts
cat > ~/.sparkii/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec sparkii teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.sparkii/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
sparkii cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/sparkii-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Sparkii Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=sparkii
EnvironmentFile=/etc/sparkii/env
ExecStart=/usr/local/bin/sparkii teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/sparkii-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Sparkii Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now sparkii-teams-pipeline-maintain.timer
systemctl list-timers sparkii-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/sparkii teams-pipeline maintain-subscriptions >> /var/log/sparkii/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.sparkii/.env` at the top of a wrapper script that crontab calls.
...
```bash
sparkii teams-pipeline subscriptions   # should show expirationDateTime advanced
sparkii teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
sparkii teams-pipeline list
sparkii teams-pipeline list --status failed
sparkii teams-pipeline show <job-id>
```

### Replay a stored job

```bash
sparkii teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
sparkii teams-pipeline fetch --meeting-id <meeting-id>
sparkii teams-pipeline fetch --join-web-url "<join-url>"
sparkii teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `sparkii teams-pipeline maintain-subscriptions --dry-run`
- inspect `sparkii teams-pipeline list --status failed`
...
- run `sparkii teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `sparkii teams-pipeline run`
...
- [ ] `sparkii teams-pipeline validate` returns an OK snapshot
- [ ] `sparkii teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Sparkii cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
