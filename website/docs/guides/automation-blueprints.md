...
sparkii cron create "0 2 * * *" \
  "You are a project manager triaging the YueDJ/SpaikiiDesktop GitHub repo.

1. Run: gh issue list --repo YueDJ/SpaikiiDesktop --state open --json number,title,labels,author,createdAt --limit 30
...
sparkii cron create "0 9 * * 1" \
  "Scan the YueDJ/SpaikiiDesktop repo for documentation drift.

1. Run: gh pr list --repo YueDJ/SpaikiiDesktop --state merged --json number,title,files,mergedAt --limit 30
