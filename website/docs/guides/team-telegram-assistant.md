...
This tutorial walks you through setting up a Telegram bot powered by Sparkii Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Sparkii Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.sparkii/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Sparkii itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Sparkii Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_sparkii_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Sparkii Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.sparkii/.env`:
...
```
[Gateway] Starting Sparkii Gateway...
...
# View live logs
journalctl --user -u sparkii-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `sparkii gateway install` to update the plist.
:::
...
```bash
sparkii gateway stop && sparkii gateway start
```
...
sparkii pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.sparkii/.env`:
...
Control how much detail the bot shows when using tools. In `~/.sparkii/config.yaml`:
...
Customize how the bot communicates by editing `~/.sparkii/SOUL.md`:

For a full guide, see [Use SOUL.md with Sparkii](/guides/use-soul-with-sparkii).
...
```bash
# From the CLI
sparkii cron list          # View all scheduled jobs
sparkii cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u sparkii-gateway` (Linux) or `~/.sparkii/logs/gateway.log` (macOS) |
| Cron job output | `~/.sparkii/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.sparkii/cron/jobs.json` |
| Pairing data | `~/.sparkii/pairing/` |
| Session history | `~/.sparkii/sessions/` |
