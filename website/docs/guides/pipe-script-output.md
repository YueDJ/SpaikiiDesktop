...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `sparkii send`."
---

# Pipe Script Output to Messaging Platforms

`sparkii send` is a small, scriptable CLI that pushes a message to any
messaging platform Sparkii is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | sparkii send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `sparkii
...
sparkii send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
sparkii send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
sparkii send --list

# Filter by platform
sparkii send --list telegram
```
...
Any platform Sparkii ships adapters for works as a target:
...
`sparkii send` resolves the message body in this order:

1. **Positional argument** — `sparkii send --to telegram "hi"`
2. **`--file PATH`** — `sparkii send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | sparkii send --to telegram`

When stdin is a TTY (no pipe), Sparkii does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  sparkii send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `sparkii send` reuses your Sparkii config, the same script works on
any host where Sparkii is installed — no need to export bot tokens into
...
`sparkii send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  sparkii send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | sparkii send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.sparkii/bin/sparkii send \
...
```bash
./train.py --epochs 200 && \
  sparkii send --to telegram "training done" || \
  sparkii send --to telegram "training failed (exit $?)"
```
...
sparkii send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(sparkii send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `sparkii send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `sparkii send` calls
the platform's REST endpoint directly using credentials from
`~/.sparkii/.env` and `~/.sparkii/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `sparkii gateway start` and retry.
...
# Machine-readable
sparkii send --list --json
```

The listing is built from `~/.sparkii/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`sparkii gateway
...
| Approach | Multi-platform | Reuses Sparkii creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `sparkii send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`sparkii send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `sparkii send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `sparkii send` shares with cron delivery.
