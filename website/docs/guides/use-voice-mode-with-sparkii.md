---
sidebar_position: 8
title: "Use Voice Mode with Sparkii"
description: "A practical guide to setting up and using Sparkii voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Sparkii
...
- you want Sparkii sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Sparkii.
...
## Step 1: make sure normal Sparkii works first
...
Sparkii supports both local and cloud speech stacks.
...
### If you use `sparkii setup`

If you choose NeuTTS in the setup wizard, Sparkii checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Sparkii transcribes and responds
...
- asking Sparkii to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Sparkii loop.

## Tuning CLI behavior

### Silence threshold

If Sparkii starts/stops too aggressively, tune:
...
Sparkii stays a normal chat bot, but can speak replies.
...
- you want Sparkii to function like a portable research or ops assistant
...
Sparkii joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Sparkii detects speech boundaries
- transcripts are posted in the associated text channel
- Sparkii responds in text and audio
...
1. get text Sparkii working
2. run `sparkii setup tts` to enable voice support
