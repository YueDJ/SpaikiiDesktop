...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Sparkii Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Sparkii instantly when PRs are opened or updated.
:::
...
- **Sparkii Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  sparkii gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.sparkii/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Sparkii can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/sparkii-agent --state open --limit 3
```
...
Still in the chat, ask Sparkii to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/sparkii-agent
```
...
A skill gives Sparkii consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.sparkii/skills/code-review
```

Create `~/.sparkii/skills/code-review/SKILL.md`:
...
Verify it loaded — start `sparkii` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Sparkii your team's standards:
...
2. Teach Sparkii your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
sparkii gateway status    # Is the gateway running?
sparkii cron list         # Is the job enabled?
```
