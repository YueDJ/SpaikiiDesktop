...
description: "Diagnose and fix common Sparkii cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`sparkii gateway` for foreground, or `sparkii gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `sparkii cron tick`.
...
```bash
date
sparkii cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.sparkii/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.sparkii/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.sparkii/.env` |
...
| `local` | Write access to `~/.sparkii/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `sparkii cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `sparkii skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `sparkii skills list`.
...
2. `~/.sparkii/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `sparkii cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Sparkii config directory). Verify:
```bash
ls ~/.sparkii/scripts/your-script.py   # Must exist
sparkii cron edit <job_id> --script ~/.sparkii/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `sparkii skills install <skill-name>`.
...
Jobs are stored in `~/.sparkii/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.sparkii/cron/jobs.json
chmod 600 ~/.sparkii/cron/jobs.json   # Your user should own it
```
...
```bash
sparkii cron list                    # Show all jobs, states, next_run times
sparkii cron run <job_id>            # Schedule for next tick (for testing)
sparkii cron edit <job_id>           # Fix configuration issues
sparkii logs                         # View recent Sparkii logs
sparkii skills list                  # Verify installed skills
```
...
3. Open an issue at [github.com/YueDJ/SpaikiiDesktop](https://github.com/YueDJ/SpaikiiDesktop) with:
