...
This page covers all commands related to [Sparkii profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `sparkii profile` without a subcommand shows help.
...
```bash
sparkii profile use <name>
```

Sets `<name>` as the active profile. All subsequent `sparkii` commands (without `-p`) will use this profile.
...
sparkii profile use default
```

## `sparkii profile create`

```bash
sparkii profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `sparkii profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `sparkii update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.sparkii`), use `sparkii skills opt-out` / `sparkii skills opt-in`. |
...
sparkii profile create work2 --clone-from work

# Clone everything from a specific profile
sparkii profile create work2-backup --clone-from work --clone-all
```

## `sparkii profile describe`

```bash
sparkii profile describe [<name>] [options]
```
...
sparkii profile describe researcher

# Set it explicitly
sparkii profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
sparkii profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
sparkii profile describe --all --auto
```

## `sparkii profile delete`

```bash
sparkii profile delete <name> [options]
```
...
```bash
sparkii profile delete mybot
sparkii profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.sparkii`) cannot be deleted — use `sparkii uninstall` to remove everything.
:::

## `sparkii profile show`

```bash
sparkii profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Sparkii home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ sparkii profile show work
Profile: work
Path:    ~/.sparkii/profiles/work
...
## `sparkii profile alias`

```bash
sparkii profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Sparkii installation.
...
```bash
sparkii profile alias work
# Creates/updates ~/.local/bin/work

sparkii profile alias work --name mywork
# Creates ~/.local/bin/mywork

sparkii profile alias work --remove
# Removes the wrapper script
```

## `sparkii profile rename`

```bash
sparkii profile rename <old-name> <new-name>
```
...
```bash
sparkii profile rename mybot assistant
# ~/.sparkii/profiles/mybot → ~/.sparkii/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `sparkii profile export`

```bash
sparkii profile export <name> [options]
```
...
```bash
sparkii profile export work
# Creates work.tar.gz in the current directory

sparkii profile export work -o ./work-2026-03-29.tar.gz
```
...
## `sparkii profile import`

```bash
sparkii profile import <archive> [options]
```
...
```bash
sparkii profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

sparkii profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `sparkii -p telemetry`). |
...
sparkii profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
sparkii profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
sparkii profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
sparkii profile install ./telemetry/
```

### `sparkii profile update`

```bash
sparkii profile update <name> [--force-config] [--yes]
```
...
### `sparkii profile info`

```bash
sparkii profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Sparkii version, author, env var requirements, the source URL/path, and
...
`sparkii profile list` also shows the distribution name and version in a
`Distribution` column, and `sparkii profile show <name>` / `delete <name>`
...
sparkii profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
sparkii profile install https://github.com/your-org/internal-assistant.git
```
...
`sparkii_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Sparkii version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Sparkii can clone from.
3. Tell recipients to run `sparkii profile install <your-repo-url>`.
...
## `sparkii -p` / `sparkii --profile`

```bash
sparkii -p <name> <command> [options]
sparkii --profile <name> <command> [options]
```

Global flag to run any Sparkii command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
sparkii -p work chat -q "Check the server status"
sparkii --profile dev gateway start
sparkii -p personal skills list
sparkii -p work config edit
```

## `sparkii completion`

```bash
sparkii completion <shell>
```
...
- `sparkii profile <TAB>` — subcommands (list, use, create, etc.)
- `sparkii profile use <TAB>` — profile names
- `sparkii -p <TAB>` — profile names
