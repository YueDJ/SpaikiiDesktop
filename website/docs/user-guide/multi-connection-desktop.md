...
# Connecting Desktop to Many Sparkii Instances

Register every Sparkii backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Sparkii Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Sparkii gateway…"**) deep-links straight to
...
list of Sparkii gateways. Its intro says it plainly: *"Manage this device and
every Sparkii gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Sparkii runtime managed by this app." | automatic |
| **Remote gateway** | "A Sparkii gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Sparkii install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Sparkii Cloud** | "A hosted instance discovered through your Sparkii Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Sparkii Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Sparkii Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Sparkii Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Sparkii Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `sparkii serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Sparkii agents
...
connection is registered) dispatches `sparkii update` to every eligible
...
- **Sparkii Cloud** instances are skipped with a *"Managed by Sparkii Cloud"*
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `sparkii serve` is running on the remote host, the
...
- **"Update Sparkii Desktop to chat with agents on other connections"** — the
