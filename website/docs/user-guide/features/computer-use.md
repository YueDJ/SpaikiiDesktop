...
1. Run `sparkii tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Sparkii app). `sparkii computer-use doctor` will tell you which permission is missing. |
...
Sparkii maps its existing approval UX onto cua-driver 0.10's immutable daemon
...
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Sparkii approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Sparkii does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Sparkii risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Sparkii session. Turning `/yolo`
...
daemon's mode or grants another Sparkii conversation the same authority.
...
agent is acting here." Each Sparkii run declares its own cua-driver
**session id** (something like `sparkii-3a7b9c14d2e8`); the cursor's
...
Sparkii intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Sparkii-side `computer_use` action vocabulary — the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Sparkii (planned follow-up), this happens
