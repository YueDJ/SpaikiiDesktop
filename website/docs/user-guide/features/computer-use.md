...
Sparkii Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Sparkii integration. It
...
**Fresh installs already have the driver.** The Sparkii installer
...
- **`sparkii tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
sparkii computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `sparkii computer-use
status` to verify the install.

Already have cua-driver? Sparkii reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `sparkii update`, and the first
`computer_use` call of a session, Sparkii checks the local version and
...
selected with `SPARKII_CUA_DRIVER_CMD` stays
under your control, so Sparkii reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Sparkii autodetection is a
planned cua-driver follow-up, so currently point Sparkii at that directory or
...
interface. The built-in toolset provides Sparkii actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `sparkii computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Sparkii host identity. |
...
```
sparkii -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.sparkii/config.yaml`.

## Permission modes and logged-in browser profiles

Sparkii maps its existing approval UX onto cua-driver's immutable runtime
...
| Sparkii session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Sparkii approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Sparkii risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Sparkii then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.sparkii/cua-manifest.yaml
```
...
for the format). Sparkii launches a private runtime with
...
resetting or closing the Sparkii session, cancellation cleanup, or process exit
closes that transport session. Sparkii also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Sparkii
...
embedded service under the Sparkii host identity.
...
## `sparkii computer-use doctor` — your first triage stop

`sparkii computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ sparkii computer-use doctor
...
Sparkii run declares a public cua-driver **session name** (something like
`sparkii-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Sparkii keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Sparkii-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Sparkii
autodetection is a planned cua-driver follow-up, so currently point Sparkii at
...
These are **platform deep dives, not duplicates of the Sparkii skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Sparkii
...
Sparkii applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.sparkii/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Sparkii applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Sparkii agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Sparkii agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Sparkii warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Sparkii from inside the RDP / console
...
```
SPARKII_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SPARKII_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Sparkii disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Sparkii sets
...
When it's on, `sparkii computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Sparkii at a binary you built from source instead
of the published release. Sparkii resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SPARKII_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Sparkii testing (see notes).
...
### Option B — point Sparkii straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SPARKII_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SPARKII_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SPARKII_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Sparkii is using your build

- `sparkii computer-use status` prints the resolved binary path and
  version.
- `sparkii computer-use doctor` confirms the binary is reachable and
...
- **Sparkii spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Sparkii YOLO, Sparkii instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Sparkii
  change needed either way.
- **Local builds skip the version check.** Sparkii warns when the
...
**First action when anything's off: run `sparkii computer-use doctor`.**
...
Run `sparkii computer-use install` to fetch the cua-driver binary, or
run `sparkii tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `sparkii computer-use doctor` will
...
- **Sparkii-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Sparkii `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Sparkii autodetection is a
  planned follow-up; currently point Sparkii at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Sparkii among them)
