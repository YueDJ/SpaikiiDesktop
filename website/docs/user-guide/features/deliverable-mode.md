...
When Sparkii Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.sparkii/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.sparkii/config.yaml` (switchable per session
via `/personality`).
...
If you use Sparkii' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.sparkii/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Sparkii Agent's deliverable
