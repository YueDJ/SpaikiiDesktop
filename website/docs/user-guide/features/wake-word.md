...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Sparkii' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Sparkii")

The wake word turns Sparkii into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Sparkii listens in the background for a spoken
trigger phrase. Say it, and Sparkii starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Sparkii backend (for example a
...
Sparkii supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey sparkii"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey sparkii"** — a model for it ships with Sparkii, so
...
```bash
cd ~/.sparkii/sparkii-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `sparkii` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.sparkii/config.yaml`,
...
wake_word:
  ...
  phrase: "hey sparkii"        # cosmetic label only — detection is keyed by the model/keyword below
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey sparkii" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey sparkii" utterances are missed.
...
default) to let Sparkii choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Sparkii installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Sparkii surfaces, and `surface` picks which one
...
| `cli` | Only the classic `sparkii` CLI. |
| `tui` | Only `sparkii --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Sparkii surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Sparkii does not silently fail over to another open surface.
...
command (`sparkii -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.sparkii/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("sparkii" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Sparkii" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `sparkii tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `sparkii-agent[wake]`).
...
but the phrase never fires. Sparkii detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Sparkii
backend (it may appear as your terminal, `python`, or Sparkii), then toggle the
...
```bash
sparkii config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
sparkii config set wake_word.input_device null
```
