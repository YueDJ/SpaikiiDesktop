...
Sparkii runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Sparkii
...
1. Sparkii captures a baseline of current diagnostics for the file.
...
…). Sparkii auto-detects the binary on PATH or in
`<SPARKII_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Sparkii at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SPARKII_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`sparkii lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Sparkii installs both packages together when you
run `sparkii lsp install typescript` or auto-install fires on first
use.

## CLI

```
sparkii lsp status          # service state + per-server install status
sparkii lsp list            # registry, optionally --installed-only
sparkii lsp install <id>    # eagerly install one server
sparkii lsp install-all     # try every server with a known recipe
sparkii lsp restart         # tear down running clients
sparkii lsp which <id>      # print resolved binary path
```

`sparkii lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SPARKII_HOME>/lsp/bin
...
When `install_strategy: auto`, Sparkii installs binaries into
`<SPARKII_HOME>/lsp/bin/`. NPM packages land in
`<SPARKII_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Sparkii-owned and is
...
**`sparkii lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SPARKII_HOME>/lsp/bin/`. Run
`sparkii lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `sparkii lsp status`**
...
When `sparkii lsp status` shows a `Backend warnings` section, install
...
`~/.sparkii/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.sparkii/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `sparkii lsp restart` to clear the set;
