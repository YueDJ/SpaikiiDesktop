...
Type `@` followed by a reference to inject content directly into your message. Sparkii expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Sparkii env: `$SPARKII_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SPARKII_HOME/skills/.hub/`
