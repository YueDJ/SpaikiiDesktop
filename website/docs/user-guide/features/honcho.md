...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Sparkii's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Sparkii instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
sparkii memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.sparkii/.env
```
...
- `per-session` — each `sparkii` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Sparkii syncs them back at session init.
...
The `sparkii honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `sparkii memory setup honcho` (or run `sparkii memory setup` and pick it from the list); the `sparkii honcho` subcommand then appears on the next invocation.

```bash
sparkii memory setup honcho    # Configure Honcho directly (works before activation)
sparkii honcho status          # Connection status, config, and key settings
sparkii honcho setup           # Redirects to `sparkii memory setup` (post-activation alias)
sparkii honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
sparkii honcho peer            # Show or update peer names + dialectic reasoning level
sparkii honcho mode            # Show or set recall mode (hybrid/context/tools)
sparkii honcho tokens          # Show or set token budget for context and dialectic
sparkii honcho identity        # Seed or show the AI peer's Honcho identity
sparkii honcho sync            # Sync Honcho config to all existing profiles
sparkii honcho peers           # Show peer identities across all profiles
sparkii honcho sessions        # List known Honcho session mappings
sparkii honcho map             # Map current directory to a Honcho session name
sparkii honcho enable          # Enable Honcho for the active profile
sparkii honcho disable         # Disable Honcho for the active profile
sparkii honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `sparkii honcho`

If you previously used the standalone `sparkii honcho setup`:
...
No re-login or re-setup needed. Run `sparkii memory setup` and select "honcho" — the wizard detects your existing config.
