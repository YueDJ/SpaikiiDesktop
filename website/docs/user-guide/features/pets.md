...
Sparkii can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SPARKII_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Sparkii renders the sprite at full fidelity when your
...
sparkii pets list
sparkii pets list cat

# Install a pet and make it active in one step
sparkii pets install boba --select
...
## `sparkii pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `sparkii pets list [query] [--limit N]` |
| List installed pets | `sparkii pets list --installed` |
| Install a pet | `sparkii pets install <slug> [--select] [--force]` |
| Set the active pet | `sparkii pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `sparkii pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `sparkii pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `sparkii pets off` |
| Remove an installed pet | `sparkii pets remove <slug>` |
| Diagnose setup | `sparkii pets doctor` |

`sparkii pets show` flags:
...
Beyond installing pre-made pets from the gallery, Sparkii can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `sparkii pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `sparkii tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SPARKII_PET_IMAGE_PROVIDER` env var (e.g. `SPARKII_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Sparkii is
...
Run `sparkii pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Sparkii uses its own
  profile-scoped `<SPARKII_HOME>/pets/` instead — install through `sparkii pets`.

## See also

- The [`sparkii-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sparkii-agent.md)
