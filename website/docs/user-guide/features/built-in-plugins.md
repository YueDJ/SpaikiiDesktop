...
description: "Plugins shipped with Sparkii Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Sparkii ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.sparkii/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Sparkii Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.sparkii/plugins/<name>/`
3. **Project** — `./.sparkii/plugins/<name>/` (requires `SPARKII_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `sparkii_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `sparkii memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `sparkii plugins list` and the interactive `sparkii plugins` UI), but none load until you explicitly enable them:

```bash
sparkii plugins enable disk-cleanup
```

Or via `~/.sparkii/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Sparkii. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
sparkii plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `sparkii plugins enable <name>`.
...
| `sparkii-achievements` | dashboard tab | Steam-style collectible badges generated from your real Sparkii session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `sparkii memory` and `sparkii plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SPARKII_HOME` or `/tmp/sparkii-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SPARKII_HOME | always | Never |
...
**State** — everything lives at `$SPARKII_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SPARKII_HOME` or `/tmp/sparkii-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `sparkii plugins enable disk-cleanup` (or check the box in `sparkii plugins`).

**Disabling again:** `sparkii plugins disable disk-cleanup`.
...
**Enabling:** `sparkii plugins enable security-guidance` (or check the box in `sparkii plugins`).

**Disabling again:** `sparkii plugins disable security-guidance`.
...
Traces Sparkii turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Sparkii' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `sparkii logs`.
...
```bash
sparkii tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Sparkii and the next turn ships a trace.
...
Then put the credentials in `~/.sparkii/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Sparkii turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SPARKII_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Sparkii session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `sparkii chat` session lives under one Langfuse session.

**Verify:**

```bash
sparkii plugins list                 # observability/langfuse should show "enabled"
sparkii chat -q "hello"              # check the Langfuse UI for a "Sparkii turn" trace
```
...
| `SPARKII_LANGFUSE_RELEASE` | — | Release/version tag |
| `SPARKII_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SPARKII_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SPARKII_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Sparkii-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Sparkii-prefixed wins when both are set.
...
**Disabling:** `sparkii plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### observability/nemo_relay

Relays Sparkii execution boundaries — sessions, turns, LLM calls, and tool invocations — to an [NVIDIA NeMo Relay](https://docs.nvidia.com/nemo/relay/about-nemo-relay/overview) endpoint. Sparkii core owns the Relay session/turn/LLM/tool scopes; the plugin configures exporters (ATOF JSONL, ATIF trajectories, OpenTelemetry) and adds observer marks for approvals and delegated subagents. Full exporter setup lives in the plugin's `README.md` under `plugins/observability/nemo_relay/`.

**Enabling:**

```bash
sparkii plugins enable observability/nemo_relay
```
...
Rotated segments keep the same `session_id` attribute and add `sparkii.session.segment` (0-based index) plus `sparkii.session.segment_reason` (`compaction` or `max_turns`), so dashboards that group on `session_id` are unaffected. Rotation happens exclusively at turn boundaries and rides the same bounded scope-op executor as every other native Relay call — a wedged exporter costs one segment span, never the agent.
...
- Post-meeting artifacts (transcript, status) saved under `~/.sparkii/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
sparkii plugins enable google_meet
sparkii meet setup   # preflight: playwright, chromium, auth file
sparkii meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `sparkii plugins disable google_meet`. Any saved transcripts stay in `~/.sparkii/workspace/meetings/` until you remove them.

### sparkii-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Sparkii session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Sparkii feature changes.

**How it works:**

- Scans your entire `~/.sparkii/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SPARKII_HOME/plugins/sparkii-achievements/state.json`
...
| Secret | Hidden until Sparkii detects the first related signal in your history |

**API** — routes mount under `/api/plugins/sparkii-achievements/`:
...
**State files** — live under `$SPARKII_HOME/plugins/sparkii-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Sparkii updates. |
...
**Enabling:** Nothing to enable — `sparkii-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `sparkii dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/sparkii-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.sparkii/plugins/sparkii-achievements/` that ships no dashboard. The plugin's state files under `$SPARKII_HOME/plugins/sparkii-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Sparkii plugin — see [Build a Sparkii Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.sparkii/plugins/<name>/`
- Manifest source is reported as `bundled` in `sparkii plugins list`
