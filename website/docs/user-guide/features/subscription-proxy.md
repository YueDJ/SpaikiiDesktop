...
chat completions — use your Sparkii-managed provider subscription as their
...
| Use case | "Use Sparkii as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Sparkii stores
the refresh token in `~/.sparkii/auth.json` — the same place all Sparkii
...
Model:      Sparkii-4-70B    # or Sparkii-4.3-36B, Sparkii-4-405B
```
...
interface in `sparkii_cli/proxy/adapters/`.
...
If you see `not logged in`, run `sparkii portal`. If you see
...
`sparkii_cli/proxy/adapters/<provider>.py` and registering it in
