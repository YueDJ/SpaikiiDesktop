...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Sparkii Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---
...
1. **Sparkii Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `sparkii model` or set your preferred provider credentials in `~/.sparkii/.env`
3. **A working base setup** — run `sparkii` to verify the agent responds to text before enabling voice

:::tip
The `~/.sparkii/` directory and default `config.yaml` are created automatically the first time you run `sparkii`. You only need to create `~/.sparkii/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `sparkii setup --portal` wires both up at once.
:::
...
cd ~/.sparkii/sparkii-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.sparkii/sparkii-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.sparkii/sparkii-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.sparkii/sparkii-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`sparkii chat`) and the **TUI** (`sparkii --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.sparkii/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
sparkii                # Start the interactive CLI
```
...
1. Start the CLI with `sparkii` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.sparkii/config.yaml` (default: `ctrl+b`).
:::
...
```bash
sparkii gateway        # Start the gateway (connects to configured platforms)
sparkii gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@sparkiibyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.sparkii/.env`:
```bash
...
```bash
sparkii gateway        # Start with existing configuration
```
...
If you are running Sparkii inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.sparkii/.env`
...
- Review gateway logs: `tail -f ~/.sparkii/logs/gateway.log`
