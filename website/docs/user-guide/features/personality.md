...
description: "Customize Sparkii Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Sparkii Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SPARKII_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Sparkii is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Sparkii now seeds a default `SOUL.md` automatically in:

```text
~/.sparkii/SOUL.md
```

More precisely, it uses the current instance's `SPARKII_HOME`, so if you run Sparkii with a custom home directory, it will use:

```text
$SPARKII_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Sparkii creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Sparkii loads `SOUL.md` only from `SPARKII_HOME`
- Sparkii does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Sparkii falls back to a built-in default identity
...
If Sparkii loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SPARKII_HOME`, the personality belongs to the Sparkii instance itself.

That also makes it easier to teach users:
- "Edit `~/.sparkii/SOUL.md` to change Sparkii' default personality."
...
- how Sparkii should handle uncertainty, disagreement, or ambiguity
...
## What Sparkii injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Sparkii falls back to a built-in default identity ("You are Sparkii Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Sparkii ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Sparkii, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Sparkii its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.sparkii/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.sparkii/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-sparkii)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Sparkii speaks
- `display.skin` and `/skin` affect how Sparkii looks in the terminal
