...
**Tool Search** is Sparkii' opt-in progressive-disclosure layer for that
...
:::info Built-in Sparkii tools never defer
The tools that make up Sparkii' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_SPARKII_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Sparkii **unwraps the bridge** and
...
- **No JS sandbox.** Sparkii uses the simpler "structured tools" mode
