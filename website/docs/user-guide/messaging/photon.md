...
Connect Sparkii to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Sparkii — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Sparkii runs it in a
...
sparkii photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Sparkii Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.sparkii/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.sparkii/.env`
...
(device token, dashboard project id) lives in `~/.sparkii/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Sparkii
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Sparkii replies with a pairing code. Approve it with:

```bash
sparkii pairing approve photon <CODE>
```

Use `sparkii pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.sparkii/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.sparkii/.env`):
...
By default Sparkii responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Sparkii` and
`@Sparkii agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Sparkii will reply.

## Status & troubleshooting

```bash
sparkii photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Sparkii uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run sparkii photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `sparkii photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `sparkii photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `sparkii photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Sparkii sends images, voice
  ...
  media.
- **Native polls are supported.** Sparkii sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Sparkii sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `sparkii send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<sparkii-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Sparkii is set
...
| `PHOTON_MENTION_PATTERNS` | Sparkii wake words  | JSON list / comma / newline regex patterns for group mentions |
