...
description: "Chat with Sparkii from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
```mermaid
flowchart TB
    subgraph Gateway["Sparkii Gateway"]
...
For group chats, hooks, and automation flows, Sparkii supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Sparkii keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Sparkii does not hide failures just because the text resembles a silence token.
...
```bash
sparkii gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
sparkii gateway              # Run in foreground
sparkii gateway setup        # Configure messaging platforms interactively
sparkii gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo sparkii gateway install --system   # Linux only: install a boot-time system service
sparkii gateway start        # Start the default service
sparkii gateway stop         # Stop the default service
sparkii gateway status       # Check default service status
sparkii gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.sparkii/config.yaml"
gateway:
...
```bash
sparkii gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Sparkii sends heartbeats
...
| `/update` | Update Sparkii Agent to the latest version |
...
with the `session_reset` section in `~/.sparkii/config.yaml`:
...
Configure per-platform overrides in `~/.sparkii/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.sparkii/gateway-config.yaml`:
...
sparkii pairing approve telegram XKGH5N7P

# Other pairing commands:
sparkii pairing list          # View pending + approved users
sparkii pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Sparkii appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.sparkii/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.sparkii/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `SPARKII_HOME`:

- `~/.sparkii/status_phrases.yaml` or any `*.yaml` in `~/.sparkii/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to SPARKII_HOME
...
Off by default. When enabled, Sparkii prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.sparkii/config.yaml`:
...
```bash
SPARKII_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
sparkii gateway install               # Install as user service
sparkii gateway start                 # Start the service
sparkii gateway stop                  # Stop the service
sparkii gateway status                # Check status
journalctl --user -u sparkii-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo sparkii gateway install --system
sudo sparkii gateway start --system
sudo sparkii gateway status --system
journalctl -u sparkii-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Sparkii installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit sparkii-gateway` (or `sudo systemctl edit sparkii-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `sparkii update`. When `sparkii update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart sparkii-gateway` command (it never blocks on an interactive password prompt).
...
```bash
sparkii gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `sparkii update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo sparkii update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/sparkii-gateway`:

```
sparkii ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed sparkii-gateway*, /usr/bin/systemctl --no-ask-password start sparkii-gateway*, /usr/bin/systemctl --no-ask-password restart sparkii-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Sparkii will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Sparkii installations on the same machine (with different `SPARKII_HOME` directories), each gets its own systemd service name. The default `~/.sparkii` uses `sparkii-gateway`; other installations use `sparkii-gateway-<hash>`. The `sparkii gateway` commands automatically target the correct service for your current `SPARKII_HOME`.
:::

### macOS (launchd)

```bash
sparkii gateway install               # Install as launchd agent
sparkii gateway start                 # Start the service
sparkii gateway stop                  # Stop the service
sparkii gateway status                # Check status
tail -f ~/.sparkii/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.sparkii.gateway.plist`. It includes three environment variables:
...
- **SPARKII_HOME** — scopes the gateway to your Sparkii installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `sparkii gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SPARKII_HOME` directory gets its own launchd label. The default `~/.sparkii` uses `ai.sparkii.gateway`; other installations use `ai.sparkii.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `sparkii-cli` | Full access |
| Telegram | `sparkii-telegram` | Full tools including terminal |
| Discord | `sparkii-discord` | Full tools including terminal |
| WhatsApp | `sparkii-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `sparkii-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `sparkii-slack` | Full tools including terminal |
| Google Chat | `sparkii-google_chat` | Full tools including terminal |
| Signal | `sparkii-signal` | Full tools including terminal |
| SMS | `sparkii-sms` | Full tools including terminal |
| Email | `sparkii-email` | Full tools including terminal |
| Home Assistant | `sparkii-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `sparkii-mattermost` | Full tools including terminal |
| Matrix | `sparkii-matrix` | Full tools including terminal |
| DingTalk | `sparkii-dingtalk` | Full tools including terminal |
| Feishu/Lark | `sparkii-feishu` | Full tools including terminal |
| WeCom | `sparkii-wecom` | Full tools including terminal |
| WeCom Callback | `sparkii-wecom-callback` | Full tools including terminal |
| Weixin | `sparkii-weixin` | Full tools including terminal |
| BlueBubbles | `sparkii-bluebubbles` | Full tools including terminal |
| QQBot | `sparkii-qqbot` | Full tools including terminal |
| Yuanbao | `sparkii-yuanbao` | Full tools including terminal |
| Microsoft Teams | `sparkii-teams` | Full tools including terminal |
| API Server | `sparkii-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `sparkii-webhook` | Full tools including terminal |
| Raft | `sparkii-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.sparkii/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
