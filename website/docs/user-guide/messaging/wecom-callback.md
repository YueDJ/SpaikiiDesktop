...
Connect Sparkii to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Sparkii supports two WeCom integration modes:
...
> Run `sparkii gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Sparkii decrypts the message, queues it for the agent
...
(Use `sparkii gateway start` only after `sparkii gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Sparkii and the token the
...
in `~/.sparkii/.env` values around `=` will also break signature checks. After
fixing, restart `sparkii gateway run`.
...
Check `sparkii gateway run` logs for the bound host/port. If the adapter bound to
