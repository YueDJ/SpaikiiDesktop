...
sidebar_position: 12
title: "Google Chat"
description: "Set up Sparkii Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Sparkii Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Sparkii
...
> Run `sparkii gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Sparkii can read (e.g.,
`~/.sparkii/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a sparkii restart)
...
- Principal: `sparkii-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Sparkii calls
...
- **App name**: whatever you want users to see ("Sparkii" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/sparkii-chat-events`.
...
`ADDED_TO_SPACE` event that Sparkii uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Sparkii

Add the Google Chat section to `~/.sparkii/.env`:
...
Send "hola" in the test DM. The bot posts a "Sparkii is thinking…" marker, then
...
`~/.sparkii/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Sparkii is thinking…").
...
Thread support: when a user replies inside a thread, Sparkii detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Sparkii session.
...
3. Download the JSON. Move it onto the host that runs Sparkii.
4. Register the client with Sparkii (run under the profile you want it scoped to):
...
sparkii -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Sparkii home (e.g.
`~/.sparkii/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.sparkii/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.sparkii/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Sparkii isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `sparkii gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Sparkii will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.sparkii/google_chat_user_tokens/<sanitized_email>.json` (filesystem
