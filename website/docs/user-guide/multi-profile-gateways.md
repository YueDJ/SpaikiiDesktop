...
If you only run one Sparkii agent, you don't need this page — see
...
[Connecting Desktop to Many Sparkii Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Sparkii agents that should all
...
(`ai.sparkii.gateway-<name>.plist`) or systemd user service
(`sparkii-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
sparkii profile create coder
sparkii profile create personal-bot
sparkii profile create research
...
```bash
sparkii config set gateway.multiplex_profiles true
sparkii gateway restart
```

Equivalently, in the default profile's `~/.sparkii/config.yaml`:
...
You do **not** run `sparkii gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `sparkii gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.sparkii/profiles/coder/.env`; the default listener key is rejected.
...
home). `sparkii status` reports the multiplexer and the profiles it serves;
`sparkii status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/sparkii-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: sparkii-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    sparkii gateway "$action"
  else
    sparkii -p "$profile" gateway "$action"
  fi
}
...
```bash
sparkii-gateways start      # start every configured profile
sparkii-gateways stop       # stop every configured profile
sparkii-gateways restart    # restart all
sparkii-gateways status     # status across all
sparkii-gateways list       # delegates to `sparkii gateway list`
```

:::tip
The `default` profile is targeted with `sparkii gateway <action>` (no `-p`),
not `sparkii -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `sparkii -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.sparkii.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/sparkii-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.sparkii.gateway.plist` /
`sparkii-gateway.service`.
...
```bash
# Default profile
tail -f ~/.sparkii/logs/gateway.log
tail -f ~/.sparkii/logs/gateway.error.log

# Named profile
tail -f ~/.sparkii/profiles/<name>/logs/gateway.log
tail -f ~/.sparkii/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.sparkii/logs/gateway.log ~/.sparkii/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
sparkii logs -f                  # follow default profile
sparkii -p coder logs -f         # follow one profile
sparkii logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
sparkii profile list             # profiles + model + gateway state
sparkii-gateways status          # full status across every profile
launchctl list | grep sparkii    # macOS — PIDs and labels
systemctl --user list-units 'sparkii-gateway-*'   # Linux — units
```
...
```
~/.sparkii/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.sparkii/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
sparkii config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.sparkii/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=sparkii --why="gateways running" \
  sleep infinity &
...
`sparkii-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.sparkii/.env ~/.sparkii/profiles/*/.env
```

## Updating the code

`sparkii update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `sparkii gateway start` after a previous `sparkii gateway stop`. The
...
```bash
ps -ef | grep "sparkii_cli.*-p <profile>"
cat ~/.sparkii/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.sparkii.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.sparkii.gateway-<profile>.plist

# Linux
systemctl --user restart sparkii-gateway-<profile>.service
```

### Health check

```bash
sparkii doctor                  # default profile
sparkii -p <profile> doctor     # one profile
```
