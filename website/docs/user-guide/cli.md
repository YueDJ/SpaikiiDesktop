...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Sparkii Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Sparkii Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `sparkii setup --portal` — and you're ready to `sparkii chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Sparkii also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `sparkii --tui` — see the [TUI](tui.md) guide.
:::
...
sparkii chat -q "Hello"

# Single query from a file or stdin — nothing is shell-interpreted, so
# arbitrary text (quotes, $(...), backticks) arrives verbatim
sparkii chat --query-file prompt.txt
sparkii chat --query-file - < prompt.txt

# With a specific model
sparkii chat --model "anthropic/claude-sonnet-4"

# With a specific provider
sparkii chat --provider nous        # Use Nous Portal
sparkii chat --provider openrouter  # Force OpenRouter
...
sparkii chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
sparkii --continue             # Resume the most recent CLI session (-c)
sparkii --resume <session_id>  # Resume a specific session by ID (-r)
sparkii --resume latest        # Resume the most recent session (same as -c)
sparkii --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
sparkii chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
sparkii -w                         # Interactive mode in worktree
sparkii -w -z "Fix issue #123"     # Single query in worktree
```

### Plugin management

The `sparkii plugins` commands manage native Sparkii plugins and portable Agent
...
sparkii plugins disable <plugin-name>
sparkii plugins update <plugin-name>
sparkii plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Sparkii currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Sparkii CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Sparkii CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SPARKII_YOLO_MODE` is on (either `sparkii --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Sparkii to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.sparkii/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Sparkii supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Sparkii replies |
...
```bash
sparkii -s sparkii-agent-dev,github-auth
sparkii chat -s github-pr-workflow -s github-auth
```

Sparkii loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.sparkii/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.sparkii/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Sparkii..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Sparkii also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Sparkii recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Sparkii — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Sparkii is working, Sparkii prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Sparkii to the background — just like any terminal process. The shell prints a confirmation:

```
Sparkii Agent has been suspended. Run `fg` to bring Sparkii Agent back.
```
...
Resume this session with:
  sparkii --resume 20260225_143052_a1b2c3
...
```bash
sparkii --continue                          # Resume the most recent CLI session
sparkii -c                                  # Short form
sparkii -c "my project"                     # Resume a named session (latest in lineage)
sparkii --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
sparkii --resume "refactoring auth"         # Resume by title
sparkii --resume latest                     # Resume the most recent session (same as -c)
sparkii --resume latest --in ./my-project   # Latest session for ./my-project's workspace
sparkii -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `sparkii sessions rename <id> <title>` from the command line. Use `sparkii sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Sparkii's SQLite state database under `~/.sparkii/state.db`. The database keeps:
...
Sparkii immediately confirms the task and gives you back the prompt:
