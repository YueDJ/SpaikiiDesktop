...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `sparkii config set` only writes scalar values, but it can't carry a custom `base_url`:
