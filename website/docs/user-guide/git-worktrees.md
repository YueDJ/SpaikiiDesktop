...
title: "Git Worktrees"
description: "Run multiple Sparkii agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Sparkii Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Sparkii so each session has a clean, isolated working directory.

## Why Use Worktrees with Sparkii?

Sparkii treats the **current working directory** as the project root:

- CLI: the directory where you run `sparkii` or `sparkii chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.sparkii/config.yaml`
...
Sparkii creates `.worktrees/my-experiment/` inside the repo (branch
`sparkii/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `sparkii-<id>`
...
git worktree add ../repo-feature feature/sparkii-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sparkii-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Sparkii there:

```bash
cd ../repo-feature

# Start Sparkii in the worktree
sparkii
```
...
git worktree add ../repo-experiment-a feature/sparkii-a
git worktree add ../repo-experiment-b feature/sparkii-b
```
...
Each Sparkii process:

- Works on its own branch (`feature/sparkii-a` vs `feature/sparkii-b`).
...
- Sparkii checkpoint data under `~/.sparkii/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Sparkii experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sparkii-checkpoints-docs`, `feature/sparkii-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Sparkii from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sparkii -w` (Automatic Worktree Mode)

Sparkii has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sparkii/sparkii-<hash>`).
...
```bash
sparkii -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `sparkii -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Sparkii session its own clean checkout.
