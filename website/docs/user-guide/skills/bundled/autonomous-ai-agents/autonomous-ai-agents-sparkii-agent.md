---
title: "Sparkii Agent — Use, configure, theme, extend, and orchestrate Sparkii Agent"
...
Use, configure, theme, extend, and orchestrate Sparkii Agent.
...
| Path | `skills/autonomous-ai-agents/sparkii-agent` |
| Version | `3.1.0` |
...
Sparkii Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Sparkii works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Sparkii different:

- **Self-improving through skills** — Sparkii learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Sparkii instances with isolated configs, sessions, skills, and memory.
...
# Other surfaces
sparkii desktop                 # launch the native desktop app (alias: sparkii gui)
sparkii dashboard               # web admin panel + embedded chat
sparkii proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```

## Key Paths

```
~/.sparkii/config.yaml       Main configuration (settings — never secrets)
~/.sparkii/.env              API keys and secrets ONLY (under $SPARKII_HOME if set)
$SPARKII_HOME/skills/        Installed skills
~/.sparkii/skins/            Custom themes (see references/themes.md)
~/.sparkii/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.sparkii/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.sparkii/pets/             Installed pet mascots (see references/petdex.md)
~/.sparkii/state.db          Canonical session store (SQLite + FTS5)
~/.sparkii/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
Profiles use `~/.sparkii/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$SPARKII_HOME` — never hardcode `~/.sparkii`.
...
| AGENTS.md / .sparkii.md / CLAUDE.md project rules | `references/project-context-files.md` |
...
| MCP servers (add, catalog, `sparkii mcp`) | `references/native-mcp.md` |
...
## Spawning Additional Sparkii Instances

Run additional Sparkii processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `sparkii` process |
|-|-----------------|--------------------------|
...
Sparkii uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `sparkii chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Sparkii; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`sparkii desktop` / `sparkii gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`sparkii dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `sparkii --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`sparkii --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`sparkii proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
