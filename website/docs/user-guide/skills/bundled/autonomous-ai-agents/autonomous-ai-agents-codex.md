...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`sparkii-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sparkii-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Sparkii terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Sparkii itself, `model.provider: openai-codex` uses Sparkii-managed Codex
OAuth from `~/.sparkii/auth.json` after `sparkii auth add openai-codex`. For the
...
## Sparkii Gateway Caveat

When invoking the Codex CLI from a Sparkii gateway/service context (for example,
