...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Sparkii MCP catalog. The
...
Two halves, in this order: the editor side must be up before Sparkii connects.
...
### One-time, Sparkii side

    sparkii mcp install unreal-engine
...
`~/.sparkii/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Sparkii — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Sparkii connects
...
2. Start the Sparkii session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Sparkii
they appear as:

| Sparkii tool | Purpose |
|---|---|
...
then happens at `sparkii mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Sparkii runs batched calls
...
- **Start order matters.** Editor + server up first, then the Sparkii
...
- **Timeouts on long operations.** Sparkii' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.sparkii/config.yaml` for
