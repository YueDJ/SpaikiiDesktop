---
title: "Honcho — Configure and troubleshoot Honcho memory for Sparkii"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Sparkii"
---
...
Configure and troubleshoot Honcho memory for Sparkii.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Sparkii

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Sparkii profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/sparkii#running-honcho-locally-with-sparkii

### Verify

```bash
sparkii honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Sparkii profile's AI peer
...
Honcho models conversations as interactions between **peers**. Sparkii creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Sparkii instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Sparkii run |
| `global` | Single session across all directories |

Manual override: `sparkii honcho map my-project-name`
...
Each Sparkii profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block sparkii.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `sparkii.coder` host block in `honcho.json`
...
```bash
sparkii honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Sparkii when Honcho memory is active.
...
Config file: `$SPARKII_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `sparkii honcho setup`. Ensure `memory.provider: honcho` is in `~/.sparkii/config.yaml`.

### Memory not persisting across sessions
Check `sparkii honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sparkii profile create <name> --clone`. For existing profiles: `sparkii honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sparkii honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sparkii honcho status` | Show resolved config, connection test, peer info for active profile |
| `sparkii honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sparkii honcho disable` | Disable Honcho for the active profile |
| `sparkii honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sparkii honcho peers` | Show peer identities across all profiles |
| `sparkii honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sparkii honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sparkii honcho sessions` | List known directory-to-session-name mappings |
| `sparkii honcho map <name>` | Map current working directory to a Honcho session name |
| `sparkii honcho identity` | Seed AI peer identity or show both peer representations |
| `sparkii honcho sync` | Create host blocks for all Sparkii profiles that don't have one yet |
| `sparkii honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Sparkii + Honcho |
| `sparkii memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sparkii memory status` | Show active memory provider and config |
| `sparkii memory off` | Disable external memory provider |
