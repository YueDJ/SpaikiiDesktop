...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SPARKII_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SPARKII_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `sparkii`)    |
