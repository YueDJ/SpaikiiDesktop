...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/mcp/fastmcp` |
...
| Related skills | [`sparkii-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sparkii-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Sparkii or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Sparkii. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sparkii/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sparkii/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Sparkii integration, either:

- configure the server in `~/.sparkii/config.yaml` using the `native-mcp` skill, or
...
### Sparkii cannot see the deployed server

The server-building part may be correct while the Sparkii config is not. Load the `native-mcp` skill and configure the server in `~/.sparkii/config.yaml`, then restart Sparkii.
