...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/email/agentmail` |
| Path | `optional-skills/email/agentmail` |
| Version | `1.0.0` |
| Author | teyrebaz33, Sparkii Agent |
| License | MIT |
...
Add to `~/.sparkii/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `sparkii-agent`)
   - The agent gets address: `sparkii-agent@agentmail.to`
2. Send an email:
