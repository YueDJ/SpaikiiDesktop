...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Sparkii Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Sparkii terminal calls (desktop app flow only)
...
### Service Account (recommended for Sparkii)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SPARKII_HOME:-~/.sparkii}/.env` (the skill will prompt for this on first load).
...
## Sparkii Execution Pattern (desktop app flow)

Sparkii terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/sparkii-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sparkii-op.sock"
