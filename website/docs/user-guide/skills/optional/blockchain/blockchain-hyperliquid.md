...
| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Sparkii Agent |
| License | MIT |
...
The script reads `${SPARKII_HOME:-~/.sparkii}/.env` for two optional defaults:
...
Helper script: `~/.sparkii/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${SPARKII_HOME:-~/.sparkii}/.env`.
