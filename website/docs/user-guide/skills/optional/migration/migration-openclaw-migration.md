---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Sparkii"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Sparkii"
---
...
Import an OpenClaw setup (memories, skills) into Sparkii.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sparkii skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Sparkii Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Sparkii`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Sparkii Migration

Use this skill when a user wants to move their OpenClaw setup into Sparkii Agent with minimal manual cleanup.
...
```bash
sparkii claw migrate              # Full interactive migration
sparkii claw migrate --dry-run    # Preview what would be migrated
sparkii claw migrate --preset user-data   # Migrate without secrets
sparkii claw migrate --overwrite  # Overwrite existing conflicts
sparkii claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `sparkii setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Sparkii home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Sparkii memory entries
- merge OpenClaw command approval patterns into Sparkii `command_allowlist`
- migrate Sparkii-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Sparkii working-directory configuration
- copy OpenClaw skills into `~/.sparkii/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Sparkii workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.sparkii/tts/`
- archive non-secret docs that do not have a direct Sparkii destination
...
- `~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.sparkii/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.sparkii/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Sparkii-compatible secrets, currently:
...
Sparkii CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Sparkii. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Sparkii workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Sparkii skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.sparkii/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Sparkii unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Sparkii targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Sparkii destination. Unsupported auth blobs must still be reported as skipped.
...
- Sparkii persona state imported
- Sparkii memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.sparkii/skills/openclaw-imports/`
