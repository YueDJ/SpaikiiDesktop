...
`~/.sparkii/config.yaml`, `~/.sparkii/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/sparkii`:

```text
/etc/sparkii/
├── config.yaml     # managed config layer (wins over ~/.sparkii/config.yaml)
└── .env            # managed env layer (wins over ~/.sparkii/.env + shell)
```
...
The location can be relocated with the `SPARKII_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SPARKII_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Sparkii.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SPARKII_MANAGED_DIR=/opt/org/sparkii-policy
```

:::warning
A user who can set `SPARKII_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `sparkii doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/sparkii/config.yaml` (managed) | `/etc/sparkii/.env` (managed) |
| 2 | `~/.sparkii/config.yaml` (user) | `~/.sparkii/.env` (user) |
...
```bash
sparkii config        # shows a header naming the managed source + the pinned keys
sparkii doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Sparkii refuses and names the source:

```bash
$ sparkii config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/sparkii/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `sparkii config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/sparkii

# Pin some config values for every user on this machine
sudo tee /etc/sparkii/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/sparkii/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/sparkii
sudo chmod 0644 /etc/sparkii/config.yaml /etc/sparkii/.env
```

Changes take effect on the next Sparkii start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`sparkii doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Sparkii as `root`), managed scope is advisory.
