...
sidebar_position: 4
title: "工具集参考"
description: "Sparkii 核心、复合、平台及动态工具集参考"
---
...
- **平台（Platform）** — 针对特定部署环境的完整工具配置（例如，`sparkii-cli` 是交互式 CLI 会话的默认配置）
...
| `code_execution` | `execute_code` | 运行以编程方式调用 Sparkii 工具的 Python 脚本。 |
...
`sparkii tools` 命令提供基于 curses 的 UI，用于按平台切换单个工具的启用/禁用状态。该操作在工具级别进行（比工具集更细粒度），并持久化到 `config.yaml`。即使工具集已启用，被禁用的工具也会被过滤掉。
