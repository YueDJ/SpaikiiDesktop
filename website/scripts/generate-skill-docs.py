...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sparkii skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Sparkii Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Sparkii ships with a large built-in skill library copied into `~/.sparkii/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sparkii also syncs bundled skills on `sparkii update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sparkii/skills/` tree, it is still shipped with Sparkii; restore it with `sparkii skills reset <name> --restore`.",
        "",
