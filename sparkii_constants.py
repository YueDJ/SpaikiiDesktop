"""Shared constants for Sparkii Agent.
...
_UNSET = object()
_SPARKII_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SPARKII_HOME_OVERRIDE", default=_UNSET
)
...
def set_sparkii_home_override(path: str | Path | None) -> Token:
    """Set a context-local Sparkii home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _SPARKII_HOME_OVERRIDE.set(value)


def reset_sparkii_home_override(token: Token) -> None:
    """Restore the previous context-local Sparkii home override."""
    _SPARKII_HOME_OVERRIDE.reset(token)


def get_sparkii_home_override() -> str | None:
    """Return the active context-local Sparkii home override, if any."""
    override = _SPARKII_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_sparkii_home() -> Path:
    """Return the platform-native default Sparkii home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "sparkii"
    return Path.home() / ".sparkii"


def _sparkii_home_from_env() -> Path:
    """Resolve SPARKII_HOME from the process environment only.

    Reads the ``SPARKII_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_sparkii_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_sparkii_home`
    and :func:`get_process_sparkii_home` so the two never drift.
    """
    val = os.environ.get("SPARKII_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_sparkii_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``SPARKII_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_sparkii_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[SPARKII_HOME fallback] SPARKII_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass SPARKII_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_sparkii_home() -> Path:
    """Return the Sparkii home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_sparkii_home_override`) → ``SPARKII_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``SPARKII_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not os.environ.get("SPARKII_HOME", "").strip():
        _warn_profile_fallback_once()

    return _sparkii_home_from_env()


def sparkii_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Sparkii home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_sparkii_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_sparkii_home() -> Path:
    """Return the Sparkii home for the running process, ignoring task overrides.

    Unlike :func:`get_sparkii_home`, this never follows the context-local
    override set by :func:`set_sparkii_home_override`.  It resolves only the
    process ``SPARKII_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_sparkii_root(). The function resolves
# SPARKII_HOME against the native home on every call (~80us of path
...
# Its result depends only on (SPARKII_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates SPARKII_HOME mid-process.
_default_sparkii_root_memo: "tuple[str, str, Path] | None" = None
...
def get_default_sparkii_root() -> Path:
    ...
    (``~/.sparkii`` on POSIX, ``%LOCALAPPDATA%\\sparkii`` on native Windows).

    In Docker or custom deployments where ``SPARKII_HOME`` points outside
    ``~/.sparkii`` (e.g. ``/opt/data``), returns ``SPARKII_HOME`` directly
    — that IS the root.

    In profile mode where ``SPARKII_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.sparkii/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_sparkii_root_memo
    native_home = _get_platform_default_sparkii_home()
    env_home = os.environ.get("SPARKII_HOME", "")
    if _default_sparkii_root_memo is not None:
        memo_native, memo_env, memo_result = _default_sparkii_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # SPARKII_HOME is under ~/.sparkii (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — SPARKII_HOME itself is the root
                result = env_path
    _default_sparkii_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``SPARKII_OPTIONAL_SKILLS``.
    """
    override = os.getenv("SPARKII_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_sparkii_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``SPARKII_OPTIONAL_MCPS``.
    """
    override = os.getenv("SPARKII_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_sparkii_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``SPARKII_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<SPARKII_HOME>/skills`` last-resort
    """
    override = os.getenv("SPARKII_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_sparkii_dir(
    ...
    *,
    ...
    """Resolve a Sparkii subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to SPARKII_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SPARKII_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Sparkii home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local SPARKII_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_sparkii_home()
...
def iter_sparkii_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    into ``%LOCALAPPDATA%\\sparkii\\node``. POSIX installs use
    ``$SPARKII_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_sparkii_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with sparkiiManagedNodePathEntries() in
...
_SPARKII_NODE_TARGET_MAJOR = int(os.environ.get("SPARKII_NODE_TARGET_MAJOR", "22"))
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Sparkii-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `sparkii update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%SPARKII_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the sparkii.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_SPARKII_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_SPARKII_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$SPARKII_HOME/node`` on POSIX.
    ...
    ``SPARKII_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_sparkii_managed_node() -> str | None:
    ...
    Sparkii never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$SPARKII_HOME/node`` (the same tree a fresh install
...
def heal_sparkii_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SPARKII_HOME": str(get_sparkii_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for directory in iter_sparkii_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _SPARKII_NODE_TARGET_MAJOR
    return False
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Sparkii-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Sparkii-managed installs.

    This is for Sparkii-owned subprocesses that should not be broken by a bad,
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``sparkii update`` — leaving a **dangling symlink** that ``which`` still
...
def display_sparkii_home() -> str:
    """Return a user-friendly display string for the current SPARKII_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.sparkii``
        profile:  ``~/.sparkii/profiles/coder``
        custom:   ``/opt/sparkii-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.sparkii``.  For code that needs a real ``Path``, use
    :func:`get_sparkii_home` instead.
    """
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("SPARKII_REAL_HOME") or os.getenv("SPARKII_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Sparkii profile HOME.

    ``SPARKII_HOME`` scopes Sparkii state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={SPARKII_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{SPARKII_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{SPARKII_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Sparkii' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["SPARKII_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Sparkii itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SPARKII_HOME.

    Replaces the ``get_sparkii_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, sparkii_logging.py, sparkii_time.py, etc.).
    """
    return get_sparkii_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under SPARKII_HOME."""
    return get_sparkii_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SPARKII_HOME."""
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``sparkii_cli`` modules using three different Windows
    ...
    A few sites outside ``sparkii_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``sparkii_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Sparkii itself. An ImportError
...
# `sparkii_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Sparkii.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `sparkii update` is exactly
    ...
    return [
        ...
        "    sparkii update",
        "If that also fails, reinstall: https://sparkii-agent.nousresearch.com",
    ]
