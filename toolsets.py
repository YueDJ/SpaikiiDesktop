...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `sparkii tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``sparkii tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SPARKII_KANBAN_TASK env "
    ...
    # Coding posture (base Sparkii — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in sparkii_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Sparkii toolsets (CLI + messaging platforms)
    #
    ...
    # the `sparkii send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "sparkii-cron": {
        # Mirrors sparkii-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `sparkii tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as sparkii-cli; gated by `sparkii tools`",
        "tools": _SPARKII_CORE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sparkii-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SPARKII_CORE_TOOLS + [platform extras]``.
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (sparkii-<name>).
        # Gives them _SPARKII_CORE_TOOLS plus any tools the plugin registered
