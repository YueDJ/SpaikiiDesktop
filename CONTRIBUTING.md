# Contributing to Sparkii Agent

Thank you for contributing to Sparkii Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Sparkii to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo YueDJ/SparkiiDesktop "<your terms>"
  gh search prs --repo YueDJ/SparkiiDesktop --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/YueDJ/SparkiiDesktop/issues?q=) · [PRs (all states)](https://github.com/YueDJ/SparkiiDesktop/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Sparkii itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Sparkii install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `sparkii skills browse` (labeled "official") and install it with `sparkii skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `sparkii skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.sparkii/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(sparkii_home, config)` for setup-wizard integration
...
- Integrate with `sparkii memory setup` via `post_setup()` — no need to touch core code
...
uv venv ~/.sparkii/venvs/sparkii-dev --python 3.11
...
```bash
mkdir -p ~/.sparkii/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.sparkii/config.yaml
touch ~/.sparkii/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.sparkii/.env
```

### Run

```bash
# The standard installer already put `sparkii` on PATH.
sparkii doctor
sparkii chat -q "Hello"
```

If you used the manual clone fallback, run `./sparkii` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/sparkii" ~/.local/bin/sparkii
```
...
| Path | Purpose |
|------|---------|
| `~/.sparkii/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.sparkii/.env` | API keys and secrets |
| `~/.sparkii/auth.json` | OAuth credentials (Nous Portal) |
...
- **Session persistence**: All conversations are stored in SQLite (`sparkii_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.sparkii/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_SPARKII_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.sparkii`:
...
The user may skip setup and keep loading the skill. Sparkii only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `sparkii setup` or update `~/.sparkii/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Sparkii tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Sparkii Agent" is the secondary collaborator. If the contributor's commit shows "Sparkii Agent" as author because they used Sparkii to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Sparkii tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `sparkii --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Sparkii uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.sparkii/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `sparkii_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `sparkii_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Sparkii runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the sparkii wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `sparkii_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `sparkii_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `sparkii_cli/gateway_windows.py::
...
Sparkii has terminal access. Security matters.
...
2. **Test manually**: Run `sparkii` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/YueDJ/SparkiiDesktop/issues)
