# Sparkii Agent Security Policy

This document describes Sparkii Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/sparkii-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Sparkii Agent does not operate a bug
bounty program.**
...
- Environment details (`sparkii version`, commit SHA, OS, Python
  version).
...
Sparkii Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Sparkii Agent,
...
- **Stance.** An explicit statement in Sparkii Agent's documentation
...
Sparkii Agent supports two OS-level isolation postures. They address
...
Sparkii Agent supports this in two ways:

- **Sparkii Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Sparkii Agent's in-process heuristics
...
A malicious or buggy plugin is not a vulnerability in Sparkii Agent
itself. Bugs in Sparkii Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Sparkii Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Sparkii Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Sparkii Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  sparkii-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Sparkii Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Sparkii Agent. Bugs in Sparkii
...
- Sparkii Agent includes supply-chain guards for MCP server
