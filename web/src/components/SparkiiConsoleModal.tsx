...
interface SparkiiConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SparkiiConsoleModal({ open, onClose }: SparkiiConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("sparkii> ");
  const inputPromptRef = useRef("sparkii> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "sparkii> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Sparkii Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sparkii-console-title"
    >
      <div
        className={cn(
          themedBody,
