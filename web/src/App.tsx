...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSparkii = status?.can_update_sparkii === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    ...
    return (
      t.status.updateSparkiiConfirmMessage ??
      `This will run 'sparkii update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSparkiiConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSparkii) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSparkii,
      runningLabel: t.status.updatingSparkii,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSparkiiConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSparkiiConfirmTitle ?? `${t.status.updateSparkii}?`}
    />
    </>
  );
}
