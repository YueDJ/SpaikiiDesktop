import { buildSparkiiWebSocketUrl } from "@sparkii/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/sparkii/). The Python backend
// injects ``window.__SPARKII_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SPARKII_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SPARKII_BASE_PATH = readBasePath();
const BASE = SPARKII_BASE_PATH;
...
declare global {
  interface Window {
    __SPARKII_SESSION_TOKEN__?: string;
    __SPARKII_BASE_PATH__?: string;
    ...
    __SPARKII_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Sparkii-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SPARKII_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``sparkii update``, ``sparkii gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SPARKII_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SPARKII_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SPARKII_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``sparkii_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SPARKII_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Sparkii-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``sparkii_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SPARKII_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildSparkiiWebSocketUrl({
...
export const api = {
  ...
  updateSparkii: () =>
    fetchJSON<ActionResponse>("/api/sparkii/update", { method: "POST" }),
  checkSparkiiUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/sparkii/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** sparkii-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``sparkii update``. */
  can_update_sparkii?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SPARKII_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SPARKII_HOME volume — no staleness dimension, so no
 * sampled_at. */
