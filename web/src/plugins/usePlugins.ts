...
import { api, SPARKII_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "sparkii:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SPARKII_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SPARKII_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?sparkii_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-sparkii-plugin", manifest.name);
