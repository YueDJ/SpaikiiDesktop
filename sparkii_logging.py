"""Centralized logging setup for Sparkii Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sparkii/logs/`` (profile-aware via ``get_sparkii_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``sparkii_cli.web_server``,
    ``sparkii_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Sparkii (TUI, gateway, ``hy_memory`` server, MCP
...
from sparkii_constants import get_config_path, get_sparkii_home
...
# Used by _ComponentFilter and exposed for ``sparkii logs --component``.
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "sparkii_cli.web_server",
        "sparkii_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Sparkii logging subsystem.
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the sparkii group. However, both ``_open()`` (initial creation)
...
# the module header): when several Sparkii processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="sparkii-log-drain", daemon=True)
    t.start()
    t.join(timeout)
