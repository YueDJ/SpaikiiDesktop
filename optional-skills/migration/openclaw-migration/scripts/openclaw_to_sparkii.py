#!/usr/bin/env python3
"""OpenClaw -> Sparkii migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Sparkii Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Sparkii.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Sparkii memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Sparkii memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Sparkii-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Sparkii-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Sparkii command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.sparkii/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.sparkii/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Sparkii .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Sparkii .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Sparkii .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Sparkii .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Sparkii .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Sparkii config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Sparkii config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Sparkii.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Sparkii MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Sparkii config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'sparkii cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Sparkii config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Sparkii session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Sparkii custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Sparkii config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Sparkii config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Sparkii config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `sparkii config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`sparkii config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.sparkii/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Sparkii config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Sparkii in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Sparkii`` (prose), but lowercase matches
# like ``openclaw`` → ``sparkii`` (so filesystem paths like ``~/.openclaw``
# become ``~/.sparkii`` — the real Sparkii home — not the broken ``~/.Sparkii``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Sparkii'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Sparkii'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Sparkii'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Sparkii`` but maps ``openclaw`` → ``sparkii`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.sparkii/config.yaml`` (the real Sparkii home) instead of the broken
    ``~/.Sparkii/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Sparkii.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Sparkii memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Sparkii Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-sparkii/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Sparkii config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-sparkii/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Sparkii session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Sparkii config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Sparkii
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Sparkii-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Sparkii-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: sparkii config set",
                        )
                    continue
        ...
        for oc_key, sparkii_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and sparkii_key not in secret_additions:
                secret_additions[sparkii_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, sparkii_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and sparkii_key not in secret_additions:
                        secret_additions[sparkii_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sparkii_config = load_yaml_file(destination)
        current_model = sparkii_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = sparkii_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                sparkii_config["model"] = {"default": model_str}
            dump_yaml_file(destination, sparkii_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Sparkii still uses "edge"
        ...
        sparkii_config = load_yaml_file(destination)
        existing_tts = sparkii_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Sparkii destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Sparkii-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Sparkii-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sparkii_cfg_path = self.target_root / "config.yaml"
        sparkii_cfg = load_yaml_file(sparkii_cfg_path)
        existing_mcp = sparkii_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if srv.get("url"):
                ...
                if srv.get("headers"):
                    sparkii_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    sparkii_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                sparkii_srv["enabled"] = False
            if srv.get("timeout"):
                sparkii_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                sparkii_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                sparkii_srv["tools"] = {}
                if tools_cfg.get("include"):
                    sparkii_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    sparkii_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                sparkii_srv["sampling"] = {
                    k: v for k, v in {
        ...
        if added > 0 and self.execute:
            self.maybe_backup(sparkii_cfg_path)
            sparkii_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(sparkii_cfg_path, sparkii_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'sparkii cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = sparkii_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Sparkii reasoning_effort
        ...
        if compaction:
            compression = sparkii_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = sparkii_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            sparkii_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = sparkii_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'sparkii gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("SPARKII_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sparkii_cfg_path = self.target_root / "config.yaml"
        sparkii_cfg = load_yaml_file(sparkii_cfg_path)
        sr = sparkii_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sparkii_cfg_path = self.target_root / "config.yaml"
        sparkii_cfg = load_yaml_file(sparkii_cfg_path)
        custom_providers = sparkii_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(sparkii_cfg_path)
            sparkii_cfg["custom_providers"] = custom_providers
            dump_yaml_file(sparkii_cfg_path, sparkii_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Sparkii config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            sparkii_cfg_path = self.target_root / "config.yaml"
            sparkii_cfg = load_yaml_file(sparkii_cfg_path)
            discord_sparkii = sparkii_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_sparkii["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_sparkii["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                sparkii_cfg["discord"] = discord_sparkii
                dump_yaml_file(sparkii_cfg_path, sparkii_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        sparkii_cfg_path = self.target_root / "config.yaml"
        sparkii_cfg = load_yaml_file(sparkii_cfg_path)
        browser_sparkii = sparkii_cfg.get("browser") or {}
        changed = False

        # Map fields that have Sparkii equivalents
        if browser.get("cdpUrl"):
            browser_sparkii["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_sparkii["headless"] = browser["headless"]
            changed = True

        if changed:
            sparkii_cfg["browser"] = browser_sparkii
            if self.execute:
                self.maybe_backup(sparkii_cfg_path)
                dump_yaml_file(sparkii_cfg_path, sparkii_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            sparkii_mode = mode_map.get(mode, "manual")
            sparkii_cfg.setdefault("approvals", {})["mode"] = sparkii_mode
            if self.execute:
                self.maybe_backup(sparkii_cfg_path)
                dump_yaml_file(sparkii_cfg_path, sparkii_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{sparkii_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Sparkii Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Sparkii. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Sparkii Config Not Overwritten)",
                "",
                "These items already existed in your Sparkii config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Sparkii agent discovers these",
            "directories, it may read/write to them instead of the Sparkii state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `sparkii claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `sparkii claw cleanup` to fix it.",
            "",
            "## Sparkii-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `sparkii claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `sparkii setup` to configure any remaining settings",
            "- Run `sparkii mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `sparkii cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `sparkii cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `sparkii gateway install` if you need the gateway service",
            "- Review `~/.sparkii/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Sparkii Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("SPARKII_HOME") or str(Path.home() / ".sparkii"), help="Sparkii home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Sparkii targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Sparkii-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Sparkii Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.sparkii/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.sparkii/config.yaml")
        print("    2. Run: sparkii mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: sparkii cron")
