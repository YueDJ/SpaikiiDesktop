...
is typically only in `~/.sparkii/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Sparkii,
...
`sparkii-custom-providers` skill, pitfall 2). Sparkii hits this because it forwards
