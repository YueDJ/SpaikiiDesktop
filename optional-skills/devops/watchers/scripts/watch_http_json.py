...
Usage (via cron with --no-agent):
    ...
    sparkii cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SPARKII_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Sparkii-Watcher/1.0"})
