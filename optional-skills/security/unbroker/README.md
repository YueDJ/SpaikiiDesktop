...
The design is **Sparkii-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
sparkii skills install official/security/unbroker
```

Then start a new Sparkii session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Sparkii session:
...
- **PII handling.** Dossiers live under the Sparkii home directory (`0600`, optionally
