...
## 5. GODMODE FAST — Sparkii 4 405B (Zero Refusal)

**Model:** `nousresearch/sparkii-4-405b`
**Strategy:** Sparkii models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Sparkii Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.sparkii/config.yaml`:
