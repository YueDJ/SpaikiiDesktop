#!/usr/bin/env python3
"""Canvas LMS API CLI for Sparkii Agent.
...
def _check_config():
    ...
    if missing:
        sparkii_env = os.path.join(
            os.environ.get("SPARKII_HOME", os.path.expanduser("~/.sparkii")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {sparkii_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Sparkii Agent"
    )
