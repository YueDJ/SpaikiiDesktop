...
### 1. Start order: editor first, Sparkii second

Sparkii probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Sparkii session.
...
restart the editor, restart the Sparkii session.
...
server fails to bind (Output Log) or Sparkii' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.sparkii/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Sparkii

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Sparkii' connection
lives in `~/.sparkii/config.yaml` via `sparkii mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Sparkii — just don't
mistake it for the Sparkii setup step.
...
explicitly warns against overlapping calls. Sparkii executes same-turn tool
...
### 9. Timeouts: Sparkii gives up before Unreal does

Sparkii' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Sparkii runs elsewhere (SSH backend,
