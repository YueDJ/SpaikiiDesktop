...
Maps each role archetype to the Sparkii skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public sparkii-agent
...
## Sparkii skills relevant to video production

### Visual / rendering skills (`sparkii-agent/skills/creative/`)
...
### Design / pre-production skills (`sparkii-agent/skills/creative/`)
...
### Audio / media skills (`sparkii-agent/skills/creative/` + `skills/media/`)
...
These are **not** Sparkii skills but external CLIs / APIs that profiles invoke.
...
## Built-in Sparkii tools for media review

These are native Sparkii tools — not invoked via terminal but through their own
...
key is present in `${SPARKII_HOME:-~/.sparkii}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Sparkii profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SPARKII_HOME:-~/.sparkii}/.env` → environment variable.
...
When a new Sparkii-public video skill ships:
