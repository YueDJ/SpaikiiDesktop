...
the actual `setup.sh` that creates the project workspace, configures Sparkii
...
2. **Create profiles** — `sparkii profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.sparkii/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `sparkii kanban create` assigned to the director
...
```bash
sparkii profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.sparkii/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.sparkii/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Sparkii
...
Each profile gets a `SOUL.md` at `~/.sparkii/profiles/<name>/SOUL.md` that
...
```bash
sparkii kanban create "Direct production of <video title>" \
    --assignee director \
...
the Sparkii `.env` (`${SPARKII_HOME:-$HOME/.sparkii}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _sparkii_env="${SPARKII_HOME:-$HOME/.sparkii}/.env"
    if grep -q "^${var}=" "$_sparkii_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_sparkii_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_sparkii_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY sparkii ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY sparkii OPENROUTER_API_KEY || exit 1
# ...
```
