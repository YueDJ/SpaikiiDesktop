...
sparkii kanban watch --tenant <project-slug>

# Snapshot of the board
sparkii kanban list --tenant <project-slug>
sparkii kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
sparkii kanban stats --tenant <project-slug>

# Visual dashboard (browser)
sparkii dashboard

# Inspect a specific task (includes comments + events)
sparkii kanban show <task-id>

# Follow a single task's event stream
sparkii kanban tail <task-id>
```

Verify available subcommands with `sparkii kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `sparkii kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `sparkii kanban block <id>` then `sparkii kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `sparkii kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `sparkii kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `sparkii profile list` (profile exists?), `sparkii status` (gateway/dispatcher up?), and verify tenant. |
...
sparkii kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
sparkii kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(sparkii kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
sparkii kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
sparkii kanban block <task-id>
sparkii kanban archive <task-id>

# Diagnose what happened
sparkii kanban show <task-id>      # task body, comments, recent events
sparkii kanban tail <task-id>      # follow the live event stream
sparkii kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `sparkii kanban create` for the director
...
while true; do
    clear
    sparkii kanban list --tenant <slug>
    echo "---"
    sparkii kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `sparkii kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `sparkii profile list` and start
