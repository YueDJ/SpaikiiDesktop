...
three virtual model IDs so the ``sparkii tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `sparkii tools` → Image "
                    "Generation → OpenAI to configure, or `sparkii setup` "
                    "to add the key."
                ),
