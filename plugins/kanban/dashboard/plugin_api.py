...
own pages inject it via ``window.__SPARKII_SESSION_TOKEN__`` so logged-in
...
This means ``sparkii dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def get_board(
    ...
    through to the active board (``SPARKII_KANBAN_BOARD`` env → on-disk
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``sparkii kanban
...
def list_diagnostics(
    ...
    AND for ``sparkii kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``sparkii kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``sparkii kanban specify <task_id>``.
    ...
    # SPARKII_KANBAN_BOARD env var — this endpoint runs in FastAPI's
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``sparkii kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.sparkii/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.sparkii/profiles/*`` on disk and every
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Sparkii
...
def list_kanban_projects():
    """List first-class Sparkii projects for board scoping.
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            ...
            profile_dir = _Path(get_sparkii_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``sparkii profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``sparkii kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.sparkii/config.yaml.
