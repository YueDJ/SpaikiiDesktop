/**
 * Sparkii Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.sparkii/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SPARKII_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SPARKII_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "sparkii-kanban-dot-triage",
    todo: "sparkii-kanban-dot-todo",
    ready: "sparkii-kanban-dot-ready",
    running: "sparkii-kanban-dot-running",
    blocked: "sparkii-kanban-dot-blocked",
    review: "sparkii-kanban-dot-review",
    done: "sparkii-kanban-dot-done",
    archived: "sparkii-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-sparkii-task";
  ...
  const DOCS_URL = "https://sparkii-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://sparkii-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "sparkii.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="sparkii-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])sparkii-kanban-md-code\1/i.test(attrs)) {
      return ' class="sparkii-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "sparkii-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "sparkii-kanban-md",
  ...
  // standard `drop` event and our `sparkii-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("sparkii-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("sparkii-kanban-column--drop");
          if (target) target.classList.add("sparkii-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("sparkii-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("sparkii-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("sparkii-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SPARKII_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "sparkii-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "sparkii-kanban-attention",
        "sparkii-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "sparkii-kanban-attention-bar" },
        h("span", { className: "sparkii-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "sparkii-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "sparkii-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "sparkii-kanban-attention-row",
                  "sparkii-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "sparkii-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "sparkii-kanban-attention-row-id" }, task.id),
                h("span", { className: "sparkii-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "sparkii-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "sparkii-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "sparkii-kanban-diag-action-btn",
      action.suggested ? "sparkii-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " sparkii-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".sparkii-kanban-drawer-comment-row input, .sparkii-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "sparkii-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("sparkii-kanban-diag", sevClass) },
      h("div", { className: "sparkii-kanban-diag-header" },
        h("span", { className: "sparkii-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "sparkii-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "sparkii-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "sparkii-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "sparkii-kanban-diag-data-row" },
                  h("span", { className: "sparkii-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "sparkii-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "sparkii-kanban-diag-data-row" },
                h("span", { className: "sparkii-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "sparkii-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "sparkii-kanban-diag-reassign-row" },
            h("span", { className: "sparkii-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "sparkii-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "sparkii-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "sparkii-kanban-diag-msg",
              msg.ok ? "sparkii-kanban-diag-msg--ok" : "sparkii-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "sparkii-kanban-section" },
      h("div", { className: "sparkii-kanban-section-head-row" },
        h("span", { className: "sparkii-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "sparkii-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "sparkii-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "sparkii-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "sparkii-kanban-docs-link",
      title: "Open Sparkii Kanban docs in a new tab",
      "aria-label": "Sparkii Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `sparkii kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "sparkii-kanban-msg-ok" : "sparkii-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "sparkii-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "sparkii-kanban-boardswitcher" },
      h("div", { className: "sparkii-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "sparkii-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "sparkii-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "sparkii-kanban-dialog-title" },
        ...
        h("div", { className: "sparkii-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Sparkii profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "sparkii-kanban-bulk" },
      h("span", { className: "sparkii-kanban-bulk-count" },
      ...
      h("div", { className: "sparkii-kanban-bulk-priority",
      ...
      h("div", { className: "sparkii-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Sparkii profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "sparkii-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sparkii-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("sparkii-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "sparkii-kanban-trash",
        dragOver ? "sparkii-kanban-trash--drop" : "",
        props.draggingTaskId ? "sparkii-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "sparkii-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "sparkii-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".sparkii-kanban-card")) return true;
      if (target.closest && target.closest(".sparkii-kanban-column-add")) return true;
      if (target.closest && target.closest(".sparkii-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("sparkii-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("sparkii-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".sparkii-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "sparkii-kanban-columns",
        isScrollable ? "sparkii-kanban-columns--scrollable" : "",
        isPanning ? "sparkii-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sparkii-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("sparkii-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "sparkii-kanban-column",
        dragOver ? "sparkii-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "sparkii-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "sparkii-kanban-col-check",
        ...
        h("span", { className: cn("sparkii-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "sparkii-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "sparkii-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "sparkii-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "sparkii-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "sparkii-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "sparkii-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "sparkii-kanban-lane" },
                  h("div", { className: "sparkii-kanban-lane-head" },
                    h("span", { className: "sparkii-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "sparkii-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "sparkii-kanban-card--stale-red";
    if (age >= tier.amber) return "sparkii-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".sparkii-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "sparkii-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "sparkii-kanban-card",
        props.selected ? "sparkii-kanban-card--selected" : "",
        props.failed ? "sparkii-kanban-card--failed" : "",
        props.draggingSource ? "sparkii-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "sparkii-kanban-card-content" },
          h("div", { className: "sparkii-kanban-card-row" },
            h("label", {
              className: "sparkii-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "sparkii-kanban-card-check",
            ...
            h("span", { className: "sparkii-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or sparkii kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "sparkii-kanban-warning-badge",
                    "sparkii-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "sparkii-kanban-needs-assignee",
          ...
          h("div", { className: "sparkii-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "sparkii-kanban-card-row sparkii-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "sparkii-kanban-assignee",
                            title: `Assigned to Sparkii profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "sparkii-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "sparkii-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "sparkii-kanban-ago",
  ...
  // centered modal reusing the sparkii-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "sparkii-kanban-dialog sparkii-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Sparkii profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Sparkii profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "sparkii-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "sparkii-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "sparkii-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "sparkii-kanban-drawer-close",
        ...
        data ? h("div", { className: "sparkii-kanban-drawer-comment-foot" },
          h("div", {
            className: "sparkii-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "sparkii-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "sparkii-kanban-section" },
      h("div", { className: "sparkii-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "sparkii-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "sparkii-kanban-drawer-body" },
      h("div", { className: "sparkii-kanban-drawer-title" },
        h("span", { className: cn("sparkii-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "sparkii-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "sparkii-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "sparkii-kanban-section" },
            h("div", { className: "sparkii-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "sparkii-kanban-section" },
            h("div", { className: "sparkii-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sparkii-kanban-done-no-result sparkii-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "sparkii-kanban-section" },
            h("div", { className: "sparkii-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "sparkii-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "sparkii-kanban-section" },
        h("div", { className: "sparkii-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "sparkii-kanban-comment" },
            h("div", { className: "sparkii-kanban-comment-head" },
              h("span", { className: "sparkii-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "sparkii-kanban-diag-action-btn",
      ...
      h("div", { className: "sparkii-kanban-section" },
        h("div", { className: "sparkii-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "sparkii-kanban-comment" },
                h("div", { className: "sparkii-kanban-comment-head" },
                  h("span", { className: "sparkii-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "sparkii-kanban-comment-ago" },
      ...
      h("div", { className: "sparkii-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "sparkii-kanban-event",
              isDiag ? "sparkii-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "sparkii-kanban-event-header" },
                  h("span", { className: "sparkii-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "sparkii-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "sparkii-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "sparkii-kanban-event-header-plain" },
                  h("span", { className: "sparkii-kanban-event-kind" }, e.kind),
                  h("span", { className: "sparkii-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "sparkii-kanban-event-phantom-row" },
                  h("span", { className: "sparkii-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "sparkii-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "sparkii-kanban-event-payload" },
    ...
    return h("div", { className: "sparkii-kanban-section" },
      h("div", { className: "sparkii-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "sparkii-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `sparkii-kanban-run--${r.outcome || r.status || "ended"}`
          : "sparkii-kanban-run--active";
        return h("div", { key: r.id, className: cn("sparkii-kanban-run", outcomeClass) },
          h("div", { className: "sparkii-kanban-run-head" },
            h("span", { className: "sparkii-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "sparkii-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "sparkii-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "sparkii-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "sparkii-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "sparkii-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "sparkii-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "sparkii-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "sparkii-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "sparkii-kanban-pre sparkii-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "sparkii-kanban-meta-row" },
      h("span", { className: "sparkii-kanban-meta-label" }, props.label),
      h("span", { className: "sparkii-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "sparkii-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sparkii-kanban-meta-row" },
        h("span", { className: "sparkii-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "sparkii-kanban-meta-value sparkii-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sparkii-kanban-meta-row" },
        h("span", { className: "sparkii-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "sparkii-kanban-meta-value sparkii-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sparkii-kanban-meta-row" },
        h("span", { className: "sparkii-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "sparkii-kanban-meta-value sparkii-kanban-editable",
    ...
    return h("div", { className: "sparkii-kanban-meta-row" },
      h("span", { className: "sparkii-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "sparkii-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "sparkii-kanban-section" },
      h("div", { className: "sparkii-kanban-section-head-row" },
        h("span", { className: "sparkii-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "sparkii-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "sparkii-kanban-section" },
      h("div", { className: "sparkii-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "sparkii-kanban-deps-row" },
        h("span", { className: "sparkii-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "sparkii-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "sparkii-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "sparkii-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "sparkii-kanban-dep-chip-x",
      ...
      h("div", { className: "sparkii-kanban-deps-row" },
        h("span", { className: "sparkii-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "sparkii-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "sparkii-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "sparkii-kanban-msg-ok"
          : "sparkii-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "sparkii-kanban-section" },
      ...
      h("div", { className: "sparkii-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "sparkii-kanban-home-sub sparkii-kanban-home-sub--on"
              : "sparkii-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SPARKII_PLUGINS__ && typeof window.__SPARKII_PLUGINS__.register === "function") {
    window.__SPARKII_PLUGINS__.register("kanban", KanbanPage);
  }
})();
