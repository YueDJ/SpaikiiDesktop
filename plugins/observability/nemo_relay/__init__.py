...
class _Runtime:
    ...
    def export_atif(self, state: _SessionState) -> None:
        ...
        thread = threading.Thread(
            target=_runner,
            name=f"sparkii-nemo-relay-atif-export-{state.session_id}",
            daemon=True,
        )
        thread.start()
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("SPARKII_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atif_export_timeout_s=_env_float(
            "SPARKII_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", 30.0
        ),
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("SPARKII_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
