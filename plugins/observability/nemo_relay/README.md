...
Optional Sparkii observability plugin that configures exporters and maps
Sparkii-specific observer hooks to NeMo Relay marks and ATIF state. Sparkii core
...
not replace Sparkii Agent's planner, tools, memory, model provider routing, or
CLI UX. Sparkii core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Sparkii Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Sparkii core.
- Add Sparkii session, turn, approval, and subagent mark events.
...
The `SPARKII_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SPARKII_HOME` that the
agent run will use:

```bash
env SPARKII_HOME=/tmp/sparkii-nemo-relay-test \
  sparkii plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SPARKII_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SPARKII_HOME` is the Sparkii profile/config home used by both
`sparkii plugins enable ...` and the later `sparkii chat ...` run. If unset,
Sparkii uses the user's default home, usually `~/.sparkii`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SPARKII_HOME=/tmp/sparkii-nemo-relay-test
sparkii plugins enable observability/nemo_relay
sparkii chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `sparkii` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run sparkii plugins enable observability/nemo_relay
uv run sparkii chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Sparkii installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/sparkii_agent-*.whl
sparkii plugins enable observability/nemo_relay
```
...
behavior; it does not control Sparkii shared client metrics.
...
`sparkii.session.segment` / `sparkii.session.segment_reason`
...
The plugin can configure exporters directly from `SPARKII_NEMO_RELAY_*`
...
```bash
export SPARKII_NEMO_RELAY_ATOF_ENABLED=1
export SPARKII_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SPARKII_NEMO_RELAY_ATIF_ENABLED=1
export SPARKII_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SPARKII_NEMO_RELAY_ATOF_FILENAME`
- `SPARKII_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SPARKII_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SPARKII_NEMO_RELAY_ATIF_AGENT_NAME`
- `SPARKII_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SPARKII_NEMO_RELAY_ATIF_MODEL_NAME`
- `SPARKII_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Sparkii at it:

```bash
export SPARKII_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Sparkii Agent"
agent_version = "local"
```

When `SPARKII_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SPARKII_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SPARKII_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Sparkii does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Sparkii keeps the direct env-var fallbacks active for that run.

Sparkii core routes provider and tool execution through NeMo Relay managed APIs
...
Sparkii identifies managed LLM operations by provider wire protocol, not by the
...
| Sparkii API mode | Relay operation |
| --- | --- |
| `chat_completions` | `openai.chat_completions` |
| `codex_responses` | `openai.responses` |
| `anthropic_messages` | `anthropic.messages` |

Middleware written against earlier Sparkii builds might match physical provider
names such as `openrouter`, `nous`, or `anthropic` as the operation. Match the
canonical operation instead. Sparkii retains the original provider in the
managed invocation metadata field `sparkii.provider`. Unknown or custom API
modes continue to use the physical provider name as their operation.

### Dynamic Plugins

Sparkii uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Sparkii-owned
...
with this Sparkii-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Sparkii rejects
...
Sparkii activates these plugins before registering its managed LLM and tool
...
For the full generic Sparkii middleware contract, see
...
with Sparkii and a local Ollama model served through the OpenAI-compatible API.

```bash
export SPARKII_HOME=/tmp/sparkii-nemo-relay-docs/sparkii-home
mkdir -p "$SPARKII_HOME"

cat > "$SPARKII_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Sparkii session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SPARKII_NEMO_RELAY_ATOF_ENABLED=1
export SPARKII_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sparkii-nemo-relay-docs/subagent/atof
export SPARKII_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SPARKII_NEMO_RELAY_ATOF_MODE=overwrite
export SPARKII_NEMO_RELAY_ATIF_ENABLED=1
export SPARKII_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sparkii-nemo-relay-docs/subagent/atif
export SPARKII_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SPARKII_NEMO_RELAY_ATIF_AGENT_NAME='Sparkii Agent E2E'
export SPARKII_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SPARKII_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"sparkii.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Sparkii Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Sparkii dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/sparkii-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/sparkii-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/sparkii-nemo-relay-docs/workdir/beta.txt
cd /tmp/sparkii-nemo-relay-docs/workdir

export SPARKII_NEMO_RELAY_ATOF_ENABLED=1
export SPARKII_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sparkii-nemo-relay-docs/parallel/atof
export SPARKII_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SPARKII_NEMO_RELAY_ATOF_MODE=overwrite
export SPARKII_NEMO_RELAY_ATIF_ENABLED=1
export SPARKII_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sparkii-nemo-relay-docs/parallel/atif
export SPARKII_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SPARKII_NEMO_RELAY_ATIF_AGENT_NAME='Sparkii Agent E2E'
export SPARKII_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Sparkii sessions map to `agent` scopes.
- Sparkii core managed provider calls map to `llm` scope start/end events.
- Sparkii core managed tool calls map to `tool` scope start/end events.
...
Sparkii core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Sparkii-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Sparkii calls the
...
Enable it for Sparkii:

```bash
export SPARKII_NEMO_RELAY_PLUGINS_TOML=/tmp/sparkii-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
Sparkii provider call
  -> nemo_relay.llm.execute(...)
    -> Sparkii provider adapter callback(...)

Sparkii tool call
  -> nemo_relay.tools.execute(...)
    -> Sparkii authorization and dispatch callback(...)
```
...
middleware for a local Sparkii run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export SPARKII_HOME=/tmp/sparkii-middleware-test/sparkii-home
mkdir -p "$SPARKII_HOME" /tmp/sparkii-middleware-test/nemo-relay
...
cat > /tmp/sparkii-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/sparkii-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/sparkii-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Sparkii Middleware E2E"
agent_version = "local"
...
export SPARKII_NEMO_RELAY_PLUGINS_TOML=/tmp/sparkii-middleware-test/nemo-relay/plugins.toml
