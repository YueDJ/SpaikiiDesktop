# Sparkii Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: sparkii-achievements plugin
...
### `GET /api/plugins/sparkii-achievements/achievements`
...
### `POST /api/plugins/sparkii-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/sparkii-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/sparkii-achievements/overview`
...
- Persist plugin data under:
  - `~/.sparkii/plugins/sparkii-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.sparkii/plugins/sparkii-achievements/scan_checkpoint.json`
...
- `~/.sparkii/plugins/sparkii-achievements/`
