...
``<sparkii home>/plugins/<name>/``. That tree is the plugin *install* dir —
``sparkii plugins remove`` deletes it and ``sparkii plugins update`` git-pulls
...
``<sparkii home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Sparkii.
...
# Mirrors the plugin-name shape `sparkii plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<sparkii home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`sparkii_constants.get_sparkii_home` on every call. Don't cache the
    result across profile switches.
    """
    from sparkii_constants import get_sparkii_home

    root = get_sparkii_home() / "plugin-data" / _validate_name(name)
