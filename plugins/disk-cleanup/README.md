...
Auto-tracks and cleans up ephemeral files created during Sparkii Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SPARKII_HOME` and `/tmp/sparkii-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SPARKII_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SPARKII_HOME` or `/tmp/sparkii-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SPARKII_HOME/disk-cleanup/` is itself excluded
- `$SPARKII_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
