...
Rules:
  ...
  - empty dirs    → always delete (under SPARKII_HOME)
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SPARKII_HOME/logs/``."""
    return get_sparkii_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SPARKII_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(sparkii_home)
        return True
...
# Paths under $SPARKII_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SPARKII_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        sparkii_home = get_sparkii_home()
        for parent in ("cron", "cronjobs"):
            base = sparkii_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SPARKII_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    try:
        for top in sparkii_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sparkii_home = get_sparkii_home()
    try:
        rel = path.resolve().relative_to(sparkii_home)
