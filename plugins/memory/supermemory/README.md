...
- Hosted: API key from [app.supermemory.ai/integrations?connect=sparkii](http://app.supermemory.ai/integrations?connect=sparkii)
...
Before running `sparkii memory setup`, add the local endpoint to
...
Then run `sparkii memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SPARKII_HOME/supermemory.json`
...
| `container_tag` | `sparkii` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `sparkii-{identity}` → `sparkii-coder`). |
...
`https://api.supermemory.ai`. Sparkii resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: sparkii`, and document writes stamp
`metadata.sm_source: sparkii`. This is a **functional routing key, not telemetry**:
it groups Sparkii-written memories into a dedicated "Sparkii" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Sparkii profile:

```json
{
  "container_tag": "sparkii-{identity}"
}
```

For a profile named `coder`, this resolves to `sparkii-coder`. The default profile resolves to `sparkii-default`. Without `{identity}`, all profiles share the same container.
