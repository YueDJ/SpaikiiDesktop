...
def _save_mem0_json(sparkii_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(sparkii_home) / "mem0.json"
    existing = {}
...
def _setup_platform(sparkii_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if env_writes:
        _write_env(Path(sparkii_home) / ".env", env_writes)
...
def _setup_selfhosted(sparkii_home: str, config: dict, flags: dict[str, str]) -> None:
...
def _setup_oss(sparkii_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(sparkii_home, config)
        return
    ...
    if env_writes:
        _write_env(Path(sparkii_home) / ".env", env_writes)
    _save_mem0_json(sparkii_home, {"mode": "oss", "user_id": user_id, "agent_id": "sparkii", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, sparkii_home: str) -> str:
...
def _setup_oss_interactive(sparkii_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], sparkii_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], sparkii_home)
        if key:
    ...
    if env_writes:
        _write_env(Path(sparkii_home) / ".env", env_writes)
    _save_mem0_json(sparkii_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(sparkii_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(sparkii_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(sparkii_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(sparkii_home, config, flags)
        return
