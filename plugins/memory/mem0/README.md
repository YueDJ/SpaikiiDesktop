...
```bash
sparkii memory setup    # select "mem0"
```
...
Behavioral settings live in `$SPARKII_HOME/mem0.json` (set them via `sparkii memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.sparkii/.env`.
...
| `user_id` | `sparkii-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Sparkii session and call `mem0_search` — it connects to your server.
...
```bash
sparkii memory setup mem0 --mode oss \
...
```bash
sparkii memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SPARKII_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.sparkii/mem0_qdrant"}}
  }
}
```
...
```bash
sparkii memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SPARKII_HOME/mem0.json`).
