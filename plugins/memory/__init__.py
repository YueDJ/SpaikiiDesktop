...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with sparkii-agent)
2. User-installed providers: ``$SPARKII_HOME/plugins/<name>/``
3. Project-local providers: ``./.sparkii/plugins/<name>/``, opt-in via
   ``SPARKII_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``sparkii_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "sparkii_agent.memory_providers"
...
_USER_NAMESPACE = "_sparkii_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_sparkii_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_sparkii_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$SPARKII_HOME/plugins/`` or None if unavailable."""
    try:
        from sparkii_constants import get_sparkii_home
        d = get_sparkii_home() / "plugins"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.sparkii/plugins/`` or None if unavailable or not opted in.

    Gated on ``SPARKII_ENABLE_PROJECT_PLUGINS`` exactly as the general
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($SPARKII_HOME/plugins/<name>/)
    # 3. Project-local providers (./.sparkii/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``sparkii <provider>`` subcommands — working, but a second-class citizen next
...
def load_memory_provider(
    ...
    *,
    ...
    (``$SPARKII_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``sparkii memory status``, the setup picker — leaves no
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _sparkii_user_memory.<name>.cli, usually
