...
- OpenViking server running and reachable from Sparkii

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Sparkii can identify older servers that expose the legacy status-only health
...
connection values into Sparkii, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.sparkii/.env`; for a named profile use
...
OpenViking's server config is separate from Sparkii:
...
Sparkii-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `sparkii` | Sparkii peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Sparkii lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Sparkii sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Sparkii built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Sparkii action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Sparkii
...
file URIs, such as
`viking://user/peers/sparkii/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/sparkii/memories/preferences/mem_abc123.md`. Files
