...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Sparkii peer ID in OpenViking (default: sparkii)
...
_DEFAULT_AGENT = "sparkii"
_AGENT_PROMPT_LABEL = "Sparkii peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Sparkii slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Sparkii to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Sparkii refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("SPARKII_HOME", "")).expanduser() if os.environ.get("SPARKII_HOME") else Path.home() / ".sparkii"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Sparkii did not start "
    ...
    try:
        ...
        # The Sparkii Desktop backend can include the Sparkii venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Sparkii venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Sparkii venv,
        # aborting `sparkii update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Sparkii will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Sparkii session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Sparkii only", "write values only to Sparkii .env"),
    ...
    _print_openviking_ready("Connection saved to Sparkii .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See sparkii-agent#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Sparkii peer ID in OpenViking, sent as the actor peer and "
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Sparkii will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.sparkii/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts sparkii (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Sparkii will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Sparkii' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Sparkii canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See sparkii-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/sparkii/.overview.md).
