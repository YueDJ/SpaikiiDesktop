...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: sparkii)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: sparkii)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $SPARKII_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "sparkii").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Sparkii can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``sparkii memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'sparkii memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SPARKII_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_sparkii_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "sparkii": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "sparkii"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Sparkii config."""
    profile = config.get("profile", "sparkii")
    return str(profile or "sparkii")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Sparkii .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Sparkii profile name (from agent_identity)
      {workspace} — Sparkii workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``sparkii-{user}`` with no user becomes ``sparkii``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Sparkii Agent and the User"
    ...
    def save_config(self, values, sparkii_home):
        """Write config to $SPARKII_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(sparkii_home) / "hindsight"
    ...
    def post_setup(self, sparkii_home: str, config: dict) -> None:
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(sparkii_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "sparkii")
        ...
        self.save_config(provider_config, sparkii_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(sparkii_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(sparkii_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        ...
        bank_id = provider_config.get("bank_id", "sparkii")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "sparkii"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: sparkii-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Sparkii Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "sparkii"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "sparkii"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "sparkii", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "sparkii")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Sparkii Agent and the User")
        ...
        # Sparkii emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Sparkii as a non-root user, or switch "
                    "to cloud / local_external mode via 'sparkii memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Sparkii get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See NousResearch/sparkii-agent#5820.
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See sparkii-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Sparkii created/used that client on the shared async loop,
