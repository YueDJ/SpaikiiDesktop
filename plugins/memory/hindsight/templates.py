...
the ``sparkii`` integration, and applies a chosen manifest to the user's bank
...
def fetch_sparkii_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``sparkii`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "sparkii" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_sparkii_templates()
