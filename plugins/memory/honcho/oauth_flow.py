...
``sparkii://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Sparkii OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "sparkii-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``sparkii-desktop`` / ``sparkii-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Sparkii.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SPARKII_HOME override can't reach.
