...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/sparkii>
...
```bash
sparkii memory setup honcho   # configure Honcho directly (works on a fresh install)
sparkii memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
sparkii config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.sparkii/.env
```

> `sparkii honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `sparkii memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `sparkii model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SPARKII_HOME/honcho.json` | Profile-local (isolated Sparkii instances) |
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `sparkii honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Sparkii session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`sparkii-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`sparkii`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-sparkii-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening sparkii in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Sparkii session ID (timestamp + hex). Every `sparkii` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Sparkii profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
| `SPARKII_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `sparkii-agent`) |
...
|---------|-------------|
| `sparkii memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `sparkii honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `sparkii memory setup`) |
| `sparkii honcho status` | Show resolved config for active profile |
| `sparkii honcho enable` / `disable` | Toggle Honcho for active profile |
| `sparkii honcho mode <mode>` | Change recall or observation mode |
| `sparkii honcho peer --user <name>` | Update user peer name |
| `sparkii honcho peer --ai <name>` | Update AI peer name |
| `sparkii honcho tokens --context <N>` | Set context token budget |
| `sparkii honcho tokens --dialectic <N>` | Set dialectic max chars |
| `sparkii honcho map <name>` | Map current directory to a session name |
| `sparkii honcho sync` | Create host blocks for all Sparkii profiles |
