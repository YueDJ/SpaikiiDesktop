...
Handles: sparkii honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "sparkii" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Sparkii profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'sparkii honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'sparkii honcho setup' first.\n")
        return
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Sparkii profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SPARKII_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_sparkii_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        sparkii_host.get("pinUserPeer"),
        sparkii_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in sparkii_host:
        aliases_src = sparkii_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in sparkii_host:
        prefix_src = sparkii_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(sparkii_host: dict) -> None:
...
def _apply_runtime_prefix(
    sparkii_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        sparkii_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(sparkii_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = sparkii_host.get("userPeerAliases")
    prefix = sparkii_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(sparkii_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(sparkii_host)
    sparkii_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        sparkii_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        sparkii_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/sparkii (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Sparkii persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = sparkii_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            sparkii_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(sparkii_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'sparkii honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'sparkii honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'sparkii honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(sparkii_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'sparkii honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = sparkii_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        sparkii_host["peerName"] = new_peer

    current_ai = sparkii_host.get("aiPeer") or cfg.get("aiPeer", "sparkii")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        sparkii_host["aiPeer"] = new_ai

    current_workspace = sparkii_host.get("workspace") or cfg.get("workspace", "sparkii")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        sparkii_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Sparkii GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, sparkii_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Sparkii gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = sparkii_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(sparkii_host)
            sparkii_host["pinUserPeer"] = False
            if prior_aliases:
                sparkii_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                sparkii_host, current_prefix, prefix_from_root,
            ...
            if merged:
                sparkii_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                sparkii_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                sparkii_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = sparkii_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        sparkii_host["observationMode"] = new_obs
    else:
        sparkii_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(sparkii_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        sparkii_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        sparkii_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = sparkii_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        sparkii_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = sparkii_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        sparkii_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                sparkii_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(sparkii_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            sparkii_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        sparkii_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        sparkii_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        sparkii_host["dialecticReasoningLevel"] = new_reasoning
    else:
        sparkii_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = sparkii_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        sparkii_host["sessionStrategy"] = new_strat

    sparkii_host["enabled"] = True
    sparkii_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        print("  Run: sparkii config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    sparkii honcho status     -- show full config")
    print("    sparkii honcho mode       -- change recall/observation mode")
    print("    sparkii honcho tokens     -- tune context and dialectic budgets")
    print("    sparkii honcho peer       -- update peer names")
    print("    sparkii honcho map <name> -- map this directory to a session name\n")
...
def _apply_grant_to_host(sparkii_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    sparkii_host["apiKey"] = cred.access_token
    sparkii_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        sparkii_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Sparkii profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("sparkii.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: sparkii honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'sparkii honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'sparkii honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: sparkii honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        sparkii = hosts.get(_host_key(), {})
        user = sparkii.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = sparkii.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = sparkii.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = sparkii.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Sparkii' identity in Honcho. Seed with 'sparkii honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: sparkii honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: sparkii honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = sparkii.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = sparkii.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = sparkii.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Sparkii asks Honcho's AI peer a question")
        ...
        print("\n  Set with: sparkii honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'sparkii honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'sparkii honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    sparkii honcho identity --show        — show both peer representations")
        print("    sparkii honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Sparkii + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Sparkii\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Sparkii persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  sparkii honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'sparkii honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'sparkii honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  sparkii honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'sparkii'")
        print("  with Honcho configured and no prior session history.")
        print("  (Sparkii calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    sparkii honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'sparkii honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Sparkii, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'sparkii honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    sparkii honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  sparkii honcho identity <path/to/SOUL.md>")
    ...
    print("    Sparkii:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Sparkii:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Sparkii:   Honcho observes every message and updates representations")
    ...
    print("    Sparkii:   per-session by default — each run gets its own session")
    print("              Map a custom name:  sparkii honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. sparkii honcho setup              — configure API key (required)")
        print("  2. sparkii honcho migrate            — re-run this walkthrough")
    else:
        print("  1. sparkii honcho status             — verify Honcho connection")
        print("  2. sparkii                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. sparkii honcho identity --show    — verify AI peer representation")
        print("  4. sparkii honcho tokens             — tune context and dialectic budgets")
        print("  5. sparkii honcho mode               — view or change memory mode")
    print()
...
def register_cli(subparser) -> None:
    """Build the ``sparkii honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``sparkii honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to sparkii memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Sparkii Honcho",
    )
