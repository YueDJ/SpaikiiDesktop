...
``{title, url, description, position}`` rows every other Sparkii web
provider produces.
...
prefers Sparkii-managed xAI Grok OAuth (via ``sparkii auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.sparkii/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``sparkii tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `sparkii auth` to sign in with "
