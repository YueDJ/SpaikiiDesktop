...
Let the sparkii agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where sparkii runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `sparkii meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `sparkii meet` CLI |
...
| `cli.py` | `sparkii meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SPARKII_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `sparkii meet node {run,list,approve,remove,status,ping}` |
...
sparkii meet install                                      # pip + Chromium
sparkii meet setup                                        # preflight
sparkii meet auth                                         # optional
sparkii meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
sparkii meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.sparkii/.env
sparkii meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
sparkii meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
sparkii meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.sparkii/.env
sparkii meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, sparkii will **not** switch your system audio input automatically — the
...
sparkii plugins enable google_meet
sparkii meet node run --display-name my-mac --host 0.0.0.0 --port 18789
