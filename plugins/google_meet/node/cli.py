"""`sparkii meet node ...` subcommand tree.

Wired into the existing ``sparkii meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``sparkii meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="sparkii-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``sparkii meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             sparkii meet node approve <name> ws://<host>:{args.port} {token}")
        try:
