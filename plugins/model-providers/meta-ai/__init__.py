"""Meta Model API (Muse Spark) provider plugin for Sparkii Agent.
...
Bundled from https://github.com/albertodepaola/sparkii-meta-provider. Sparkii'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to sparkii' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Sparkii' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in sparkii_cli/providers.py and the retention hint in
