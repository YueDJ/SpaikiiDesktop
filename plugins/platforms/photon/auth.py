...
Credential storage mirrors every other Sparkii channel:

    * runtime SDK creds  -> ``~/.sparkii/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.sparkii/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a sparkii dependency
...
# registers Sparkii as its own client_id.
...
# Default name of the project Sparkii provisions for the operator.
DEFAULT_PROJECT_NAME = "Sparkii Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of sparkii-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.sparkii/auth.json`` honouring the active Sparkii profile."""
    try:
        from sparkii_constants import get_sparkii_home
        return Path(get_sparkii_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.sparkii")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # sparkii_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from sparkii_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.sparkii/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.sparkii/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.sparkii/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: sparkii_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `sparkii photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `sparkii photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `sparkii photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `sparkii photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `sparkii photon setup`)"
