...
This plugin connects Sparkii Agent to iMessage (and other Spectrum
...
TypeScript-only, Sparkii runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Sparkii channel,
...
Run `sparkii photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.sparkii/.env` (the same place every other
...
Management metadata lives in `~/.sparkii/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `sparkii photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Sparkii
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Sparkii coalesces
...
- **Native polls are supported.** Sparkii posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `sparkii send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<sparkii-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `sparkii photon status`, a DM and a group roundtrip,
