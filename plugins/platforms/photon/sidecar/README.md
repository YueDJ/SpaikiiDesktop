...
Small Node helper that bridges Sparkii Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Sparkii is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Sparkii-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Sparkii can receive inbound messages
...
The Sparkii plugin's `sparkii photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Sparkii is Python, so replies go through this sidecar
