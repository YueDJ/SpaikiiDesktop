// Sparkii Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Sparkii): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Sparkii -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Sparkii-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `sparkii photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Sparkii can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "sparkii-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Sparkii'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Sparkii allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Sparkii, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-sparkii-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Sparkii supplied them so a known-good
