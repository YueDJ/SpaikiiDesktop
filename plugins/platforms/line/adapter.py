"""
LINE Messaging API platform adapter for Sparkii Agent.
...
to Sparkii Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SPARKII_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            sparkii_home = Path(get_sparkii_home()).resolve()
        except Exception:
            sparkii_home = Path.home().joinpath(".sparkii").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``sparkii status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``sparkii status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sparkii setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.sparkii/.env`` via ``sparkii_cli.config``.
    """
    print()
    ...
    try:
        from sparkii_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("sparkii_cli.config not available; set LINE_* vars manually in ~/.sparkii/.env")
        return
