"""SimpleX Chat platform adapter (Sparkii plugin).
...
This adapter ships as a Sparkii platform plugin under
``plugins/platforms/simplex/``. The Sparkii plugin loader scans the
...
Optional environment variables:
    ...
    SPARKII_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``sparkii setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Sparkii — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SPARKII_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sparkii setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.sparkii/.env`` via
    ``sparkii_cli.config``.
    """
    print()
