"""
IRC Platform Adapter for Sparkii Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Sparkii agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sparkii-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Sparkii Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Sparkii Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: sparkii_, sparkii_1, sparkii_2...
...
def check_requirements() -> bool:
    ...
    # sparkii setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `sparkii gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Sparkii to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #sparkii — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.sparkii/.env")
    print_info("Restart the gateway for changes to take effect: sparkii gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sparkii-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "sparkii-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Sparkii Agent (cron)")
