"""
Email platform adapter for the Sparkii gateway.

Allows users to interact with Sparkii by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "sparkii-agent" "version" "{_sparkii_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `sparkii config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `sparkii gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Sparkii Agent")
        ...
        msg_id = f"<sparkii-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# sparkii_cli/gateway.py, and the _send_email dispatch in
