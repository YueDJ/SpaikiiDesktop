# A2A — Agent-to-Agent protocol for Sparkii
...
peer (another Sparkii, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
sparkii gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Sparkii serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.sparkii/a2a_audit.jsonl`.
- Conversations persist to `~/.sparkii/a2a_conversations/` — they survive context
