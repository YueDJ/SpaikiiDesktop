...
# ``model_tools`` import time, so ``sparkii status``, ``sparkii chat``, etc.
...
# ``sparkii_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Sparkii is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a sparkii MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "sparkii-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SPARKII_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _sparkii_home = _get_sparkii_home()
        except (ModuleNotFoundError, ImportError):
            _sparkii_home = _Path.home() / ".sparkii"
        self._thread_count_store = _ThreadCountStore(
            _sparkii_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Sparkii is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SPARKII_HOME", str(_Path.home() / ".sparkii"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SPARKII_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a sparkii MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Sparkii is thinking…' marker message.
        ...
        "Sparkii is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Sparkii is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Sparkii is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Sparkii is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``sparkii setup``.

    The setup wizard at ``sparkii_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.sparkii/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. sparkii-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.sparkii/.env")
    print_info("Restart the gateway: sparkii gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``sparkii cron`` running as a
    separate process from ``sparkii gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Sparkii plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `sparkii setup` to install Google Chat support.",
        ...
        # the "Sparkii is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Sparkii is thinking…' marker message "
