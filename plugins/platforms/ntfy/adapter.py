"""ntfy platform adapter (Sparkii plugin).
...
a Sparkii dependency.

This adapter ships as a Sparkii platform plugin under
``plugins/platforms/ntfy/``. The Sparkii plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "sparkii-in"              # subscribe topic (incoming)
          publish_topic: "sparkii-out"     # optional — defaults to topic
...
_ECHO_TAG = "sparkii-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``sparkii cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Sparkii dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `sparkii gateway status` without
