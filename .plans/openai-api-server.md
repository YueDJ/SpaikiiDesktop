# OpenAI-Compatible API Server for Sparkii Agent
...
SSE streaming. By exposing this endpoint, sparkii-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  sparkii-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.sparkii/.env`
2. Run `sparkii gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with sparkii-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns sparkii-agent as a model) |
...
Authorization: Bearer sparkii-api-key-here
...
But sparkii-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "sparkii-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured sparkii-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of sparkii-agent's core prompt
...
Once implemented, sparkii-agent works as a drop-in backend for:
