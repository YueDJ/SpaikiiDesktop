# Contribuir a Sparkii Agent

¡Gracias por contribuir a Sparkii Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Sparkii funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Sparkii. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es oficial y útil pero no universalmente necesaria (ej., una integración de servicio de pago, una dependencia pesada), ponla en **`optional-skills/`** — se envía con el repositorio pero no está activada por defecto. Los usuarios pueden descubrirla a través de `sparkii skills browse` (etiquetada como "oficial") e instalarla con `sparkii skills install` (sin advertencia de terceros, confianza integrada).

Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [Discord de Nous Research](https://discord.gg/NousResearch). Los usuarios pueden instalarla con `sparkii skills install`.

---

## Proveedores de Memoria: Publicar como Plugin Independiente

**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.sparkii/plugins/` (o a través de un entry point de pip).
...
│
├── sparkii_cli/               # Implementaciones de comandos CLI
...
### Configuración del usuario (almacenada en `~/.sparkii/`)

| Ruta | Propósito |
|------|-----------|
| `~/.sparkii/config.yaml` | Configuración (modelo, terminal, toolsets, compresión, etc.) |
| `~/.sparkii/.env` | Claves API y secretos |
| `~/.sparkii/auth.json` | Credenciales OAuth (Nous Portal) |
| `~/.sparkii/skills/` | Todas las habilidades activas (incluidas + instaladas desde hub + creadas por el agente) |
| `~/.sparkii/memories/` | Memoria persistente (MEMORY.md, USER.md) |
| `~/.sparkii/state.db` | Base de datos de sesiones SQLite |
| `~/.sparkii/sessions/` | Índice de enrutamiento del gateway (`sessions.json`), migas de pan de solicitudes, transcripciones `*.jsonl` del gateway y (opcionalmente) snapshots JSON por sesión cuando `sessions.write_json_snapshots: true` está configurado. Los snapshots por sesión están desactivados por defecto; state.db es canónica. |
| `~/.sparkii/cron/` | Datos de trabajos programados |
| `~/.sparkii/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`sparkii_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_SPARKII_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Sparkii o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Sparkii usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.sparkii/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `sparkii_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Sparkii se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Sparkii tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `sparkii` y ejercita la ruta de código que cambiaste
...
- Usa [GitHub Issues](https://github.com/YueDJ/SparkiiDesktop/issues)
