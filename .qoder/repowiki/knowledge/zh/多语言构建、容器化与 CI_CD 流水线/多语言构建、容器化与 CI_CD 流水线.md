...
- **Shell 安装器**：`scripts/install.sh`（Linux/macOS/Termux）与 `install.ps1`（Windows）负责探测系统、安装 uv/Python/Node、克隆仓库、创建 venv、安装依赖、配置 FHS 布局（root 下 `/usr/local/lib/hermes-agent`）。
...
### 容器进程模型
- s6-overlay 作为 PID 1，管理 main hermes、dashboard、per-profile gateway 三个服务。
- `entrypoint-dispatch.sh` 兼容被外部 init（Fly Machines、`docker run --init`）包裹的场景，回退到 `stage2-hook.sh` + `main-wrapper.sh` 路径。
- 非 root 用户 `hermes`（UID 10000），数据目录 `/opt/data` 通过 volume 持久化。

### 安装布局
- 普通用户：`~/.hermes/hermes-agent`，命令链接到 `~/.local/bin/hermes`。
- Linux root：FHS 布局 `/usr/local/lib/hermes-agent` + `/usr/local/bin/hermes`，数据仍在 `$HERMES_HOME`。
- Termux：代码留在 `$HERMES_HOME/hermes-agent`，命令链接到 `$PREFIX/bin`。

### 版本与发布
- Python 版本约束 `>=3.11,<3.14`（防止 uv 自动选择 3.14 导致 Rust 依赖无 wheel）。
- Node 版本统一为 26（Dockerfile 注释标注 pin 在多处）。
- 构建时注入 `HERMES_GIT_SHA` 到 `.hermes_build_sha`，供 `hermes dump` 报告实际 commit。
...
- **Docker 构建不可变**：`COPY --chmod=a+rX,go-w . .` 将源码层设为只读，配合 `HERMES_LAZY_INSTALL_TARGET=/opt/data/lazy-packages` 确保运行时只能向数据卷写入。
- **Nix 沙箱构建**：`SPARKII_NIX_BUILD=1` 环境变量是绕过 `setup.py` 构建守卫的唯一合法方式，由 `nix/python.nix` 设置。
- **Playwright 浏览器预装**：构建期安装 Chromium 到 `/opt/hermes/.playwright`，避开运行时 `/opt/data` 挂载覆盖导致的丢失问题。
