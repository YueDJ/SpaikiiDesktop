...
G->>G : 构建会话ID/加载历史(可选X-Sparkii-Session-Id)
...
- 其他行为
  ...
  - 支持通过 X-Sparkii-Session-Id 继续已有会话并加载历史；需要 API Key 鉴权。
  - 支持通过 X-Sparkii-Session-Key 限定长期记忆范围。
...
- 会话历史
  - 当未提供 X-Sparkii-Session-Id 时，系统基于首条用户消息与系统提示派生稳定会话 ID。
  - 当提供 X-Sparkii-Session-Id 且已配置 API Key 时，会从状态数据库加载历史消息。
...
- 提供商特殊参数
  - Gemini：
    - thinking_config：将 Sparkii/OpenRouter 风格的 reasoning_config 翻译为 Gemini 的 thinkingConfig（includeThoughts、thinkingLevel、thinkingBudget）。
...
- 常见错误
  ...
  - Invalid session ID / Session ID too long：检查 X-Sparkii-Session-Id 的安全性与长度。
- 调试建议
