...
- 功能
  - 记录允许清单内的计数器（如 hermes.model_route.count、hermes.tool_call.count 等）
...
- 应用业务指标
  ...
  - sparkii.cron.jobs.*（enabled、running、overdue）
  - 客户端共享指标：hermes.model_route.count、hermes.tool_call.count、hermes.task_run.started/finished、hermes.skill.lifecycle/load/count、hermes.client.active
- 自定义业务指标
