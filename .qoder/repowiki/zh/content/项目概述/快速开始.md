...
## 简介
本指南面向首次接触 Hermes Agent 的用户，目标是在最短时间内完成环境准备、安装与基础配置，并通过 CLI 启动对话、切换模型、启用工具等常见操作，帮助你快速体验核心能力。Hermes 支持在 Linux、macOS、Windows（原生 PowerShell）上安装，并提供一键脚本自动处理 Python、Node.js、Git 等依赖。

## 项目结构
- 安装入口：跨平台安装脚本位于 scripts/install.sh（Linux/macOS/WSL/Termux）与 scripts/install.ps1（Windows）。
- 运行时入口：CLI 主程序由 sparkii_cli/main.py 提供，暴露 hermes/sparkii 命令集。
...
- 使用一键脚本安装：
  - 在终端执行 curl 管道脚本，将自动安装 uv、Python、Node、Git、克隆仓库、创建 venv、安装依赖并写入 hermes 命令。
  - 安装完成后刷新 shell（source ~/.bashrc 或 ~/.zshrc），然后运行 hermes 进入交互模式。
...
### 安装步骤（Windows 原生）
- 在 PowerShell 中执行安装脚本，将自动安装 uv、Python、Node、Git（MinGit）、克隆仓库、创建 venv、安装依赖并写入 hermes 命令。
- 安装路径位于 %LOCALAPPDATA%\hermes；若已存在系统 Git，则复用该 Git。
...
### 基本配置
- 模型提供商与 API 密钥：
  - 通过 hermes model 选择提供商与模型；或通过 hermes setup 一次性配置。
  - 密钥通常保存在 ~/.sparkii/.env（或对应平台的 .env），也可在 config.yaml 中设置（注意敏感信息优先来自环境变量）。
- 工具启用：
  - 使用 hermes tools 启用或禁用工具；部分工具需额外依赖（如浏览器工具需要 Chromium）。
- 终端与环境：
...
- 启动对话：
  - 直接运行 hermes 进入交互式聊天。
- 切换模型：
  - 使用 /model 或 hermes model 选择提供商与模型。
- 使用工具：
  - 在对话中使用斜杠命令或自然语言触发工具；必要时先通过 hermes tools 启用。
- 其他常用命令：
  - hermes gateway：启动消息网关（Telegram、Discord、Slack、WhatsApp、Signal 等）。
  - hermes config set/get：查看或设置配置项。
  - hermes doctor：诊断问题。
...
### 验证安装成功
- 运行 hermes 或 sparkii 应能进入交互界面或显示帮助信息。
- 执行 hermes doctor 检查依赖与配置是否正常。
...
## 结论
通过官方安装脚本，你可以在 Linux、macOS、Windows 上快速搭建 Hermes Agent 的运行环境，并使用 CLI 进行对话、模型切换与工具调用。建议在首次安装后运行 hermes doctor 验证环境，并根据需求调整配置（模型、工具、压缩、记忆等）。遇到问题时，优先查看日志与诊断输出，并按故障排除指南逐步定位。
