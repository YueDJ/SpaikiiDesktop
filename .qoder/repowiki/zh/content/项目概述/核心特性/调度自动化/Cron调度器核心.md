...
## 简介
本文件面向Hermes Agent的Cron调度器核心，系统性说明任务调度引擎的架构与实现：tick机制、任务解析与时间表达式支持、任务生命周期管理（创建—执行—完成）、并发控制（并行执行池与顺序执行池）、任务锁与文件锁（多进程互斥）、失败处理与重试策略、监控与可观测性、配置示例与调优建议。目标是让非专业读者也能理解并安全运维该子系统。
