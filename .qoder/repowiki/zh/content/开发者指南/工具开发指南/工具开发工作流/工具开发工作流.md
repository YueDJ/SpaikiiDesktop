...
- 配置：复制示例配置到 ~/.hermes/config.yaml，设置 .env 中的密钥。
- 运行与诊断：hermes doctor、hermes chat -q 快速验证；日志位于 ~/.hermes/logs。
...
- 环境变量：确保 ~/.hermes/.env 中设置了必要的密钥；配置文件 ~/.hermes/config.yaml 正确。
- 日志定位：查看 ~/.hermes/logs 下的日志，结合 hermes doctor 输出定位问题。
