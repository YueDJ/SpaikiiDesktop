...
- Nix 开发壳：nix/devShell.nix 聚合常用开发工具，设置 PATH、HERMES_PYTHON_SRC_ROOT、VIRTUAL_ENV，并提供一键启动提示。
...
- 安装步骤（Linux/macOS/WSL2/Termux）：
  - 通过脚本安装 uv、Python、Node.js、Git，并在 $HERMES_HOME/hermes-agent 下克隆源码。
...
- Nix 用户：
  - 使用 nix develop 进入 devShell，已预装 uv、playwright-test、ghostty 等，并导出 HERMES_PYTHON_SRC_ROOT 与 VIRTUAL_ENV。
...
RunPS --> CreateVenv
UseNix --> SetEnv["设置 PATH/VIRTUAL_ENV/HERMES_PYTHON_SRC_ROOT"]
...
### IDE 配置建议
- 解释器与 venv：
  - 将项目根目录下的 .venv 或 ~/.hermes/hermes-agent/venv 设置为解释器。
  - Windows 注意 Scripts/python.exe；Linux/macOS 使用 bin/python。
- 调试器：
  - VS Code：使用内置 Debug Adapter，配置 launch.json 指向 hermes 入口或 pytest 会话。
...
- 权限问题：
  - Linux 根安装使用 FHS 布局，代码位于 /usr/local/lib/hermes-agent，命令链接到 /usr/local/bin；数据仍位于 $HERMES_HOME。
...
## 故障排查指南
- 无法找到 Python/uv：
  - 确认安装脚本成功安装 uv 与 Python 3.11；检查 $HERMES_HOME/bin 是否在 PATH。
...
## 附录
- 快速参考
  - 安装（Linux/macOS/WSL2/Termux）：curl 安装脚本 → 进入 $HERMES_HOME/hermes-agent → uv pip install -e ".[all,dev]"
