...
- [hermes-achievements/manifest.json](file://plugins/hermes-achievements/dashboard/manifest.json)
- [hermes-achievements/plugin_api.py](file://plugins/hermes-achievements/dashboard/plugin_api.py)
...
## 简介
本教程面向希望在 Hermes Agent 仪表盘中开发 Web 插件的开发者，涵盖从项目初始化、前端组件与后端 API 集成、状态管理与主题定制，到打包部署与用户体验优化的完整流程。仓库内已内置两个典型仪表板插件示例：成就系统（hermes-achievements）与看板管理（kanban），可作为参考实现。

## 项目结构
Hermes 仪表板插件采用“前后端分离 + 清单驱动”的组织方式：
...
图表来源
- [hermes-achievements/manifest.json:1-12](file://plugins/hermes-achievements/dashboard/manifest.json#L1-L12)
...
- [hermes-achievements/plugin_api.py:560-671](file://plugins/hermes-achievements/dashboard/plugin_api.py#L560-L671)
...
### 成就系统插件（hermes-achievements）
- 功能概述
  - 扫描本地 Hermes 会话历史，基于行为指标解锁徽章
...
图表来源
- [hermes-achievements/plugin_api.py:560-671](file://plugins/hermes-achievements/dashboard/plugin_api.py#L560-L671)
- [hermes-achievements/plugin_api.py:674-755](file://plugins/hermes-achievements/dashboard/plugin_api.py#L674-L755)
- [hermes-achievements/plugin_api.py:758-800](file://plugins/hermes-achievements/dashboard/plugin_api.py#L758-L800)
...
## 结论
通过清单驱动的插件体系，Hermes 仪表板提供了灵活、可扩展的前后端扩展能力。借助现有示例（成就系统与看板），开发者可以快速上手，遵循统一的注册、路由与鉴权规范，实现高质量的用户体验与稳定的性能表现。
