...
participant CMD as "sparkii/sparkii 命令"
Dev->>SH : 运行安装脚本
SH->>OS : 检测系统/发行版
SH->>UV : 安装或复用 uv
...
SH->>CMD : 将 sparkii/sparkii 加入 PATH
Dev-->>CMD : 执行 sparkii / sparkii
```
...
T["安装脚本<br/>install.sh / install.ps1"] --> U["命令可用<br/>sparkii/sparkii"]
```
...
- Windows
  ...
  - 验证命令可用（sparkii --help 或 sparkii --help）
- Node.js 与前端
...
- Python
  ...
  - 运行入口：run_agent.py 或 sparkii CLI
- Node.js
...
### 常见环境变量与配置位置
- 用户数据目录：~/.sparkii（或 Windows 下 %LOCALAPPDATA%\sparkii）
- 配置文件：config.yaml（行为设置）、.env（仅密钥）
- 日志：~/.sparkii/logs/（agent.log、errors.log、gateway.log）
