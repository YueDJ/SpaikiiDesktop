...
U->>WS : 发起 /api/* 请求
alt 本地回环绑定
WS-->>U : 注入会话令牌，校验 X-Hermes-Session-Token
else 非回环绑定
...
- 认证策略：
  - 本地回环模式：通过注入的会话令牌（X-Hermes-Session-Token 或 Bearer）校验敏感接口。
...
AuthMode --> |否| Token["会话令牌校验<br/>X-Hermes-Session-Token/Bearer"]
Gate --> Next["继续处理请求"]
Token --> Next
Next --> End(["响应"])
```
...
- 数据卷：挂载 ~/.hermes 到容器内 /opt/data，保证配置与状态持久化。
