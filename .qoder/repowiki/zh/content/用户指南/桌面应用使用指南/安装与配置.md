...
## 简介
本指南面向首次安装和配置桌面应用（Sparkii Desktop）的用户，覆盖系统要求、多种安装方式、首次启动配置流程、配置文件与环境变量、跨平台注意事项以及常见问题排查。推荐通过 Sparkii CLI 一键安装并引导完成配置；也提供预构建安装包下载与手动安装路径。
...
- 桌面应用：Electron + React 前端，连接后端运行时（sparkii serve/tui_gateway），支持首次运行向导、远程连接与本地安装引导。
...
## 架构总览
桌面应用启动时按优先级探测可用后端：环境变量指向的开发目录、当前源码检出、已完成的托管安装、PATH 中的 sparkii、系统 Python 可导入运行时，最后回退到首次启动引导安装。成功后以 headless 模式运行 sparkii serve，并通过 tui_gateway 暴露 JSON-RPC/WebSocket API 供渲染层使用。

```mermaid
sequenceDiagram
participant U as "用户"
participant D as "桌面应用(Electron)"
participant R as "后端运行时(sparkii serve)"
...
- 安装完成后，执行 sparkii 进入交互界面，或使用 sparkii setup 一次性配置提供商、模型与工具。
...
- 安装后同样通过 sparkii desktop 启动，或在首次启动时选择“连接现有 Sparkii”或“本地安装引导”。
...
### 首次启动配置流程
- 连接现有 Sparkii 服务：若检测到已有网关，可在首次启动时选择连接，应用会测试 HTTP 与 WebSocket 连通性并保存加密连接信息。
...
Check -- 是 --> Connect["连接现有 Sparkii 服务"]
...
### 配置文件位置与结构
- 主配置文件：~/.sparkii/config.yaml（或 ~/.sparkii/config.yaml，取决于安装布局），用于设置模型、终端、压缩、内存、会话策略等。
- 密钥与环境变量：~/.sparkii/.env（或 ~/.sparkii/.env），存放 API Key 与平台通道配置。
...
- Linux：
  - 安装器会根据发行版自动安装 Git 与依赖；root 安装采用 FHS 布局，代码位于 /usr/local/lib/sparkii-agent，命令链接至 /usr/local/bin。
...
- 桌面应用依赖 Electron、React 与共享库，后端通过 sparkii serve 暴露 API。
...
E["桌面应用<br/>Electron + React"] --> F["后端运行时<br/>sparkii serve"]
...
- Windows Defender/杀毒软件误报 uv.exe：
  ...
  - 将 sparkii bin 目录加入白名单，而非仅针对单个文件哈希。
...
## 结论
通过 Sparkii CLI 的一键安装与首次启动引导，用户可以快速完成提供商与模型配置，并使用桌面应用获得流畅的对话体验。对于受限或企业环境，可借助环境变量与配置选项进行定制。遇到问题时，参考本指南的跨平台注意事项与故障排除步骤，通常能迅速定位并解决。

## 附录
- 常用命令参考：
  - sparkii：启动交互界面
  - sparkii model：选择提供商与模型
  - sparkii tools：配置工具集
  - sparkii config set/get：读写配置项
  - sparkii gateway：启动消息网关
  - sparkii setup：一次性设置向导
