...
A --> C["远程路径: 文件式 RPC"]
B --> D["子进程: script.py + sparkii_tools.py"]
...
- 工具白名单与 RPC 存根：SANDBOX_ALLOWED_TOOLS 限定可调用工具；generate_sparkii_tools_module 动态生成 sparkii_tools.py 供脚本导入。
- 两种 RPC 传输：
...
alt 本地执行
Exec->>Child : 启动子进程(临时目录+sparkii_tools.py)
...
- 功能要点
  ...
  - 根据终端后端选择本地或远程路径。
  - 本地：生成 sparkii_tools.py 与 script.py，启动子进程，建立 UDS/TCP 连接。
...
participant Client as "RPC客户端(嵌入sparkii_tools.py)"
...
Prefix --> |是| KeepP["保留"]
Prefix --> |否| Sparkii{"精确SPARKII_*白名单?"}
Sparkii --> |是| KeepH["保留"]
Sparkii --> |否| Win{"Windows必需变量?"}
Win --> |是| KeepW["保留"]
Win --> |否| Drop2["丢弃"]
```
...
- 其他
  - PYTHONPATH：注入 sparkii-agent 根目录与临时目录，便于 import。
...
### 实际示例与最佳实践
- 执行脚本
  - 在 sparkii_tools 中导入允许的工具（如 web_search、read_file、write_file、search_files、patch、terminal），打印最终结果到 stdout。
