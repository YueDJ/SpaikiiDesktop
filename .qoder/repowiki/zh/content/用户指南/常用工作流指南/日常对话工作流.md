...
## 简介
本指南面向初学者与进阶用户，系统讲解如何在 SparkiiDesktop（Hermes Agent）中快速开始日常对话工作流：启动 CLI、选择模型、设置会话；深入说明上下文压缩与记忆系统的机制与最佳实践；介绍如何使用内置工具增强对话体验（文件操作、代码执行、网络搜索等）；提供常见场景示例（代码开发辅助、文档编写、数据分析）；并给出性能优化建议（上下文管理、工具调用策略、内存使用优化）。
