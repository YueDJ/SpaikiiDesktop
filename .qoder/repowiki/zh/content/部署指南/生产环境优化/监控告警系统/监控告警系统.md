...
- 参考命令与配置位置：
  - 启用与查看状态：见 docs/observability/monitoring.md 中的配置与 hermes monitoring status。
...
### 告警规则配置建议
- 阈值与级别
  - 网关存活：hermes.gateway.up == 0 持续 N 分钟 → critical
  - 平台下线：hermes.platform.up == 0 → critical
  - 平台降级：hermes.platform.degraded == 1 → error/warning（依据 error_class）
  - 调度心跳老化：hermes.cron.scheduler.heartbeat_age_seconds > 180 → warning
  - 最近成功老化：hermes.cron.scheduler.last_success_age_seconds > 300 → warning
  - 作业过期：hermes.cron.jobs.overdue > 0 → warning
  - 积压增加：increase(hermes.cron.scheduler.catch_up_occurrences[15m]) > 0 → info/warning
- 通知渠道
...
### 健康检查端点与故障检测
- 健康检查
  - 通过指标 hermes.gateway.up 与 hermes.platform.up 判断服务可用性；结合 heartbeat_age/last_success_age 判断调度健康。
- 故障检测
