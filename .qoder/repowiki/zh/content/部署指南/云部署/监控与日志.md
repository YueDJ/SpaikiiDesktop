...
## 简介
本指南面向运维与平台工程团队，提供在 Hermes Agent/Gateway 中启用并配置可观测性的完整方案。内容覆盖：
...
- 监控未生效
  ...
  - 使用 hermes monitoring status 查看姿态
- 指标/事件未到达后端
...
### 告警规则配置
- 指标告警
  - hermes.gateway.up == 0（网关不可用）
  - hermes_platform_up == 0（本地桥不可用）
  - hermes_cron_scheduler_heartbeat_age_seconds > 180（调度心跳过期）
  - hermes_cron_jobs_overdue > 0（任务逾期）
- 事件告警
