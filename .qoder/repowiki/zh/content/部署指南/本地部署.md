...
## 简介
本指南面向希望在本地（Windows、macOS、Linux）搭建并运行 Sparkii Agent 的开发者与用户。内容涵盖：
- 各平台安装要求与前置依赖（Python、Node.js、系统工具）
- 使用 setup-sparkii.sh 的一键安装流程（环境变量、配置文件、初始引导）
...
图表来源
- [setup-sparkii.sh:169-267](file://setup-sparkii.sh#L169-L267)
...
## 核心组件
- 安装器与引导
  - Linux/macOS/WSL/Termux：setup-sparkii.sh 或 scripts/install.sh
...
- CLI 与入口
  - 命令名：sparkii（通过脚本创建符号链接到 venv/bin/sparkii）
  - 子命令：model、tools、config、gateway、setup、doctor 等

章节来源
- [setup-sparkii.sh:329-396](file://setup-sparkii.sh#L329-L396)
...
S->>N : 准备 Node 工作区可选
S->>H : 创建 sparkii 命令链接
U->>H : 运行 sparkii setup
H->>G : 初始化配置/向导
U->>H : 运行 sparkii / sparkii gateway / TUI
```

图表来源
- [setup-sparkii.sh:133-182](file://setup-sparkii.sh#L133-L182)
- [setup-sparkii.sh:236-267](file://setup-sparkii.sh#L236-L267)
- [setup-sparkii.sh:348-396](file://setup-sparkii.sh#L348-L396)
- [scripts/install.sh:549-653](file://scripts/install.sh#L549-L653)

## 详细组件分析

### 安装器与一键安装（setup-sparkii.sh）
- 功能要点
  ...
  - 将 sparkii 命令软链到用户可访问目录（~/.local/bin 或 $PREFIX/bin）
  - 同步内置 skills 到 ~/.sparkii/skills
  - 提示是否立即运行 setup 向导
- 关键行为与位置
  - Python 版本与 venv 创建：[setup-sparkii.sh:133-182](file://setup-sparkii.sh#L133-L182)
  - 依赖安装（优先 uv.lock）：[setup-sparkii.sh:236-267](file://setup-sparkii.sh#L236-L267)
  - 环境变量与 .env 处理：[setup-sparkii.sh:329-342](file://setup-sparkii.sh#L329-L342)
  - PATH 与命令链接：[setup-sparkii.sh:348-396](file://setup-sparkii.sh#L348-L396)
  - Skills 同步与完成提示：[setup-sparkii.sh:399-462](file://setup-sparkii.sh#L399-L462)
...
PipFallback --> EnvFile
EnvFile --> LinkCmd["创建 sparkii 命令链接"]
LinkCmd --> Skills["同步内置 skills"]
Skills --> Wizard{"是否运行 setup 向导?"}
Wizard -- 是 --> RunSetup["执行 sparkii setup"]
Wizard -- 否 --> End(["结束"])
RunSetup --> End
```

图表来源
- [setup-sparkii.sh:66-127](file://setup-sparkii.sh#L66-L127)
- [setup-sparkii.sh:133-182](file://setup-sparkii.sh#L133-L182)
- [setup-sparkii.sh:236-267](file://setup-sparkii.sh#L236-L267)
- [setup-sparkii.sh:329-396](file://setup-sparkii.sh#L329-L396)
- [setup-sparkii.sh:399-462](file://setup-sparkii.sh#L399-L462)
...
- Linux/macOS/WSL/Termux
  - 推荐使用 scripts/install.sh，支持更多选项（如 --no-venv、--skip-setup、--dir、--sparkii-home 等）
...
- 启动聊天（TUI）
  - 命令：sparkii
- 选择模型/工具/配置
  - sparkii model、sparkii tools、sparkii config set/get
- 启动消息网关（Telegram/Discord/Slack 等）
  - sparkii gateway（前台）或 sparkii gateway install（后台服务）
- 诊断与更新
  - sparkii doctor、sparkii update
- 开发模式
...
- 路径配置
  - 确认 sparkii 命令可被当前 shell 找到；必要时手动 source 对应 shell 配置文件
...
## 结论
通过 setup-sparkii.sh 或平台专用安装器，可在主流操作系统上快速完成 Python/Node 环境、依赖与 CLI 的本地部署。配合 .env 与 cli-config.yaml 完成密钥与行为配置后，即可使用 sparkii 进入 TUI 或启动 Gateway。开发模式下，借助 venv、pytest 与 run_tests.sh 可高效进行调试与测试。

## 附录
- 常用命令速查
  - sparkii：启动聊天（TUI）
  - sparkii model：切换模型/provider
  - sparkii tools：启用/禁用工具集
  - sparkii config set/get：读写配置项
  - sparkii gateway：启动消息网关（前台/后台）
  - sparkii setup：运行完整设置向导
  - sparkii doctor：诊断环境问题
- 开发测试
