...
- 进程监督与特权降级
  ...
  - 启动时以 root 执行 stage2-hook，完成 UID/GID 重映射与数据卷权限修复后，再以 sparkii 用户运行服务。
- 数据持久化与只读代码树
  - /opt/sparkii 为只读安装树，/opt/data 为持久化数据卷，所有可写状态位于数据卷。
...
participant Service as "sparkii/dashboard"
...
Hook-->>S6 : 初始化完成
S6->>Service : 启动 sparkii/dashboard
...
- 安全加固
  ...
  - 将 /opt/sparkii 设为只读，可写状态全部置于 /opt/data，防止运行时篡改代码与依赖。
  - 非 root 用户 sparkii 运行服务，并通过 s6-setuidgid 进行特权降级。
...
- 阶段二钩子职责
  - 拒绝不支持的 --user 方式启动，强制通过 SPARKII_UID/SPARKII_GID 或 PUID/PGID 别名进行用户映射。
  - 创建并修复 /opt/data 下各子目录与文件的所有权，确保 sparkii 用户可读写。
...
```mermaid
flowchart TD
S(["启动阶段二钩子"]) --> CheckUser{"是否以非 sparkii 且非 root 启动?"}
CheckUser --> |是| Error["报错并退出<br/>建议使用 SPARKII_UID/PUID"]
...
- Deployment
  - 使用 sparkii-agent 镜像，设置副本数、滚动更新策略。
...
### 容器安全最佳实践
- 非 root 运行
  - 默认以 sparkii 用户运行服务，通过 s6-setuidgid 进行特权降级。
  - 禁止使用 --user 绑定任意 UID，改用 SPARKII_UID/SPARKII_GID 或 PUID/PGID。
- 最小化镜像
...
## 故障排查指南
- 启动失败与权限问题
  - 若以非 root 且非 sparkii 用户启动，会收到明确错误提示；应改为通过 SPARKII_UID/SPARKII_GID 或 PUID/PGID 进行映射。
