...
## 简介
本文件面向Sparkii Agent的长期记忆系统，系统性阐述其存储架构、语义索引构建、跨会话知识检索、用户画像构建、技能自动创建机制、记忆压缩与去重策略、外部记忆提供商集成（Honcho、OpenViking、Mem0等），以及与工具系统的深度集成方式。文档同时为初学者提供基础概念，并为高级用户提供存储架构与算法实现细节。
