...
## 简介
本文详细阐述 Hermes Agent 的响应标准化处理机制，即将不同 LLM 提供商的原始响应统一转换为 NormalizedResponse 标准格式的核心技术。由于不同提供商（OpenAI、Anthropic、Bedrock、Codex 等）的响应格式各异，标准化处理是系统多提供商支持的关键。
...
## 结论
响应标准化处理是 Hermes Agent 多提供商支持的基石。通过统一的 NormalizedResponse 格式，上层逻辑无需关心具体提供商的实现细节，实现了真正的提供商无关架构。
