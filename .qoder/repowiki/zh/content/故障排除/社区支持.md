...
A --> C["社区论坛<br/>Discord"]
A --> D["官方文档站点"]
A --> E["诊断命令<br/>sparkii doctor / sparkii debug share"]
B --> F["维护者评审与修复"]
C --> F
E --> F
```
...
### 问题报告与最佳实践
- 使用模板：
  - Bug 报告：包含问题描述、复现步骤、期望与实际行为、受影响组件、平台、Sparkii 版本、Python 版本、Debug Report 链接、操作系统、附加日志与可选根因分析与修复建议。
  - 功能请求：说明问题或使用场景、提出方案、考虑过的替代方案、类型与范围、是否愿意贡献。
  - 安装/设置帮助：描述问题、已执行步骤、安装方式、操作系统、Python/Sparkii 版本、Debug Report、完整错误输出与已尝试方法。
- 必要信息收集：
  - 版本与环境：sparkii version、python --version、操作系统。
  - 复现步骤：最小化可复现的命令序列与输入。
  - 日志与诊断：优先使用 sparkii debug share 生成报告链接；必要时附额外 traceback。
- 日志格式化：
...
### 常用调试工具与资源
- 诊断命令：
  - sparkii doctor：检查环境、配置、证书、数据库状态、过时项并提供修复建议。
  - sparkii debug share：采集系统信息与日志摘要，脱敏后上传并返回分享链接；支持 --local 本地查看。
- 工具级调试：
...
## 故障排查指南
- 快速自检：
  - 运行 sparkii doctor 检查环境与健康度。
  - 运行 sparkii debug share 生成报告链接，附带 agent.log、gateway.log 等。
- 常见问题定位：
