...
## 简介
本“常见问题解答”聚焦安装失败、配置错误、权限问题、环境依赖（Python/Node.js）、以及自动化工具（如 hermes doctor）诊断结果的解读与修复。文档按操作系统（Windows、Linux、macOS）分类，提供分步骤的诊断与修复流程，并给出命令示例与配置文件修改说明。同时汇总社区常见问题的最佳实践建议。

## 项目结构
本项目包含跨平台安装脚本、CLI 诊断工具、启动引导模块、配置管理与错误类型定义等关键部分：
- 安装脚本：scripts/install.sh（Linux/macOS/Termux）、scripts/install.ps1（Windows）
- CLI 诊断：sparkii_cli/doctor.py（hermes doctor）
...
- 启动引导（sparkii_bootstrap.py）：在 Windows 上启用 UTF-8 输出、抑制平台版本查询的窗口闪烁与解码崩溃、防止当前目录包遮蔽 Hermes 模块、激活持久化懒加载目标。
...
- 诊断与修复
  ...
  - Windows：通过 install.ps1 安装 uv、Python、Node、Git（含便携 Git Bash），并确保 %LOCALAPPDATA%\hermes\bin 在 PATH。
  - Termux：使用 .[termux] 或 .[termux-all] 额外集，避免不兼容依赖。
- 命令参考
  - Linux/macOS：curl 管道执行安装脚本；source ~/.bashrc 后运行 hermes。
  - Windows PowerShell：iex (irm ...) 执行安装脚本；完成后运行 hermes。
- 注意
  - 安装脚本会清理继承的 PYTHONPATH/PYTHONHOME，避免模块遮蔽。
  - 根用户 Linux 安装采用 FHS 布局，代码位于 /usr/local/lib/hermes-agent，命令链接至 /usr/local/bin。
...
- 命令参考
  - 使用 hermes config get/set 查看和修改配置项。
  - 使用 hermes setup 重新运行向导。
- 注意
...
- 原因
  - Windows 控制台默认非 UTF-8；platform.win32_ver 调用触发 cmd 子进程；当前目录包遮蔽 Hermes 模块。
- 诊断与修复
  - 启动时自动启用 UTF-8 模式与重配置 stdio；抑制 platform._syscmd_ver 以避免闪烁与解码崩溃；加固 sys.path 防止遮蔽。
- 命令参考
  - 直接运行 hermes；如遇编码问题，确认已正确安装并更新到最新版本。
...
- 诊断与修复
  - 使用 hermes doctor 检查证书；必要时使用 --fix 强制重装 certifi；或手动 pip install --force-reinstall certifi。
...
- 诊断与修复
  - 使用 hermes doctor 查看数据库大小、WAL 状态与持有者数量；若处于易受攻击的 WAL 模式，升级 SQLite 或执行修复提示。
...
- 原因
  - 仍在使用旧版配置键或环境变量。
- 诊断与修复
  - 使用 hermes doctor 报告弃用项，并按提示迁移到新键（如 terminal.cwd、display.tool_progress）。
- 命令参考
  - hermes doctor
...
- 诊断与修复
  - 使用 hermes doctor 查看工具集可用性；为 CLI 启用的工具集补充 API Key 或切换 OAuth。
- 命令参考
  - hermes tools
  - hermes model
...
- 安装失败
  ...
  - Windows 下确认 PATH 包含 %LOCALAPPDATA%\hermes\bin。
- 配置错误
...
- 证书与网络
  - 运行 hermes doctor --fix；必要时重装 certifi；检查自定义证书路径。
- 数据库与 WAL
...
## 结论
通过统一的安装脚本、健壮的启动引导、完善的诊断工具与安全配置管理，大多数安装、配置与权限问题可在数分钟内定位与修复。建议优先使用 hermes doctor 进行健康检查，并结合日志与配置备份快速恢复。遵循最佳实践（最小权限、显式路径、定期维护数据库）可显著提升稳定性与可维护性。

## 附录
- 常用命令
  - 安装：Linux/macOS 使用 curl 管道安装脚本；Windows 使用 PowerShell 安装脚本。
  - 诊断：hermes doctor；必要时加 --fix。
  - 配置：hermes config get/set；hermes setup。
  - 模型与工具：hermes model；hermes tools。
- 社区资源
