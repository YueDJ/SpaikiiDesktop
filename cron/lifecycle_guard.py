...
``sparkii gateway restart`` (or ``launchctl kickstart ai.sparkii.gateway``
or ``systemctl restart sparkii-gateway``).  When the cron fires, the
...
``sparkii cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``sparkii gateway``, ``launchctl ... sparkii-gateway``,
``systemctl ... sparkii-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_SPARKII_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``sparkii gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `sparkii gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:sparkii\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a sparkii-gateway label. macOS launchd
    # labels look like `ai.sparkii.gateway` / `sparkii-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated sparkii services (e.g.
    # `launchctl unload ai.sparkii.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.sparkii.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a sparkii-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bsparkii[.\-]?gateway)"
    # Branch D: pkill / kill targeting the sparkii gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.sparkii.gateway-... \` / `  -- ...`,
...
# for `systemctl restart sparkii-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart sparkii-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.sparkii.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart sparkii-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart sparkii-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".sparkii"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<SPARKII_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.sparkii/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_sparkii_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_sparkii_home() falls back to Path.home(), which raises when
        # neither SPARKII_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "sparkii gateway restart"
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.sparkii/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `sparkii gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `sparkii gateway restart` from a shell outside "
