...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``sparkii://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``sparkii://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"sparkii://blueprint/{quote(blueprint.key)}{qs}"
