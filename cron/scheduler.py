...
Uses a file-based lock (~/.sparkii/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `sparkii update` reloads
# the module) fail with ModuleNotFoundError for sparkii_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `sparkii fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `sparkii cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Sparkii, pin it explicitly: "
                f"`sparkii cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``sparkii tools`` config for the ``cron`` platform.
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``SPARKII_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("SPARKII_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid SPARKII_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _sparkii_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_sparkii_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_sparkii_home())
...
# (SPARKII_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse SPARKII_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("SPARKII_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid SPARKII_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_sparkii_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_sparkii_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.sparkii/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``sparkii update`` / ``sparkii gateway stop`` / systemd restart, or an
...
_sparkii_home: Path | None = None


def _get_sparkii_home() -> Path:
    """Resolve Sparkii home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active SPARKII_HOME at call time
    ...
    """
    return _sparkii_home or get_sparkii_home()
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Sparkii — {task_name}"
    try:
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # SPARKII_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `sparkii cron run` from the CLI,
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SPARKII_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SPARKII_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    SPARKII_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SPARKII_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid SPARKII_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within SPARKII_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Sparkii-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SPARKII_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_sparkii_home() / "scripts"
    ...
    # escape — scripts MUST reside within SPARKII_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `sparkii setup`), or pin a "
            "working provider via `sparkii cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`sparkii setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    #
    # Bounded with its own timeout (separate from SPARKII_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("SPARKII_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid SPARKII_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    #     (notify_on_complete / watch_patterns) reads SPARKII_SESSION_PLATFORM
    #     and SPARKII_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SPARKII_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SPARKII_SESSION_PLATFORM == "telegram".
    ...
    _ctx_tokens = set_session_vars(
        ...
        # SPARKII_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ SPARKII_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "SPARKII_CRON_AUTO_DELIVER_PLATFORM",
        "SPARKII_CRON_AUTO_DELIVER_CHAT_ID",
        "SPARKII_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["SPARKII_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `sparkii chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with SPARKII_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $SPARKII_KANBAN_TASK -- letting
        ...
        if delivery_target:
            _VAR_MAP["SPARKII_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SPARKII_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SPARKII_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > SPARKII_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``sparkii cron edit --model``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"SPARKII_MODEL={os.getenv('SPARKII_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`sparkii cron edit {job_id} --model <name>` or set a "
                "default with `sparkii model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from sparkii_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("SPARKII_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_sparkii_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject SPARKII_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Sparkii "
                        "pin it explicitly: "
                        f"`sparkii cron edit {job_id} --provider <provider> "
        ...
        agent = AIAgent(
            ...
            # SPARKII_HOME. When a workdir is configured, also inject project
        ...
        # override via SPARKII_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from sparkii_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
def run_one_job(
    ...
    *,
    ...
    profile_home = _get_sparkii_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_sparkii_home())
        )
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`sparkii pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `sparkii resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `sparkii cron run` that claimed a job
        ...
        # Set SPARKII_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SPARKII_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SPARKII_CRON_MAX_PARALLEL value; defaulting to unbounded")
