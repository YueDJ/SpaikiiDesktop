...
A *suggestion* is a ready-to-run cron job spec that Sparkii surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.sparkii/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_sparkii_home() (profile home), not the
# shared default root. See cron/jobs.py for the full rationale.
CRON_DIR = get_sparkii_home().resolve() / "cron"
