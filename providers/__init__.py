...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with sparkii-agent)
2. User plugins: ``$SPARKII_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``sparkii_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SPARKII_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SPARKII_HOME/plugins/model-providers/`` if it exists."""
    try:
        from sparkii_constants import get_sparkii_home

        d = get_sparkii_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SPARKII_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_sparkii_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``sparkii_agent.plugins``
    ...
    * **Provider targets only.** The ``sparkii_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SPARKII_HOME``) keep their
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SPARKII_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``sparkii_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SPARKII_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with sparkii-agent.
    ...
    # 2. User plugins — under $SPARKII_HOME/plugins/model-providers/<name>/.
