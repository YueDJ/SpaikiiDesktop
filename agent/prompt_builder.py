...
def _find_sparkii_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .sparkii.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SPARKII_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Sparkii Agent, an intelligent AI assistant created by Nous Research. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sparkii/kanban.db`. Your task id is in "
    "`$SPARKII_KANBAN_TASK`; your workspace is `$SPARKII_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SPARKII_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SPARKII_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SPARKII_KANBAN_BRANCH:-wt/$SPARKII_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SPARKII_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`sparkii profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sparkii kanban <verb>` for board operations. Use "
...
#
# The sparkii-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to sparkii-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Sparkii YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Sparkii launches that runtime.\n\n"
        ...
        "run `sparkii computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Sparkii "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Sparkii floating over another application, so the
    user is rarely asking about Sparkii — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Sparkii "
        ...
        "rather than anything inside Sparkii. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Sparkii terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Sparkii desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.sparkii.send(\"prompt\") — or a "
        "data-sparkii-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Sparkii WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Sparkii itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Sparkii runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Sparkii itself is running. The host OS, home, and cwd "
                f"of the Sparkii process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Sparkii "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Sparkii
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sparkii_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SPARKII_PLATFORM") or os.environ.get("SPARKII_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sparkii/skills/`` directory.  External dirs
    ...
    # SPARKII_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_sparkii_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.sparkii/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_sparkii_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Sparkii Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `sparkii-agent` skill "
            "first. It has the actual commands (e.g. `sparkii config set …`, `sparkii tools`, "
            "`sparkii setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: sparkii setup, sparkii setup tools, sparkii setup terminal, sparkii status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from SPARKII_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the SPARKII_HOME ContextVar falls
    ...
    _home = Path(home_override) if home_override is not None else get_sparkii_home()
...
def build_context_files_prompt(
    ...
    SOUL.md from SPARKII_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Sparkii install/source
    ...
    # explicitly configured cwd is honored verbatim — the Sparkii tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Sparkii in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Sparkii install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from SPARKII_HOME only — skip when already loaded as identity
    if not skip_soul:
