...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Sparkii with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.sparkii/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Sparkii
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sparkii tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``sparkii setup`` for
