...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SPARKII_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "SPARKII_KANBAN_DB", "SPARKII_KANBAN_WORKSPACES_ROOT", "SPARKII_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SPARKII_KANBAN_",
    "SPARKII_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``sparkii_cli.config._parse_env_value``) but never mutates the process
    ...
    from sparkii_cli.config import _parse_env_value
...
def build_profile_secret_scope(sparkii_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(sparkii_home)
    secrets = load_env_file(home / ".env")

    try:
        from sparkii_cli.env_loader import get_secret_source_values
