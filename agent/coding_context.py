"""Coding-context awareness — base Sparkii, every interactive surface.

When the user runs Sparkii inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Sparkii shifts into a **coding posture**. This module is the
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "sparkii", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _SPARKII_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``SPARKII_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Sparkii should operate in its coding posture right now."""
