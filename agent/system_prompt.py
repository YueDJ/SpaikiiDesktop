...
upstream prefix cache warm.  See ``sparkii-agent-dev``'s
...
from sparkii_constants import get_default_sparkii_root, get_sparkii_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- sparkii-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SPARKII_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SPARKII_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the SPARKII_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound SPARKII_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_sparkii_root` (NOT ``get_sparkii_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_sparkii_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Sparkii never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SPARKII_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the SPARKII_HOME ContextVar
    ...
    # Pointer to the sparkii-agent skill + docs for user questions about Sparkii itself.
    stable_parts.append(SPARKII_AGENT_HELP_GUIDANCE)
    ...
    # SPARKII_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Sparkii, any interactive coding surface in a code
    ...
    # `sparkii -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['sparkii-bots']).
    ...
    # Active-profile hint — names the Sparkii profile the agent is running
    # under so it doesn't conflate ~/.sparkii/skills/ (default profile) with
    # ~/.sparkii/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient SPARKII_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_sparkii_root(): get_sparkii_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_sparkii_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_sparkii_root())
    else:
        _home_str = _root_str = str(get_sparkii_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_sparkii_root()), which in ambient
        # profile mode is NOT get_sparkii_home().
        profile_home = _home_str
        default_root = get_default_sparkii_root()
        post_workspace_parts.append(
            f"Active Sparkii profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Sparkii). Every other surface (desktop chat panel,
    ...
    from sparkii_time import get_timezone as _sparkii_tz, now as _sparkii_now
    now = _sparkii_now()
