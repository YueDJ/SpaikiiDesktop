...
row taxonomy is inferred from the atlas shape so Sparkii can render both legacy
...
# rows (1536x1872), while older Sparkii/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Sparkii' activity state names. They are not always identical to the
...
# Legacy Sparkii/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Sparkii activity names -> accepted row-name aliases in descending
