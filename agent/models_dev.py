...
# ---------------------------------------------------------------------------
# Provider ID mapping: Sparkii ↔ models.dev
# ---------------------------------------------------------------------------

# Sparkii provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Sparkii' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Sparkii ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_sparkii_ids(mdev_id: str) -> List[str]:
    """Return the Sparkii provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for sparkii_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(sparkii_id)
...
def _get_etag_path() -> Path:
    ...
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "models_dev_cache.etag"
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Sparkii provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Sparkii id.
    for sparkii_id in _models_dev_to_sparkii_ids(provider_key):
        if sparkii_id != provider_key:
            candidates.append(sparkii_id)
...
def _get_provider_models(
    ...
    """Resolve a Sparkii provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Sparkii agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Sparkii provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Sparkii ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Sparkii or models.dev provider ID.  Tries exact match then
