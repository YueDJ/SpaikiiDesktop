"""Anthropic Messages API adapter for Sparkii Agent.

Translates between Sparkii's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in sparkii_cli/runtime_provider.py.
    """
...
# Sparkii effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to sparkii's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to sparkii's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SparkiiAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://sparkii-agent.nousresearch.com")
        headers.setdefault("X-Title", "Sparkii Agent")
        headers.setdefault("User-Agent", f"SparkiiAgent/{_SPARKII_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Sparkii loads that into the process env from
    # ``~/.sparkii/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Sparkii notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Sparkii historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``sparkii models``) never mutates ``~/.sparkii/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Sparkii)
      ...
      5. Anthropic credential_pool OAuth entry (~/.sparkii/auth.json)
    ...
    # 1. Sparkii-managed OAuth/setup token env var
    ...
    # 5. Sparkii credential_pool OAuth entry.
...
# ── Sparkii-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.sparkii/.anthropic_oauth.json (our own file).
...
def run_sparkii_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Sparkii with your Claude Pro/Max subscription.")
    print()
...
def read_sparkii_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Sparkii OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See sparkii-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See sparkii-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See sparkii-agent#13848 (Kimi) and
    sparkii-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Sparkii Agent", "Claude Code")
                text = text.replace("Sparkii agent", "Claude Code")
                text = text.replace("sparkii-agent", "claude-code")
        ...
        #      a) bare Sparkii-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Sparkii surfaces in its CLI. We
