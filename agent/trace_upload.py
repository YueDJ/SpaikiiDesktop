"""Upload a Sparkii session transcript to Hugging Face as an agent trace.

Sparkii stores sessions in its own SQLite store (``sparkii_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``sparkii trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Sparkii' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "sparkii-traces"
_SPARKII_VERSION = "sparkii-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Sparkii OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Sparkii' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Sparkii conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.sparkii/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `sparkii trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/sparkii-traces`` dataset. Returns a
