...
# request needs the bearer key below.  ``sparkii egress reload`` is the
# client.
#
...
# credential (Sparkii' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.sparkii/proxy/`` just to
    check whether a pidfile exists.
    """
    from sparkii_constants import get_sparkii_home

    return get_sparkii_home() / "proxy"
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``sparkii proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="sparkii-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "sparkii-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``sparkii egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="sparkii-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "sparkii-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="sparkii-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `sparkii egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `sparkii egress setup` and use "
            "`sparkii egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `sparkii egress setup`, "
            "then `sparkii egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``sparkii egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `sparkii egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a Sparkii dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``sparkii egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``sparkii egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_SPARKII_IRON_PROXY_NONCE_ENV = "SPARKII_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_SPARKII_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `sparkii egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `sparkii egress setup` first."
        )
    ...
    env[_SPARKII_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Sparkii
    # process.
    #
    ...
    # ``sparkii egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``sparkii egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `sparkii egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`sparkii egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `sparkii secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
