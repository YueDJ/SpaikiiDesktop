...
Plugins built on Sparkii Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Sparkii
...
:class:`~sparkii_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Sparkii' main config
...
handles every provider, fallback chain, and per-task override Sparkii
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sparkii_cli.plugins.PluginContext`
