...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Sparkii starts and finds ``~/.openclaw/``.

    Points users at ``sparkii claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sparkii claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Sparkii, run "
        "`sparkii claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sparkii claw cleanup` (renames it to ~/.openclaw.pre-migration — "
