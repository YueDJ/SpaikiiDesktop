...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=sparkii-agent",
        "client=sparkii-client-v<__version__>",
    ]

The version is sourced live from ``sparkii_cli.__version__`` so it auto-aligns
...
``sparkii_cli.__version__`` is the canonical source of truth.
"""
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=sparkii-agent", sparkii_client_tag()]
