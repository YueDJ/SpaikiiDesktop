...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `sparkii backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "sparkii").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'sparkii memory setup' to walk the user through configuration.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SPARKII_HOME.

        ``sparkii backup`` only walks SPARKII_HOME, so any provider state kept
        ...
        of the archive, and ``sparkii import`` restores them to their original
