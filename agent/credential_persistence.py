...
``auth.json``.  They intentionally have no dependency on ``sparkii_cli.auth`` so
...
# Sources Sparkii owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Sparkii-owned OAuth/device-code state)
