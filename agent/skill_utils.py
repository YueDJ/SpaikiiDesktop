...
from sparkii_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.sparkii/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    SPARKII_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SPARKII_KANBAN_TASK`` / ``SPARKII_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SPARKII_KANBAN_TASK") or os.getenv("SPARKII_KANBAN_BOARD"):
        ...
        # The Sparkii Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``sparkii_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SPARKII_PLATFORM`` or
            ``SPARKII_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("SPARKII_PLATFORM")
        or get_session_env("SPARKII_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``sparkii config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``sparkii`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sparkii/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``sparkii`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SPARKII_HOME, not cwd
        if not p.is_absolute():
            p = (sparkii_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sparkii/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.sparkii/skills`` at the git root) are
...
#   <root>/.sparkii/skills/   — Sparkii-native location
...
# a one-line "run `sparkii skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when SPARKII_HOME itself lives inside a git checkout:
    ``<root>/.sparkii/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``sparkii skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`sparkii skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under SPARKII_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_sparkii_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.sparkii/skills/`` (including via symlinks) or configured
    ...
    # ~/.sparkii/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Sparkii can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sparkii = metadata.get("sparkii") or {}
    if not isinstance(sparkii, dict):
        sparkii = {}
    return {
        "fallback_for_toolsets": sparkii.get("fallback_for_toolsets", []),
        "requires_toolsets": sparkii.get("requires_toolsets", []),
        "fallback_for_tools": sparkii.get("fallback_for_tools", []),
        "requires_tools": sparkii.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    sparkii = metadata.get("sparkii")
    if not isinstance(sparkii, dict):
        return []
    raw = sparkii.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Sparkii metadata, VCS, virtualenv/dependency, cache, and skill
