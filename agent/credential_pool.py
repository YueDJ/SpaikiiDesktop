...
# un-throttled log fires on *every* selection. On Windows several Sparkii
...
# readiness handshake ("Timed out connecting to Sparkii backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``sparkii model`` / ``sparkii auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Sparkii process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``sparkii`` invocation that defaults to the active provider
        (e.g. setup wizard, ``sparkii auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Sparkii
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``SPARKII_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "SPARKII_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "SPARKII_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("sparkii_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Sparkii
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("sparkii_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Sparkii process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Sparkii profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `sparkii model` / `sparkii auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `sparkii model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``sparkii auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `sparkii auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Sparkii PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `sparkii setup` ("Claude
        ...
        # Claude Code CLI, sparkii_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.sparkii/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `sparkii setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"sparkii_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_sparkii_oauth_credentials

        for source_name, creds in (
            ("sparkii_pkce", read_sparkii_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `sparkii auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (sparkii auth remove copilot gh_cli) must not pay either on
...
# Prefer ~/.sparkii/.env over os.environ — the user's config file is the
# authoritative source for Sparkii credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_sparkii_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `sparkii auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.sparkii/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.sparkii/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `sparkii auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Sparkii
