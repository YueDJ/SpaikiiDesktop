...
Storage
-------
Bundles live in ``~/.sparkii/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SPARKII_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``sparkii bundles``)
...
def _bundles_dir() -> Path:
    ...
    Honors ``SPARKII_BUNDLES_DIR`` for tests; falls back to
    ``<SPARKII_HOME>/skill-bundles``.
    """
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `sparkii bundles` CLI subcommand.
# ---------------------------------------------------------------------------
