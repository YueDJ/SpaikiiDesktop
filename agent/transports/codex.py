...
# Wire-name used when Sparkii keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Sparkii's configured provider
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # "ultra" is Sparkii-internal ladder vocabulary (the Codex product
        ...
        if params.get("is_xai_responses", False):
            ...
            # out at high. max/ultra are Sparkii ladder aliases for "this
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp Sparkii' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs Sparkii web providers.
        #
        ...
        #    keep Sparkii dispatch so ``web.backend`` / ``web.search_backend``
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Sparkii dispatches
