...
Owns one Codex thread per Sparkii session. Drives `turn/start`, consumes
...
# Sparkii' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SPARKII_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Sparkii/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Sparkii session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SPARKII_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SPARKII_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Sparkii' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Sparkii' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own sparkii-tools callback we
            # auto-accept — the user already approved Sparkii' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Sparkii
        approval-mode/timeout logic. The Sparkii-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Sparkii approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Sparkii approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Sparkii returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Sparkii approval mode/timeout resolution
    ...
    # "prompt expired"; the Sparkii-side messaging already distinguishes them.
    return "decline"
