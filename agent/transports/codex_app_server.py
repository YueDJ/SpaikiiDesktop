...
projection into Sparkii' display, approval bridging, transcript projection into
...
"codex_app_server"`. Sparkii' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Sparkii secret — gateway
        ...
        spawn_env = sparkii_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SPARKII_KANBAN_TASK"):
            kanban_db = spawn_env.get("SPARKII_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SPARKII_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SPARKII_HOME", os.path.expanduser("~/.sparkii")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "sparkii",
        client_title: str = "Sparkii Agent",
