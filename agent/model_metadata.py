...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Sparkii also honours SPARKII_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("SPARKII_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_sparkii_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Sparkii Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Sparkii from probing-down to
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Sparkii minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from sparkii_constants import get_sparkii_home
    return get_sparkii_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Sparkii grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    # ``Sparkii-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Sparkii sends to providers:
