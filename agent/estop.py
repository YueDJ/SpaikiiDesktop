...
``sparkii pause`` writes a sentinel file at ``$SPARKII_HOME/ESTOP``;
``sparkii resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Sparkii is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.sparkii/ESTOP``.
...
def _sparkii_home() -> Path:
    """Resolve the active SPARKII_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SPARKII_HOME."""
    return _sparkii_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SPARKII_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Sparkii is paused ({reason}). New work is on hold; "
            "run `sparkii resume` to pick things back up."
        )
    return (
        "⏸️ Sparkii is paused. New work is on hold; "
        "run `sparkii resume` to pick things back up."
    )
