...
as ``kind: backend``) or ``~/.sparkii/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``sparkii tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SPARKII_HOME/cache/videos/``, creating parents as needed."""
    from sparkii_constants import get_sparkii_home

    path = get_sparkii_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SPARKII_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SPARKII_HOME/cache/videos/``.
