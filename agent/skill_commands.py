...
#
# When a user invokes a /skill (or /bundle), Sparkii expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_sparkii_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``SPARKII_PLATFORM`` env var and
    ``SPARKII_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("SPARKII_PLATFORM")
            or get_session_env("SPARKII_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("SPARKII_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Sparkii home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_sparkii_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.sparkii.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_sparkii_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.sparkii/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Sparkii slash command (name or alias). The
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Sparkii home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.sparkii/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``sparkii -s <skill>`` or
    a deployment's ``SPARKII_TUI_SKILLS`` env var could force-load a skill an
