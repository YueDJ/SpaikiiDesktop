...
(async () => {
  ...
  const svgContent = fs.readFileSync(path.resolve(__dirname, 'sparkii-favicon.svg'), 'utf-8');
  ...
  const tmp = path.resolve(__dirname, '_rf.html');
  ...
  for (const size of sizes) {
    ...
    const outPath = path.resolve(__dirname, 'sparkii-favicon-' + size + '.png');
    await page.screenshot({ path: outPath, omitBackground: true });
    console.log('sparkii-favicon-' + size + '.png (' + fs.statSync(outPath).size + ' bytes)');
    await page.close();
  }
  fs.unlinkSync(tmp);
  await browser.close();
