#!/usr/bin/env python3
"""
SQLite State Store for Sparkii Agent.
...
_STATE_DB_GUARD_BYPASS_ENV = "SPARKII_STATE_DB_GUARD_BYPASS"
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Sparkii root for the guard.
...
#: redirects ``SPARKII_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Sparkii test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SPARKII_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SPARKII_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SPARKII_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``sparkii`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Sparkii home *root*.
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SPARKII_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Sparkii root {root}). "
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``sparkii sessions optimize`` (FTS merge + VACUUM)
    ...
    ``sparkii_cli/kanban_db.py`` already bounds its WAL growth with
...
def _wal_reset_repair_hint() -> str:
    ...
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Sparkii-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Sparkii"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Sparkii-managed install (uv-managed venv)
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``sparkii doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Sparkii host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``sparkii serve`` backend, interactive CLI sessions,
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Sparkii home tree (``get_sparkii_home()``) — the safe repair scope:
      Sparkii owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Sparkii needs read-write access to open the database. "
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Sparkii
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Sparkii process held the "
    ...
    # offline repair path behind ``sparkii sessions repair-routing``.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Sparkii, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Sparkii session.

        A Sparkii session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Sparkii session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `sparkii sessions vacuum` runs in a transient
