...
<p align="center">
  <a href="https://github.com/YueDJ/SparkiiAgent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
<tr><td><b>Voice</b></td><td>Talk to Sparkii and hear it back.</td></tr>
...
### Install with Sparkii (recommended)

Already have the Sparkii CLI? Just run:

```bash
sparkii desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Sparkii gateway or install Sparkii locally. Local onboarding then walks you through choosing a provider and model.
...
```bash
# throwaway SPARKII_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Sparkii Agent runtime into `SPARKII_HOME`
(`~/.sparkii`, or `%LOCALAPPDATA%\sparkii` on Windows), using the same layout as a
CLI install.
...
- **Sparkii Agent** runs as a headless `sparkii serve` process and exposes the
...
Desktop supports a managed local backend, explicit remote gateways, and Sparkii
...
screen offers **Connect to existing Sparkii** before starting the local installer.
...
Boot logs land in `SPARKII_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\sparkii\sparkii-agent\.sparkii-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\sparkii\sparkii-agent\venv"
```

> The default Sparkii home on Windows is `%LOCALAPPDATA%\sparkii`. Set the `SPARKII_HOME` env var if you've relocated it.
