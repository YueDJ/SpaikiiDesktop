...
How to build Sparkii Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Sparkii surface can also
...
- A **runtime home change** (switching the underlying `SPARKII_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Sparkii — do not assume
