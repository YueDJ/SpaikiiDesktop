...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.sparkiiHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.sparkiiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.sparkiiHome)
  const builder = await RealSessionBuilder.start(sandbox.sparkiiHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.sparkiiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.sparkiiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.sparkiiHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
