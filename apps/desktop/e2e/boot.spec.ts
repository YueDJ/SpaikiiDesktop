 ...
 * packaged binary) with a real `sparkii serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → sparkii serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Sparkii title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { sparkiiDesktop?: Record<string, unknown> }).sparkiiDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → sparkii serve is spawned → WS connects → config
