 ...
 *     (electron → sparkii serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `sparkii desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sparkii-e2e-${prefix}-${Math.random()}`))
  const sparkiiHome = path.join(root, 'sparkii-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(sparkiiHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(sparkiiHome, 'config.yaml')
 ...
 */
export function writeEnvFile(sparkiiHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(sparkiiHome, '.env')
 ...
 */
function writeEmptyConfig(sparkiiHome: string): void {
  const configPath = path.join(sparkiiHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SPARKII_HOME → sandbox sparkii-home (isolated config/sessions)
 *  - SPARKII_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SPARKII_DESKTOP_IGNORE_EXISTING=1 → don't pick up `sparkii` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SPARKII_DESKTOP_SPARKII_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SPARKII_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SPARKII_HOME: sandbox.sparkiiHome,
    SPARKII_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SPARKII_DESKTOP_IGNORE_EXISTING: '1',
    SPARKII_DESKTOP_SPARKII_ROOT: REPO_ROOT,
    SPARKII_DESKTOP_APP_NAME: `SparkiiE2E-${Date.now()}`,
    ...
    SPARKII_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SPARKII_HOME + userData
 * @param env      - the process environment (already has SPARKII_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.sparkiiHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.sparkiiHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SPARKII_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`sparkii serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.sparkiiHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SPARKII_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Sparkii backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Sparkii.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Sparkii.app', 'Contents', 'MacOS', 'Sparkii')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'sparkii')
}
 ...
 * SPARKII_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SPARKII_DESKTOP_BOOT_FAKE: '1',
    SPARKII_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })
