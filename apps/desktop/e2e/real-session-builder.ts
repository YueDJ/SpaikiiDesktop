 ...
 * AIAgent loop, using the E2E mock provider configured in `sparkiiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(sparkiiHome: string) {
  ...
  static async start(sparkiiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(sparkiiHome)
