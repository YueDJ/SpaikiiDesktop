...
const UPSTREAM_URL = 'https://github.com/NousResearch/sparkii-agent.git'
...
const LEGACY_MIGRATION_KEY = 'sparkii.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.sparkiiDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `sparkii-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const sparkiiHome = join(sandbox, 'sparkii-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(sparkiiHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(sparkiiHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SPARKII_DESKTOP_APP_NAME: `SparkiiShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SPARKII_DESKTOP_SPARKII_ROOT: target.targetRoot,
        SPARKII_DESKTOP_IGNORE_EXISTING: '1',
        SPARKII_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'sparkii-short-session-ab-'))
  const prepared = []
