 ...
 * product name (`Sparkii`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Sparkii'
 *
 * This is a hard failure with no obvious cause for the user — `sparkii desktop`
 ...
 * (the mac-unpacked Sparkii.app bundle) and Windows (win-unpacked), so we
 ...
 * Sparkii.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `sparkii desktop --build-only` (sparkii_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Sparkii.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Sparkii'}.exe`
