...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sparkii-dev-mock-${Date.now()}`))
  ...
  return { root, sparkiiHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(sparkiiHome, mockUrl) {
  fs.writeFileSync(
    path.join(sparkiiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(sparkiiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.sparkiiHome, mock.url)
  console.log(`  SPARKII_HOME: ${sandbox.sparkiiHome}`)
  ...
  const env = {
    ...
    SPARKII_DESKTOP_APP_NAME: `SparkiiDevMock-${Date.now()}`,
  }
