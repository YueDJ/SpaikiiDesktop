import { JsonRpcGatewayClient } from '@sparkii/shared'
...
// "Error invoking remote method 'sparkii:api': Error: 404: ..." through the
...
export function scanSessionPullRequests(
  ...
  return window.sparkiiDesktop.api<{
...
// curation `sparkii model` does — for Nous it honors the free/paid tier so a
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `sparkii skills`
...
// `sparkii mcp` and the dashboard MCP page). Raw JSON editing stays in
