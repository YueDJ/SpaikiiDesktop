...
import type { SparkiiApiRequest } from '@/global'
...
// process to route them to that host (sparkii:api's registry branch). Null
...
export function sparkiiApi<T>(request: SparkiiApiRequest): Promise<T> {
  return window.sparkiiDesktop.api<T>({ ...connectionScoped(), ...request })
}
