import type { McpCatalogResponse, McpServerSummary } from '@/types/sparkii'
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return sparkiiApi<{ ok: boolean; status: string }>({
...
export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return sparkiiApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return sparkiiApi<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
