...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Sparkii doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in sparkii_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `sparkii model` uses — instead of
...
export interface SparkiiConfig {
  agent?: {
...
export type SparkiiConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `sparkii model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Sparkii simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (sparkii.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors sparkii_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Sparkii, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `sparkii tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Sparkii); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/sparkii/update/check` — the backend's own update state.
