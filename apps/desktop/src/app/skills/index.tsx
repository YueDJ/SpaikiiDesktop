...
import type { SkillInfo, ToolsetInfo } from '@/types/sparkii'
...
export function SkillsView({
  ...
  const registryBridge = window.sparkiiDesktop?.connections
  const rosterBridge = window.sparkiiDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Sparkii (default)' : p.name,
      value: p.name
    }))
