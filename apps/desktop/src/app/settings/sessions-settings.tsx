...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveSparkiiConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Sparkii wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.sparkiiDesktop` is
    ...
    const settings = window.sparkiiDesktop?.settings
