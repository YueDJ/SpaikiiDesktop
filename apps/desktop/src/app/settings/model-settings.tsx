...
import { sparkiiConfigCacheWriter, invalidateSparkiiConfig, useSparkiiConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `sparkii model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in sparkii_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile = null }: ModelSettingsProps) {
  ...
  const { data: config } = useSparkiiConfigRecord(scopeProfile)
  const setConfig = useMemo(() => sparkiiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateSparkiiConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveSparkiiConfig(next, scopeProfile ?? undefined)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `sparkii model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Sparkii runs the flow for you.`}
          </p>
        )}
