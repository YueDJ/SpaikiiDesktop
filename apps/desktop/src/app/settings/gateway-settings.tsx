...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.sparkiiDesktop.applyConnectionConfig(payload())
        : await window.sparkiiDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.sparkiiDesktop.getConnectionConfig(scope)
