...
import { sparkiiConfigCacheWriter, useSparkiiConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<SparkiiConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSparkiiConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => sparkiiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['sparkii-config-schema'] : ['sparkii-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSparkiiConfigSchema(scopeProfile ?? undefined),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveSparkiiConfig(config, scopeProfile ?? undefined)
  ...
  const updateConfig = (next: SparkiiConfigRecord) => {
