...
import { getEnvVars, getSparkiiConfigSchema } from '@/sparkii'
...
import { useSparkiiConfigRecord } from '../hooks/use-config-record'
...
export function useSettingsSearchCatalog(enabled: boolean) {
  const { t } = useI18n()
  const configQuery = useSparkiiConfigRecord()
