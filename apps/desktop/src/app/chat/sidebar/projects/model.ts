...
import type { SessionInfo } from '@/sparkii'
...
// zero Sparkii sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, SparkiiGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, SparkiiGitWorktree[]>>({})
