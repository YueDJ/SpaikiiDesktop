...
import type { SparkiiReadDirEntry, SparkiiReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SparkiiReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SparkiiReadDirEntry) {
...
async function filterIgnored(entries: SparkiiReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SparkiiReadDirResult> {
  if (!window.sparkiiDesktop) {
