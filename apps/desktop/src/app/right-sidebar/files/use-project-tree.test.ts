...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { sparkiiDesktop: { readDir: typeof readDir } }).sparkiiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SparkiiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { sparkiiDesktop: unknown }).sparkiiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SparkiiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SparkiiReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { sparkiiDesktop: unknown }).sparkiiDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.sparkiiDesktop is missing', async () => {
    delete (window as unknown as { sparkiiDesktop?: unknown }).sparkiiDesktop
