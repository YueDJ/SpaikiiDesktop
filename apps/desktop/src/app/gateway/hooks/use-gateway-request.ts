...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SparkiiGateway | null
  const gatewayRef = useRef<SparkiiGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.sparkiiDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SparkiiGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SparkiiGateway | null
      }),
    []
  )
