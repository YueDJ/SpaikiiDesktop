...
import { pathFromSparkiiDeepLink, resolveSparkiiOpenPath } from '@/lib/sparkii-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sparkiiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.sparkiiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // sparkii://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future sparkiiDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSparkiiOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (sparkii://index-network/intent/1, sparkii://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSparkiiDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.sparkiiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.sparkiiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
