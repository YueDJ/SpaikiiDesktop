...
import { getSparkiiConfigRecord, saveMcpServers } from '@/sparkii'
...
import { setSparkiiConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `sparkii://mcp/install` deep links. The payload is
 ...
 * confirming lets Sparkii spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSparkiiConfigRecord())
      ...
      setSparkiiConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
