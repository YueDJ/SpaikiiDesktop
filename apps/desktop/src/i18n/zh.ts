  ...
  settings: {
    ...
    plugins: {
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '运行在 Sparkii 后端——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        empty: '尚未安装智能体插件。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Sparkii Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Sparkii 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      sshHermesPathTitle: 'Sparkii 路径（可选）',
      sshHermesPathDesc: '远程 sparkii 可执行文件的完整路径。留空 = 自动检测。',
