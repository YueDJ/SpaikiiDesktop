...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Sparkii Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Sparkii バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
  ...
  commandCenter: {
    ...
    hermesActiveSessions: (version, count) => `Sparkii ${version} · アクティブセッション ${count}`,
