...
const SPARKII_DIRECTIVE_RE = referenceRe()
...
export const sparkiiDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SPARKII_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...sparkiiDirectiveFormatter.parse(text)]
  } catch {
