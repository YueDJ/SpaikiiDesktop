 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.sparkii.send(prompt)` (or declarative
 * `data-sparkii-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'sparkii-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'sparkii-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `sparkii.send(prompt)`.
 ...
 *  wires `data-sparkii-send` so declarative HTML works with zero script:
 *  `<button data-sparkii-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.sparkii={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-sparkii-send]"):null;' +
    'if(el)send(el.getAttribute("data-sparkii-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.sparkiiDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
