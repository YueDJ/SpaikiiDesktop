...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Sparkii gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.sparkiiDesktop
  Object.defineProperty(window, 'sparkiiDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'sparkiiDesktop', { configurable: true, value: original })
}
