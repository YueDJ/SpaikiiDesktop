...
function sessionsBotsTree() {
  ...
  registerPane('sparkii-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'sparkii-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('sparkii-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('sparkii-bots:pane')
    expect($hiddenStripTabs.get()).toContain('sparkii-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('sparkii-bots:pane')

    expect(setStripTabHidden('sparkii-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('sparkii-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('sparkii-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('sparkii-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('sparkii.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('sparkii-bots:pane')
    ...
    revealTreePane('sparkii-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('sparkii-bots:pane')
    expect(window.localStorage.getItem('sparkii.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'sparkii-bots:pane', title: 'sparkii-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'sparkii-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
