...
vi.mock('@/sparkii', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn() }))
...
const { deleteProfile } = await import('@/sparkii')
...
describe('connection-aware plugin host APIs', () => {
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { sparkiiDesktop: unknown }).sparkiiDesktop = { getProfileRoutes }
