/**
 * @sparkii/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__SPARKII_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, type SparkiiGateway } from '@/sparkii'
import {
  $gateway,
...
import type { UsageStats } from '@/types/sparkii'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Sparkii profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.sparkiiDesktop?.getAgentRoster
...
//
// ensureGatewayProfile awaits the store's dial, and SparkiiGateway.connect() has
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.sparkiiDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Sparkii Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.sparkiiDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Sparkii Desktop.')
    }
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Sparkii Desktop connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Sparkii gateway unavailable')
    }
...
export type { SparkiiGateway } from '@/sparkii'
...
export type { SparkiiOpenTarget } from '@/lib/sparkii-open-target'
...
/** Sparkii' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/sparkii'
