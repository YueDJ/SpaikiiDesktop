/**
 * Live skin sync from the Sparkii backend.
 *
 * The backend resolves the active skin (built-in or `$SPARKII_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Sparkii
 ...
 */
export function ingestBackendSkin(skin: SparkiiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SparkiiSkin)
