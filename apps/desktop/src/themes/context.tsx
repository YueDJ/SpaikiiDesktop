...
const SKIN_KEY = 'sparkii-desktop-theme-v2'
const MODE_KEY = 'sparkii-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'sparkii-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'sparkii-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'sparkii-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.sparkiiTheme = skinName
  root.dataset.sparkiiMode = rendered
  ...
  window.sparkiiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('sparkii-boot-background', chromeBg)
    window.localStorage.setItem('sparkii-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.sparkiiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.sparkiiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Sparkii authoring/activating a skin from a
