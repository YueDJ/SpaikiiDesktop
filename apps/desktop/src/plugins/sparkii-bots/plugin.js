/**
 * Sparkii Bot Mode — a "one chat per agent" roster for the Sparkii desktop.
 *
 * Left pane "Bots": one row per Sparkii profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Sparkii cron jobs) scoped to the
 ...
 * bot-initiated sends use `sparkii -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@sparkii/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
async function saveBotMeta(name, patch) {
  ...
  try {
    ...
    serverRequest = Promise.resolve(host.request('profiles.configure', { name, ui_meta: { 'sparkii-bots': rest } }))
  } catch {
...
// handoffs (`sparkii -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      host
        ...
        .then(() => queryClient.invalidateQueries({ queryKey: ['sparkii-bots', 'roster'] }))
...
function mergeServerMeta(roster) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['sparkii-bots']
...
/** Permanently delete a bot's Sparkii profile, then remove plugin-local state
 ...
 * directory (sparkii-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('sparkii-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'sparkii-bots-roster-css'
  style.textContent =
    '.sparkii-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.sparkii-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes sparkii-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.sparkii-bots-pulse { animation: sparkii-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run sparkii mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.sparkiiDesktop && window.sparkiiDesktop.openExternal) {
        window.sparkiiDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Sparkii), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `sparkii pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (sparkii-agent bot_mode_probe).
 ...
 *  click opens (sparkii-agent#88200). Unknown params are ignored by older
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (sparkii-agent #86875) also expose the union
 ...
 *  callable alias is 'sparkii' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'sparkii' : name
}
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'sparkii') {
      token = byForm.has('sparkii') ? 'sparkii' : token
    }
...
async function ensureRemoteCanonicalChat(route, profile) {
  ...
  try {
    ...
    pinned = owner?.ui_meta?.['sparkii-bots']?.chat || null
  } catch {
...
async function openStoredBotChat(name, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Sparkii Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (sparkii-agent#89617) — clicking Retry
 ...
 *
 *  Identity rules (sparkii-agent#88200 — the row must open the session its
 *  preview describes):
...
async function prepareBotSource(bot, pinnedChat) {
  ...
  if (typeof host.ensureAgent !== 'function') {
    throw new Error('Update Sparkii Desktop to chat with agents on other connections.')
  }
  ...
  try {
    ...
    return owner?.ui_meta?.['sparkii-bots']?.chat || null
  } catch {
...
function displayName(bot, meta) {
  ...
  // "Sparkii". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `sparkii profile rename
  ...
  // that reads like nobody bothered. Present it as Sparkii (the agent it is)
...
// turn in its OWN persistent per-group Sparkii session and is fed only the
 ...
 *  literally named "default" — render it as Sparkii (matching displayName and
 *  the @sparkii handle) so the main agent never loses its name in rooms. */
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'sparkii -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `sparkii profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @sparkii, never @default (#89484).
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              activeNow
                ? jsx('span', {
                    className: 'sparkii-bots-pulse size-1.5 shrink-0 rounded-full bg-(--ui-accent,#4f9cf9)',
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Sparkii).'
    })
  }
  ...
  // (sparkii-agent#87317): Skills (installed list + one-click hub installs +
...
// https://sparkii-agent.nousresearch.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'sparkii-skill-pick', ...} to us (sparkii-agent#86243). We validate
...
const HUB_ORIGIN = 'https://sparkii-agent.nousresearch.com'
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'sparkii-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'sparkii-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Sparkii).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// sparkii -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `sparkii -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Sparkii-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Sparkii schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Sparkii, remote dupes
...
function GroupRow({ active, group, members, needsYou, onOpen }) {
  ...
  // and the group prompt: the primary profile is @sparkii, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['sparkii-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Sparkii and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : filteredRoster.length === 0 && rosterRows.length === 0
              ...
              : jsx(ScrollArea, {
                  className: 'sparkii-bots-roster min-h-0 flex-1',
                  children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Sparkii profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @sparkii.
    ctx.register({
    ...
    if (!document.getElementById('sparkii-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'sparkii-bots-keyframes'
      style.textContent = '@keyframes sparkii-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
        ...
        handler: async draft => {
          ...
          if (!roster) {
            ...
            for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
              ...
              if (name === 'sparkii' && !names.includes('sparkii') && names.includes('default')) {
                name = 'default'
              }
          ...
          if (localMentions.length) {
            note +=
              ...
              localMentions.map(bot => '`sparkii -p ' + shellQuote(bot.name) + ' chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(live.name)) + '): <your composed message>"`').join('\n') +
          ...
          if (remoteMentions.length) {
            ...
            note +=
              ...
              ' over Connections in the background. Do not run sparkii -p for them and do not switch Gateway. Tell the user they were messaged here; when a reply lands, relay it attributed to that agent.]'
          }
