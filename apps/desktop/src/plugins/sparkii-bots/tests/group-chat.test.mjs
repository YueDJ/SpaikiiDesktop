...
test('default profile speaks as Sparkii in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Sparkii: hello room')
...
test('turn prompt addresses the default profile as @sparkii', () => {
  ...
  assert.match(prompt, /You are @sparkii,/)
  ...
  assert.match(peerView, /group chat with @sparkii/)
})

test('mention routing: @sparkii resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@sparkii take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Sparkii)…
...
test('group room preview renders the bot HANDLE, not the raw profile name', () => {
  ...
  // @sparkii, so users concluded mention routing was broken.
