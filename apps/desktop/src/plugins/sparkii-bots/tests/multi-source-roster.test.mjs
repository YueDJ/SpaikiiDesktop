...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@sparkii\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Sparkii". Regression:
  ...
  assert.equal(name(active, undefined), 'Sparkii')
})

test('botHandle: precomputed multi-source handle wins; default stays sparkii', () => {
  ...
  assert.equal(botHandle('default'), 'sparkii')
})
...
test('displayName: local default stays Sparkii; remote default uses the device label', () => {
...
test('resolveRosterMentions: @sparkii in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'sparkii' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @sparkii and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
