...
function renderBotRow(input = 'alpha') {
  ...
  const context = {
    ...
    ROSTER_KEY: ['sparkii-bots', 'roster'],
    $botMeta: atom({}),
    $botUnread: atom({}),
    ...
    host: {
      ...
      request: async method =>
        method === 'profiles.list'
          ? { profiles: [{ name, ui_meta: { 'sparkii-bots': { chat: 'owner-chat' } } }] }
          : { sessions: [] },
      notify: () => undefined,
...
test('behavior: remote default does not open this-device chat when the source did not activate', async () => {
  ...
  const context = {
    ...
    host: {
      ...
      request: async () => ({ profiles: [{ name: 'default', ui_meta: { 'sparkii-bots': { chat: 'this-device-chat' } } }] }),
      notify: () => undefined,
