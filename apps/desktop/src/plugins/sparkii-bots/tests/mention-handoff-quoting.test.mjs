...
// The @mention middleware appends a handoff note whose sparkii command the
...
function load({ activeProfile = 'research', profiles = ['research', 'ops'], title = null } = {}) {
  ...
  const source = pluginSource
    .replace(/^import\s+\*\s+as\s+sdk\s+from '@sparkii\/plugin-sdk'\r?\n/m, '')
    .replace(/^import\s+\{[\s\S]*?\}\s+from '@sparkii\/plugin-sdk'\r?\n/m, '')
...
/** Run the note's first sparkii command under a stub that echoes each argv
 *  element — proves the shell received the interpolations as LITERALS. */
function runHandoffCommand(noteText) {
  const command = noteText.match(/`sparkii -p [^`]*`/)[0].slice(1, -1)
  const script = `sparkii() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: a poisoned bot title stays literal in the handoff command', async () => {
  const quoteSentinel = `/tmp/sparkii-bot-mode-mention-quote-${process.pid}`
  const subSentinel = `/tmp/sparkii-bot-mode-mention-sub-${process.pid}`
...
test('regression: the handoff command quotes the recipient argument', async () => {
  ...
  assert.match(result.text, /`sparkii -p 'ops' chat --in ~/)
})

test('behavior: @dixie on a Connections bot stays in this chat and does not sparkii -p', async () => {
  const values = new Map()
  const atom = initial => {
    const slot = { get: () => values.get(slot), set: value => values.set(slot, value) }
    values.set(slot, initial)
  ...
  assert.doesNotMatch(result.text, /sparkii -p 'dixie'/)
