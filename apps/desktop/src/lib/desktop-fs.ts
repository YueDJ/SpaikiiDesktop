import type {
  SparkiiConnection,
  SparkiiReadDirResult,
  SparkiiReadFileTextResult,
  SparkiiSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SparkiiConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SparkiiConnection | null = $connection.get()) {
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Sparkii Desktop bridge is unavailable')
  }
...
export async function readDesktopDir(path: string): Promise<SparkiiReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SparkiiReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SparkiiReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SparkiiReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.sparkiiDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SparkiiSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
