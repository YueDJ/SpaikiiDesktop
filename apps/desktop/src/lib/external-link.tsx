...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|sparkii):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.sparkiiDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.sparkiiDesktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of Sparkii, and it is the surface the agent
