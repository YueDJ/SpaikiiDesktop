 ...
 * `sparkii://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `sparkii://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `sparkii://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type SparkiiOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeSparkiiOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('sparkii://') || trimmed.startsWith(`${SPARKII_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // sparkii://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // sparkii://index-network/intent/1 → /index-network/intent/1
...
export function resolveSparkiiOpenPath(target: SparkiiOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeSparkiiOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeSparkiiOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeSparkiiOpenString(target.path)
 ...
 * (`sparkii://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromSparkiiDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveSparkiiOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveSparkiiOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `sparkii://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromSparkiiDeepLink('open', name, params)
}
