...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `sparkii-media://remote/${file}${profile}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `sparkii-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.sparkii/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.sparkiiDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.sparkiiDesktop.saveGatewayFile({
    path: file,
