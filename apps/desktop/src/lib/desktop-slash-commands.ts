...
/** Why a known Sparkii command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Sparkii profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Sparkii Agent version', surface: exec() },
...
function isKnownSparkiiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Sparkii' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSparkiiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Sparkii and were never asked for. It is
