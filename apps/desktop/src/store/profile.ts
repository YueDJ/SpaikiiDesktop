...
import { getProfiles, sparkiiApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/sparkii'
...
// /api/profiles/active `current`). "default" is the root ~/.sparkii. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'sparkii.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'sparkii.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await sparkiiApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SPARKII_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.sparkiiDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SparkiiConnection | null> {
  const getConnection = window.sparkiiDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SparkiiConnection | null> {
  const getConnectionFor = window.sparkiiDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'sparkii.desktop.showAllProfiles'
...
// Switch to the default (root ~/.sparkii) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.sparkiiDesktop?.touchBackend?.(target).catch(() => undefined)
}
