...
describe('transcript tail cache', () => {
  ...
  it('self-evicts a corrupt entry instead of returning garbage', () => {
    window.localStorage.setItem('sparkii.transcript-tail.v1:sess-bad', '{not json')

    expect(loadTranscriptTail('sess-bad')).toBeNull()
    expect(window.localStorage.getItem('sparkii.transcript-tail.v1:sess-bad')).toBeNull()
  })
