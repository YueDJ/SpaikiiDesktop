...
export function setZoomPercent(percent: number): void {
  window.sparkiiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.sparkiiDesktop?.zoom) {
  void window.sparkiiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.sparkiiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
