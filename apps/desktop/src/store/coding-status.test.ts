...
import type { SparkiiRepoStatus } from '@/global'
...
const sampleStatus: SparkiiRepoStatus = {
...
const otherStatus: SparkiiRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<SparkiiRepoStatus | null>) {
  ;(window as unknown as { sparkiiDesktop?: unknown }).sparkiiDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: SparkiiRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: SparkiiRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
...
describe('repoChangeKindForPath', () => {
  it('does not notify a row when only another path changes', () => {
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'b.ts', untracked: true } as SparkiiRepoStatus['files'][number]]
      }
    })
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'a.ts', untracked: true } as SparkiiRepoStatus['files'][number]]
      }
    })
