...
import type { SparkiiGitWorktree, SparkiiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SparkiiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SparkiiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SparkiiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SparkiiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SparkiiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SparkiiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SparkiiGitWorktree[]>>()
const $noRepoStatus = atom<SparkiiRepoStatus | null>(null)
const $noWorktrees = atom<SparkiiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SparkiiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SparkiiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SparkiiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SparkiiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SparkiiRepoStatus | null>
  seq: number
  target: string
}
