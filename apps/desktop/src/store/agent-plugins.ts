...
/**
 * Feature store for backend (agent) plugins — the native Sparkii plugins plus
 ...
 * primitives `sparkii plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Sparkii. */
  portable?: boolean
}
 ...
 *  SPARKII_HOME. Always refetches (it's a cheap local disk scan on the
