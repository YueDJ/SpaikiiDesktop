...
//
// Scope is always "uncommitted" — Sparkii' flow is agent edits you review BEFORE
...
export const $reviewFiles = atom<SparkiiReviewFile[]>([])
...
export const $reviewShipInfo = atom<SparkiiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['sparkiiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SparkiiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SparkiiReviewFile[], path: string): SparkiiReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.sparkiiDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.sparkiiDesktop?.openExternal?.(url)
    }
