 ...
 * app's bounds and stays visible while Sparkii is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'sparkii.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'sparkii.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.sparkiiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.sparkiiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.sparkiiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.sparkiiDesktop?.petOverlay?.close()
}
