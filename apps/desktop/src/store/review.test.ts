...
import type { SparkiiReviewFile, SparkiiReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.sparkiiDesktop.git,
...
function file(path: string, over: Partial<SparkiiReviewFile> = {}): SparkiiReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as SparkiiReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.sparkiiDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as SparkiiReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.sparkiiDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: SparkiiReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as SparkiiReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { sparkiiDesktop?: unknown }).sparkiiDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as SparkiiReviewShipInfo)
