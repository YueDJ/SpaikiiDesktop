...
const agentConn = (over: Partial<SparkiiConnection> = {}): SparkiiConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as SparkiiConnection

const localConn = (over: Partial<SparkiiConnection> = {}): SparkiiConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SparkiiConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SparkiiConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<SparkiiConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { sparkiiDesktop: { getConnection, getConnectionFor } })
})
