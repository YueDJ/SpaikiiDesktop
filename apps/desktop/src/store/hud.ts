 ...
 * composer with the reply scrolling above it, so Sparkii can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.sparkiiDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.sparkiiDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.sparkiiDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.sparkiiDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
