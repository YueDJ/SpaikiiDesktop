 ...
 *  Working in `~/www/sparkii-agent`, the draft says "sparkii-agent" constantly
 *  — a "Use skill: sparkii-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`sparkii-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/sparkii-agent-dev`, `plugin:skill`). */
