...
const { getProfiles } = await import('@/sparkii')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/sparkii/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<SparkiiConnection> = {}): SparkiiConnection =>
  ({ baseUrl: 'https://sparkii-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SparkiiConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { sparkiiDesktop: { getConnection } })
