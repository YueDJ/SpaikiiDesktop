 ...
 *   (`@sparkii/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 ...
 * and the two on-disk doors — `<sparkii home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<sparkii home>/plugins/<name>/desktop/
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.sparkii/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@sparkii/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @sparkii/plugin-sdk and react`
      )
    }
    ...
    // standalone install of sparkii-bots predating its adoption in-tree) must
...
//  - `<sparkii home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<sparkii home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's sparkii_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.sparkii/plugins` is installed-but-inert until the
