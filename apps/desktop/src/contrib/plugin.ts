 ...
 * `SparkiiPlugin`; it never touches the registry directly — it receives a
 ...
 *  in one attributed namespace instead of the raw `window.sparkiiDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Sparkii — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `sparkii://index-network/intent/1`),
...
export interface SparkiiPlugin {
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.sparkiiDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.sparkiiDesktop
