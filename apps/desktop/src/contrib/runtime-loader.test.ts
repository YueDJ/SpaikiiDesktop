...
import type * as SparkiiModule from '@/sparkii'
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend sparkii_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sparkii/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sparkii/plugins')
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.sparkii/plugins')
  ...
  it('probes desktop/plugin.js inside agent-plugin packages (unified packaging)', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.sparkii/plugins'
        ? { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.sparkii/plugins/my-feature' }] }
        : { entries: [] }
    )
    ...
    expect(readFileText).toHaveBeenCalledWith('/local/.sparkii/plugins/my-feature/desktop/plugin.js')
    // The Python half's files must never be probed as a desktop entry.
    expect(readFileText).not.toHaveBeenCalledWith('/local/.sparkii/plugins/my-feature/plugin.js')
  })

  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.sparkiiDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.sparkii/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.sparkii/plugins'
        ? { entries: [{ isDirectory: true, name: 'uni', path: '/local/.sparkii/plugins/uni' }] }
        : { entries: [] }
    )
    ...
    try {
      ...
      // ~/.sparkii/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend sparkii_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.sparkii/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.sparkii/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sparkii/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.sparkii/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.sparkii/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'sparkii-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "sparkii-bots", name: "Bot Mode", register() {} }',
        'sparkii-bots',
        { file: '/local/.sparkii/desktop-plugins/sparkii-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['sparkii-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['sparkii-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.sparkii/desktop-plugins/sparkii-bots/plugin.js'
      })
