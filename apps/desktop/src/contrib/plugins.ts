 ...
 *    `SparkiiPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `sparkii-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<sparkii home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: SparkiiPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
