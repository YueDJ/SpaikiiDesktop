...
import type { TranslucencyState } from '@sparkii/shared/translucency'
...
declare global {
  interface Window {
    sparkiiDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SparkiiConnection>
      ...
      // Resume this session in the user's own terminal emulator (`sparkii --tui
      ...
      // Sparkii Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SPARKII_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SparkiiApiRequest) => Promise<T>
      notify: (payload: SparkiiNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SparkiiReadFileTextResult>
      selectPaths: (options?: SparkiiSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SparkiiPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SparkiiPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SparkiiPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SparkiiActiveWork) => void
      setTitleBarTheme?: (payload: SparkiiTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SparkiiReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SPARKII_HOME>/desktop-plugins),
      ...
      // Local AGENT-plugin root (<SPARKII_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SparkiiGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SparkiiGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SparkiiGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SparkiiRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SparkiiReviewScope, baseRef?: null | string) => Promise<SparkiiReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SparkiiReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SparkiiReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SparkiiRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SparkiiPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SparkiiTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SparkiiTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SparkiiWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SparkiiPreviewFileChanged) => void) => () => void
...
export interface SparkiiTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SparkiiTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface SparkiiConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface SparkiiTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SparkiiActiveWork {
  count: number
  titles: string[]
}

export interface SparkiiWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Sparkii Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Sparkii Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Sparkii Cloud org (slug or id) to
...
// A discovered Sparkii Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SparkiiApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SparkiiNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SparkiiPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SparkiiReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SparkiiPreviewWatch {
  id: string
  path: string
}
...
export interface SparkiiGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SparkiiGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SparkiiGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SparkiiRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SparkiiRepoStatus {
  branch: null | string
...
// turn began.
export type SparkiiReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SparkiiReviewFile {
  path: string
  added: number
  removed: number
...
export interface SparkiiReviewList {
  files: SparkiiReviewFile[]
...
export interface SparkiiReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SparkiiBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SparkiiRepoPullRequests {
  ghReady: boolean
  prs: SparkiiBranchPullRequest[]
}
...
export interface SparkiiPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SparkiiReviewShipInfo {
  ghReady: boolean
  pr: SparkiiReviewPr | null
}

export interface SparkiiReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SparkiiReadDirResult {
  entries: SparkiiReadDirEntry[]
  error?: string
}

export interface SparkiiPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SparkiiSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
