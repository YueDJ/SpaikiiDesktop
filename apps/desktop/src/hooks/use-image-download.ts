...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'sparkii:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.sparkiiDesktop?.saveImageFromUrl) {
        if (await window.sparkiiDesktop.saveImageFromUrl(src)) {
