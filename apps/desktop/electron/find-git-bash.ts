 ...
 *   1. SPARKII_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\sparkii\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SPARKII_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SPARKII_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'sparkii', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'sparkii', 'git', 'usr', 'bin', 'bash.exe'))
  }
