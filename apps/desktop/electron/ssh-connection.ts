...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SPARKII_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Sparkii-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.sparkii/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'sparkii-desktop-ssh')
  }

  return path.join(os.homedir(), '.sparkii', 'desktop-ssh')
}
