 ...
 * the three user-facing uninstall modes to the `sparkii uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m sparkii_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `sparkii_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'sparkii_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Sparkii.app/Contents/MacOS/Sparkii  → …/Sparkii.app
 *   Windows: …\Sparkii\Sparkii.exe                 → …\Sparkii  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Sparkii.app/Contents/MacOS/Sparkii → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Sparkii.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Sparkii.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Sparkii$/i.test(dir) || /[\\/]sparkii-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/sparkii
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import sparkii_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, sparkiiHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import sparkii_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Sparkii install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
