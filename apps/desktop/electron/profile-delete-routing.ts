// Profile-delete routing logic for the `sparkii:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_sparkii_home() and
...
/** Parse a `sparkii:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SPARKII_HOME.
 *
...
/**
 * Route the next `sparkii:api` request away from the primary/window backend
 ...
 * ensure_sparkii_home() recreates the directory the delete just removed.
 */
