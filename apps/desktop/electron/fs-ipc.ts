...
// hardening, SPARKII_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(sparkiiHome, 'profiles', profile) : sparkiiHome
  ...
  ipcMain.handle('sparkii:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL agent-plugin root (`<SPARKII_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('sparkii:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('sparkii:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('sparkii:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
