export type ParentWatchdogEnv = {
  SPARKII_PARENT_PID: string
  SPARKII_PARENT_START_MARKER?: string
  SPARKII_PARENT_NONCE?: string
}
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SPARKII_PARENT_PID: String(pid) }
  ...
  env.SPARKII_PARENT_START_MARKER = startMarker
  env.SPARKII_PARENT_NONCE = nonce
