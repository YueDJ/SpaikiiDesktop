...
// have sparkii_cli" (since `node -c "import sparkii_cli"` will exit
...
test('canImportSparkiiCli returns false when interpreter cannot run -c', () => {
  // node IS an interpreter, but `node -c "import sparkii_cli"` is a
...
test('canImportSparkiiCli returns false when binary does not exist', () => {
  const ghost = path.join(os.tmpdir(), 'sparkii-probes-ghost-' + Date.now() + '.exe')
  assert.equal(canImportSparkiiCli(ghost), false)
})

test('sparkii runtime import probe checks config dependencies', () => {
  const probe = sparkiiRuntimeImportProbe()
  assert.match(probe, /\bimport yaml\b/)
  // dotenv is the first third-party import on the CLI boot path
  // (sparkii_cli/env_loader.py); a mid-update venv missing python-dotenv
  // passed the old probe and produced an unrecoverable boot loop.
  assert.match(probe, /\bimport dotenv\b/)
  assert.match(probe, /\bimport sparkii_cli\.config\b/)
})

test('explicit Sparkii override is authoritative', () => {
  assert.equal(shouldTrustSparkiiOverride('/nix/store/abc/bin/sparkii'), true)
})
...
test('verifySparkiiCli returns true when --version exits 0', () => {
  ...
  // it through node. This stands in for a working sparkii binary --
  // verifySparkiiCli only cares about the exit code.
  const scriptPath = path.join(os.tmpdir(), `sparkii-probes-ok-${Date.now()}-${process.pid}.cjs`)
...
test('default probe timeout is 15s (not the old 5s death-loop value)', () => {
  ...
  // matches the default (tests run without SPARKII_PROBE_TIMEOUT_MS).
  assert.equal(PROBE_TIMEOUT_MS, DEFAULT_PROBE_TIMEOUT_MS)
})

test('resolveProbeTimeoutMs honours SPARKII_PROBE_TIMEOUT_MS', () => {
  assert.equal(resolveProbeTimeoutMs({}), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ SPARKII_PROBE_TIMEOUT_MS: '30000' }), 30_000)
  assert.equal(resolveProbeTimeoutMs({ SPARKII_PROBE_TIMEOUT_MS: '0' }), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ SPARKII_PROBE_TIMEOUT_MS: 'nope' }), DEFAULT_PROBE_TIMEOUT_MS)
  // Cap runaway values
  assert.equal(resolveProbeTimeoutMs({ SPARKII_PROBE_TIMEOUT_MS: '999999' }), 120_000)
})
