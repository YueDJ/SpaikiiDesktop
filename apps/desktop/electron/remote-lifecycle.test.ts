...
test('listRemoteSparkiiProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/SPARKII_HOME/, '/Users/zillajr/.sparkii\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteSparkiiProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteSparkiiProfiles rejects a hostile SPARKII_HOME', async () => {
  const ssh = fakeSsh([[/SPARKII_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteSparkiiProfiles(ssh),
    (err: any) => {
...
test('locateSparkii prefers the explicit profile path when executable', async () => {
  ...
  assert.equal(await locateSparkii(ssh, '/opt/sparkii'), '/opt/sparkii')
})
...
test('locateSparkii throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  ...
  await assert.rejects(
    () => locateSparkii(ssh, '/bad/path/sparkii'),
    (err: any) => {
...
test('locateSparkii falls back to ~/.local/bin/sparkii when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateSparkii(ssh, ''), '~/.local/bin/sparkii')
})

test('locateSparkii tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/sparkii/, 'OK']])
  assert.equal(await locateSparkii(ssh, ''), '~/.sparkii/sparkii-agent/venv/bin/sparkii')
})

test('locateSparkii throws a sparkii-not-found error with an install hint', async () => {
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/sparkii', '/Users/cd9c/.sparkii', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /sparkii-agent.*venv.*bin.*sparkii/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'sparkii wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'sparkii')
    const launcher = path.join(temp, 'sparkii launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/sparkii-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { sparkiiPath: '/x/sparkii', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { sparkiiPath: '/x/sparkii', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('connect() respawns when the lockfile sparkiiPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ sparkiiPath: '/old/stale/sparkii', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteSparkiiPath: '/new/sparkii', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() fresh spawn writes sparkiiHome + protocolVersion into the lockfile', async () => {
  ...
  assert.match(lockWrite, /"sparkiiHome":"\/home\/alice\/\.sparkii"/)
})
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { sparkiiPath: '/x/sparkii', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { sparkiiPath: '/x/sparkii', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
