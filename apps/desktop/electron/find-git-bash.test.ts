...
test('SPARKII_GIT_BASH_PATH override takes precedence', () => {
  const result = findGitBash({
    isWindows: true,
    env: { SPARKII_GIT_BASH_PATH: 'D:\\CustomGit\\bin\\bash.exe' },
    fileExists: yes,
    findOnPath: () => null
  })
...
test('SPARKII_GIT_BASH_PATH invalid path falls through to candidates', () => {
  const env = {
    SPARKII_GIT_BASH_PATH: 'X:\\Missing\\bash.exe',
...
test('SPARKII_GIT_BASH_PATH empty string is ignored', () => {
  const result = findGitBash({
    isWindows: true,
    env: { SPARKII_GIT_BASH_PATH: '', LOCALAPPDATA: '' },
    fileExists: no,
