...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/sparkii', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/sparkii/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/sparkii'), 'https://gw.example.com/sparkii/auth/native/refresh')
})
