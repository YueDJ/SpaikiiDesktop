...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so Sparkii Desktop cannot save remote gateway tokens. ' +
        'Set SPARKII_DESKTOP_REMOTE_URL and SPARKII_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
