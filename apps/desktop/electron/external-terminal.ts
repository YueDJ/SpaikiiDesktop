// Launching the Sparkii TUI in the user's OWN terminal emulator.
//
...
// `sparkii --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Sparkii runtime is often a venv Python invoked
//     as `python -m sparkii_cli.main`, not a `sparkii` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Sparkii
