 ...
 *
 * The staged Tauri `sparkii-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `sparkii update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `sparkii update`, swaps/relaunches
 * the app, and writes .sparkii-update-result.json. With the app gone before
 * the update starts, the SPARKII_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `sparkii update` card — old checkouts pull the script on their next update).
 */
