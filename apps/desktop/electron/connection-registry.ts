 ...
 * runtime, remote gateways, Sparkii Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@sparkii/shared`) — tsconfig project
...
/** Turn `ls ~/.sparkii/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Sparkii Cloud
 * instances are platform-managed — we never run `sparkii update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteSparkiiPath: input.remoteSparkiiPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Sparkii-managed names; an empty result stores nothing.
 ...
 * `remoteSparkiiPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Sparkii Cloud' : 'Remote gateway'),
