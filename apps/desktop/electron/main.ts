...
import { buildDesktopBackendEnv, hermesManagedNodePathEntries, normalizeSparkiiHomeRoot } from './backend-env'
...
function pathWithHermesManagedNode(...entries) {
  const managed = hermesManagedNodePathEntries(SPARKII_HOME).filter(directoryExists)
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    isHermesSourceRoot(ACTIVE_SPARKII_ROOT) ? ACTIVE_SPARKII_ROOT : null
  ].filter(Boolean)
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const sparkii = resolveHermesCliBinary(updateRoot)

  if (!sparkii) {
    emitUpdateProgress({ stage: 'manual', message: 'sparkii update', percent: null })

    return { ok: true, manual: true, command: 'sparkii update', hermesRoot: updateRoot }
  }
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_SPARKII_ROOT) &&
...
function resolveHermesBackend(backendArgs) {
  ...
  if (process.env.SPARKII_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    const hermesOverride = process.env.SPARKII_DESKTOP_HERMES
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Sparkii override under WSL: ${hermesOverride}`)
      }
    } else {
      hermesCommand = findOnPath('sparkii')
    }

    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Sparkii CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (shouldTrustHermesOverride(hermesOverride) || verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        ...
        return {
          label: `existing Sparkii CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Sparkii CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
...
async function ensureRuntime(backend) {
  ...
  if (!isHermesSourceRoot(ACTIVE_SPARKII_ROOT)) {
    throw new Error(
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.hermesVersion || 'sparkii (version unknown)'} at ${result.hermesPath || '?'}`
  )
...
ipcMain.handle('sparkii:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: hermesLog.slice(-200) }))
