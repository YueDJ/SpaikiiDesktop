...
// it). The value arrives via SPARKII_DESKTOP_PASSWORD_STORE, bridged by the
// `sparkii desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[sparkii] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[sparkii] using password-store backend: ${PASSWORD_STORE.store}`)
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'sparkii-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.sparkiiBackendIdentity = identity

    return identity
  } catch (error) {
    stopBackendChild(child)
    await waitForBackendExit(child)
    throw new Error(`Could not persist ownership for the Sparkii backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.sparkiiBackendIdentity
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `sparkii update` → relaunch),
      ...
      // in: the SPARKII_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `sparkii update` card once; their next update pulls it.
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Sparkii will keep running — try again, or run \`sparkii update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // The updater invokes the venv's Sparkii launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the sparkii
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.sparkii-bootstrap-complete')),
      hasVenvSparkii: fileExists(venvSparkii),
...
// out, runs `sparkii update`, swaps/relaunches the app bundle, and writes
// .sparkii-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Sparkii — this window will close. Don’t reopen Sparkii yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: sparkii desktop --force-build`
    )
...
async function waitForSparkii(baseUrl, token, signal?, authMode?, headers = {}) {
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('sparkii:preview-nav', command)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('sparkii:context-menu-spellcheck', {
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Sparkii Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
async function discoverCloudAgents(org?: string) {
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Sparkii Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every sparkii:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSparkiiPath`/`remoteProfile` —
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SPARKII_DESKTOP_REMOTE_TOKEN,
      url: process.env.SPARKII_DESKTOP_REMOTE_URL
    },
    profile,
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('sparkii:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  // spawning, so two children never share one profile's SPARKII_HOME.
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteSparkiiPath: source.remoteSparkiiPath,
    ...
    return {
      ...
      // is only the routing label. sparkii:api uses it to translate explicit
  ...
  await waitForSparkii(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSparkii(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SPARKII_HOME, 'profiles', key))
  )
...
async function startSparkii() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Sparkii Desktop is already running in another window.')
  }
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      return createPrimaryRemoteConnection(remote, sparkiiLog.slice(-80), getWindowState())
    }
    ...
    await claimBackendChild(sparkiiProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, sparkiiProcess)

    if (!processOwner) {
      stopBackendChild(sparkiiProcess)
      await waitForBackendExit(sparkiiProcess)
      releaseBackendChild(sparkiiProcess)
      throw new Error('Sparkii backend start was superseded by a newer connection attempt.')
    }
...
ipcMain.handle('sparkii:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('sparkii:connection:for', async (_event, payload) => {
...
ipcMain.handle('sparkii:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSparkiiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Sparkii is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `sparkii-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SPARKII_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('sparkii:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('sparkii:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('sparkii:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('sparkii:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('sparkii:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sparkii:connections:set-launch-mode', async (_event, mode) => {
  const registry = setConnectionLaunchMode(readDesktopConnectionsRegistry(), String(mode || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sparkii:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('sparkii:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSparkiiPath: entry.remoteSparkiiPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startSparkii()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSparkiiPath: connection.remoteSparkiiPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSparkiiProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SPARKII_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('sparkii:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// sparkii:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('sparkii:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `sparkii update` to every eligible registered connection at once.
...
// /api/sparkii/update endpoint (the dashboard updater), which runs
// `sparkii update` on THAT machine.
ipcMain.handle('sparkii:connections:update-all', async () => {
  const registry = readDesktopConnectionsRegistry()
  ...
  const results = await Promise.all(
    registry.connections.map(async connection => {
      ...
      try {
        ...
        const body: any = await postJsonForBackend(descriptor, '/api/sparkii/update', {}, { timeoutMs: 15_000 })
...
// analogue of the sparkii:api handler's own auth split: OAuth backends prefer a
...
async function handleSparkiiApiRequest(request) {
...
ipcMain.handle('sparkii:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SPARKII_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSparkiiApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSparkiiApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('sparkii:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    // for renderer callbacks). Same vocabulary as sparkii://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('sparkii:notification-activate', {
...
ipcMain.handle('sparkii:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))
...
ipcMain.handle('sparkii:context-menu:edit', (event, command) => {
...
ipcMain.handle('sparkii:context-menu:copy-image', event => {
...
ipcMain.handle('sparkii:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('sparkii:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.on('sparkii:translucency:support', event => {
...
ipcMain.on('sparkii:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('sparkii:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sparkii:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))
...
// Embedded terminal PTY host (sparkii:terminal:*) — see terminal-ipc.ts.
...
// Renderer-bundle skew: `sparkii update` moves the SOURCE TREE, but the UI
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSparkiiVersion()} — app build out of date, update the desktop app`
        : resolveSparkiiVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
// sparkii://mcp/install?name=NAME&config=B64 — the vendor "Add to Sparkii"
// button, or sparkii://plugin/install?repo=owner/repo). Dev
// (`SPARKII_DESKTOP_DEV_SERVER`) registers sparkii-dev:// instead — bare
// Electron or a stale OS handler often owns sparkii:// on dev machines.
...
// ---------------------------------------------------------------------------
const SPARKII_PROTOCOL = DEV_SERVER ? 'sparkii-dev' : 'sparkii'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['sparkii-dev', 'sparkii'] : ['sparkii']
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SPARKII_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SPARKII_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SPARKII_PROTOCOL}:// handler`)
  } catch (err) {
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSparkii), whose
