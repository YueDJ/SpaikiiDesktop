...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', SPARKII_HOME: '/home/b/.sparkii' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'sparkii_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.sparkii/sparkii-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.sparkii/sparkii-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.sparkii/sparkii-agent'`,
      `exec '/home/b/.sparkii/sparkii-agent/venv/bin/python' '-m' 'sparkii_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\sparkii\\venv\\Scripts\\sparkii.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\sparkii\\venv\\Scripts\\sparkii.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
