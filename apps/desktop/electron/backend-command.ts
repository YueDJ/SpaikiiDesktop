// Backend subcommand routing for the desktop-managed Sparkii process.
//
// The desktop app launches its own headless backend via `sparkii serve` — it
...
// app hasn't updated yet, or an older `sparkii` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Sparkii profile to pin via `--profile`.
 */
 ...
 * `-m sparkii_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `sparkii_cli/subcommands/dashboard.py` source registers
