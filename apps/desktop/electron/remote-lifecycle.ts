...
async function locateSparkii(ssh, remoteSparkiiPath) {
  ...
  if (remoteSparkiiPath) {
    if (await isExecutable(remoteSparkiiPath)) {
      return resolveLauncher(remoteSparkiiPath)
    }

    const err: any = new Error(
      `The Sparkii path you set is not an executable on the remote host: "${remoteSparkiiPath}". ` +
...
async function probeSparkiiVersion(ssh, sparkiiPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(sparkiiPath)} --version 2>&1`)).trim()
...
async function listRemoteSparkiiProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteSparkiiHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Sparkii profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Sparkii home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'sparkiiPath', 'sparkiiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !sparkiiPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(sparkiiPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/sparkii`, leaving neither
      // launcher nor SPARKII_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `sparkii_home=os.path.expanduser(${shq(sparkiiHome)}) if ${shq(sparkiiHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if sparkii_home:\n' +
      ' expected_entries.add(os.path.join(sparkii_home,"sparkii-agent","venv","bin","sparkii"))\n' +
...
function buildSpawnCommand(sparkiiPath, profile, opts: any = {}) {
  const sparkii = expandRemotePath(sparkiiPath)
...
async function remoteSupportsSshOwnership(ssh, sparkiiPath) {
  const sparkii = expandRemotePath(sparkiiPath)
...
async function spawnRemoteDashboard(ssh, { sparkiiPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, sparkiiPath))) {
    const err: any = new Error(
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(sparkiiPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
...
async function connect(deps) {
  ...
  const sparkiiPath = await locateSparkii(ssh, remoteSparkiiPath)
  log(`located sparkii at ${sparkiiPath}`)
  const sparkiiVersion = await probeSparkiiVersion(ssh, sparkiiPath)

  if (sparkiiVersion) {
    log(`remote sparkii version: ${sparkiiVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const sparkiiHome = await probeRemoteSparkiiHome(ssh)
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.sparkiiPath === sparkiiPath &&
      lock.sparkiiHome === sparkiiHome
