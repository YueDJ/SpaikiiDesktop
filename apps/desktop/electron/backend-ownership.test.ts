...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'sparkii serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/sparkii serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m sparkii_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/sparkii --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Sparkii Runtime\\sparkii.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('sparkii chat --query serve'), false)
