...
function bootstrapCacheDir(sparkiiHome) {
  return path.join(sparkiiHome, 'bootstrap-cache')
}
...
function installedAgentInstallScript(sparkiiHome) {
  if (!sparkiiHome) {
    return null
  }

  const candidate = path.join(sparkiiHome, 'sparkii-agent', 'scripts', installScriptName())
...
function cachedScriptPath(sparkiiHome, commit) {
  return path.join(bootstrapCacheDir(sparkiiHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/YueDJ/SparkiiDesktop/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(sparkiiHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(sparkiiHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, sparkiiHome }: any = {}) {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, sparkiiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, sparkiiHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--sparkii-home', sparkiiHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, sparkiiHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, sparkiiHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, sparkiiHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(sparkiiHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, sparkiiHome, emit })
