 ...
 * console. It writes SPARKII_HOME/.sparkii-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Sparkii within 30 minutes.
...
export function handoffResultPath(sparkiiHome: string): string {
  return path.join(sparkiiHome, '.sparkii-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(sparkiiHome)
  let raw: string
