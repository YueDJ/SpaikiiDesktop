...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'sparkii-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Sparkii Test')
    git('config', 'user.email', 'sparkii@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/NousResearch/sparkii-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/NousResearch/sparkii-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/NousResearch/sparkii-agent.git',
