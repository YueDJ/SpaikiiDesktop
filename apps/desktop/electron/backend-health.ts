...
export interface SparkiiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSparkiiReady`).
   */
...
export async function waitForSparkiiReady(baseUrl: string, options: SparkiiReadyOptions): Promise<void> {
  ...
  throw new Error(`Sparkii backend did not become ready: ${detail}`)
}
