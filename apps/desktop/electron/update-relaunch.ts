 ...
 * Background
 * ----------
 * After `sparkii update` + `sparkii desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SPARKII_DESKTOP_* is preserved
// plus SPARKII_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SPARKII_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SPARKII_DESKTOP_']
