//! Sparkii Setup — Tauri entrypoint.
//!
...
pub fn run() {
    // Tracing → bootstrap-installer.log under SPARKII_HOME/logs/ so install
    ...
    // Sparkii is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Sparkii installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Sparkii is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Sparkii" doubles as a normal launcher (first run installs, every
        ...
        .expect("error while running Sparkii Setup");
}
