...
//!
//! Driven when the installer is launched as `Sparkii-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Sparkii desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `sparkii update`
//!      or repair bootstrap can race locked files),
//!   2. run `sparkii update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in sparkii_cli/main.py),
//!   3. run `sparkii desktop --build-only` (the rebuild step update skips),
...
/// `sparkii update` exit code meaning "another sparkii process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// sparkii_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `sparkii update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `sparkii update` claims
/// the same file (see `sparkii_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `sparkii update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in sparkii_cli/update_lock.py — all three read
...
/// owner that is itself and aborts ("Another Sparkii update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `sparkii update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(sparkiiHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    //
    // The same marker is the cross-process update lock (sparkii_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `sparkii update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Sparkii update is already running (PID {}, started {} ago). \
    ...
    // async on Windows. If it still holds the venv shim, `sparkii update`
    ...
    // ---- stage 2: sparkii update -----------------------------------------
    // Pass --branch so `sparkii update` targets the branch this installer was
    ...
    // without --branch, `sparkii update` switches the checkout to `main` (a
    ...
    // --force skips `sparkii update`'s Windows running-exe guard (which would
    ...
    // time `sparkii update` runs there is no legitimate sparkii.exe to protect,
    // and the guard would only produce a false "Sparkii is still running" stop.
    //
    ...
    // probe only checks the sparkii.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Sparkii windows" message.
    ...
    // Retry-once for the update-boundary crash. `sparkii update` lazily imports
    ...
    // second `sparkii update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Sparkii" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Sparkii is still running. Close all Sparkii windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
    ...
    // ---- stage 3: sparkii desktop --build-only ----------------------------
    // `sparkii update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `sparkii update` already does above, and `sparkii update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `sparkii desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Sparkii manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Sparkii to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend sparkii.exe (or the desktop Sparkii.exe
            ...
            // force-kill every Sparkii.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Sparkii still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Sparkii.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Sparkii.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `sparkii.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `sparkii.exe` image tree via
...
/// Spawn `sparkii <args>` from `cwd`, stream stdout/stderr as Log events on the
...
fn venv_hermes(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        ...
        install_root.join("venv").join("bin").join("sparkii")
    }
}
...
fn resolve_hermes(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "sparkii.exe" } else { "sparkii" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `sparkii update` child claims that SAME lock (sparkii_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Sparkii is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in sparkii_cli/update_lock.py.
    envs.push((
        "SPARKII_UPDATE_HANDOFF_PID".to_string(),
...
async fn install_macos_app_update(
    ...
    let tmp = PathBuf::from(format!("{}.sparkii-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.sparkii-update-old", target_app.display()));
...
mod tests {
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/sparkii-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/sparkii-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "SPARKII_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the sparkii update child claims the same marker we hold; without our pid \
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/sparkii-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".sparkii-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `sparkii update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(sparkiiHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Sparkii.app"]),
            Some(PathBuf::from("/Applications/Sparkii.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Sparkii.app");
        let tmp = base.join("Sparkii.app.sparkii-update-new");
        let old = base.join("Sparkii.app.sparkii-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Sparkii.app");
        let tmp = base.join("Sparkii.app.sparkii-update-new"); // intentionally absent
        let old = base.join("Sparkii.app.sparkii-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Sparkii.app");
        let tmp = base.join("Sparkii.app.sparkii-update-new"); // absent
        let old = base.join("Sparkii.app.sparkii-update-old");
        write_marker(&target, "OLD");
