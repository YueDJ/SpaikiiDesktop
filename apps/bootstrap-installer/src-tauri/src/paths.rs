...
//!
//! Mirrors `sparkii_constants.get_sparkii_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\sparkii
//!   macOS:   ~/.sparkii
//!   Linux:   ~/.sparkii  (override via $SPARKII_HOME)
//!
//! NOTE (macOS): Python's get_sparkii_home(), scripts/install.sh, and the
//! Electron desktop's resolveSparkiiHome() ALL use ~/.sparkii on macOS — there
...
pub fn sparkii_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\sparkii — matches scripts/install.ps1's $SparkiiHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("sparkii");
        }
    }

    // macOS + Linux + fallback: ~/.sparkii (matches Python get_sparkii_home(),
    // install.sh, and the Electron desktop's resolveSparkiiHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".sparkii");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".sparkii")
}
...
/// SPARKII_HOME so it survives repo checkout deletion (unlike anything under
/// sparkii-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\sparkii\sparkii-setup.exe`; on other
...
pub fn copy_self_to_sparkii_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SPARKII_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SPARKII_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".sparkii-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SPARKII_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[sparkii-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SPARKII_BOOTSTRAP_LOG")
