...
//! SPARKII_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
/// Spawn the locally-built Sparkii desktop binary, then close the installer
...
pub async fn launch_sparkii_desktop(
    ...
    tracing::info!(?exe_path, "launching Sparkii desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Sparkii
...
pub(crate) fn resolve_sparkii_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Sparkii.exe"),
            ("win-arm64-unpacked", "Sparkii.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Sparkii.app/Contents/MacOS", "Sparkii"),
            ("mac-arm64/Sparkii.app/Contents/MacOS", "Sparkii"),
        ]
    } else {
        &[("linux-unpacked", "sparkii")]
    };
...
async fn run_bootstrap(
    ...
    // Copy ourselves to SPARKII_HOME/sparkii-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_sparkii_home() {
        tracing::warn!(?err, "failed to copy installer into SPARKII_HOME (non-fatal)");
        emit_log(&format!(
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Sparkii"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Sparkii.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Sparkii.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("sparkii");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".sparkii-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".sparkii-bootstrap-complete");
        let tmp_path = root.join(".sparkii-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".sparkii-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".sparkii-bootstrap-complete.tmp").exists(),
