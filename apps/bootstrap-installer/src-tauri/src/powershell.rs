...
///
/// `sparkii_home_override` propagates to the child as $SPARKII_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, sparkii_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = sparkii_home_override {
        cmd.env("SPARKII_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, sparkii_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = sparkii_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_sparkii_home() {
