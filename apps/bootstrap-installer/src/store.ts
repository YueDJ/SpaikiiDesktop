...
/// desktop app handing off via `Sparkii-Setup.exe --update`.
...
export const $sparkiiHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.sparkii/logs/bootstrap-installer.log')
    $sparkiiHome.set('~/.sparkii')
  ...
  try {
    const [logPath, sparkiiHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_sparkii_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $sparkiiHome.set(sparkiiHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Sparkii" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Sparkii-Setup.exe --update);
...
export async function launchSparkiiDesktop(): Promise<void> {
  ...
  await invoke('launch_sparkii_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Sparkii repository' },
