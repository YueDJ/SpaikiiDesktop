...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``sparkii -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = os.environ.get("SPARKII_SESSION_SOURCE", "")
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Sparkii-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.sparkii/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[sparkii-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `sparkii --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``sparkii chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``SPARKII_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``SPARKII_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("SPARKII_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``SPARKII_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("SPARKII_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``SPARKII_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("SPARKII_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        sparkii-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See sparkii-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Sparkii against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Sparkii can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SPARKII_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _redact_message_content(content):
        ...
        Respects ``SPARKII_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.sparkii/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Sparkii-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SPARKII_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Sparkii' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Sparkii model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Sparkii-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``SPARKII_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("SPARKII_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.sparkii/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``SPARKII_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("SPARKII_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Sparkii process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Sparkii process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `sparkii doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.sparkii/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.sparkii/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`sparkii doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (SPARKII_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (SPARKII_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SPARKII_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SPARKII_DEV_CREDITS"))
        ...
        if _dev:
            # SPARKII_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `sparkii logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Sparkii YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``SPARKII_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        ...
        A Settings save (desktop ``PUT /api/env``, ``sparkii setup``) updates
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated sparkii_logging._session_context (a
            ...
            try:
                from sparkii_logging import set_session_context
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across Sparkii
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Sparkii process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Sparkii process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Sparkii process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Sparkii process kept this session busy too "
